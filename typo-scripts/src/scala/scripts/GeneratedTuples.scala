  ...
  override def run(started: Started, commands: Commands, targets: List[GeneratedTuples.Target], args: List[String]): Unit = {
    ...
    val tupleExprRecords = 1.to(N).map { n =>
      ...
      s"""    record TupleExpr$n<$tparamsDecl>($fields) implements TupleExpr<Tuple$n<$tparamsDecl>> {
         ...
         |        public typo.runtime.DbType<Tuple$n<$tparamsDecl>> dbType() {
         ...
         |
         |        @Override
         |        public typo.runtime.Fragment render(typo.dsl.RenderCtx ctx, java.util.concurrent.atomic.AtomicInteger counter) {
         |            // Render all sub-expressions as a comma-separated list
         |            java.util.List<typo.runtime.Fragment> fragments = java.util.List.of($renderArgs);
         |            return typo.runtime.Fragment.comma(fragments);
         |        }
         |    }""".stripMargin
    }
    ...
    val tuplesContents =
      s"""|package typo.dsl;
          ...
          |         */
          |        @Override
          |        default java.util.List<typo.runtime.DbType<?>> flattenedDbTypes() {
          |            java.util.List<typo.runtime.DbType<?>> result = new java.util.ArrayList<>();
          |            for (SqlExpr<?> expr : exprs()) {
    ...
    targets.foreach { target =>
      FileUtils.writeString(started.logger, Some("writing"), target.sources.resolve("typo/dsl/Tuples.java"), tuplesContents)
      FileUtils.writeString(started.logger, Some("writing"), target.sources.resolve("typo/dsl/SelectBuilderMap.java"), selectBuilderMapContents)
    }
  }
}
