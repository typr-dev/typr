...
object GeneratedSqlServer {
  ...
  def main(args: Array[String]): Unit =
    Loggers
      ...
      .use { logger =>
        val ds = TypoDataSource.hikariSqlServer(
          server = "localhost",
          port = 1433,
          databaseName = "typo",
          username = "sa",
        ...
        val scriptsPath = buildDir.resolve("sqlserver_sql")
        ...
        val variants: Seq[(Lang, DbLibName, JsonLibName, String, String)] = List(
          (LangJava, DbLibName.Typo, JsonLibName.Jackson, "typo-tester-sqlserver-java", ""),
          (LangScala.scalaDsl(Dialect.Scala3, TypeSupportScala), DbLibName.Typo, JsonLibName.Jackson, "typo-tester-sqlserver-scala", ""),
          (LangKotlin(TypeSupportKotlin), DbLibName.Typo, JsonLibName.Jackson, "typo-tester-sqlserver-kotlin", "")
        )
