...
/** Oracle-specific JDBC metadata extraction for views and queries */
case class OracleJdbcMetadata(columns: MaybeReturnsRows[NonEmptyList[MetadataColumn]])

object OracleJdbcMetadata {

  def from(sql: String)(implicit c: Connection): Either[String, OracleJdbcMetadata] = {
    val ps = c.prepareStatement(sql)
    try from(ps)
    finally ps.close()
  }

  def from(ps: PreparedStatement): Either[String, OracleJdbcMetadata] = {
    def nonEmpty(str: String): Option[String] = if (str == null || str.isEmpty) None else Some(str)

    Option(ps.getMetaData) match {
      case None =>
        Right(OracleJdbcMetadata(MaybeReturnsRows.Update))
      ...
      case Some(metadata: ResultSetMetaData) =>
        val cols = 0
          ...
          .map(_ + 1)
          .map { n =>
            // Oracle JDBC driver provides base table/column info via standard JDBC methods
            ...
            MetadataColumn(
              ...
              format = 0, // Oracle doesn't have this PostgreSQL-specific field
        ...
        NonEmptyList.fromList(cols.toList) match {
          case Some(cols) =>
            Right(OracleJdbcMetadata(MaybeReturnsRows.Query(cols)))
          case None =>
