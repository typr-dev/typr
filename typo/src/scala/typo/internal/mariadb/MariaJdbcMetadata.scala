...
/** MariaDB-specific JDBC metadata extraction for views and queries */
case class MariaJdbcMetadata(columns: MaybeReturnsRows[NonEmptyList[MetadataColumn]])

object MariaJdbcMetadata {

  def from(sql: String)(implicit c: Connection): Either[String, MariaJdbcMetadata] = {
    val ps = c.prepareStatement(sql)
    try from(ps)
    finally ps.close()
  }

  def from(ps: PreparedStatement): Either[String, MariaJdbcMetadata] = {
    def nonEmpty(str: String): Option[String] = if (str == null || str.isEmpty) None else Some(str)

    Option(ps.getMetaData) match {
      case None =>
        Right(MariaJdbcMetadata(MaybeReturnsRows.Update))
      ...
      case Some(metadata: ResultSetMetaData) =>
        val cols = 0
          ...
          .map(_ + 1)
          .map { n =>
            // MariaDB JDBC driver provides base table/column info via standard JDBC methods
            ...
            MetadataColumn(
              ...
              format = 0, // MariaDB doesn't have this PostgreSQL-specific field
        ...
        NonEmptyList.fromList(cols.toList) match {
          case Some(cols) =>
            Right(MariaJdbcMetadata(MaybeReturnsRows.Query(cols)))
          case None =>
