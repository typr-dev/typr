...
object LViewRow {
  val `_rowParser`: RowParser[LViewRow] = RowParsers.of(LocationId.pgType, LocationId.pgType, Name.pgType, PgTypes.numeric, PgTypes.numeric, PgTypes.timestamp, LViewRow.apply, row => Array[Any](row.id, row.locationid, row.name, row.costrate, row.availability, row.modifieddate))
}