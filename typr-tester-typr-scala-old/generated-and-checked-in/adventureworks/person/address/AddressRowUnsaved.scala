...
object AddressRowUnsaved {
  given pgText: PgText[AddressRowUnsaved] = PgText.instance((row, sb) => { PgTypes.text.pgText.unsafeEncode(row.addressline1, sb); sb.append(PgText.DELIMETER); PgTypes.text.opt().pgText.unsafeEncode(row.addressline2, sb); sb.append(PgText.DELIMETER); PgTypes.text.pgText.unsafeEncode(row.city, sb); sb.append(PgText.DELIMETER); StateprovinceId.pgType.pgText.unsafeEncode(row.stateprovinceid, sb); sb.append(PgText.DELIMETER); PgTypes.text.pgText.unsafeEncode(row.postalcode, sb); sb.append(PgText.DELIMETER); PgTypes.bytea.opt().pgText.unsafeEncode(row.spatiallocation, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using AddressId.pgType.pgText).unsafeEncode(row.addressid, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using PgTypes.uuid.pgText).unsafeEncode(row.rowguid, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using PgTypes.timestamp.pgText).unsafeEncode(row.modifieddate, sb) })
}