...
object PersonRowUnsaved {
  given pgText: PgText[PersonRowUnsaved] = PgText.instance((row, sb) => { BusinessentityId.pgType.pgText.unsafeEncode(row.businessentityid, sb); sb.append(PgText.DELIMETER); PgTypes.bpchar.pgText.unsafeEncode(row.persontype, sb); sb.append(PgText.DELIMETER); PgTypes.text.opt().pgText.unsafeEncode(row.title, sb); sb.append(PgText.DELIMETER); FirstName.pgType.pgText.unsafeEncode(row.firstname, sb); sb.append(PgText.DELIMETER); Name.pgType.opt().pgText.unsafeEncode(row.middlename, sb); sb.append(PgText.DELIMETER); Name.pgType.pgText.unsafeEncode(row.lastname, sb); sb.append(PgText.DELIMETER); PgTypes.text.opt().pgText.unsafeEncode(row.suffix, sb); sb.append(PgText.DELIMETER); PgTypes.xml.opt().pgText.unsafeEncode(row.additionalcontactinfo, sb); sb.append(PgText.DELIMETER); PgTypes.xml.opt().pgText.unsafeEncode(row.demographics, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using NameStyle.pgType.pgText).unsafeEncode(row.namestyle, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using PgTypes.int4.pgText).unsafeEncode(row.emailpromotion, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using PgTypes.uuid.pgText).unsafeEncode(row.rowguid, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using PgTypes.timestamp.pgText).unsafeEncode(row.modifieddate, sb) })
}