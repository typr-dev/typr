...
object SalespersonquotahistoryRowUnsaved {
  given pgText: PgText[SalespersonquotahistoryRowUnsaved] = PgText.instance((row, sb) => { BusinessentityId.pgType.pgText.unsafeEncode(row.businessentityid, sb); sb.append(PgText.DELIMETER); PgTypes.timestamp.pgText.unsafeEncode(row.quotadate, sb); sb.append(PgText.DELIMETER); PgTypes.numeric.pgText.unsafeEncode(row.salesquota, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using PgTypes.uuid.pgText).unsafeEncode(row.rowguid, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using PgTypes.timestamp.pgText).unsafeEncode(row.modifieddate, sb) })
}