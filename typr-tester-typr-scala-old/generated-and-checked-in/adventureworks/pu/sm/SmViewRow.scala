...
object SmViewRow {
  val `_rowParser`: RowParser[SmViewRow] = RowParsers.of(ShipmethodId.pgType, ShipmethodId.pgType, Name.pgType, PgTypes.numeric, PgTypes.numeric, PgTypes.uuid, PgTypes.timestamp, SmViewRow.apply, row => Array[Any](row.id, row.shipmethodid, row.name, row.shipbase, row.shiprate, row.rowguid, row.modifieddate))
}