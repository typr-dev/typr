...
object SoViewRow {
  val `_rowParser`: RowParser[SoViewRow] = RowParsers.of(SpecialofferId.pgType, SpecialofferId.pgType, PgTypes.text, PgTypes.numeric, PgTypes.text, PgTypes.text, PgTypes.timestamp, PgTypes.timestamp, PgTypes.int4, PgTypes.int4, PgTypes.uuid, PgTypes.timestamp, SoViewRow.apply, row => Array[Any](row.id, row.specialofferid, row.description, row.discountpct, row.`type`, row.category, row.startdate, row.enddate, row.minqty, row.maxqty, row.rowguid, row.modifieddate))
}