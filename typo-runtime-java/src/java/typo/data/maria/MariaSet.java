 ...
 * each chosen from a list of permitted values. SET values are returned
 * by JDBC as comma-separated strings (e.g., "email,sms,push").
 * <p>
 * This wrapper provides type-safe access to the individual values.
 */
public final class MariaSet {
    ...
    public static MariaSet fromString(String commaSeparated) {
        ...
        Set<String> values = Arrays.stream(commaSeparated.split(","))
                .map(String::trim)
                .filter(s -> !s.isEmpty())
