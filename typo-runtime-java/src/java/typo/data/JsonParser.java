...
/**
 * Simple JSON parser for parsing JSON strings into JsonValue ADT.
 * Handles standard JSON format as produced by PostgreSQL and MariaDB.
 */
final class JsonParser {
    ...
    static JsonValue parse(String json) {
        ...
        if (parser.pos < parser.json.length()) {
            throw new IllegalArgumentException("Unexpected content after JSON value at position " + parser.pos);
        }
        return result;
    }
    ...
    private JsonValue parseValue() {
        ...
        return switch (c) {
            ...
            default -> throw new IllegalArgumentException("Unexpected character '" + c + "' at position " + pos);
        };
    }
    ...
    private JsonValue.JNumber parseNumber() {
        ...
        return new JsonValue.JNumber(new BigDecimal(numStr));
    }
