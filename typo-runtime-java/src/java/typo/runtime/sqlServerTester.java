 ...
 * <p>Then run:
 *
 * <pre>java -cp ... typo.runtime.sqlServerTester</pre>
 */
public interface sqlServerTester {
  ...
  static void withConnection(SqlFunction<Connection, ?> f) {
    try (var conn =
        DriverManager.getConnection(
            "jdbc:sqlserver://localhost:1433;databaseName=typo;user=sa;password=YourStrong@Passw0rd;encrypt=false;trustServerCertificate=true")) {
  ...
  static <A> void testJsonRoundtrip(SqlServerTypeAndExample<A> t) {
    ...
    typo.data.JsonValue jsonValue = jsonCodec.toJson(original);
    String encoded = jsonValue.encode();
    typo.data.JsonValue parsed = typo.data.JsonValue.parse(encoded);
  ...
  static <A> void testJsonDbRoundtrip(Connection conn, SqlServerTypeAndExample<A> t)
    ...
    try {
      ...
      typo.data.JsonValue parsedFromDb = typo.data.JsonValue.parse(jsonFromDb);
      typo.data.JsonValue valueJson = ((typo.data.JsonValue.JObject) parsedFromDb).get("v");
