...
public sealed interface Operation<Out>
    ...
    record UpdateReturning<Out>(Fragment query, ResultSetParser<Out> parser) implements Operation<Out> {
        @Override
        public Out run(Connection conn) throws SQLException {
            try (PreparedStatement stmt = conn.prepareStatement(query.render())) {
                query.set(stmt);
    ...
    record UpdateMany<Row>(Fragment query,
    ...
    record UpdateManyReturning<Row>(
            Fragment query,
            RowParser<Row> parser,
            Iterator<Row> rows
        ...
        public List<Row> run(Connection conn) throws SQLException {
            try (PreparedStatement stmt = conn.prepareStatement(query.render(), java.sql.Statement.RETURN_GENERATED_KEYS)) {
                query.set(stmt);
                while (rows.hasNext()) {
                    Row row = rows.next();
                    parser.writeRow(stmt, row);
