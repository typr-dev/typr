...
public sealed interface PgRead<A> extends DbRead<A> permits PgRead.NonNullable, PgRead.Nullable {
    ...
    /**
     * Read an array where JDBC driver returns Object[] containing elements that need casting.
     * Used for PostgreSQL geometric types (box[], circle[], etc.) where driver returns PGobject[].
     */
    @SuppressWarnings("unchecked")
    ...
    PgRead<Json[]> readJsonArray = PgRead.readStringArray.map(as -> arrayMap.map(as, Json::new, Json.class));
    PgRead<Jsonb[]> readJsonbArray = PgRead.readStringArray.map(as -> arrayMap.map(as, Jsonb::new, Jsonb.class));
    ...
    PgRead<Money[]> readMoneyArray = PgRead.readString.map(str -> {
        if (str.equals("{}")) return new Money[0];
        return arrayMap.map(str.substring(1, str.length() - 1).split(","), Money::new, Money.class);
    });
