...
case class AddressRepoMock(
  ...
  override def deleteById(addressid: AddressId)(implicit c: Connection): Boolean = map.remove(addressid).isDefined

  override def deleteByIds(addressids: Array[AddressId])(implicit c: Connection): Int = addressids.map(id => map.remove(id)).count(_.isDefined)
  ...
  override def insert(unsaved: AddressRowUnsaved)(implicit c: Connection): AddressRow = insert(toRow(unsaved))
  ...
  override def selectAll(implicit c: Connection): List[AddressRow] = map.values.toList

  override def selectById(addressid: AddressId)(implicit c: Connection): Option[AddressRow] = map.get(addressid)

  override def selectByIds(addressids: Array[AddressId])(implicit c: Connection): List[AddressRow] = addressids.flatMap(map.get).toList
