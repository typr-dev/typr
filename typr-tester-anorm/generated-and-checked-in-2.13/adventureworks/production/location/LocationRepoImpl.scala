...
class LocationRepoImpl extends LocationRepo {
  ...
  override def deleteById(locationid: LocationId)(implicit c: Connection): Boolean = SQL"""delete from "production"."location" where "locationid" = ${ParameterValue(locationid, null, LocationId.toStatement)}""".executeUpdate() > 0
  ...
  override def insertStreaming(
    ...
    batchSize: Int = 10000
  )(implicit c: Connection): Long = streamingInsert(s"""COPY "production"."location"("locationid", "name", "costrate", "availability", "modifieddate") FROM STDIN""", batchSize, unsaved)(LocationRow.pgText, c)

  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(
    unsaved: Iterator[LocationRowUnsaved],
    batchSize: Int = 10000
  )(implicit c: Connection): Long = streamingInsert(s"""COPY "production"."location"("name", "locationid", "costrate", "availability", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(LocationRowUnsaved.pgText, c)
  ...
  override def upsertStreaming(
    ...
    streamingInsert(s"""copy location_TEMP("locationid", "name", "costrate", "availability", "modifieddate") from stdin""", batchSize, unsaved)(LocationRow.pgText, c): @nowarn
    SQL"""insert into "production"."location"("locationid", "name", "costrate", "availability", "modifieddate")
