...
case class DocumentRepoMock(
  ...
  override def deleteById(documentnode: DocumentId)(implicit c: Connection): Boolean = map.remove(documentnode).isDefined

  override def deleteByIds(documentnodes: Array[DocumentId])(implicit c: Connection): Int = documentnodes.map(id => map.remove(id)).count(_.isDefined)
  ...
  override def insert(unsaved: DocumentRowUnsaved)(implicit c: Connection): DocumentRow = insert(toRow(unsaved))
  ...
  override def selectAll(implicit c: Connection): List[DocumentRow] = map.values.toList

  override def selectById(documentnode: DocumentId)(implicit c: Connection): Option[DocumentRow] = map.get(documentnode)

  override def selectByIds(documentnodes: Array[DocumentId])(implicit c: Connection): List[DocumentRow] = documentnodes.flatMap(map.get).toList
  ...
  override def selectByUniqueRowguid(rowguid: TypoUUID)(implicit c: Connection): Option[DocumentRow] = map.values.find(v => rowguid == v.rowguid)
