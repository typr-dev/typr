...
case class ProductRepoMock(
  ...
  override def deleteById(productid: ProductId)(implicit c: Connection): Boolean = map.remove(productid).isDefined

  override def deleteByIds(productids: Array[ProductId])(implicit c: Connection): Int = productids.map(id => map.remove(id)).count(_.isDefined)
  ...
  override def insert(unsaved: ProductRowUnsaved)(implicit c: Connection): ProductRow = insert(toRow(unsaved))
  ...
  override def selectAll(implicit c: Connection): List[ProductRow] = map.values.toList

  override def selectById(productid: ProductId)(implicit c: Connection): Option[ProductRow] = map.get(productid)

  override def selectByIds(productids: Array[ProductId])(implicit c: Connection): List[ProductRow] = productids.flatMap(map.get).toList
