...
case class PurchaseorderheaderRepoMock(
  ...
  override def deleteById(purchaseorderid: PurchaseorderheaderId)(implicit c: Connection): Boolean = map.remove(purchaseorderid).isDefined

  override def deleteByIds(purchaseorderids: Array[PurchaseorderheaderId])(implicit c: Connection): Int = purchaseorderids.map(id => map.remove(id)).count(_.isDefined)
  ...
  override def insert(unsaved: PurchaseorderheaderRowUnsaved)(implicit c: Connection): PurchaseorderheaderRow = insert(toRow(unsaved))
  ...
  override def selectAll(implicit c: Connection): List[PurchaseorderheaderRow] = map.values.toList

  override def selectById(purchaseorderid: PurchaseorderheaderId)(implicit c: Connection): Option[PurchaseorderheaderRow] = map.get(purchaseorderid)

  override def selectByIds(purchaseorderids: Array[PurchaseorderheaderId])(implicit c: Connection): List[PurchaseorderheaderRow] = purchaseorderids.flatMap(map.get).toList
