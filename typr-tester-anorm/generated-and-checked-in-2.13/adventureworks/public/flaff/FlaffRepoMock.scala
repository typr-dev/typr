...
case class FlaffRepoMock(map: scala.collection.mutable.Map[FlaffId, FlaffRow] = scala.collection.mutable.Map.empty[FlaffId, FlaffRow]) extends FlaffRepo {
  ...
  override def deleteById(compositeId: FlaffId)(implicit c: Connection): Boolean = map.remove(compositeId).isDefined

  override def deleteByIds(compositeIds: Array[FlaffId])(implicit c: Connection): Int = compositeIds.map(id => map.remove(id)).count(_.isDefined)
  ...
  override def selectAll(implicit c: Connection): List[FlaffRow] = map.values.toList

  override def selectById(compositeId: FlaffId)(implicit c: Connection): Option[FlaffRow] = map.get(compositeId)

  override def selectByIds(compositeIds: Array[FlaffId])(implicit c: Connection): List[FlaffRow] = compositeIds.flatMap(map.get).toList
