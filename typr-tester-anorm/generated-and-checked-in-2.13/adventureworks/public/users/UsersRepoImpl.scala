...
class UsersRepoImpl extends UsersRepo {
  ...
  override def deleteById(userId: UsersId)(implicit c: Connection): Boolean = SQL"""delete from "public"."users" where "user_id" = ${ParameterValue(userId, null, UsersId.toStatement)}""".executeUpdate() > 0
  ...
  override def insertStreaming(
    ...
    batchSize: Int = 10000
  )(implicit c: Connection): Long = streamingInsert(s"""COPY "public"."users"("user_id", "name", "last_name", "email", "password", "created_at", "verified_on") FROM STDIN""", batchSize, unsaved)(UsersRow.pgText, c)

  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(
    unsaved: Iterator[UsersRowUnsaved],
    batchSize: Int = 10000
  )(implicit c: Connection): Long = streamingInsert(s"""COPY "public"."users"("user_id", "name", "last_name", "email", "password", "verified_on", "created_at") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(UsersRowUnsaved.pgText, c)
  ...
  override def upsertStreaming(
    ...
    streamingInsert(s"""copy users_TEMP("user_id", "name", "last_name", "email", "password", "created_at", "verified_on") from stdin""", batchSize, unsaved)(UsersRow.pgText, c): @nowarn
    SQL"""insert into "public"."users"("user_id", "name", "last_name", "email", "password", "created_at", "verified_on")
