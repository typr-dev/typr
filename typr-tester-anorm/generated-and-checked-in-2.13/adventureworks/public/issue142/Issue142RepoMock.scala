...
case class Issue142RepoMock(map: scala.collection.mutable.Map[Issue142Id, Issue142Row] = scala.collection.mutable.Map.empty[Issue142Id, Issue142Row]) extends Issue142Repo {
  ...
  override def deleteById(tabellkode: Issue142Id)(implicit c: Connection): Boolean = map.remove(tabellkode).isDefined

  override def deleteByIds(tabellkodes: Array[Issue142Id])(implicit c: Connection): Int = tabellkodes.map(id => map.remove(id)).count(_.isDefined)
  ...
  override def selectAll(implicit c: Connection): List[Issue142Row] = map.values.toList

  override def selectById(tabellkode: Issue142Id)(implicit c: Connection): Option[Issue142Row] = map.get(tabellkode)

  override def selectByIds(tabellkodes: Array[Issue142Id])(implicit c: Connection): List[Issue142Row] = tabellkodes.flatMap(map.get).toList
