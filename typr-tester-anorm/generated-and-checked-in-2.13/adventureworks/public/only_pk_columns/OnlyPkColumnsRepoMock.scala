...
case class OnlyPkColumnsRepoMock(map: scala.collection.mutable.Map[OnlyPkColumnsId, OnlyPkColumnsRow] = scala.collection.mutable.Map.empty[OnlyPkColumnsId, OnlyPkColumnsRow]) extends OnlyPkColumnsRepo {
  ...
  override def deleteById(compositeId: OnlyPkColumnsId)(implicit c: Connection): Boolean = map.remove(compositeId).isDefined

  override def deleteByIds(compositeIds: Array[OnlyPkColumnsId])(implicit c: Connection): Int = compositeIds.map(id => map.remove(id)).count(_.isDefined)
  ...
  override def selectAll(implicit c: Connection): List[OnlyPkColumnsRow] = map.values.toList

  override def selectById(compositeId: OnlyPkColumnsId)(implicit c: Connection): Option[OnlyPkColumnsRow] = map.get(compositeId)

  override def selectByIds(compositeIds: Array[OnlyPkColumnsId])(implicit c: Connection): List[OnlyPkColumnsRow] = compositeIds.flatMap(map.get).toList
