...
class OnlyPkColumnsRepoImpl extends OnlyPkColumnsRepo {
  ...
  override def deleteByIds(compositeIds: Array[OnlyPkColumnsId])(implicit c: Connection): Int = {
    ...
    in (select unnest(${ParameterValue(keyColumn1, null, ToStatement.arrayToParameter(ParameterMetaData.StringParameterMetaData))}), unnest(${ParameterValue(keyColumn2, null, adventureworks.IntArrayToStatement)}))
    """.executeUpdate()
  }
  ...
  override def upsertStreaming(
    ...
    streamingInsert(s"""copy only_pk_columns_TEMP("key_column_1", "key_column_2") from stdin""", batchSize, unsaved)(OnlyPkColumnsRow.pgText, c): @nowarn
    SQL"""insert into "public"."only_pk_columns"("key_column_1", "key_column_2")
