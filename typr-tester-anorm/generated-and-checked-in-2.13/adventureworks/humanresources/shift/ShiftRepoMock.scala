...
case class ShiftRepoMock(
  ...
  override def deleteById(shiftid: ShiftId)(implicit c: Connection): Boolean = map.remove(shiftid).isDefined

  override def deleteByIds(shiftids: Array[ShiftId])(implicit c: Connection): Int = shiftids.map(id => map.remove(id)).count(_.isDefined)
  ...
  override def insert(unsaved: ShiftRowUnsaved)(implicit c: Connection): ShiftRow = insert(toRow(unsaved))
  ...
  override def selectAll(implicit c: Connection): List[ShiftRow] = map.values.toList

  override def selectById(shiftid: ShiftId)(implicit c: Connection): Option[ShiftRow] = map.get(shiftid)

  override def selectByIds(shiftids: Array[ShiftId])(implicit c: Connection): List[ShiftRow] = shiftids.flatMap(map.get).toList
