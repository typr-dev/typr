...
object LocationRow {
  ...
  def rowParser(idx: Int): RowParser[LocationRow] = {
    RowParser[LocationRow] { row =>
      Success(
        LocationRow(
          locationid = row(idx + 0)(using LocationId.column),
          name = row(idx + 1)(using Name.column),
          costrate = row(idx + 2)(using Column.columnToScalaBigDecimal),
          availability = row(idx + 3)(using Column.columnToScalaBigDecimal),
