...
class UnitmeasureRepoImpl extends UnitmeasureRepo {
  override def delete: DeleteBuilder[UnitmeasureFields, UnitmeasureRow] = DeleteBuilder.of(""""production"."unitmeasure"""", UnitmeasureFields.structure, UnitmeasureRow.rowParser(1).*)

  override def deleteById(unitmeasurecode: UnitmeasureId)(using c: Connection): Boolean = SQL"""delete from "production"."unitmeasure" where "unitmeasurecode" = ${ParameterValue(unitmeasurecode, null, UnitmeasureId.toStatement)}""".executeUpdate() > 0
  ...
  override def insertStreaming(
    ...
    batchSize: Int = 10000
  )(using c: Connection): Long = streamingInsert(s"""COPY "production"."unitmeasure"("unitmeasurecode", "name", "modifieddate") FROM STDIN""", batchSize, unsaved)(using UnitmeasureRow.pgText, c)

  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(
    unsaved: Iterator[UnitmeasureRowUnsaved],
    batchSize: Int = 10000
  )(using c: Connection): Long = streamingInsert(s"""COPY "production"."unitmeasure"("unitmeasurecode", "name", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(using UnitmeasureRowUnsaved.pgText, c)

  override def select: SelectBuilder[UnitmeasureFields, UnitmeasureRow] = SelectBuilder.of(""""production"."unitmeasure"""", UnitmeasureFields.structure, UnitmeasureRow.rowParser)
  ...
  override def update: UpdateBuilder[UnitmeasureFields, UnitmeasureRow] = UpdateBuilder.of(""""production"."unitmeasure"""", UnitmeasureFields.structure, UnitmeasureRow.rowParser(1).*)
  ...
  override def upsertStreaming(
    ...
    streamingInsert(s"""copy unitmeasure_TEMP("unitmeasurecode", "name", "modifieddate") from stdin""", batchSize, unsaved)(using UnitmeasureRow.pgText, c): @nowarn
    SQL"""insert into "production"."unitmeasure"("unitmeasurecode", "name", "modifieddate")
