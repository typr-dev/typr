...
class ProductlistpricehistoryRepoImpl extends ProductlistpricehistoryRepo {
  ...
  override def deleteById(compositeId: ProductlistpricehistoryId)(using c: Connection): Boolean = SQL"""delete from "production"."productlistpricehistory" where "productid" = ${ParameterValue(compositeId.productid, null, ProductId.toStatement)} AND "startdate" = ${ParameterValue(compositeId.startdate, null, TypoLocalDateTime.toStatement)}""".executeUpdate() > 0
  ...
  override def insertStreaming(
    ...
    batchSize: Int = 10000
  )(using c: Connection): Long = streamingInsert(s"""COPY "production"."productlistpricehistory"("productid", "startdate", "enddate", "listprice", "modifieddate") FROM STDIN""", batchSize, unsaved)(using ProductlistpricehistoryRow.pgText, c)

  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(
    unsaved: Iterator[ProductlistpricehistoryRowUnsaved],
    batchSize: Int = 10000
  )(using c: Connection): Long = streamingInsert(s"""COPY "production"."productlistpricehistory"("productid", "startdate", "enddate", "listprice", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(using ProductlistpricehistoryRowUnsaved.pgText, c)
  ...
  override def upsertStreaming(
    ...
    streamingInsert(s"""copy productlistpricehistory_TEMP("productid", "startdate", "enddate", "listprice", "modifieddate") from stdin""", batchSize, unsaved)(using ProductlistpricehistoryRow.pgText, c): @nowarn
    SQL"""insert into "production"."productlistpricehistory"("productid", "startdate", "enddate", "listprice", "modifieddate")
