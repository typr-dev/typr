...
trait ProductmodelproductdescriptioncultureFields {
  ...
  def compositeIdIn(compositeIds: Array[ProductmodelproductdescriptioncultureId]): SqlExpr[Boolean] =
    new CompositeIn(compositeIds)(TuplePart[ProductmodelproductdescriptioncultureId](productmodelid)(_.productmodelid)(using as[Array[ProductmodelId]](using ToParameterValue(using null, ProductmodelId.arrayToStatement), adventureworks.arrayParameterMetaData(using ProductmodelId.parameterMetadata)), implicitly), TuplePart[ProductmodelproductdescriptioncultureId](productdescriptionid)(_.productdescriptionid)(using as[Array[ProductdescriptionId]](using ToParameterValue(using null, ProductdescriptionId.arrayToStatement), adventureworks.arrayParameterMetaData(using ProductdescriptionId.parameterMetadata)), implicitly), TuplePart[ProductmodelproductdescriptioncultureId](cultureid)(_.cultureid)(using as[Array[CultureId]](using ToParameterValue(using null, CultureId.arrayToStatement), adventureworks.arrayParameterMetaData(using CultureId.parameterMetadata)), implicitly))
