...
object BillofmaterialsRow {
  given pgText: Text[BillofmaterialsRow] = {
  ...
  given reads: Reads[BillofmaterialsRow] = {
    Reads[BillofmaterialsRow](json => JsResult.fromTry(
        Try(
          BillofmaterialsRow(
            billofmaterialsid = json.\("billofmaterialsid").as(using Reads.IntReads),
            productassemblyid = json.\("productassemblyid").toOption.map(_.as(using ProductId.reads)),
            componentid = json.\("componentid").as(using ProductId.reads),
            ...
            bomlevel = json.\("bomlevel").as(using TypoShort.reads),
            perassemblyqty = json.\("perassemblyqty").as(using Reads.bigDecReads),
  ...
  def rowParser(idx: Int): RowParser[BillofmaterialsRow] = {
    RowParser[BillofmaterialsRow] { row =>
      Success(
        BillofmaterialsRow(
          billofmaterialsid = row(idx + 0)(using Column.columnToInt),
          productassemblyid = row(idx + 1)(using Column.columnToOption(using ProductId.column)),
          componentid = row(idx + 2)(using ProductId.column),
          startdate = row(idx + 3)(using TypoLocalDateTime.column),
          enddate = row(idx + 4)(using Column.columnToOption(using TypoLocalDateTime.column)),
          unitmeasurecode = row(idx + 5)(using UnitmeasureId.column),
          bomlevel = row(idx + 6)(using TypoShort.column),
          perassemblyqty = row(idx + 7)(using Column.columnToScalaBigDecimal),
  ...
  given writes: OWrites[BillofmaterialsRow] = {
