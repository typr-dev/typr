...
class ProductcosthistoryRepoImpl extends ProductcosthistoryRepo {
  ...
  override def deleteById(compositeId: ProductcosthistoryId)(using c: Connection): Boolean = SQL"""delete from "production"."productcosthistory" where "productid" = ${ParameterValue(compositeId.productid, null, ProductId.toStatement)} AND "startdate" = ${ParameterValue(compositeId.startdate, null, TypoLocalDateTime.toStatement)}""".executeUpdate() > 0
