...
case class WorkorderroutingRepoMock(
  ...
  override def deleteByIds(compositeIds: Array[WorkorderroutingId])(using c: Connection): Int = compositeIds.map(id => map.remove(id)).count(_.isDefined)
  ...
  override def insert(unsaved: WorkorderroutingRowUnsaved)(using c: Connection): WorkorderroutingRow = insert(toRow(unsaved))
  ...
  override def selectByIds(compositeIds: Array[WorkorderroutingId])(using c: Connection): List[WorkorderroutingRow] = compositeIds.flatMap(map.get).toList
