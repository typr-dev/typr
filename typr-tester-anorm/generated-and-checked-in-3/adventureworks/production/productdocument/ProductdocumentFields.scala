...
trait ProductdocumentFields {
  ...
  def compositeIdIn(compositeIds: Array[ProductdocumentId]): SqlExpr[Boolean] =
    new CompositeIn(compositeIds)(TuplePart[ProductdocumentId](productid)(_.productid)(using as[Array[ProductId]](using ToParameterValue(using null, ProductId.arrayToStatement), adventureworks.arrayParameterMetaData(using ProductId.parameterMetadata)), implicitly), TuplePart[ProductdocumentId](documentnode)(_.documentnode)(using as[Array[DocumentId]](using ToParameterValue(using null, DocumentId.arrayToStatement), adventureworks.arrayParameterMetaData(using DocumentId.parameterMetadata)), implicitly))

}

object ProductdocumentFields {
  lazy val structure: RelationStructure[ProductdocumentFields, ProductdocumentRow] =
    new Impl(List())

  private final class Impl(val _path: List[Path])
    extends RelationStructure[ProductdocumentFields, ProductdocumentRow] {
