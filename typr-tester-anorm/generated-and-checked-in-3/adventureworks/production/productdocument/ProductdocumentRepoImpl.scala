...
class ProductdocumentRepoImpl extends ProductdocumentRepo {
  ...
  override def deleteById(compositeId: ProductdocumentId)(using c: Connection): Boolean = SQL"""delete from "production"."productdocument" where "productid" = ${ParameterValue(compositeId.productid, null, ProductId.toStatement)} AND "documentnode" = ${ParameterValue(compositeId.documentnode, null, DocumentId.toStatement)}""".executeUpdate() > 0
  ...
  override def insertStreaming(
    ...
    batchSize: Int = 10000
  )(using c: Connection): Long = streamingInsert(s"""COPY "production"."productdocument"("productid", "modifieddate", "documentnode") FROM STDIN""", batchSize, unsaved)(using ProductdocumentRow.pgText, c)

  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(
    unsaved: Iterator[ProductdocumentRowUnsaved],
    batchSize: Int = 10000
  )(using c: Connection): Long = streamingInsert(s"""COPY "production"."productdocument"("productid", "modifieddate", "documentnode") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(using ProductdocumentRowUnsaved.pgText, c)
  ...
  override def upsertStreaming(
    ...
    streamingInsert(s"""copy productdocument_TEMP("productid", "modifieddate", "documentnode") from stdin""", batchSize, unsaved)(using ProductdocumentRow.pgText, c): @nowarn
    SQL"""insert into "production"."productdocument"("productid", "modifieddate", "documentnode")
