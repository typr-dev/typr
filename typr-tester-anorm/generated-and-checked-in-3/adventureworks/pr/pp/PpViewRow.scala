/**
 * File has been automatically generated by `typo`.
 *
 * IF YOU CHANGE THIS FILE YOUR CHANGES WILL BE OVERWRITTEN.
 */
package adventureworks.pr.pp

import adventureworks.customtypes.TypoBytea
import adventureworks.customtypes.TypoLocalDateTime
import adventureworks.production.productphoto.ProductphotoId
import anorm.Column
import anorm.RowParser
import anorm.Success
import play.api.libs.json.JsObject
import play.api.libs.json.JsResult
import play.api.libs.json.JsValue
import play.api.libs.json.OWrites
import play.api.libs.json.Reads
import play.api.libs.json.Writes
import scala.collection.immutable.ListMap
import scala.util.Try

/** View: pr.pp */
case class PpViewRow(
  /** Points to [[adventureworks.production.productphoto.ProductphotoRow.productphotoid]] */
  id: ProductphotoId,
  /** Points to [[adventureworks.production.productphoto.ProductphotoRow.productphotoid]] */
  productphotoid: ProductphotoId,
  /** Points to [[adventureworks.production.productphoto.ProductphotoRow.thumbnailphoto]] */
  thumbnailphoto: TypoBytea,
  /** Points to [[adventureworks.production.productphoto.ProductphotoRow.thumbnailphotofilename]] */
  thumbnailphotofilename: String,
  /** Points to [[adventureworks.production.productphoto.ProductphotoRow.largephoto]] */
  largephoto: TypoBytea,
  /** Points to [[adventureworks.production.productphoto.ProductphotoRow.largephotofilename]] */
  largephotofilename: String,
  /** Points to [[adventureworks.production.productphoto.ProductphotoRow.modifieddate]] */
  modifieddate: TypoLocalDateTime
)

object PpViewRow {
  given reads: Reads[PpViewRow] = {
    Reads[PpViewRow](json => JsResult.fromTry(
        Try(
          PpViewRow(
            id = json.\("id").as(using ProductphotoId.reads),
            productphotoid = json.\("productphotoid").as(using ProductphotoId.reads),
            thumbnailphoto = json.\("thumbnailphoto").as(using TypoBytea.reads),
            thumbnailphotofilename = json.\("thumbnailphotofilename").as(using Reads.StringReads),
            largephoto = json.\("largephoto").as(using TypoBytea.reads),
            largephotofilename = json.\("largephotofilename").as(using Reads.StringReads),
            modifieddate = json.\("modifieddate").as(using TypoLocalDateTime.reads)
          )
        )
      ),
    )
  }

  def rowParser(idx: Int): RowParser[PpViewRow] = {
    RowParser[PpViewRow] { row =>
      Success(
        PpViewRow(
          id = row(idx + 0)(using ProductphotoId.column),
          productphotoid = row(idx + 1)(using ProductphotoId.column),
          thumbnailphoto = row(idx + 2)(using TypoBytea.column),
          thumbnailphotofilename = row(idx + 3)(using Column.columnToString),
          largephoto = row(idx + 4)(using TypoBytea.column),
          largephotofilename = row(idx + 5)(using Column.columnToString),
          modifieddate = row(idx + 6)(using TypoLocalDateTime.column)
        )
      )
    }
  }

  given writes: OWrites[PpViewRow] = {
    OWrites[PpViewRow](o =>
      new JsObject(ListMap[String, JsValue](
        "id" -> ProductphotoId.writes.writes(o.id),
        "productphotoid" -> ProductphotoId.writes.writes(o.productphotoid),
        "thumbnailphoto" -> TypoBytea.writes.writes(o.thumbnailphoto),
        "thumbnailphotofilename" -> Writes.StringWrites.writes(o.thumbnailphotofilename),
        "largephoto" -> TypoBytea.writes.writes(o.largephoto),
        "largephotofilename" -> Writes.StringWrites.writes(o.largephotofilename),
        "modifieddate" -> TypoLocalDateTime.writes.writes(o.modifieddate)
      ))
    )
  }
}