...
object ThViewRow {
  given reads: Reads[ThViewRow] = {
    Reads[ThViewRow](json => JsResult.fromTry(
        Try(
          ThViewRow(
            id = json.\("id").as(using TransactionhistoryId.reads),
  ...
  def rowParser(idx: Int): RowParser[ThViewRow] = {
    RowParser[ThViewRow] { row =>
      Success(
        ThViewRow(
          id = row(idx + 0)(using TransactionhistoryId.column),
          transactionid = row(idx + 1)(using TransactionhistoryId.column),
  ...
  given writes: OWrites[ThViewRow] = {
    OWrites[ThViewRow](o =>
