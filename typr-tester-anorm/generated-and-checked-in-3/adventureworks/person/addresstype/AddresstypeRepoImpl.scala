...
class AddresstypeRepoImpl extends AddresstypeRepo {
  override def delete: DeleteBuilder[AddresstypeFields, AddresstypeRow] = DeleteBuilder.of(""""person"."addresstype"""", AddresstypeFields.structure, AddresstypeRow.rowParser(1).*)

  override def deleteById(addresstypeid: AddresstypeId)(using c: Connection): Boolean = SQL"""delete from "person"."addresstype" where "addresstypeid" = ${ParameterValue(addresstypeid, null, AddresstypeId.toStatement)}""".executeUpdate() > 0
  ...
  override def insertStreaming(
    ...
    batchSize: Int = 10000
  )(using c: Connection): Long = streamingInsert(s"""COPY "person"."addresstype"("addresstypeid", "name", "rowguid", "modifieddate") FROM STDIN""", batchSize, unsaved)(using AddresstypeRow.pgText, c)

  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(
    unsaved: Iterator[AddresstypeRowUnsaved],
    batchSize: Int = 10000
  )(using c: Connection): Long = streamingInsert(s"""COPY "person"."addresstype"("name", "addresstypeid", "rowguid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(using AddresstypeRowUnsaved.pgText, c)

  override def select: SelectBuilder[AddresstypeFields, AddresstypeRow] = SelectBuilder.of(""""person"."addresstype"""", AddresstypeFields.structure, AddresstypeRow.rowParser)
  ...
  override def update: UpdateBuilder[AddresstypeFields, AddresstypeRow] = UpdateBuilder.of(""""person"."addresstype"""", AddresstypeFields.structure, AddresstypeRow.rowParser(1).*)

  override def update(row: AddresstypeRow)(using c: Connection): Option[AddresstypeRow] = {
  ...
  override def upsertBatch(unsaved: Iterable[AddresstypeRow])(using c: Connection): List[AddresstypeRow] = {
  ...
  override def upsertStreaming(
    ...
    streamingInsert(s"""copy addresstype_TEMP("addresstypeid", "name", "rowguid", "modifieddate") from stdin""", batchSize, unsaved)(using AddresstypeRow.pgText, c): @nowarn
    SQL"""insert into "person"."addresstype"("addresstypeid", "name", "rowguid", "modifieddate")
