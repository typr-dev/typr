...
case class PhonenumbertypeRepoMock(
  ...
  override def deleteByIds(phonenumbertypeids: Array[PhonenumbertypeId])(using c: Connection): Int = phonenumbertypeids.map(id => map.remove(id)).count(_.isDefined)
  ...
  override def insert(unsaved: PhonenumbertypeRowUnsaved)(using c: Connection): PhonenumbertypeRow = insert(toRow(unsaved))
  ...
  override def selectByIds(phonenumbertypeids: Array[PhonenumbertypeId])(using c: Connection): List[PhonenumbertypeRow] = phonenumbertypeids.flatMap(map.get).toList
