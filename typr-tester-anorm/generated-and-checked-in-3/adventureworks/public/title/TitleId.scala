...
object TitleId {
  ...
  given arrayColumn: Column[Array[TitleId]] = Column.columnToArray[String](using Column.columnToString, summon).map(_.map(TitleId.apply))
  ...
  given parameterMetadata: ParameterMetaData[TitleId] = {
    new ParameterMetaData[TitleId] {
  ...
  given reads: Reads[TitleId] = Reads[TitleId]{(value: JsValue) => value.validate(using Reads.StringReads).map(TitleId.apply)}
