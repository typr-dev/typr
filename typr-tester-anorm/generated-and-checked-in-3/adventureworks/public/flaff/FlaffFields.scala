...
trait FlaffFields {
  ...
  def compositeIdIn(compositeIds: Array[FlaffId]): SqlExpr[Boolean] =
    new CompositeIn(compositeIds)(TuplePart[FlaffId](code)(_.code)(using as[Array[ShortText]](using ToParameterValue(using null, ShortText.arrayToStatement), adventureworks.arrayParameterMetaData(using ShortText.parameterMetadata)), implicitly), TuplePart[FlaffId](anotherCode)(_.anotherCode)(using as[Array[String]](using ToParameterValue(using null, ToStatement.arrayToParameter(using ParameterMetaData.StringParameterMetaData)), adventureworks.arrayParameterMetaData(using ParameterMetaData.StringParameterMetaData)), implicitly), TuplePart[FlaffId](someNumber)(_.someNumber)(using as[Array[Int]](using ToParameterValue(using null, adventureworks.IntArrayToStatement), adventureworks.arrayParameterMetaData(using ParameterMetaData.IntParameterMetaData)), implicitly), TuplePart[FlaffId](specifier)(_.specifier)(using as[Array[ShortText]](using ToParameterValue(using null, ShortText.arrayToStatement), adventureworks.arrayParameterMetaData(using ShortText.parameterMetadata)), implicitly))
