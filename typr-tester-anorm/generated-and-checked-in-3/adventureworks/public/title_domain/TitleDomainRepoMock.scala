...
case class TitleDomainRepoMock(map: scala.collection.mutable.Map[TitleDomainId, TitleDomainRow] = scala.collection.mutable.Map.empty[TitleDomainId, TitleDomainRow]) extends TitleDomainRepo {
  ...
  override def deleteByIds(codes: Array[TitleDomainId])(using c: Connection): Int = codes.map(id => map.remove(id)).count(_.isDefined)
  ...
  override def selectByIds(codes: Array[TitleDomainId])(using c: Connection): List[TitleDomainRow] = codes.flatMap(map.get).toList
