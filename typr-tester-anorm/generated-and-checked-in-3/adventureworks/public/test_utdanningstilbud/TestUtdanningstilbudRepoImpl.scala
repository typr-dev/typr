...
class TestUtdanningstilbudRepoImpl extends TestUtdanningstilbudRepo {
  ...
  override def deleteById(compositeId: TestUtdanningstilbudId)(using c: Connection): Boolean = SQL"""delete from "public"."test_utdanningstilbud" where "organisasjonskode" = ${ParameterValue(compositeId.organisasjonskode, null, TestOrganisasjonId.toStatement)} AND "utdanningsmulighet_kode" = ${ParameterValue(compositeId.utdanningsmulighetKode, null, ToStatement.stringToStatement)}""".executeUpdate() > 0
  ...
  override def deleteByIds(compositeIds: Array[TestUtdanningstilbudId])(using c: Connection): Int = {
    ...
    in (select unnest(${ParameterValue(organisasjonskode, null, TestOrganisasjonId.arrayToStatement)}), unnest(${ParameterValue(utdanningsmulighetKode, null, ToStatement.arrayToParameter(using ParameterMetaData.StringParameterMetaData))}))
    """.executeUpdate()
  }
  ...
  override def insertStreaming(
    ...
    batchSize: Int = 10000
  )(using c: Connection): Long = streamingInsert(s"""COPY "public"."test_utdanningstilbud"("organisasjonskode", "utdanningsmulighet_kode") FROM STDIN""", batchSize, unsaved)(using TestUtdanningstilbudRow.pgText, c)
  ...
  override def upsertBatch(unsaved: Iterable[TestUtdanningstilbudRow])(using c: Connection): List[TestUtdanningstilbudRow] = {
  ...
  override def upsertStreaming(
    ...
    streamingInsert(s"""copy test_utdanningstilbud_TEMP("organisasjonskode", "utdanningsmulighet_kode") from stdin""", batchSize, unsaved)(using TestUtdanningstilbudRow.pgText, c): @nowarn
    SQL"""insert into "public"."test_utdanningstilbud"("organisasjonskode", "utdanningsmulighet_kode")
