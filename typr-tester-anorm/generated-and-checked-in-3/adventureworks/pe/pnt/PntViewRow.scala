/**
 * File has been automatically generated by `typo`.
 *
 * IF YOU CHANGE THIS FILE YOUR CHANGES WILL BE OVERWRITTEN.
 */
package adventureworks.pe.pnt

import adventureworks.customtypes.TypoLocalDateTime
import adventureworks.person.phonenumbertype.PhonenumbertypeId
import adventureworks.public.Name
import anorm.RowParser
import anorm.Success
import play.api.libs.json.JsObject
import play.api.libs.json.JsResult
import play.api.libs.json.JsValue
import play.api.libs.json.OWrites
import play.api.libs.json.Reads
import scala.collection.immutable.ListMap
import scala.util.Try

/** View: pe.pnt */
case class PntViewRow(
  /** Points to [[adventureworks.person.phonenumbertype.PhonenumbertypeRow.phonenumbertypeid]] */
  id: PhonenumbertypeId,
  /** Points to [[adventureworks.person.phonenumbertype.PhonenumbertypeRow.phonenumbertypeid]] */
  phonenumbertypeid: PhonenumbertypeId,
  /** Points to [[adventureworks.person.phonenumbertype.PhonenumbertypeRow.name]] */
  name: Name,
  /** Points to [[adventureworks.person.phonenumbertype.PhonenumbertypeRow.modifieddate]] */
  modifieddate: TypoLocalDateTime
)

object PntViewRow {
  given reads: Reads[PntViewRow] = {
    Reads[PntViewRow](json => JsResult.fromTry(
        Try(
          PntViewRow(
            id = json.\("id").as(using PhonenumbertypeId.reads),
            phonenumbertypeid = json.\("phonenumbertypeid").as(using PhonenumbertypeId.reads),
            name = json.\("name").as(using Name.reads),
            modifieddate = json.\("modifieddate").as(using TypoLocalDateTime.reads)
          )
        )
      ),
    )
  }

  def rowParser(idx: Int): RowParser[PntViewRow] = {
    RowParser[PntViewRow] { row =>
      Success(
        PntViewRow(
          id = row(idx + 0)(using PhonenumbertypeId.column),
          phonenumbertypeid = row(idx + 1)(using PhonenumbertypeId.column),
          name = row(idx + 2)(using Name.column),
          modifieddate = row(idx + 3)(using TypoLocalDateTime.column)
        )
      )
    }
  }

  given writes: OWrites[PntViewRow] = {
    OWrites[PntViewRow](o =>
      new JsObject(ListMap[String, JsValue](
        "id" -> PhonenumbertypeId.writes.writes(o.id),
        "phonenumbertypeid" -> PhonenumbertypeId.writes.writes(o.phonenumbertypeid),
        "name" -> Name.writes.writes(o.name),
        "modifieddate" -> TypoLocalDateTime.writes.writes(o.modifieddate)
      ))
    )
  }
}