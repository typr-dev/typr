...
object TypoLine {
  given arrayColumn: Column[Array[TypoLine]] = {
    Column.nonNull[Array[TypoLine]]((v1: Any, _) =>
      v1 match {
          case v: PgArray =>
           v.getArray match {
             case v: Array[?] =>
               Right(v.map(v => new TypoLine(v.asInstanceOf[PGline].a, v.asInstanceOf[PGline].b, v.asInstanceOf[PGline].c)))
  ...
  given arrayToStatement: ToStatement[Array[TypoLine]] = ToStatement[Array[TypoLine]]((s, index, v) => s.setArray(index, s.getConnection.createArrayOf("line", v.map(v => new PGline(v.a, v.b, v.c)))))

  given column: Column[TypoLine] = {
    Column.nonNull[TypoLine]((v1: Any, _) =>
      v1 match {
        case v: PGline => Right(new TypoLine(v.a, v.b, v.c))
        case other => Left(TypeDoesNotMatch(s"Expected instance of org.postgresql.geometric.PGline, got ${other.getClass.getName}"))
      }
    )
  }

  given parameterMetadata: ParameterMetaData[TypoLine] = {
  ...
  given reads: Reads[TypoLine] = {
    Reads[TypoLine](json => JsResult.fromTry(
        Try(
          TypoLine(
            a = json.\("a").as(using Reads.DoubleReads),
            b = json.\("b").as(using Reads.DoubleReads),
            c = json.\("c").as(using Reads.DoubleReads)
          )
        )
      ),
    )
  ...
  given toStatement: ToStatement[TypoLine] = ToStatement[TypoLine]((s, index, v) => s.setObject(index, new PGline(v.a, v.b, v.c)))

  given writes: OWrites[TypoLine] = {
    OWrites[TypoLine](o =>
