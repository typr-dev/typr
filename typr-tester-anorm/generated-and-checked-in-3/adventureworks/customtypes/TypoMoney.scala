...
object TypoMoney {
  given arrayColumn: Column[Array[TypoMoney]] = {
    Column.nonNull[Array[TypoMoney]]((v1: Any, _) =>
      v1 match {
          case v: PgArray =>
           v.getArray match {
             case v: Array[?] =>
               Right(v.map(v => new TypoMoney(BigDecimal(v.asInstanceOf[java.math.BigDecimal]))))
  ...
  given arrayToStatement: ToStatement[Array[TypoMoney]] = ToStatement[Array[TypoMoney]]((s, index, v) => s.setArray(index, s.getConnection.createArrayOf("money", v.map(v => v.value.bigDecimal))))

  given bijection: Bijection[TypoMoney, BigDecimal] = Bijection.apply[TypoMoney, BigDecimal](_.value)(TypoMoney.apply)

  given column: Column[TypoMoney] = {
    Column.nonNull[TypoMoney]((v1: Any, _) =>
      v1 match {
        case v: java.math.BigDecimal => Right(new TypoMoney(BigDecimal(v)))
        case other => Left(TypeDoesNotMatch(s"Expected instance of java.math.BigDecimal, got ${other.getClass.getName}"))
      }
    )
  }

  given parameterMetadata: ParameterMetaData[TypoMoney] = {
  ...
  given reads: Reads[TypoMoney] = Reads.bigDecReads.map(TypoMoney.apply)

  given toStatement: ToStatement[TypoMoney] = ToStatement[TypoMoney]((s, index, v) => s.setObject(index, v.value.bigDecimal))

  given writes: Writes[TypoMoney] = Writes.BigDecimalWrites.contramap(_.value)
}