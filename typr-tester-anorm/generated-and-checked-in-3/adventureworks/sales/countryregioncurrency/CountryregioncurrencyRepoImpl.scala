...
class CountryregioncurrencyRepoImpl extends CountryregioncurrencyRepo {
  ...
  override def deleteById(compositeId: CountryregioncurrencyId)(using c: Connection): Boolean = SQL"""delete from "sales"."countryregioncurrency" where "countryregioncode" = ${ParameterValue(compositeId.countryregioncode, null, CountryregionId.toStatement)} AND "currencycode" = ${ParameterValue(compositeId.currencycode, null, CurrencyId.toStatement)}""".executeUpdate() > 0
