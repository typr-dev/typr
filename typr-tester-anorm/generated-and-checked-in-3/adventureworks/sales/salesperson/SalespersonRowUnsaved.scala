    ...
    Reads[SalespersonRowUnsaved](json => JsResult.fromTry(
        Try(
          SalespersonRowUnsaved(
            ...
            salesquota = json.\("salesquota").toOption.map(_.as(using Reads.bigDecReads)),
            bonus = json.\("bonus").as(using Defaulted.reads(using Reads.bigDecReads)),
            commissionpct = json.\("commissionpct").as(using Defaulted.reads(using Reads.bigDecReads)),
