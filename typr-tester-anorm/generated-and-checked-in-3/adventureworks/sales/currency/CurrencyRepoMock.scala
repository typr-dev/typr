...
case class CurrencyRepoMock(
  ...
  override def deleteByIds(currencycodes: Array[CurrencyId])(using c: Connection): Int = currencycodes.map(id => map.remove(id)).count(_.isDefined)
  ...
  override def insert(unsaved: CurrencyRowUnsaved)(using c: Connection): CurrencyRow = insert(toRow(unsaved))
  ...
  override def selectByIds(currencycodes: Array[CurrencyId])(using c: Connection): List[CurrencyRow] = currencycodes.flatMap(map.get).toList
