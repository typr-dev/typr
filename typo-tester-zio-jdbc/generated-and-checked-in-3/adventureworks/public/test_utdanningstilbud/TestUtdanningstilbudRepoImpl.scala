...
class TestUtdanningstilbudRepoImpl extends TestUtdanningstilbudRepo {
  ...
  override def insertStreaming(unsaved: ZStream[ZConnection, Throwable, TestUtdanningstilbudRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "public"."test_utdanningstilbud"("organisasjonskode", "utdanningsmulighet_kode") FROM STDIN""", batchSize, unsaved)(using TestUtdanningstilbudRow.text)
  }
  ...
  override def selectById(compositeId: TestUtdanningstilbudId): ZIO[ZConnection, Throwable, Option[TestUtdanningstilbudRow]] = {
    sql"""select "organisasjonskode", "utdanningsmulighet_kode" from "public"."test_utdanningstilbud" where "organisasjonskode" = ${Segment.paramSegment(compositeId.organisasjonskode)(using TestOrganisasjonId.setter)} AND "utdanningsmulighet_kode" = ${Segment.paramSegment(compositeId.utdanningsmulighetKode)(using Setter.stringSetter)}""".query(using TestUtdanningstilbudRow.jdbcDecoder).selectOne
  }
  ...
  override def upsertStreaming(unsaved: ZStream[ZConnection, Throwable, TestUtdanningstilbudRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    ...
    val copied = streamingInsert(s"""copy test_utdanningstilbud_TEMP("organisasjonskode", "utdanningsmulighet_kode") from stdin""", batchSize, unsaved)(using TestUtdanningstilbudRow.text)
