...
class TitleDomainRepoImpl extends TitleDomainRepo {
  ...
  override def deleteById(code: TitleDomainId): ZIO[ZConnection, Throwable, Boolean] = {
    sql"""delete from "public"."title_domain" where "code" = ${Segment.paramSegment(code)(using TitleDomainId.setter)}""".delete.map(_ > 0)
  }
  ...
  override def insertStreaming(unsaved: ZStream[ZConnection, Throwable, TitleDomainRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "public"."title_domain"("code") FROM STDIN""", batchSize, unsaved)(using TitleDomainRow.text)
  }
  ...
  override def selectById(code: TitleDomainId): ZIO[ZConnection, Throwable, Option[TitleDomainRow]] = {
    sql"""select "code" from "public"."title_domain" where "code" = ${Segment.paramSegment(code)(using TitleDomainId.setter)}""".query(using TitleDomainRow.jdbcDecoder).selectOne
  }
  override def selectByIds(codes: Array[TitleDomainId]): ZStream[ZConnection, Throwable, TitleDomainRow] = {
    sql"""select "code" from "public"."title_domain" where "code" = ANY(${Segment.paramSegment(codes)(using TitleDomainId.arraySetter)})""".query(using TitleDomainRow.jdbcDecoder).selectStream()
  }
  ...
  override def upsertStreaming(unsaved: ZStream[ZConnection, Throwable, TitleDomainRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    ...
    val copied = streamingInsert(s"""copy title_domain_TEMP("code") from stdin""", batchSize, unsaved)(using TitleDomainRow.text)
