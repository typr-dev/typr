...
class TitleRepoImpl extends TitleRepo {
  ...
  override def insertStreaming(unsaved: ZStream[ZConnection, Throwable, TitleRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "public"."title"("code") FROM STDIN""", batchSize, unsaved)(using TitleRow.text)
  }
  ...
  override def selectById(code: TitleId): ZIO[ZConnection, Throwable, Option[TitleRow]] = {
    sql"""select "code" from "public"."title" where "code" = ${Segment.paramSegment(code)(using TitleId.setter)}""".query(using TitleRow.jdbcDecoder).selectOne
  }
  override def selectByIds(codes: Array[TitleId]): ZStream[ZConnection, Throwable, TitleRow] = {
    sql"""select "code" from "public"."title" where "code" = ANY(${Segment.paramSegment(codes)(using TitleId.arraySetter)})""".query(using TitleRow.jdbcDecoder).selectStream()
  }
  ...
  override def upsertStreaming(unsaved: ZStream[ZConnection, Throwable, TitleRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    ...
    val copied = streamingInsert(s"""copy title_TEMP("code") from stdin""", batchSize, unsaved)(using TitleRow.text)
