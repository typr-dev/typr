...
class IdentityTestRepoImpl extends IdentityTestRepo {
  ...
  override def deleteById(name: IdentityTestId): ZIO[ZConnection, Throwable, Boolean] = {
    sql"""delete from "public"."identity-test" where "name" = ${Segment.paramSegment(name)(using IdentityTestId.setter)}""".delete.map(_ > 0)
  }
  ...
  override def insert(unsaved: IdentityTestRowUnsaved): ZIO[ZConnection, Throwable, IdentityTestRow] = {
    val fs = List(
      ...
      unsaved.defaultGenerated match {
        ...
        case Defaulted.Provided(value) => Some((sql""""default_generated"""", sql"${Segment.paramSegment(value: Int)(using Setter.intSetter)}::int4"))
      }
    ).flatten
  ...
  override def insertStreaming(unsaved: ZStream[ZConnection, Throwable, IdentityTestRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "public"."identity-test"("default_generated", "name") FROM STDIN""", batchSize, unsaved)(using IdentityTestRow.text)
  }
  ...
  override def insertUnsavedStreaming(unsaved: ZStream[ZConnection, Throwable, IdentityTestRowUnsaved], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "public"."identity-test"("name", "default_generated") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(using IdentityTestRowUnsaved.text)
  }
  ...
  override def selectById(name: IdentityTestId): ZIO[ZConnection, Throwable, Option[IdentityTestRow]] = {
    sql"""select "always_generated", "default_generated", "name" from "public"."identity-test" where "name" = ${Segment.paramSegment(name)(using IdentityTestId.setter)}""".query(using IdentityTestRow.jdbcDecoder).selectOne
  }
  override def selectByIds(names: Array[IdentityTestId]): ZStream[ZConnection, Throwable, IdentityTestRow] = {
    sql"""select "always_generated", "default_generated", "name" from "public"."identity-test" where "name" = ANY(${Segment.paramSegment(names)(using IdentityTestId.arraySetter)})""".query(using IdentityTestRow.jdbcDecoder).selectStream()
  }
  ...
  override def update(row: IdentityTestRow): ZIO[ZConnection, Throwable, Option[IdentityTestRow]] = {
    ...
    sql"""update "public"."identity-test"
          ...
          where "name" = ${Segment.paramSegment(name)(using IdentityTestId.setter)}
          returning "always_generated", "default_generated", "name""""
      .query(using IdentityTestRow.jdbcDecoder)
      .selectOne
  }
  override def upsert(unsaved: IdentityTestRow): ZIO[ZConnection, Throwable, UpdateResult[IdentityTestRow]] = {
    sql"""insert into "public"."identity-test"("default_generated", "name")
          values (
            ...
            ${Segment.paramSegment(unsaved.name)(using IdentityTestId.setter)}
          )
          on conflict ("name")
          do update set
  ...
  override def upsertStreaming(unsaved: ZStream[ZConnection, Throwable, IdentityTestRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    ...
    val copied = streamingInsert(s"""copy identity-test_TEMP("default_generated", "name") from stdin""", batchSize, unsaved)(using IdentityTestRow.text)
