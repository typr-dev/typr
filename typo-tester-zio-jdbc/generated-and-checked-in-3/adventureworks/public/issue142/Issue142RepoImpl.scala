...
class Issue142RepoImpl extends Issue142Repo {
  ...
  override def deleteById(tabellkode: Issue142Id): ZIO[ZConnection, Throwable, Boolean] = {
    sql"""delete from "public"."issue142" where "tabellkode" = ${Segment.paramSegment(tabellkode)(using Issue142Id.setter)}""".delete.map(_ > 0)
  }
  ...
  override def insertStreaming(unsaved: ZStream[ZConnection, Throwable, Issue142Row], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "public"."issue142"("tabellkode") FROM STDIN""", batchSize, unsaved)(using Issue142Row.text)
  }
  ...
  override def selectById(tabellkode: Issue142Id): ZIO[ZConnection, Throwable, Option[Issue142Row]] = {
    sql"""select "tabellkode" from "public"."issue142" where "tabellkode" = ${Segment.paramSegment(tabellkode)(using Issue142Id.setter)}""".query(using Issue142Row.jdbcDecoder).selectOne
  }
  override def selectByIds(tabellkodes: Array[Issue142Id]): ZStream[ZConnection, Throwable, Issue142Row] = {
    sql"""select "tabellkode" from "public"."issue142" where "tabellkode" = ANY(${Segment.paramSegment(tabellkodes)(using Issue142Id.arraySetter)})""".query(using Issue142Row.jdbcDecoder).selectStream()
  }
  ...
  override def upsertStreaming(unsaved: ZStream[ZConnection, Throwable, Issue142Row], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    ...
    val copied = streamingInsert(s"""copy issue142_TEMP("tabellkode") from stdin""", batchSize, unsaved)(using Issue142Row.text)
