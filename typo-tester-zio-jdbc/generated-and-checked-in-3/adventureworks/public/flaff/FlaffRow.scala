...
object FlaffRow {
  ...
  given jdbcDecoder: JdbcDecoder[FlaffRow] = new JdbcDecoder[FlaffRow] {
    override def unsafeDecode(columIndex: Int, rs: ResultSet): (Int, FlaffRow) =
      columIndex + 4 ->
        FlaffRow(
          ...
          parentspecifier = JdbcDecoder.optionDecoder(using ShortText.jdbcDecoder).unsafeDecode(columIndex + 4, rs)._2
        )
  }
  given jsonDecoder: JsonDecoder[FlaffRow] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
    ...
    val parentspecifier = jsonObj.get("parentspecifier").fold[Either[String, Option[ShortText]]](Right(None))(_.as(using JsonDecoder.option(using ShortText.jsonDecoder)))
  ...
  given jsonEncoder: JsonEncoder[FlaffRow] = new JsonEncoder[FlaffRow] {
