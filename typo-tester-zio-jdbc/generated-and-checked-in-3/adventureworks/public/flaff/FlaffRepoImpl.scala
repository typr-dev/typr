...
class FlaffRepoImpl extends FlaffRepo {
  ...
  override def insertStreaming(unsaved: ZStream[ZConnection, Throwable, FlaffRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "public"."flaff"("code", "another_code", "some_number", "specifier", "parentspecifier") FROM STDIN""", batchSize, unsaved)(using FlaffRow.text)
  }
  ...
  override def selectById(compositeId: FlaffId): ZIO[ZConnection, Throwable, Option[FlaffRow]] = {
    sql"""select "code", "another_code", "some_number", "specifier", "parentspecifier" from "public"."flaff" where "code" = ${Segment.paramSegment(compositeId.code)(using ShortText.setter)} AND "another_code" = ${Segment.paramSegment(compositeId.anotherCode)(using Setter.stringSetter)} AND "some_number" = ${Segment.paramSegment(compositeId.someNumber)(using Setter.intSetter)} AND "specifier" = ${Segment.paramSegment(compositeId.specifier)(using ShortText.setter)}""".query(using FlaffRow.jdbcDecoder).selectOne
  }
  ...
  override def upsertStreaming(unsaved: ZStream[ZConnection, Throwable, FlaffRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    ...
    val copied = streamingInsert(s"""copy flaff_TEMP("code", "another_code", "some_number", "specifier", "parentspecifier") from stdin""", batchSize, unsaved)(using FlaffRow.text)
