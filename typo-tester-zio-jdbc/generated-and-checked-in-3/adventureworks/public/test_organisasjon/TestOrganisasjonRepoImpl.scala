...
class TestOrganisasjonRepoImpl extends TestOrganisasjonRepo {
  ...
  override def deleteById(organisasjonskode: TestOrganisasjonId): ZIO[ZConnection, Throwable, Boolean] = {
    sql"""delete from "public"."test_organisasjon" where "organisasjonskode" = ${Segment.paramSegment(organisasjonskode)(using TestOrganisasjonId.setter)}""".delete.map(_ > 0)
  }
  ...
  override def insertStreaming(unsaved: ZStream[ZConnection, Throwable, TestOrganisasjonRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "public"."test_organisasjon"("organisasjonskode") FROM STDIN""", batchSize, unsaved)(using TestOrganisasjonRow.text)
  }
  ...
  override def selectById(organisasjonskode: TestOrganisasjonId): ZIO[ZConnection, Throwable, Option[TestOrganisasjonRow]] = {
    sql"""select "organisasjonskode" from "public"."test_organisasjon" where "organisasjonskode" = ${Segment.paramSegment(organisasjonskode)(using TestOrganisasjonId.setter)}""".query(using TestOrganisasjonRow.jdbcDecoder).selectOne
  }
  override def selectByIds(organisasjonskodes: Array[TestOrganisasjonId]): ZStream[ZConnection, Throwable, TestOrganisasjonRow] = {
    sql"""select "organisasjonskode" from "public"."test_organisasjon" where "organisasjonskode" = ANY(${Segment.paramSegment(organisasjonskodes)(using TestOrganisasjonId.arraySetter)})""".query(using TestOrganisasjonRow.jdbcDecoder).selectStream()
  }
  ...
  override def upsertStreaming(unsaved: ZStream[ZConnection, Throwable, TestOrganisasjonRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    ...
    val copied = streamingInsert(s"""copy test_organisasjon_TEMP("organisasjonskode") from stdin""", batchSize, unsaved)(using TestOrganisasjonRow.text)
