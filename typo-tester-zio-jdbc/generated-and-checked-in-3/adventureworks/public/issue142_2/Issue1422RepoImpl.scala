...
class Issue1422RepoImpl extends Issue1422Repo {
  ...
  override def insertStreaming(unsaved: ZStream[ZConnection, Throwable, Issue1422Row], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "public"."issue142_2"("tabellkode") FROM STDIN""", batchSize, unsaved)(using Issue1422Row.text)
  }
  ...
  override def selectById(tabellkode: Issue142Id): ZIO[ZConnection, Throwable, Option[Issue1422Row]] = {
    sql"""select "tabellkode" from "public"."issue142_2" where "tabellkode" = ${Segment.paramSegment(tabellkode)(using Issue142Id.setter)}""".query(using Issue1422Row.jdbcDecoder).selectOne
  }
  override def selectByIds(tabellkodes: Array[Issue142Id]): ZStream[ZConnection, Throwable, Issue1422Row] = {
    sql"""select "tabellkode" from "public"."issue142_2" where "tabellkode" = ANY(${Segment.paramSegment(tabellkodes)(using Issue142Id.arraySetter)})""".query(using Issue1422Row.jdbcDecoder).selectStream()
  }
  ...
  override def upsertStreaming(unsaved: ZStream[ZConnection, Throwable, Issue1422Row], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    ...
    val copied = streamingInsert(s"""copy issue142_2_TEMP("tabellkode") from stdin""", batchSize, unsaved)(using Issue1422Row.text)
