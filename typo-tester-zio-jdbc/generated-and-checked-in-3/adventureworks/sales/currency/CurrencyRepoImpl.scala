...
class CurrencyRepoImpl extends CurrencyRepo {
  ...
  override def deleteById(currencycode: CurrencyId): ZIO[ZConnection, Throwable, Boolean] = {
    sql"""delete from "sales"."currency" where "currencycode" = ${Segment.paramSegment(currencycode)(using CurrencyId.setter)}""".delete.map(_ > 0)
  }
  ...
  override def insertStreaming(unsaved: ZStream[ZConnection, Throwable, CurrencyRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "sales"."currency"("currencycode", "name", "modifieddate") FROM STDIN""", batchSize, unsaved)(using CurrencyRow.text)
  }
  ...
  override def insertUnsavedStreaming(unsaved: ZStream[ZConnection, Throwable, CurrencyRowUnsaved], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "sales"."currency"("currencycode", "name", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(using CurrencyRowUnsaved.text)
  }
  ...
  override def selectById(currencycode: CurrencyId): ZIO[ZConnection, Throwable, Option[CurrencyRow]] = {
    sql"""select "currencycode", "name", "modifieddate"::text from "sales"."currency" where "currencycode" = ${Segment.paramSegment(currencycode)(using CurrencyId.setter)}""".query(using CurrencyRow.jdbcDecoder).selectOne
  }
  override def selectByIds(currencycodes: Array[CurrencyId]): ZStream[ZConnection, Throwable, CurrencyRow] = {
    sql"""select "currencycode", "name", "modifieddate"::text from "sales"."currency" where "currencycode" = ANY(${Segment.paramSegment(currencycodes)(using CurrencyId.arraySetter)})""".query(using CurrencyRow.jdbcDecoder).selectStream()
  }
  ...
  override def upsertStreaming(unsaved: ZStream[ZConnection, Throwable, CurrencyRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    ...
    val copied = streamingInsert(s"""copy currency_TEMP("currencycode", "name", "modifieddate") from stdin""", batchSize, unsaved)(using CurrencyRow.text)
