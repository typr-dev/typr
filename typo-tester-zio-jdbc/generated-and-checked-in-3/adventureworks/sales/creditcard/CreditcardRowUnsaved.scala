...
object CreditcardRowUnsaved {
  given jsonDecoder: JsonDecoder[CreditcardRowUnsaved] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
    val cardtype = jsonObj.get("cardtype").toRight("Missing field 'cardtype'").flatMap(_.as(using JsonDecoder.string))
    val cardnumber = jsonObj.get("cardnumber").toRight("Missing field 'cardnumber'").flatMap(_.as(using JsonDecoder.string))
    val expmonth = jsonObj.get("expmonth").toRight("Missing field 'expmonth'").flatMap(_.as(using TypoShort.jsonDecoder))
    val expyear = jsonObj.get("expyear").toRight("Missing field 'expyear'").flatMap(_.as(using TypoShort.jsonDecoder))
    val creditcardid = jsonObj.get("creditcardid").toRight("Missing field 'creditcardid'").flatMap(_.as(using Defaulted.jsonDecoder(using CustomCreditcardId.jsonDecoder)))
  ...
  given jsonEncoder: JsonEncoder[CreditcardRowUnsaved] = new JsonEncoder[CreditcardRowUnsaved] {
    override def unsafeEncode(a: CreditcardRowUnsaved, indent: Option[Int], out: Write): Unit = {
      ...
      Defaulted.jsonEncoder(using CustomCreditcardId.jsonEncoder).unsafeEncode(a.creditcardid, indent, out)
      out.write(",")
