...
object CustomerRow {
  given jdbcDecoder: JdbcDecoder[CustomerRow] = new JdbcDecoder[CustomerRow] {
    override def unsafeDecode(columIndex: Int, rs: ResultSet): (Int, CustomerRow) =
      columIndex + 5 ->
        CustomerRow(
          ...
          personid = JdbcDecoder.optionDecoder(using BusinessentityId.jdbcDecoder).unsafeDecode(columIndex + 1, rs)._2,
          storeid = JdbcDecoder.optionDecoder(using BusinessentityId.jdbcDecoder).unsafeDecode(columIndex + 2, rs)._2,
          territoryid = JdbcDecoder.optionDecoder(using SalesterritoryId.jdbcDecoder).unsafeDecode(columIndex + 3, rs)._2,
  ...
  given jsonDecoder: JsonDecoder[CustomerRow] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
  ...
  given jsonEncoder: JsonEncoder[CustomerRow] = new JsonEncoder[CustomerRow] {
  ...
  given text: Text[CustomerRow] = Text.instance[CustomerRow]{ (row, sb) =>
