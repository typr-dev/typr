...
object CurrencyrateRowUnsaved {
  given jsonDecoder: JsonDecoder[CurrencyrateRowUnsaved] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
    val currencyratedate = jsonObj.get("currencyratedate").toRight("Missing field 'currencyratedate'").flatMap(_.as(using TypoLocalDateTime.jsonDecoder))
    val fromcurrencycode = jsonObj.get("fromcurrencycode").toRight("Missing field 'fromcurrencycode'").flatMap(_.as(using CurrencyId.jsonDecoder))
    val tocurrencycode = jsonObj.get("tocurrencycode").toRight("Missing field 'tocurrencycode'").flatMap(_.as(using CurrencyId.jsonDecoder))
    val averagerate = jsonObj.get("averagerate").toRight("Missing field 'averagerate'").flatMap(_.as(using JsonDecoder.scalaBigDecimal))
    val endofdayrate = jsonObj.get("endofdayrate").toRight("Missing field 'endofdayrate'").flatMap(_.as(using JsonDecoder.scalaBigDecimal))
    val currencyrateid = jsonObj.get("currencyrateid").toRight("Missing field 'currencyrateid'").flatMap(_.as(using Defaulted.jsonDecoder(using CurrencyrateId.jsonDecoder)))
  ...
  given jsonEncoder: JsonEncoder[CurrencyrateRowUnsaved] = new JsonEncoder[CurrencyrateRowUnsaved] {
    override def unsafeEncode(a: CurrencyrateRowUnsaved, indent: Option[Int], out: Write): Unit = {
      ...
      Defaulted.jsonEncoder(using CurrencyrateId.jsonEncoder).unsafeEncode(a.currencyrateid, indent, out)
      out.write(",")
