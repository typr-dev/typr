...
object StoreRow {
  given jdbcDecoder: JdbcDecoder[StoreRow] = new JdbcDecoder[StoreRow] {
    override def unsafeDecode(columIndex: Int, rs: ResultSet): (Int, StoreRow) =
      columIndex + 5 ->
        StoreRow(
          ...
          salespersonid = JdbcDecoder.optionDecoder(using BusinessentityId.jdbcDecoder).unsafeDecode(columIndex + 2, rs)._2,
          demographics = JdbcDecoder.optionDecoder(using TypoXml.jdbcDecoder).unsafeDecode(columIndex + 3, rs)._2,
  ...
  given jsonDecoder: JsonDecoder[StoreRow] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
  ...
  given jsonEncoder: JsonEncoder[StoreRow] = new JsonEncoder[StoreRow] {
