...
object SalestaxrateRowUnsaved {
  given jsonDecoder: JsonDecoder[SalestaxrateRowUnsaved] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
    ...
    val taxtype = jsonObj.get("taxtype").toRight("Missing field 'taxtype'").flatMap(_.as(using TypoShort.jsonDecoder))
    val name = jsonObj.get("name").toRight("Missing field 'name'").flatMap(_.as(using Name.jsonDecoder))
    val salestaxrateid = jsonObj.get("salestaxrateid").toRight("Missing field 'salestaxrateid'").flatMap(_.as(using Defaulted.jsonDecoder(using SalestaxrateId.jsonDecoder)))
    val taxrate = jsonObj.get("taxrate").toRight("Missing field 'taxrate'").flatMap(_.as(using Defaulted.jsonDecoder(using JsonDecoder.scalaBigDecimal)))
  ...
  given jsonEncoder: JsonEncoder[SalestaxrateRowUnsaved] = new JsonEncoder[SalestaxrateRowUnsaved] {
    override def unsafeEncode(a: SalestaxrateRowUnsaved, indent: Option[Int], out: Write): Unit = {
      ...
      Defaulted.jsonEncoder(using SalestaxrateId.jsonEncoder).unsafeEncode(a.salestaxrateid, indent, out)
      out.write(",")
      out.write(""""taxrate":""")
      Defaulted.jsonEncoder(using JsonEncoder.scalaBigDecimal).unsafeEncode(a.taxrate, indent, out)
      out.write(",")
      out.write(""""rowguid":""")
