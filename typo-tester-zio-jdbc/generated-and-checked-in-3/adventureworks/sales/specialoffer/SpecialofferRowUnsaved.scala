...
object SpecialofferRowUnsaved {
  given jsonDecoder: JsonDecoder[SpecialofferRowUnsaved] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
    val description = jsonObj.get("description").toRight("Missing field 'description'").flatMap(_.as(using JsonDecoder.string))
    val `type` = jsonObj.get("type").toRight("Missing field 'type'").flatMap(_.as(using JsonDecoder.string))
    val category = jsonObj.get("category").toRight("Missing field 'category'").flatMap(_.as(using JsonDecoder.string))
    ...
    val maxqty = jsonObj.get("maxqty").fold[Either[String, Option[Int]]](Right(None))(_.as(using JsonDecoder.option(using JsonDecoder.int)))
    val specialofferid = jsonObj.get("specialofferid").toRight("Missing field 'specialofferid'").flatMap(_.as(using Defaulted.jsonDecoder(using SpecialofferId.jsonDecoder)))
    val discountpct = jsonObj.get("discountpct").toRight("Missing field 'discountpct'").flatMap(_.as(using Defaulted.jsonDecoder(using JsonDecoder.scalaBigDecimal)))
    val minqty = jsonObj.get("minqty").toRight("Missing field 'minqty'").flatMap(_.as(using Defaulted.jsonDecoder(using JsonDecoder.int)))
  ...
  given jsonEncoder: JsonEncoder[SpecialofferRowUnsaved] = new JsonEncoder[SpecialofferRowUnsaved] {
    override def unsafeEncode(a: SpecialofferRowUnsaved, indent: Option[Int], out: Write): Unit = {
      ...
      Defaulted.jsonEncoder(using SpecialofferId.jsonEncoder).unsafeEncode(a.specialofferid, indent, out)
      out.write(",")
      out.write(""""discountpct":""")
      Defaulted.jsonEncoder(using JsonEncoder.scalaBigDecimal).unsafeEncode(a.discountpct, indent, out)
      out.write(",")
      out.write(""""minqty":""")
      Defaulted.jsonEncoder(using JsonEncoder.int).unsafeEncode(a.minqty, indent, out)
      out.write(",")
      out.write(""""rowguid":""")
