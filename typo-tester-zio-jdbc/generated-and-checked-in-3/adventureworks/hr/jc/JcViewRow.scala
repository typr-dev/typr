...
object JcViewRow {
  given jdbcDecoder: JdbcDecoder[JcViewRow] = new JdbcDecoder[JcViewRow] {
    override def unsafeDecode(columIndex: Int, rs: ResultSet): (Int, JcViewRow) =
      columIndex + 4 ->
        JcViewRow(
          ...
          businessentityid = JdbcDecoder.optionDecoder(using BusinessentityId.jdbcDecoder).unsafeDecode(columIndex + 2, rs)._2,
          resume = JdbcDecoder.optionDecoder(using TypoXml.jdbcDecoder).unsafeDecode(columIndex + 3, rs)._2,
          modifieddate = TypoLocalDateTime.jdbcDecoder.unsafeDecode(columIndex + 4, rs)._2
        )
  }
  given jsonDecoder: JsonDecoder[JcViewRow] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
  ...
  given jsonEncoder: JsonEncoder[JcViewRow] = new JsonEncoder[JcViewRow] {
