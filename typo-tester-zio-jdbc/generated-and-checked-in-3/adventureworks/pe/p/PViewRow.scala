    ...
    override def unsafeDecode(columIndex: Int, rs: ResultSet): (Int, PViewRow) =
      columIndex + 13 ->
        PViewRow(
          ...
          title = JdbcDecoder.optionDecoder(using JdbcDecoder.stringDecoder).unsafeDecode(columIndex + 4, rs)._2,
          firstname = FirstName.jdbcDecoder.unsafeDecode(columIndex + 5, rs)._2,
          middlename = JdbcDecoder.optionDecoder(using Name.jdbcDecoder).unsafeDecode(columIndex + 6, rs)._2,
          lastname = Name.jdbcDecoder.unsafeDecode(columIndex + 7, rs)._2,
          suffix = JdbcDecoder.optionDecoder(using JdbcDecoder.stringDecoder).unsafeDecode(columIndex + 8, rs)._2,
          emailpromotion = JdbcDecoder.intDecoder.unsafeDecode(columIndex + 9, rs)._2,
          additionalcontactinfo = JdbcDecoder.optionDecoder(using TypoXml.jdbcDecoder).unsafeDecode(columIndex + 10, rs)._2,
          demographics = JdbcDecoder.optionDecoder(using TypoXml.jdbcDecoder).unsafeDecode(columIndex + 11, rs)._2,
