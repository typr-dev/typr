...
object UmViewRow {
  given jdbcDecoder: JdbcDecoder[UmViewRow] = new JdbcDecoder[UmViewRow] {
  ...
  given jsonDecoder: JsonDecoder[UmViewRow] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
    val id = jsonObj.get("id").toRight("Missing field 'id'").flatMap(_.as(using UnitmeasureId.jsonDecoder))
  ...
  given jsonEncoder: JsonEncoder[UmViewRow] = new JsonEncoder[UmViewRow] {
