...
object PchViewRow {
  given jdbcDecoder: JdbcDecoder[PchViewRow] = new JdbcDecoder[PchViewRow] {
    override def unsafeDecode(columIndex: Int, rs: ResultSet): (Int, PchViewRow) =
      columIndex + 5 ->
        PchViewRow(
          ...
          enddate = JdbcDecoder.optionDecoder(using TypoLocalDateTime.jdbcDecoder).unsafeDecode(columIndex + 3, rs)._2,
  ...
  given jsonDecoder: JsonDecoder[PchViewRow] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
  ...
  given jsonEncoder: JsonEncoder[PchViewRow] = new JsonEncoder[PchViewRow] {
