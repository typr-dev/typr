...
object SoViewRow {
  given jdbcDecoder: JdbcDecoder[SoViewRow] = new JdbcDecoder[SoViewRow] {
    override def unsafeDecode(columIndex: Int, rs: ResultSet): (Int, SoViewRow) =
      columIndex + 11 ->
        SoViewRow(
          ...
          maxqty = JdbcDecoder.optionDecoder(using JdbcDecoder.intDecoder).unsafeDecode(columIndex + 9, rs)._2,
  ...
  given jsonDecoder: JsonDecoder[SoViewRow] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
    val id = jsonObj.get("id").toRight("Missing field 'id'").flatMap(_.as(using SpecialofferId.jsonDecoder))
  ...
  given jsonEncoder: JsonEncoder[SoViewRow] = new JsonEncoder[SoViewRow] {
