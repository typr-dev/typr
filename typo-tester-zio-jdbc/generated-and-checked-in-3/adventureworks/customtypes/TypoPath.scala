...
object TypoPath {
  given arrayJdbcDecoder: JdbcDecoder[Array[TypoPath]] = JdbcDecoder[Array[TypoPath]]((rs: ResultSet) => (i: Int) =>
    rs.getArray(i) match {
      case null => null
  ...
  given arraySetter: Setter[Array[TypoPath]] = Setter.forSqlType((ps, i, v) =>
    ps.setArray(
      i,
  ...
  given jdbcDecoder: JdbcDecoder[TypoPath] = JdbcDecoder[TypoPath](
    (rs: ResultSet) => (i: Int) => {
  ...
  given jsonDecoder: JsonDecoder[TypoPath] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
  ...
  given jsonEncoder: JsonEncoder[TypoPath] = new JsonEncoder[TypoPath] {
  ...
  given text: Text[TypoPath] = new Text[TypoPath] {
    ...
    override def unsafeArrayEncode(v: TypoPath, sb: StringBuilder): Unit = Text.stringInstance.unsafeArrayEncode(s"""${if (v.open) "[" else "("}${v.points.map(p => s"${p.x}, ${p.y}").mkString(",")}${if (v.open) "]" else ")"}""", sb)
  }
}
