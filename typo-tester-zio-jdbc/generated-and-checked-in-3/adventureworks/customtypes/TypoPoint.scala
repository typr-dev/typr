/**
 * File has been automatically generated by `typo`.
 *
 * IF YOU CHANGE THIS FILE YOUR CHANGES WILL BE OVERWRITTEN.
 */
package adventureworks
package customtypes

import java.sql.ResultSet
import java.sql.Types
import org.postgresql.geometric.PGpoint
import typo.dsl.PGType
import zio.jdbc.JdbcDecoder
import zio.jdbc.JdbcEncoder
import zio.jdbc.SqlFragment.Setter
import zio.json.JsonDecoder
import zio.json.JsonEncoder
import zio.json.ast.Json
import zio.json.internal.Write

/** Point datatype in PostgreSQL */
case class TypoPoint(x: Double, y: Double)

object TypoPoint {
  given arrayJdbcDecoder: JdbcDecoder[Array[TypoPoint]] = JdbcDecoder[Array[TypoPoint]]((rs: ResultSet) => (i: Int) =>
    rs.getArray(i) match {
      case null => null
      case arr => arr.getArray.asInstanceOf[Array[AnyRef]].map(x => TypoPoint(x.asInstanceOf[PGpoint].x, x.asInstanceOf[PGpoint].y))
    },
    "Array[org.postgresql.geometric.PGpoint]"
  )
  given arrayJdbcEncoder: JdbcEncoder[Array[TypoPoint]] = JdbcEncoder.singleParamEncoder(using arraySetter)
  given arraySetter: Setter[Array[TypoPoint]] = Setter.forSqlType((ps, i, v) =>
    ps.setArray(
      i,
      ps.getConnection.createArrayOf(
        "point",
        v.map { vv =>
          new PGpoint(vv.x, vv.y)
        }
      )
    ),
    Types.ARRAY
  )
  given jdbcDecoder: JdbcDecoder[TypoPoint] = JdbcDecoder[TypoPoint](
    (rs: ResultSet) => (i: Int) => {
      val v = rs.getObject(i)
      if (v eq null) null else TypoPoint(v.asInstanceOf[PGpoint].x, v.asInstanceOf[PGpoint].y)
    },
    "org.postgresql.geometric.PGpoint"
  )
  given jdbcEncoder: JdbcEncoder[TypoPoint] = JdbcEncoder.singleParamEncoder(using setter)
  given jsonDecoder: JsonDecoder[TypoPoint] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
    val x = jsonObj.get("x").toRight("Missing field 'x'").flatMap(_.as(using JsonDecoder.double))
    val y = jsonObj.get("y").toRight("Missing field 'y'").flatMap(_.as(using JsonDecoder.double))
    if (x.isRight && y.isRight)
      Right(TypoPoint(x = x.toOption.get, y = y.toOption.get))
    else Left(List[Either[String, Any]](x, y).flatMap(_.left.toOption).mkString(", "))
  }
  given jsonEncoder: JsonEncoder[TypoPoint] = new JsonEncoder[TypoPoint] {
    override def unsafeEncode(a: TypoPoint, indent: Option[Int], out: Write): Unit = {
      out.write("{")
      out.write(""""x":""")
      JsonEncoder.double.unsafeEncode(a.x, indent, out)
      out.write(",")
      out.write(""""y":""")
      JsonEncoder.double.unsafeEncode(a.y, indent, out)
      out.write("}")
    }
  }
  given ordering: Ordering[TypoPoint] = Ordering.by(x => (x.x, x.y))
  given pgType: PGType[TypoPoint] = PGType.instance[TypoPoint]("point", Types.OTHER)
  given setter: Setter[TypoPoint] = Setter.other(
    (ps, i, v) => {
      ps.setObject(
        i,
        new PGpoint(v.x, v.y)
      )
    },
    "point"
  )
  given text: Text[TypoPoint] = new Text[TypoPoint] {
    override def unsafeEncode(v: TypoPoint, sb: StringBuilder): Unit = Text.stringInstance.unsafeEncode(s"(${v.x},${v.y})", sb)
    override def unsafeArrayEncode(v: TypoPoint, sb: StringBuilder): Unit = Text.stringInstance.unsafeArrayEncode(s"(${v.x},${v.y})", sb)
  }
}
