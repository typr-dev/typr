...
object VemployeeViewRow {
  given jdbcDecoder: JdbcDecoder[VemployeeViewRow] = new JdbcDecoder[VemployeeViewRow] {
    override def unsafeDecode(columIndex: Int, rs: ResultSet): (Int, VemployeeViewRow) =
      columIndex + 17 ->
        VemployeeViewRow(
          ...
          additionalcontactinfo = JdbcDecoder.optionDecoder(using TypoXml.jdbcDecoder).unsafeDecode(columIndex + 17, rs)._2
        )
  }
  given jsonDecoder: JsonDecoder[VemployeeViewRow] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
  ...
  given jsonEncoder: JsonEncoder[VemployeeViewRow] = new JsonEncoder[VemployeeViewRow] {
