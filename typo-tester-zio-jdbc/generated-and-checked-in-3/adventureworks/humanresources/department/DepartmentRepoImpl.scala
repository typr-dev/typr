...
class DepartmentRepoImpl extends DepartmentRepo {
  ...
  override def deleteById(departmentid: DepartmentId): ZIO[ZConnection, Throwable, Boolean] = {
    sql"""delete from "humanresources"."department" where "departmentid" = ${Segment.paramSegment(departmentid)(using DepartmentId.setter)}""".delete.map(_ > 0)
  }
  ...
  override def insertStreaming(unsaved: ZStream[ZConnection, Throwable, DepartmentRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "humanresources"."department"("departmentid", "name", "groupname", "modifieddate") FROM STDIN""", batchSize, unsaved)(using DepartmentRow.text)
  }
  ...
  override def insertUnsavedStreaming(unsaved: ZStream[ZConnection, Throwable, DepartmentRowUnsaved], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "humanresources"."department"("name", "groupname", "departmentid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(using DepartmentRowUnsaved.text)
  }
  ...
  override def selectById(departmentid: DepartmentId): ZIO[ZConnection, Throwable, Option[DepartmentRow]] = {
    sql"""select "departmentid", "name", "groupname", "modifieddate"::text from "humanresources"."department" where "departmentid" = ${Segment.paramSegment(departmentid)(using DepartmentId.setter)}""".query(using DepartmentRow.jdbcDecoder).selectOne
  }
  override def selectByIds(departmentids: Array[DepartmentId]): ZStream[ZConnection, Throwable, DepartmentRow] = {
    sql"""select "departmentid", "name", "groupname", "modifieddate"::text from "humanresources"."department" where "departmentid" = ANY(${Segment.paramSegment(departmentids)(using DepartmentId.arraySetter)})""".query(using DepartmentRow.jdbcDecoder).selectStream()
  }
  ...
  override def upsertStreaming(unsaved: ZStream[ZConnection, Throwable, DepartmentRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    ...
    val copied = streamingInsert(s"""copy department_TEMP("departmentid", "name", "groupname", "modifieddate") from stdin""", batchSize, unsaved)(using DepartmentRow.text)
