...
class ShiftRepoImpl extends ShiftRepo {
  ...
  override def deleteById(shiftid: ShiftId): ZIO[ZConnection, Throwable, Boolean] = {
    sql"""delete from "humanresources"."shift" where "shiftid" = ${Segment.paramSegment(shiftid)(using ShiftId.setter)}""".delete.map(_ > 0)
  }
  ...
  override def insertStreaming(unsaved: ZStream[ZConnection, Throwable, ShiftRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "humanresources"."shift"("shiftid", "name", "starttime", "endtime", "modifieddate") FROM STDIN""", batchSize, unsaved)(using ShiftRow.text)
  }
  ...
  override def insertUnsavedStreaming(unsaved: ZStream[ZConnection, Throwable, ShiftRowUnsaved], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "humanresources"."shift"("name", "starttime", "endtime", "shiftid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(using ShiftRowUnsaved.text)
  }
  ...
  override def selectById(shiftid: ShiftId): ZIO[ZConnection, Throwable, Option[ShiftRow]] = {
    sql"""select "shiftid", "name", "starttime"::text, "endtime"::text, "modifieddate"::text from "humanresources"."shift" where "shiftid" = ${Segment.paramSegment(shiftid)(using ShiftId.setter)}""".query(using ShiftRow.jdbcDecoder).selectOne
  }
  override def selectByIds(shiftids: Array[ShiftId]): ZStream[ZConnection, Throwable, ShiftRow] = {
    sql"""select "shiftid", "name", "starttime"::text, "endtime"::text, "modifieddate"::text from "humanresources"."shift" where "shiftid" = ANY(${Segment.paramSegment(shiftids)(using ShiftId.arraySetter)})""".query(using ShiftRow.jdbcDecoder).selectStream()
  }
  ...
  override def upsertStreaming(unsaved: ZStream[ZConnection, Throwable, ShiftRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    ...
    val copied = streamingInsert(s"""copy shift_TEMP("shiftid", "name", "starttime", "endtime", "modifieddate") from stdin""", batchSize, unsaved)(using ShiftRow.text)
