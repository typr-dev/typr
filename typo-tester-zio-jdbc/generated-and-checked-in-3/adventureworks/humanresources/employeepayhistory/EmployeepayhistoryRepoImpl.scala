...
class EmployeepayhistoryRepoImpl extends EmployeepayhistoryRepo {
  ...
  override def deleteById(compositeId: EmployeepayhistoryId): ZIO[ZConnection, Throwable, Boolean] = {
    sql"""delete from "humanresources"."employeepayhistory" where "businessentityid" = ${Segment.paramSegment(compositeId.businessentityid)(using BusinessentityId.setter)} AND "ratechangedate" = ${Segment.paramSegment(compositeId.ratechangedate)(using TypoLocalDateTime.setter)}""".delete.map(_ > 0)
  }
  ...
  override def insert(unsaved: EmployeepayhistoryRowUnsaved): ZIO[ZConnection, Throwable, EmployeepayhistoryRow] = {
    val fs = List(
      ...
      Some((sql""""rate"""", sql"${Segment.paramSegment(unsaved.rate)(using Setter.bigDecimalScalaSetter)}::numeric")),
      Some((sql""""payfrequency"""", sql"${Segment.paramSegment(unsaved.payfrequency)(using TypoShort.setter)}::int2")),
  ...
  override def insertStreaming(unsaved: ZStream[ZConnection, Throwable, EmployeepayhistoryRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "humanresources"."employeepayhistory"("businessentityid", "ratechangedate", "rate", "payfrequency", "modifieddate") FROM STDIN""", batchSize, unsaved)(using EmployeepayhistoryRow.text)
  }
  ...
  override def insertUnsavedStreaming(unsaved: ZStream[ZConnection, Throwable, EmployeepayhistoryRowUnsaved], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "humanresources"."employeepayhistory"("businessentityid", "ratechangedate", "rate", "payfrequency", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(using EmployeepayhistoryRowUnsaved.text)
  }
  ...
  override def update(row: EmployeepayhistoryRow): ZIO[ZConnection, Throwable, Option[EmployeepayhistoryRow]] = {
    ...
    sql"""update "humanresources"."employeepayhistory"
          set "rate" = ${Segment.paramSegment(row.rate)(using Setter.bigDecimalScalaSetter)}::numeric,
              "payfrequency" = ${Segment.paramSegment(row.payfrequency)(using TypoShort.setter)}::int2,
              "modifieddate" = ${Segment.paramSegment(row.modifieddate)(using TypoLocalDateTime.setter)}::timestamp
          where "businessentityid" = ${Segment.paramSegment(compositeId.businessentityid)(using BusinessentityId.setter)} AND "ratechangedate" = ${Segment.paramSegment(compositeId.ratechangedate)(using TypoLocalDateTime.setter)}
          returning "businessentityid", "ratechangedate"::text, "rate", "payfrequency", "modifieddate"::text"""
      .query(using EmployeepayhistoryRow.jdbcDecoder)
      .selectOne
  }
  override def upsert(unsaved: EmployeepayhistoryRow): ZIO[ZConnection, Throwable, UpdateResult[EmployeepayhistoryRow]] = {
    sql"""insert into "humanresources"."employeepayhistory"("businessentityid", "ratechangedate", "rate", "payfrequency", "modifieddate")
          values (
            ...
            ${Segment.paramSegment(unsaved.ratechangedate)(using TypoLocalDateTime.setter)}::timestamp,
            ${Segment.paramSegment(unsaved.rate)(using Setter.bigDecimalScalaSetter)}::numeric,
            ${Segment.paramSegment(unsaved.payfrequency)(using TypoShort.setter)}::int2,
  ...
  override def upsertStreaming(unsaved: ZStream[ZConnection, Throwable, EmployeepayhistoryRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    ...
    val copied = streamingInsert(s"""copy employeepayhistory_TEMP("businessentityid", "ratechangedate", "rate", "payfrequency", "modifieddate") from stdin""", batchSize, unsaved)(using EmployeepayhistoryRow.text)
