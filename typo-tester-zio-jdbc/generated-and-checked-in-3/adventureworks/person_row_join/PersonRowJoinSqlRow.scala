...
object PersonRowJoinSqlRow {
  given jdbcDecoder: JdbcDecoder[PersonRowJoinSqlRow] = new JdbcDecoder[PersonRowJoinSqlRow] {
    override def unsafeDecode(columIndex: Int, rs: ResultSet): (Int, PersonRowJoinSqlRow) =
      columIndex + 2 ->
        PersonRowJoinSqlRow(
          ...
          email = JdbcDecoder.optionDecoder(using JdbcDecoder[Array[TypoRecord]]).unsafeDecode(columIndex + 1, rs)._2,
          emails = JdbcDecoder.optionDecoder(using JdbcDecoder[Array[TypoRecord]]).unsafeDecode(columIndex + 2, rs)._2
        )
  }
  given jsonDecoder: JsonDecoder[PersonRowJoinSqlRow] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
    ...
    val email = jsonObj.get("email").fold[Either[String, Option[Array[TypoRecord]]]](Right(None))(_.as(using JsonDecoder.option(using JsonDecoder.array[TypoRecord](using TypoRecord.jsonDecoder, implicitly))))
    val emails = jsonObj.get("emails").fold[Either[String, Option[Array[TypoRecord]]]](Right(None))(_.as(using JsonDecoder.option(using JsonDecoder.array[TypoRecord](using TypoRecord.jsonDecoder, implicitly))))
  ...
  given jsonEncoder: JsonEncoder[PersonRowJoinSqlRow] = new JsonEncoder[PersonRowJoinSqlRow] {
