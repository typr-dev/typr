...
object WorkorderroutingRow {
  ...
  given jdbcDecoder: JdbcDecoder[WorkorderroutingRow] = new JdbcDecoder[WorkorderroutingRow] {
    override def unsafeDecode(columIndex: Int, rs: ResultSet): (Int, WorkorderroutingRow) =
      columIndex + 11 ->
        WorkorderroutingRow(
          ...
          actualstartdate = JdbcDecoder.optionDecoder(using TypoLocalDateTime.jdbcDecoder).unsafeDecode(columIndex + 6, rs)._2,
          actualenddate = JdbcDecoder.optionDecoder(using TypoLocalDateTime.jdbcDecoder).unsafeDecode(columIndex + 7, rs)._2,
          actualresourcehrs = JdbcDecoder.optionDecoder(using JdbcDecoder.bigDecimalDecoderScala).unsafeDecode(columIndex + 8, rs)._2,
          plannedcost = JdbcDecoder.bigDecimalDecoderScala.unsafeDecode(columIndex + 9, rs)._2,
          actualcost = JdbcDecoder.optionDecoder(using JdbcDecoder.bigDecimalDecoderScala).unsafeDecode(columIndex + 10, rs)._2,
          modifieddate = TypoLocalDateTime.jdbcDecoder.unsafeDecode(columIndex + 11, rs)._2
        )
  }
  given jsonDecoder: JsonDecoder[WorkorderroutingRow] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
  ...
  given jsonEncoder: JsonEncoder[WorkorderroutingRow] = new JsonEncoder[WorkorderroutingRow] {
