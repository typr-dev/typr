...
class ProductreviewRepoImpl extends ProductreviewRepo {
  ...
  override def deleteById(productreviewid: ProductreviewId): ZIO[ZConnection, Throwable, Boolean] = {
    sql"""delete from "production"."productreview" where "productreviewid" = ${Segment.paramSegment(productreviewid)(using ProductreviewId.setter)}""".delete.map(_ > 0)
  }
  ...
  override def insert(unsaved: ProductreviewRowUnsaved): ZIO[ZConnection, Throwable, ProductreviewRow] = {
    val fs = List(
      ...
      Some((sql""""emailaddress"""", sql"${Segment.paramSegment(unsaved.emailaddress)(using Setter.stringSetter)}")),
      Some((sql""""rating"""", sql"${Segment.paramSegment(unsaved.rating)(using Setter.intSetter)}::int4")),
      Some((sql""""comments"""", sql"${Segment.paramSegment(unsaved.comments)(using Setter.optionParamSetter(using Setter.stringSetter))}")),
      unsaved.productreviewid match {
        case Defaulted.UseDefault => None
        case Defaulted.Provided(value) => Some((sql""""productreviewid"""", sql"${Segment.paramSegment(value: ProductreviewId)(using ProductreviewId.setter)}::int4"))
      },
      unsaved.reviewdate match {
        case Defaulted.UseDefault => None
        case Defaulted.Provided(value) => Some((sql""""reviewdate"""", sql"${Segment.paramSegment(value: TypoLocalDateTime)(using TypoLocalDateTime.setter)}::timestamp"))
      },
  ...
  override def insertStreaming(unsaved: ZStream[ZConnection, Throwable, ProductreviewRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "production"."productreview"("productreviewid", "productid", "reviewername", "reviewdate", "emailaddress", "rating", "comments", "modifieddate") FROM STDIN""", batchSize, unsaved)(using ProductreviewRow.text)
  }
  ...
  override def insertUnsavedStreaming(unsaved: ZStream[ZConnection, Throwable, ProductreviewRowUnsaved], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "production"."productreview"("productid", "reviewername", "emailaddress", "rating", "comments", "productreviewid", "reviewdate", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(using ProductreviewRowUnsaved.text)
  }
  ...
  override def selectById(productreviewid: ProductreviewId): ZIO[ZConnection, Throwable, Option[ProductreviewRow]] = {
    sql"""select "productreviewid", "productid", "reviewername", "reviewdate"::text, "emailaddress", "rating", "comments", "modifieddate"::text from "production"."productreview" where "productreviewid" = ${Segment.paramSegment(productreviewid)(using ProductreviewId.setter)}""".query(using ProductreviewRow.jdbcDecoder).selectOne
  }
  override def selectByIds(productreviewids: Array[ProductreviewId]): ZStream[ZConnection, Throwable, ProductreviewRow] = {
    sql"""select "productreviewid", "productid", "reviewername", "reviewdate"::text, "emailaddress", "rating", "comments", "modifieddate"::text from "production"."productreview" where "productreviewid" = ANY(${Segment.paramSegment(productreviewids)(using ProductreviewId.arraySetter)})""".query(using ProductreviewRow.jdbcDecoder).selectStream()
  }
  ...
  override def update(row: ProductreviewRow): ZIO[ZConnection, Throwable, Option[ProductreviewRow]] = {
    ...
    sql"""update "production"."productreview"
          set "productid" = ${Segment.paramSegment(row.productid)(using ProductId.setter)}::int4,
              "reviewername" = ${Segment.paramSegment(row.reviewername)(using Name.setter)}::varchar,
              "reviewdate" = ${Segment.paramSegment(row.reviewdate)(using TypoLocalDateTime.setter)}::timestamp,
              "emailaddress" = ${Segment.paramSegment(row.emailaddress)(using Setter.stringSetter)},
              "rating" = ${Segment.paramSegment(row.rating)(using Setter.intSetter)}::int4,
              "comments" = ${Segment.paramSegment(row.comments)(using Setter.optionParamSetter(using Setter.stringSetter))},
              "modifieddate" = ${Segment.paramSegment(row.modifieddate)(using TypoLocalDateTime.setter)}::timestamp
          where "productreviewid" = ${Segment.paramSegment(productreviewid)(using ProductreviewId.setter)}
          returning "productreviewid", "productid", "reviewername", "reviewdate"::text, "emailaddress", "rating", "comments", "modifieddate"::text"""
      .query(using ProductreviewRow.jdbcDecoder)
      .selectOne
  }
  override def upsert(unsaved: ProductreviewRow): ZIO[ZConnection, Throwable, UpdateResult[ProductreviewRow]] = {
    sql"""insert into "production"."productreview"("productreviewid", "productid", "reviewername", "reviewdate", "emailaddress", "rating", "comments", "modifieddate")
          values (
            ...
            ${Segment.paramSegment(unsaved.reviewername)(using Name.setter)}::varchar,
            ${Segment.paramSegment(unsaved.reviewdate)(using TypoLocalDateTime.setter)}::timestamp,
            ${Segment.paramSegment(unsaved.emailaddress)(using Setter.stringSetter)},
            ${Segment.paramSegment(unsaved.rating)(using Setter.intSetter)}::int4,
            ${Segment.paramSegment(unsaved.comments)(using Setter.optionParamSetter(using Setter.stringSetter))},
  ...
  override def upsertStreaming(unsaved: ZStream[ZConnection, Throwable, ProductreviewRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    ...
    val copied = streamingInsert(s"""copy productreview_TEMP("productreviewid", "productid", "reviewername", "reviewdate", "emailaddress", "rating", "comments", "modifieddate") from stdin""", batchSize, unsaved)(using ProductreviewRow.text)
