...
object WorkorderRow {
  given jdbcDecoder: JdbcDecoder[WorkorderRow] = new JdbcDecoder[WorkorderRow] {
    override def unsafeDecode(columIndex: Int, rs: ResultSet): (Int, WorkorderRow) =
      columIndex + 8 ->
        WorkorderRow(
          ...
          scrapreasonid = JdbcDecoder.optionDecoder(using ScrapreasonId.jdbcDecoder).unsafeDecode(columIndex + 7, rs)._2,
          modifieddate = TypoLocalDateTime.jdbcDecoder.unsafeDecode(columIndex + 8, rs)._2
        )
  }
  given jsonDecoder: JsonDecoder[WorkorderRow] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
  ...
  given jsonEncoder: JsonEncoder[WorkorderRow] = new JsonEncoder[WorkorderRow] {
