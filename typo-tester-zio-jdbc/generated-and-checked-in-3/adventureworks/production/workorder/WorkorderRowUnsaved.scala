...
object WorkorderRowUnsaved {
  given jsonDecoder: JsonDecoder[WorkorderRowUnsaved] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
    ...
    val workorderid = jsonObj.get("workorderid").toRight("Missing field 'workorderid'").flatMap(_.as(using Defaulted.jsonDecoder(using WorkorderId.jsonDecoder)))
  ...
  given jsonEncoder: JsonEncoder[WorkorderRowUnsaved] = new JsonEncoder[WorkorderRowUnsaved] {
    override def unsafeEncode(a: WorkorderRowUnsaved, indent: Option[Int], out: Write): Unit = {
      ...
      Defaulted.jsonEncoder(using WorkorderId.jsonEncoder).unsafeEncode(a.workorderid, indent, out)
      out.write(",")
