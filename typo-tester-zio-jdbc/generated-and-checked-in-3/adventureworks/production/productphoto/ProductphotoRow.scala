...
object ProductphotoRow {
  given jdbcDecoder: JdbcDecoder[ProductphotoRow] = new JdbcDecoder[ProductphotoRow] {
    override def unsafeDecode(columIndex: Int, rs: ResultSet): (Int, ProductphotoRow) =
      columIndex + 5 ->
        ProductphotoRow(
          ...
          thumbnailphoto = JdbcDecoder.optionDecoder(using TypoBytea.jdbcDecoder).unsafeDecode(columIndex + 1, rs)._2,
          thumbnailphotofilename = JdbcDecoder.optionDecoder(using JdbcDecoder.stringDecoder).unsafeDecode(columIndex + 2, rs)._2,
          largephoto = JdbcDecoder.optionDecoder(using TypoBytea.jdbcDecoder).unsafeDecode(columIndex + 3, rs)._2,
          largephotofilename = JdbcDecoder.optionDecoder(using JdbcDecoder.stringDecoder).unsafeDecode(columIndex + 4, rs)._2,
          modifieddate = TypoLocalDateTime.jdbcDecoder.unsafeDecode(columIndex + 5, rs)._2
        )
  }
  given jsonDecoder: JsonDecoder[ProductphotoRow] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
  ...
  given jsonEncoder: JsonEncoder[ProductphotoRow] = new JsonEncoder[ProductphotoRow] {
