...
class CultureRepoImpl extends CultureRepo {
  ...
  override def deleteById(cultureid: CultureId): ZIO[ZConnection, Throwable, Boolean] = {
    sql"""delete from "production"."culture" where "cultureid" = ${Segment.paramSegment(cultureid)(using CultureId.setter)}""".delete.map(_ > 0)
  }
  ...
  override def insertStreaming(unsaved: ZStream[ZConnection, Throwable, CultureRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "production"."culture"("cultureid", "name", "modifieddate") FROM STDIN""", batchSize, unsaved)(using CultureRow.text)
  }
  ...
  override def insertUnsavedStreaming(unsaved: ZStream[ZConnection, Throwable, CultureRowUnsaved], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "production"."culture"("cultureid", "name", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(using CultureRowUnsaved.text)
  }
  ...
  override def selectById(cultureid: CultureId): ZIO[ZConnection, Throwable, Option[CultureRow]] = {
    sql"""select "cultureid", "name", "modifieddate"::text from "production"."culture" where "cultureid" = ${Segment.paramSegment(cultureid)(using CultureId.setter)}""".query(using CultureRow.jdbcDecoder).selectOne
  }
  override def selectByIds(cultureids: Array[CultureId]): ZStream[ZConnection, Throwable, CultureRow] = {
    sql"""select "cultureid", "name", "modifieddate"::text from "production"."culture" where "cultureid" = ANY(${Segment.paramSegment(cultureids)(using CultureId.arraySetter)})""".query(using CultureRow.jdbcDecoder).selectStream()
  }
  ...
  override def upsertStreaming(unsaved: ZStream[ZConnection, Throwable, CultureRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    ...
    val copied = streamingInsert(s"""copy culture_TEMP("cultureid", "name", "modifieddate") from stdin""", batchSize, unsaved)(using CultureRow.text)
