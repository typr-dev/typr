...
object BillofmaterialsRow {
  given jdbcDecoder: JdbcDecoder[BillofmaterialsRow] = new JdbcDecoder[BillofmaterialsRow] {
    override def unsafeDecode(columIndex: Int, rs: ResultSet): (Int, BillofmaterialsRow) =
      columIndex + 8 ->
        BillofmaterialsRow(
          ...
          productassemblyid = JdbcDecoder.optionDecoder(using ProductId.jdbcDecoder).unsafeDecode(columIndex + 1, rs)._2,
  ...
  given jsonDecoder: JsonDecoder[BillofmaterialsRow] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
    val billofmaterialsid = jsonObj.get("billofmaterialsid").toRight("Missing field 'billofmaterialsid'").flatMap(_.as(using JsonDecoder.int))
    ...
    val perassemblyqty = jsonObj.get("perassemblyqty").toRight("Missing field 'perassemblyqty'").flatMap(_.as(using JsonDecoder.scalaBigDecimal))
  ...
  given jsonEncoder: JsonEncoder[BillofmaterialsRow] = new JsonEncoder[BillofmaterialsRow] {
  ...
  given text: Text[BillofmaterialsRow] = Text.instance[BillofmaterialsRow]{ (row, sb) =>
