...
object AddresstypeRowUnsaved {
  given jsonDecoder: JsonDecoder[AddresstypeRowUnsaved] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
    val name = jsonObj.get("name").toRight("Missing field 'name'").flatMap(_.as(using Name.jsonDecoder))
    val addresstypeid = jsonObj.get("addresstypeid").toRight("Missing field 'addresstypeid'").flatMap(_.as(using Defaulted.jsonDecoder(using AddresstypeId.jsonDecoder)))
  ...
  given jsonEncoder: JsonEncoder[AddresstypeRowUnsaved] = new JsonEncoder[AddresstypeRowUnsaved] {
    override def unsafeEncode(a: AddresstypeRowUnsaved, indent: Option[Int], out: Write): Unit = {
      ...
      Defaulted.jsonEncoder(using AddresstypeId.jsonEncoder).unsafeEncode(a.addresstypeid, indent, out)
      out.write(",")
      out.write(""""rowguid":""")
