...
object EmailaddressRowUnsaved {
  given jsonDecoder: JsonDecoder[EmailaddressRowUnsaved] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
    ...
    val emailaddressid = jsonObj.get("emailaddressid").toRight("Missing field 'emailaddressid'").flatMap(_.as(using Defaulted.jsonDecoder(using JsonDecoder.int)))
  ...
  given jsonEncoder: JsonEncoder[EmailaddressRowUnsaved] = new JsonEncoder[EmailaddressRowUnsaved] {
    override def unsafeEncode(a: EmailaddressRowUnsaved, indent: Option[Int], out: Write): Unit = {
      ...
      Defaulted.jsonEncoder(using JsonEncoder.int).unsafeEncode(a.emailaddressid, indent, out)
      out.write(",")
      out.write(""""rowguid":""")
