...
object VViewRow {
  given jdbcDecoder: JdbcDecoder[VViewRow] = new JdbcDecoder[VViewRow] {
    override def unsafeDecode(columIndex: Int, rs: ResultSet): (Int, VViewRow) =
      columIndex + 8 ->
        VViewRow(
          ...
          purchasingwebserviceurl = JdbcDecoder.optionDecoder(using JdbcDecoder.stringDecoder).unsafeDecode(columIndex + 7, rs)._2,
          modifieddate = TypoLocalDateTime.jdbcDecoder.unsafeDecode(columIndex + 8, rs)._2
        )
  }
  given jsonDecoder: JsonDecoder[VViewRow] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
  ...
  given jsonEncoder: JsonEncoder[VViewRow] = new JsonEncoder[VViewRow] {
