...
class TitleDomainRepoImpl extends TitleDomainRepo {
  ...
  override def selectAll: ZStream[ZConnection, Throwable, TitleDomainRow] = {
    sql"""select "code" from "public"."title_domain"""".query(TitleDomainRow.jdbcDecoder).selectStream()
  }
  override def selectById(code: TitleDomainId): ZIO[ZConnection, Throwable, Option[TitleDomainRow]] = {
    sql"""select "code" from "public"."title_domain" where "code" = ${Segment.paramSegment(code)(TitleDomainId.setter)}""".query(TitleDomainRow.jdbcDecoder).selectOne
  }
  override def selectByIds(codes: Array[TitleDomainId]): ZStream[ZConnection, Throwable, TitleDomainRow] = {
    sql"""select "code" from "public"."title_domain" where "code" = ANY(${Segment.paramSegment(codes)(TitleDomainId.arraySetter)})""".query(TitleDomainRow.jdbcDecoder).selectStream()
  }
