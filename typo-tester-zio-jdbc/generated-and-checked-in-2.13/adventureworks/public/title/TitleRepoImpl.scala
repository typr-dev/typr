...
class TitleRepoImpl extends TitleRepo {
  ...
  override def selectAll: ZStream[ZConnection, Throwable, TitleRow] = {
    sql"""select "code" from "public"."title"""".query(TitleRow.jdbcDecoder).selectStream()
  }
  override def selectById(code: TitleId): ZIO[ZConnection, Throwable, Option[TitleRow]] = {
    sql"""select "code" from "public"."title" where "code" = ${Segment.paramSegment(code)(TitleId.setter)}""".query(TitleRow.jdbcDecoder).selectOne
  }
  override def selectByIds(codes: Array[TitleId]): ZStream[ZConnection, Throwable, TitleRow] = {
    sql"""select "code" from "public"."title" where "code" = ANY(${Segment.paramSegment(codes)(TitleId.arraySetter)})""".query(TitleRow.jdbcDecoder).selectStream()
  }
