...
class TestOrganisasjonRepoImpl extends TestOrganisasjonRepo {
  ...
  override def selectAll: ZStream[ZConnection, Throwable, TestOrganisasjonRow] = {
    sql"""select "organisasjonskode" from "public"."test_organisasjon"""".query(TestOrganisasjonRow.jdbcDecoder).selectStream()
  }
  override def selectById(organisasjonskode: TestOrganisasjonId): ZIO[ZConnection, Throwable, Option[TestOrganisasjonRow]] = {
    sql"""select "organisasjonskode" from "public"."test_organisasjon" where "organisasjonskode" = ${Segment.paramSegment(organisasjonskode)(TestOrganisasjonId.setter)}""".query(TestOrganisasjonRow.jdbcDecoder).selectOne
  }
  override def selectByIds(organisasjonskodes: Array[TestOrganisasjonId]): ZStream[ZConnection, Throwable, TestOrganisasjonRow] = {
    sql"""select "organisasjonskode" from "public"."test_organisasjon" where "organisasjonskode" = ANY(${Segment.paramSegment(organisasjonskodes)(TestOrganisasjonId.arraySetter)})""".query(TestOrganisasjonRow.jdbcDecoder).selectStream()
  }
