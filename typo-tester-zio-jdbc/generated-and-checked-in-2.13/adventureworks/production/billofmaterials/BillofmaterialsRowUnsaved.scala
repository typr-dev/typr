...
object BillofmaterialsRowUnsaved {
  ...
  implicit lazy val pgText: Text[BillofmaterialsRowUnsaved] = {
    Text.instance[BillofmaterialsRowUnsaved]{ (row, sb) =>
      Text.option(ProductId.pgText).unsafeEncode(row.productassemblyid, sb)
      ...
      sb.append(Text.DELIMETER)
      Defaulted.pgText(Text.intInstance).unsafeEncode(row.billofmaterialsid, sb)
      sb.append(Text.DELIMETER)
      Defaulted.pgText(TypoLocalDateTime.pgText).unsafeEncode(row.startdate, sb)
      sb.append(Text.DELIMETER)
      Defaulted.pgText(Text.bigDecimalInstance).unsafeEncode(row.perassemblyqty, sb)
      sb.append(Text.DELIMETER)
