...
object VvendorwithcontactsViewRow {
  val `_rowParser`: RowParser[VvendorwithcontactsViewRow] = RowParsers.of(BusinessentityId.pgType, Name.pgType, Name.pgType, PgTypes.text, FirstName.pgType, Name.pgType, Name.pgType, PgTypes.text, Phone.pgType, Name.pgType, PgTypes.text, ScalaDbTypes.PgTypes.int4)(VvendorwithcontactsViewRow.apply)(row => Array[Any](row.businessentityid, row.name, row.contacttype, row.title, row.firstname, row.middlename, row.lastname, row.suffix, row.phonenumber, row.phonenumbertype, row.emailaddress, row.emailpromotion))
}