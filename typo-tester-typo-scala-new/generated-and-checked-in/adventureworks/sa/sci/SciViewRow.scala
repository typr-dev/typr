...
object SciViewRow {
  val `_rowParser`: RowParser[SciViewRow] = RowParsers.of(ShoppingcartitemId.pgType, ShoppingcartitemId.pgType, PgTypes.text, ScalaDbTypes.PgTypes.int4, ProductId.pgType, PgTypes.timestamp, PgTypes.timestamp)(SciViewRow.apply)(row => Array[Any](row.id, row.shoppingcartitemid, row.shoppingcartid, row.quantity, row.productid, row.datecreated, row.modifieddate))
}