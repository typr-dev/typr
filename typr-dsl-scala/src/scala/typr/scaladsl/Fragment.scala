...
/** Scala wrapper for typr.runtime.Fragment with Scala-native APIs.
  *
  * This class wraps the Java Fragment interface and provides Scala-friendly methods that use Scala collections and types.
  */
class Fragment(val underlying: typr.runtime.Fragment) extends AnyVal {
...
object Fragment {
  val EMPTY: Fragment = new Fragment(typr.runtime.Fragment.EMPTY)

  def lit(value: String): Fragment = new Fragment(typr.runtime.Fragment.lit(value))

  def empty(): Fragment = EMPTY

  def quotedDouble(value: String): Fragment = new Fragment(typr.runtime.Fragment.quotedDouble(value))

  def quotedSingle(value: String): Fragment = new Fragment(typr.runtime.Fragment.quotedSingle(value))

  def value[A](value: A, dbType: typr.runtime.DbType[A]): Fragment =
    new Fragment(typr.runtime.Fragment.value(value, dbType))

  /** Encode a value into a SQL fragment using the provided database type. */
  def encode[A](dbType: typr.runtime.DbType[A], value: A): Fragment =
    new Fragment(typr.runtime.Fragment.encode(dbType, value))

  def and(fragments: Fragment*): Fragment =
    new Fragment(typr.runtime.Fragment.and(fragments.map(_.underlying)*))

  def and(fragments: List[Fragment]): Fragment =
    new Fragment(typr.runtime.Fragment.and(fragments.map(_.underlying).asJava))

  def or(fragments: Fragment*): Fragment =
    new Fragment(typr.runtime.Fragment.or(fragments.map(_.underlying)*))

  def or(fragments: List[Fragment]): Fragment =
    new Fragment(typr.runtime.Fragment.or(fragments.map(_.underlying).asJava))

  def whereAnd(fragments: Fragment*): Fragment =
    new Fragment(typr.runtime.Fragment.whereAnd(fragments.map(_.underlying)*))

  def whereAnd(fragments: List[Fragment]): Fragment =
    new Fragment(typr.runtime.Fragment.whereAnd(fragments.map(_.underlying).asJava))

  def whereOr(fragments: Fragment*): Fragment =
    new Fragment(typr.runtime.Fragment.whereOr(fragments.map(_.underlying)*))

  def whereOr(fragments: List[Fragment]): Fragment =
    new Fragment(typr.runtime.Fragment.whereOr(fragments.map(_.underlying).asJava))

  def set(fragments: Fragment*): Fragment =
    new Fragment(typr.runtime.Fragment.set(fragments.map(_.underlying)*))

  def set(fragments: List[Fragment]): Fragment =
    new Fragment(typr.runtime.Fragment.set(fragments.map(_.underlying).asJava))

  def parentheses(fragment: Fragment): Fragment =
    new Fragment(typr.runtime.Fragment.parentheses(fragment.underlying))

  def comma(fragments: Fragment*): Fragment =
    new Fragment(typr.runtime.Fragment.comma(fragments.map(_.underlying)*))

  def comma(fragments: Iterable[Fragment]): Fragment =
    new Fragment(typr.runtime.Fragment.comma(fragments.map(_.underlying).toList.asJava))

  def orderBy(fragments: Fragment*): Fragment =
    new Fragment(typr.runtime.Fragment.orderBy(fragments.map(_.underlying)*))

  def orderBy(fragments: List[Fragment]): Fragment =
    new Fragment(typr.runtime.Fragment.orderBy(fragments.map(_.underlying).asJava))

  def join(fragments: List[Fragment], separator: Fragment): Fragment =
    new Fragment(typr.runtime.Fragment.join(fragments.map(_.underlying).asJava, separator.underlying))

  def concat(fragments: Fragment*): Fragment =
    new Fragment(typr.runtime.Fragment.concat(fragments.map(_.underlying)*))

  def in[T <: Object](column: String, values: Array[T], dbType: typr.runtime.DbType[T]): Fragment =
    new Fragment(typr.runtime.Fragment.in(column, values, dbType))

  def compositeIn(columns: Array[String], tuples: List[Array[Object]], types: Array[typr.runtime.DbType[?]]): Fragment =
    new Fragment(typr.runtime.Fragment.compositeIn(columns, tuples.asJava, types))

  /** Scala string interpolator for creating SQL Fragments.
    *
    * Usage:
    * {{{
    * import typr.scaladsl.Fragment.interpolate
    *
  ...
  extension (sc: StringContext) {
    def sql(args: Fragment*): Fragment = {
      ...
      val frags = new scala.collection.mutable.ListBuffer[typr.runtime.Fragment]()

      // Add first string part
      if (parts.hasNext) {
        val first = parts.next()
        if (first.nonEmpty) {
          frags += typr.runtime.Fragment.lit(first)
        }
      }
      ...
      while (parts.hasNext && argsIt.hasNext) {
        ...
        val part = parts.next()
        if (part.nonEmpty) {
          frags += typr.runtime.Fragment.lit(part)
        }
      }
      ...
      frags.result() match {
        ...
        case multiple =>
          val javaList = new java.util.ArrayList[typr.runtime.Fragment](multiple.size)
          multiple.foreach(javaList.add)
          new Fragment(new typr.runtime.Fragment.Concat(javaList))
      }
    }
  }

  /** Builder for creating Fragments with a fluent API */
  class Builder(private val underlying: typr.runtime.Fragment.Builder) {
    def sql(s: String): Builder = {
      underlying.sql(s)
      this
    }

    def param[T](dbType: typr.runtime.DbType[T], value: T): Builder = {
  ...
  def interpolate(initial: String): Builder =
    new Builder(typr.runtime.Fragment.interpolate(initial))

  def interpolate(fragments: Fragment*): Fragment =
    new Fragment(typr.runtime.Fragment.interpolate(fragments.map(_.underlying)*))
}
