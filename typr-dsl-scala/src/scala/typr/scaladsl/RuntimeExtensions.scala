...
implicit class EitherOps[L, R](private val either: Either[L, R]) extends AnyVal {
  ...
  def leftOrNone: Option[L] = {
    either match {
      case left: typr.runtime.Either.Left[L, R] => Some(left.value())
      case _                                    => None
    }
  }
}

implicit class ArrOps[A](private val arr: typr.data.Arr[A]) extends AnyVal {
  def reshapeOrNone(newDims: Int*): Option[typr.data.Arr[A]] = {
...
implicit class RangeOps[T <: Comparable[T]](private val range: typr.data.Range[T]) extends AnyVal {
  def finiteOrNone: Option[typr.data.RangeFinite[T]] = {
...
implicit class FragmentOps(private val fragment: Fragment) extends AnyVal {
  def query[Out](parser: typr.scaladsl.ResultSetParser[Out]): Operation[Out] = {
