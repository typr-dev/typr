...
class AddresstypeRepoImpl extends AddresstypeRepo {
  ...
  override def deleteById(addresstypeid: AddresstypeId)(using c: Connection): Boolean = {
  ...
  override def insertStreaming(unsaved: Iterator[AddresstypeRow], batchSize: Int = 10000)(using c: Connection): Long = {
    streamingInsert(s"""COPY "person"."addresstype"("addresstypeid", "name", "rowguid", "modifieddate") FROM STDIN""", batchSize, unsaved)(using AddresstypeRow.text, c)
  }
  /* NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(unsaved: Iterator[AddresstypeRowUnsaved], batchSize: Int = 10000)(using c: Connection): Long = {
    streamingInsert(s"""COPY "person"."addresstype"("name", "addresstypeid", "rowguid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(using AddresstypeRowUnsaved.text, c)
  }
  ...
  override def upsertStreaming(unsaved: Iterator[AddresstypeRow], batchSize: Int = 10000)(using c: Connection): Int = {
    SQL"""create temporary table addresstype_TEMP (like "person"."addresstype") on commit drop""".execute(): @nowarn
    streamingInsert(s"""copy addresstype_TEMP("addresstypeid", "name", "rowguid", "modifieddate") from stdin""", batchSize, unsaved)(using AddresstypeRow.text, c): @nowarn
    SQL"""insert into "person"."addresstype"("addresstypeid", "name", "rowguid", "modifieddate")
