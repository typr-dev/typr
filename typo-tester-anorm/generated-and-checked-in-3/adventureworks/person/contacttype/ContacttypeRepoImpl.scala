...
class ContacttypeRepoImpl extends ContacttypeRepo {
  ...
  override def deleteById(contacttypeid: ContacttypeId)(using c: Connection): Boolean = {
  ...
  override def insertStreaming(unsaved: Iterator[ContacttypeRow], batchSize: Int = 10000)(using c: Connection): Long = {
    streamingInsert(s"""COPY "person"."contacttype"("contacttypeid", "name", "modifieddate") FROM STDIN""", batchSize, unsaved)(using ContacttypeRow.text, c)
  }
  /* NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(unsaved: Iterator[ContacttypeRowUnsaved], batchSize: Int = 10000)(using c: Connection): Long = {
    streamingInsert(s"""COPY "person"."contacttype"("name", "contacttypeid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(using ContacttypeRowUnsaved.text, c)
  }
  ...
  override def upsertStreaming(unsaved: Iterator[ContacttypeRow], batchSize: Int = 10000)(using c: Connection): Int = {
    SQL"""create temporary table contacttype_TEMP (like "person"."contacttype") on commit drop""".execute(): @nowarn
    streamingInsert(s"""copy contacttype_TEMP("contacttypeid", "name", "modifieddate") from stdin""", batchSize, unsaved)(using ContacttypeRow.text, c): @nowarn
    SQL"""insert into "person"."contacttype"("contacttypeid", "name", "modifieddate")
