...
object EmployeeRowUnsaved {
  given reads: Reads[EmployeeRowUnsaved] = Reads[EmployeeRowUnsaved](json => JsResult.fromTry(
      Try(
        EmployeeRowUnsaved(
  ...
  given text: Text[EmployeeRowUnsaved] = Text.instance[EmployeeRowUnsaved]{ (row, sb) =>
    ...
    sb.append(Text.DELIMETER)
    Defaulted.text(using Flag.text).unsafeEncode(row.salariedflag, sb)
    sb.append(Text.DELIMETER)
    Defaulted.text(using TypoShort.text).unsafeEncode(row.vacationhours, sb)
    sb.append(Text.DELIMETER)
    Defaulted.text(using TypoShort.text).unsafeEncode(row.sickleavehours, sb)
    sb.append(Text.DELIMETER)
    Defaulted.text(using Flag.text).unsafeEncode(row.currentflag, sb)
    sb.append(Text.DELIMETER)
    ...
    sb.append(Text.DELIMETER)
    Defaulted.text(using Text.option(using Text.stringInstance)).unsafeEncode(row.organizationnode, sb)
  }
  given writes: OWrites[EmployeeRowUnsaved] = OWrites[EmployeeRowUnsaved](o =>
