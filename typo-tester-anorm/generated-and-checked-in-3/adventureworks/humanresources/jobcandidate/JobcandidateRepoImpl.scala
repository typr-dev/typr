...
class JobcandidateRepoImpl extends JobcandidateRepo {
  ...
  override def deleteById(jobcandidateid: JobcandidateId)(using c: Connection): Boolean = {
  ...
  override def insertStreaming(unsaved: Iterator[JobcandidateRow], batchSize: Int = 10000)(using c: Connection): Long = {
    streamingInsert(s"""COPY "humanresources"."jobcandidate"("jobcandidateid", "businessentityid", "resume", "modifieddate") FROM STDIN""", batchSize, unsaved)(using JobcandidateRow.text, c)
  }
  /* NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(unsaved: Iterator[JobcandidateRowUnsaved], batchSize: Int = 10000)(using c: Connection): Long = {
    streamingInsert(s"""COPY "humanresources"."jobcandidate"("businessentityid", "resume", "jobcandidateid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(using JobcandidateRowUnsaved.text, c)
  }
  ...
  override def update(row: JobcandidateRow)(using c: Connection): Option[JobcandidateRow] = {
    ...
    SQL"""update "humanresources"."jobcandidate"
          set "businessentityid" = ${ParameterValue(row.businessentityid, null, ToStatement.optionToStatement(using BusinessentityId.toStatement, BusinessentityId.parameterMetadata))}::int4,
              "resume" = ${ParameterValue(row.resume, null, ToStatement.optionToStatement(using TypoXml.toStatement, TypoXml.parameterMetadata))}::xml,
  ...
  override def upsert(unsaved: JobcandidateRow)(using c: Connection): JobcandidateRow = {
    SQL"""insert into "humanresources"."jobcandidate"("jobcandidateid", "businessentityid", "resume", "modifieddate")
          values (
            ...
            ${ParameterValue(unsaved.businessentityid, null, ToStatement.optionToStatement(using BusinessentityId.toStatement, BusinessentityId.parameterMetadata))}::int4,
            ${ParameterValue(unsaved.resume, null, ToStatement.optionToStatement(using TypoXml.toStatement, TypoXml.parameterMetadata))}::xml,
  ...
  override def upsertStreaming(unsaved: Iterator[JobcandidateRow], batchSize: Int = 10000)(using c: Connection): Int = {
    SQL"""create temporary table jobcandidate_TEMP (like "humanresources"."jobcandidate") on commit drop""".execute(): @nowarn
    streamingInsert(s"""copy jobcandidate_TEMP("jobcandidateid", "businessentityid", "resume", "modifieddate") from stdin""", batchSize, unsaved)(using JobcandidateRow.text, c): @nowarn
    SQL"""insert into "humanresources"."jobcandidate"("jobcandidateid", "businessentityid", "resume", "modifieddate")
