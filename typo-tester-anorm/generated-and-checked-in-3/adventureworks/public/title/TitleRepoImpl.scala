...
class TitleRepoImpl extends TitleRepo {
  ...
  override def deleteById(code: TitleId)(using c: Connection): Boolean = {
  ...
  override def insertStreaming(unsaved: Iterator[TitleRow], batchSize: Int = 10000)(using c: Connection): Long = {
    streamingInsert(s"""COPY "public"."title"("code") FROM STDIN""", batchSize, unsaved)(using TitleRow.text, c)
  }
  ...
  override def upsertStreaming(unsaved: Iterator[TitleRow], batchSize: Int = 10000)(using c: Connection): Int = {
    SQL"""create temporary table title_TEMP (like "public"."title") on commit drop""".execute(): @nowarn
    streamingInsert(s"""copy title_TEMP("code") from stdin""", batchSize, unsaved)(using TitleRow.text, c): @nowarn
    SQL"""insert into "public"."title"("code")
