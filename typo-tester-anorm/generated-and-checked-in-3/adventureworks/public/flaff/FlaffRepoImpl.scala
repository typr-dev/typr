...
class FlaffRepoImpl extends FlaffRepo {
  ...
  override def deleteById(compositeId: FlaffId)(using c: Connection): Boolean = {
  ...
  override def insertStreaming(unsaved: Iterator[FlaffRow], batchSize: Int = 10000)(using c: Connection): Long = {
    streamingInsert(s"""COPY "public"."flaff"("code", "another_code", "some_number", "specifier", "parentspecifier") FROM STDIN""", batchSize, unsaved)(using FlaffRow.text, c)
  }
  ...
  override def upsert(unsaved: FlaffRow)(using c: Connection): FlaffRow = {
    SQL"""insert into "public"."flaff"("code", "another_code", "some_number", "specifier", "parentspecifier")
          values (
            ...
            ${ParameterValue(unsaved.parentspecifier, null, ToStatement.optionToStatement(using ShortText.toStatement, ShortText.parameterMetadata))}::text
          )
  ...
  override def upsertStreaming(unsaved: Iterator[FlaffRow], batchSize: Int = 10000)(using c: Connection): Int = {
    SQL"""create temporary table flaff_TEMP (like "public"."flaff") on commit drop""".execute(): @nowarn
    streamingInsert(s"""copy flaff_TEMP("code", "another_code", "some_number", "specifier", "parentspecifier") from stdin""", batchSize, unsaved)(using FlaffRow.text, c): @nowarn
    SQL"""insert into "public"."flaff"("code", "another_code", "some_number", "specifier", "parentspecifier")
