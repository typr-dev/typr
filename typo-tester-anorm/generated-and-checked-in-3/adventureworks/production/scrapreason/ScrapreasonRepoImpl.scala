...
class ScrapreasonRepoImpl extends ScrapreasonRepo {
  ...
  override def deleteById(scrapreasonid: ScrapreasonId)(using c: Connection): Boolean = {
  ...
  override def insertStreaming(unsaved: Iterator[ScrapreasonRow], batchSize: Int = 10000)(using c: Connection): Long = {
    streamingInsert(s"""COPY "production"."scrapreason"("scrapreasonid", "name", "modifieddate") FROM STDIN""", batchSize, unsaved)(using ScrapreasonRow.text, c)
  }
  /* NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(unsaved: Iterator[ScrapreasonRowUnsaved], batchSize: Int = 10000)(using c: Connection): Long = {
    streamingInsert(s"""COPY "production"."scrapreason"("name", "scrapreasonid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(using ScrapreasonRowUnsaved.text, c)
  }
  ...
  override def upsertStreaming(unsaved: Iterator[ScrapreasonRow], batchSize: Int = 10000)(using c: Connection): Int = {
    SQL"""create temporary table scrapreason_TEMP (like "production"."scrapreason") on commit drop""".execute(): @nowarn
    streamingInsert(s"""copy scrapreason_TEMP("scrapreasonid", "name", "modifieddate") from stdin""", batchSize, unsaved)(using ScrapreasonRow.text, c): @nowarn
    SQL"""insert into "production"."scrapreason"("scrapreasonid", "name", "modifieddate")
