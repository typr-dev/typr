...
class IllustrationRepoImpl extends IllustrationRepo {
  ...
  override def deleteById(illustrationid: IllustrationId)(using c: Connection): Boolean = {
  ...
  override def insertStreaming(unsaved: Iterator[IllustrationRow], batchSize: Int = 10000)(using c: Connection): Long = {
    streamingInsert(s"""COPY "production"."illustration"("illustrationid", "diagram", "modifieddate") FROM STDIN""", batchSize, unsaved)(using IllustrationRow.text, c)
  }
  /* NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(unsaved: Iterator[IllustrationRowUnsaved], batchSize: Int = 10000)(using c: Connection): Long = {
    streamingInsert(s"""COPY "production"."illustration"("diagram", "illustrationid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(using IllustrationRowUnsaved.text, c)
  }
  ...
  override def upsert(unsaved: IllustrationRow)(using c: Connection): IllustrationRow = {
    SQL"""insert into "production"."illustration"("illustrationid", "diagram", "modifieddate")
          values (
            ...
            ${ParameterValue(unsaved.diagram, null, ToStatement.optionToStatement(using TypoXml.toStatement, TypoXml.parameterMetadata))}::xml,
  ...
  override def upsertStreaming(unsaved: Iterator[IllustrationRow], batchSize: Int = 10000)(using c: Connection): Int = {
    SQL"""create temporary table illustration_TEMP (like "production"."illustration") on commit drop""".execute(): @nowarn
    streamingInsert(s"""copy illustration_TEMP("illustrationid", "diagram", "modifieddate") from stdin""", batchSize, unsaved)(using IllustrationRow.text, c): @nowarn
    SQL"""insert into "production"."illustration"("illustrationid", "diagram", "modifieddate")
