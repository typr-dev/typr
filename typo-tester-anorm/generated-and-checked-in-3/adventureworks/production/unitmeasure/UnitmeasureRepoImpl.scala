...
class UnitmeasureRepoImpl extends UnitmeasureRepo {
  ...
  override def deleteById(unitmeasurecode: UnitmeasureId)(using c: Connection): Boolean = {
  ...
  override def insertStreaming(unsaved: Iterator[UnitmeasureRow], batchSize: Int = 10000)(using c: Connection): Long = {
    streamingInsert(s"""COPY "production"."unitmeasure"("unitmeasurecode", "name", "modifieddate") FROM STDIN""", batchSize, unsaved)(using UnitmeasureRow.text, c)
  }
  /* NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(unsaved: Iterator[UnitmeasureRowUnsaved], batchSize: Int = 10000)(using c: Connection): Long = {
    streamingInsert(s"""COPY "production"."unitmeasure"("unitmeasurecode", "name", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(using UnitmeasureRowUnsaved.text, c)
  }
  ...
  override def upsertStreaming(unsaved: Iterator[UnitmeasureRow], batchSize: Int = 10000)(using c: Connection): Int = {
    SQL"""create temporary table unitmeasure_TEMP (like "production"."unitmeasure") on commit drop""".execute(): @nowarn
    streamingInsert(s"""copy unitmeasure_TEMP("unitmeasurecode", "name", "modifieddate") from stdin""", batchSize, unsaved)(using UnitmeasureRow.text, c): @nowarn
    SQL"""insert into "production"."unitmeasure"("unitmeasurecode", "name", "modifieddate")
