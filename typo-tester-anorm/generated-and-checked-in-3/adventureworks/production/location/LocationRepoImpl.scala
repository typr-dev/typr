...
class LocationRepoImpl extends LocationRepo {
  ...
  override def deleteById(locationid: LocationId)(using c: Connection): Boolean = {
  ...
  override def insertStreaming(unsaved: Iterator[LocationRow], batchSize: Int = 10000)(using c: Connection): Long = {
    streamingInsert(s"""COPY "production"."location"("locationid", "name", "costrate", "availability", "modifieddate") FROM STDIN""", batchSize, unsaved)(using LocationRow.text, c)
  }
  /* NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(unsaved: Iterator[LocationRowUnsaved], batchSize: Int = 10000)(using c: Connection): Long = {
    streamingInsert(s"""COPY "production"."location"("name", "locationid", "costrate", "availability", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(using LocationRowUnsaved.text, c)
  }
  ...
  override def upsertStreaming(unsaved: Iterator[LocationRow], batchSize: Int = 10000)(using c: Connection): Int = {
    SQL"""create temporary table location_TEMP (like "production"."location") on commit drop""".execute(): @nowarn
    streamingInsert(s"""copy location_TEMP("locationid", "name", "costrate", "availability", "modifieddate") from stdin""", batchSize, unsaved)(using LocationRow.text, c): @nowarn
    SQL"""insert into "production"."location"("locationid", "name", "costrate", "availability", "modifieddate")
