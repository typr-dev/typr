...
object CustomCreditcardId {
  ...
  given pgType: typo.runtime.PgType[CustomCreditcardId] = PgTypes.int4.bimap(CustomCreditcardId.apply, _.value)
  given pgTypeArray: typo.runtime.PgType[Array[CustomCreditcardId]] = PgTypes.int4Array.bimap(xs => xs.map(CustomCreditcardId.apply), xs => xs.map(_.value))
