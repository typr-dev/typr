...
class RepoTest {
  ...
  private def upsertStreaming(unitmeasureRepo: UnitmeasureRepo): Unit = {
    WithConnection {
      ...
      val _ = unitmeasureRepo.upsertStreaming(List(um1a, um2a).iterator.asJava, 1000)

      val all2 = unitmeasureRepo.selectAll.asScala.toList.sortBy(_.name.value)
  ...
  private def upsertBatch(unitmeasureRepo: UnitmeasureRepo): Unit = {
    WithConnection {
      ...
      val um1a = um1.copy(name = Name("name1a"))
      val um2a = um2.copy(name = Name("name2a"))
      val returned = unitmeasureRepo.upsertBatch(List(um1a, um2a).iterator.asJava).asScala.toList.sortBy(_.name.value)
      assertEquals(List(um1a, um2a), returned)

      val all = unitmeasureRepo.selectAll.asScala.toList.sortBy(_.name.value)
