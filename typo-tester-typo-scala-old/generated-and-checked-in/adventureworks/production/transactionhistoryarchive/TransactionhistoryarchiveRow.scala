...
object TransactionhistoryarchiveRow {
  val `_rowParser`: RowParser[TransactionhistoryarchiveRow] = RowParsers.of(TransactionhistoryarchiveId.pgType, PgTypes.int4, PgTypes.int4, PgTypes.int4, PgTypes.timestamp, PgTypes.bpchar, PgTypes.int4, PgTypes.numeric, PgTypes.timestamp, TransactionhistoryarchiveRow.apply, row => Array[Any](row.transactionid, row.productid, row.referenceorderid, row.referenceorderlineid, row.transactiondate, row.transactiontype, row.quantity, row.actualcost, row.modifieddate))
