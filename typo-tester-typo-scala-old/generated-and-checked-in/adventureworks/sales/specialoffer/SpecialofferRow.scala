...
object SpecialofferRow {
  val `_rowParser`: RowParser[SpecialofferRow] = RowParsers.of(SpecialofferId.pgType, PgTypes.text, PgTypes.numeric, PgTypes.text, PgTypes.text, PgTypes.timestamp, PgTypes.timestamp, PgTypes.int4, PgTypes.int4.opt(), PgTypes.uuid, PgTypes.timestamp, SpecialofferRow.apply, row => Array[Any](row.specialofferid, row.description, row.discountpct, row.`type`, row.category, row.startdate, row.enddate, row.minqty, row.maxqty, row.rowguid, row.modifieddate))
