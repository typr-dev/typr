/**
 * File has been automatically generated by `typo`.
 *
 * IF YOU CHANGE THIS FILE YOUR CHANGES WILL BE OVERWRITTEN.
 */
package adventureworks.sales.vpersondemographics

import adventureworks.person.businessentity.BusinessentityId
import java.time.LocalDate
import typo.data.Money
import typo.runtime.PgTypes
import typo.runtime.RowParser
import typo.runtime.RowParsers

/** View: sales.vpersondemographics */
case class VpersondemographicsViewRow(
  /** Points to [[adventureworks.person.person.PersonRow.businessentityid]] */
  businessentityid: BusinessentityId,
  totalpurchaseytd: Money,
  datefirstpurchase: LocalDate,
  birthdate: LocalDate,
  maritalstatus: String,
  yearlyincome: String,
  gender: String,
  totalchildren: Integer,
  numberchildrenathome: Integer,
  education: String,
  occupation: String,
  homeownerflag: java.lang.Boolean,
  numbercarsowned: Integer
)

object VpersondemographicsViewRow {
  val `_rowParser`: RowParser[VpersondemographicsViewRow] = {
    RowParsers.of(BusinessentityId.pgType, PgTypes.money, PgTypes.date, PgTypes.date, PgTypes.text, PgTypes.text, PgTypes.text, PgTypes.int4, PgTypes.int4, PgTypes.text, PgTypes.text, PgTypes.bool, PgTypes.int4, (t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12) => new VpersondemographicsViewRow(
      t0,
      t1,
      t2,
      t3,
      t4,
      t5,
      t6,
      t7,
      t8,
      t9,
      t10,
      t11,
      t12
    ), row => Array[Any](row.businessentityid, row.totalpurchaseytd, row.datefirstpurchase, row.birthdate, row.maritalstatus, row.yearlyincome, row.gender, row.totalchildren, row.numberchildrenathome, row.education, row.occupation, row.homeownerflag, row.numbercarsowned))
  }
}