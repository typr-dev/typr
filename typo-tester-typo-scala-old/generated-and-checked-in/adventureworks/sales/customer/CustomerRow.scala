...
object CustomerRow {
  val `_rowParser`: RowParser[CustomerRow] = RowParsers.of(CustomerId.pgType, BusinessentityId.pgType.opt(), BusinessentityId.pgType.opt(), SalesterritoryId.pgType.opt(), PgTypes.uuid, PgTypes.timestamp, CustomerRow.apply, row => Array[Any](row.customerid, row.personid, row.storeid, row.territoryid, row.rowguid, row.modifieddate))
