...
object DViewRow {
  val `_rowParser`: RowParser[DViewRow] = RowParsers.of(PgTypes.text, BusinessentityId.pgType, Flag.pgType, PgTypes.text, PgTypes.text, PgTypes.bpchar, PgTypes.int4, PgTypes.int2, PgTypes.text, PgTypes.bytea, PgTypes.uuid, PgTypes.timestamp, DocumentId.pgType, DViewRow.apply, row => Array[Any](row.title, row.owner, row.folderflag, row.filename, row.fileextension, row.revision, row.changenumber, row.status, row.documentsummary, row.document, row.rowguid, row.modifieddate, row.documentnode))
}