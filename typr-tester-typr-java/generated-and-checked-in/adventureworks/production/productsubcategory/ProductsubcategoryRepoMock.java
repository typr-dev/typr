...
public record ProductsubcategoryRepoMock(
  ...
  public ProductsubcategoryRepoMock(
      java.util.function.Function<ProductsubcategoryRowUnsaved, ProductsubcategoryRow> toRow) {
    this(toRow, new HashMap<ProductsubcategoryId, ProductsubcategoryRow>());
  }
  ;

  public ProductsubcategoryRepoMock withToRow(
      java.util.function.Function<ProductsubcategoryRowUnsaved, ProductsubcategoryRow> toRow) {
    return new ProductsubcategoryRepoMock(toRow, map);
  }
  ;

  public ProductsubcategoryRepoMock withMap(
      HashMap<ProductsubcategoryId, ProductsubcategoryRow> map) {
    return new ProductsubcategoryRepoMock(toRow, map);
  }
  ;
  ...
  public Long insertStreaming(
      Iterator<ProductsubcategoryRow> unsaved, Integer batchSize, Connection c) {
    var count = 0L;
  ...
  public Map<ProductsubcategoryId, ProductsubcategoryRow> selectByIdsTracked(
      ProductsubcategoryId[] productsubcategoryids, Connection c) {
    return selectByIds(productsubcategoryids, c).stream()
        .collect(
            Collectors.toMap(
                (ProductsubcategoryRow row) -> row.productsubcategoryid(), Function.identity()));
  }
  ;
  ...
  public Boolean update(ProductsubcategoryRow row, Connection c) {
    var shouldUpdate =
        Optional.ofNullable(map.get(row.productsubcategoryid()))
  ...
  public List<ProductsubcategoryRow> upsertBatch(
      Iterator<ProductsubcategoryRow> unsaved, Connection c) {
    var result = new ArrayList<ProductsubcategoryRow>();
    while (unsaved.hasNext()) {
      var row = unsaved.next();
      map.put(row.productsubcategoryid(), row);
