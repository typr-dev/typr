...
class FlaffRepoImpl extends FlaffRepo {
  ...
  override def insert(unsaved: FlaffRow): ZIO[ZConnection, Throwable, FlaffRow] = {
    ...
    """.insertReturning(FlaffRow.jdbcDecoder).map(_.updatedKeys.head)
  }

  override def insertStreaming(
    unsaved: ZStream[ZConnection, Throwable, FlaffRow],
    batchSize: Int = 10000
  ): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY "public"."flaff"("code", "another_code", "some_number", "specifier", "parentspecifier") FROM STDIN""", batchSize, unsaved)(FlaffRow.pgText)
  ...
  override def selectAll: ZStream[ZConnection, Throwable, FlaffRow] = sql"""select "code", "another_code", "some_number", "specifier", "parentspecifier" from "public"."flaff"""".query(FlaffRow.jdbcDecoder).selectStream()

  override def selectById(compositeId: FlaffId): ZIO[ZConnection, Throwable, Option[FlaffRow]] = sql"""select "code", "another_code", "some_number", "specifier", "parentspecifier" from "public"."flaff" where "code" = ${Segment.paramSegment(compositeId.code)(ShortText.setter)} AND "another_code" = ${Segment.paramSegment(compositeId.anotherCode)(Setter.stringSetter)} AND "some_number" = ${Segment.paramSegment(compositeId.someNumber)(Setter.intSetter)} AND "specifier" = ${Segment.paramSegment(compositeId.specifier)(ShortText.setter)}""".query(FlaffRow.jdbcDecoder).selectOne
  ...
  override def upsert(unsaved: FlaffRow): ZIO[ZConnection, Throwable, UpdateResult[FlaffRow]] = {
    ...
    returning "code", "another_code", "some_number", "specifier", "parentspecifier"""".insertReturning(FlaffRow.jdbcDecoder)
  }
  ...
  override def upsertStreaming(
    ...
    val copied = streamingInsert(s"""copy flaff_TEMP("code", "another_code", "some_number", "specifier", "parentspecifier") from stdin""", batchSize, unsaved)(FlaffRow.pgText)
