...
class OnlyPkColumnsRepoImpl extends OnlyPkColumnsRepo {
  ...
  override def deleteById(compositeId: OnlyPkColumnsId): ZIO[ZConnection, Throwable, Boolean] = sql"""delete from "public"."only_pk_columns" where "key_column_1" = ${Segment.paramSegment(compositeId.keyColumn1)(Setter.stringSetter)} AND "key_column_2" = ${Segment.paramSegment(compositeId.keyColumn2)(Setter.intSetter)}""".delete.map(_ > 0)
  ...
  override def insert(unsaved: OnlyPkColumnsRow): ZIO[ZConnection, Throwable, OnlyPkColumnsRow] = {
    ...
    """.insertReturning(OnlyPkColumnsRow.jdbcDecoder).map(_.updatedKeys.head)
  }

  override def insertStreaming(
    unsaved: ZStream[ZConnection, Throwable, OnlyPkColumnsRow],
    batchSize: Int = 10000
  ): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY "public"."only_pk_columns"("key_column_1", "key_column_2") FROM STDIN""", batchSize, unsaved)(OnlyPkColumnsRow.pgText)
  ...
  override def upsert(unsaved: OnlyPkColumnsRow): ZIO[ZConnection, Throwable, UpdateResult[OnlyPkColumnsRow]] = {
    ...
    returning "key_column_1", "key_column_2"""".insertReturning(OnlyPkColumnsRow.jdbcDecoder)
  }
  ...
  override def upsertStreaming(
    ...
    val copied = streamingInsert(s"""copy only_pk_columns_TEMP("key_column_1", "key_column_2") from stdin""", batchSize, unsaved)(OnlyPkColumnsRow.pgText)
