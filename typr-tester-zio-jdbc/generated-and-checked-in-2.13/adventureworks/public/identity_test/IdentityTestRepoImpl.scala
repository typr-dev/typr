...
class IdentityTestRepoImpl extends IdentityTestRepo {
  ...
  override def deleteById(name: IdentityTestId): ZIO[ZConnection, Throwable, Boolean] = sql"""delete from "public"."identity-test" where "name" = ${Segment.paramSegment(name)(IdentityTestId.setter)}""".delete.map(_ > 0)

  override def deleteByIds(names: Array[IdentityTestId]): ZIO[ZConnection, Throwable, Long] = sql"""delete from "public"."identity-test" where "name" = ANY(${Segment.paramSegment(names)(IdentityTestId.arraySetter)})""".delete
  ...
  override def insertStreaming(
    ...
    batchSize: Int = 10000
  ): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY "public"."identity-test"("default_generated", "name") FROM STDIN""", batchSize, unsaved)(IdentityTestRow.pgText)

  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(
    unsaved: ZStream[ZConnection, Throwable, IdentityTestRowUnsaved],
    batchSize: Int = 10000
  ): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY "public"."identity-test"("name", "default_generated") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(IdentityTestRowUnsaved.pgText)
  ...
  override def selectAll: ZStream[ZConnection, Throwable, IdentityTestRow] = sql"""select "always_generated", "default_generated", "name" from "public"."identity-test"""".query(IdentityTestRow.jdbcDecoder).selectStream()

  override def selectById(name: IdentityTestId): ZIO[ZConnection, Throwable, Option[IdentityTestRow]] = sql"""select "always_generated", "default_generated", "name" from "public"."identity-test" where "name" = ${Segment.paramSegment(name)(IdentityTestId.setter)}""".query(IdentityTestRow.jdbcDecoder).selectOne

  override def selectByIds(names: Array[IdentityTestId]): ZStream[ZConnection, Throwable, IdentityTestRow] = sql"""select "always_generated", "default_generated", "name" from "public"."identity-test" where "name" = ANY(${Segment.paramSegment(names)(IdentityTestId.arraySetter)})""".query(IdentityTestRow.jdbcDecoder).selectStream()
  ...
  override def upsertStreaming(
    ...
    val copied = streamingInsert(s"""copy identity-test_TEMP("default_generated", "name") from stdin""", batchSize, unsaved)(IdentityTestRow.pgText)
