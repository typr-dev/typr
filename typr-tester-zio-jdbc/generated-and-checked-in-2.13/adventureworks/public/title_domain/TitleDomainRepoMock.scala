...
case class TitleDomainRepoMock(map: scala.collection.mutable.Map[TitleDomainId, TitleDomainRow] = scala.collection.mutable.Map.empty[TitleDomainId, TitleDomainRow]) extends TitleDomainRepo {
  ...
  override def deleteById(code: TitleDomainId): ZIO[ZConnection, Throwable, Boolean] = ZIO.succeed(map.remove(code).isDefined)

  override def deleteByIds(codes: Array[TitleDomainId]): ZIO[ZConnection, Throwable, Long] = ZIO.succeed(codes.map(id => map.remove(id)).count(_.isDefined).toLong)
  ...
  override def select: SelectBuilder[TitleDomainFields, TitleDomainRow] = SelectBuilderMock(TitleDomainFields.structure, ZIO.succeed(Chunk.fromIterable(map.values)), SelectParams.empty)

  override def selectAll: ZStream[ZConnection, Throwable, TitleDomainRow] = ZStream.fromIterable(map.values)

  override def selectById(code: TitleDomainId): ZIO[ZConnection, Throwable, Option[TitleDomainRow]] = ZIO.succeed(map.get(code))

  override def selectByIds(codes: Array[TitleDomainId]): ZStream[ZConnection, Throwable, TitleDomainRow] = ZStream.fromIterable(codes.flatMap(map.get))
