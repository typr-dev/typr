...
class CurrencyRepoImpl extends CurrencyRepo {
  ...
  override def deleteById(currencycode: CurrencyId): ZIO[ZConnection, Throwable, Boolean] = sql"""delete from "sales"."currency" where "currencycode" = ${Segment.paramSegment(currencycode)(CurrencyId.setter)}""".delete.map(_ > 0)

  override def deleteByIds(currencycodes: Array[CurrencyId]): ZIO[ZConnection, Throwable, Long] = sql"""delete from "sales"."currency" where "currencycode" = ANY(${Segment.paramSegment(currencycodes)(CurrencyId.arraySetter)})""".delete
  ...
  override def insertStreaming(
    ...
    batchSize: Int = 10000
  ): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY "sales"."currency"("currencycode", "name", "modifieddate") FROM STDIN""", batchSize, unsaved)(CurrencyRow.pgText)

  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(
    unsaved: ZStream[ZConnection, Throwable, CurrencyRowUnsaved],
    batchSize: Int = 10000
  ): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY "sales"."currency"("currencycode", "name", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(CurrencyRowUnsaved.pgText)
  ...
  override def selectAll: ZStream[ZConnection, Throwable, CurrencyRow] = sql"""select "currencycode", "name", "modifieddate"::text from "sales"."currency"""".query(CurrencyRow.jdbcDecoder).selectStream()

  override def selectById(currencycode: CurrencyId): ZIO[ZConnection, Throwable, Option[CurrencyRow]] = sql"""select "currencycode", "name", "modifieddate"::text from "sales"."currency" where "currencycode" = ${Segment.paramSegment(currencycode)(CurrencyId.setter)}""".query(CurrencyRow.jdbcDecoder).selectOne

  override def selectByIds(currencycodes: Array[CurrencyId]): ZStream[ZConnection, Throwable, CurrencyRow] = sql"""select "currencycode", "name", "modifieddate"::text from "sales"."currency" where "currencycode" = ANY(${Segment.paramSegment(currencycodes)(CurrencyId.arraySetter)})""".query(CurrencyRow.jdbcDecoder).selectStream()
  ...
  override def upsertStreaming(
    ...
    val copied = streamingInsert(s"""copy currency_TEMP("currencycode", "name", "modifieddate") from stdin""", batchSize, unsaved)(CurrencyRow.pgText)
