...
class CustomerRepoImpl extends CustomerRepo {
  ...
  override def deleteById(customerid: CustomerId): ZIO[ZConnection, Throwable, Boolean] = sql"""delete from "sales"."customer" where "customerid" = ${Segment.paramSegment(customerid)(CustomerId.setter)}""".delete.map(_ > 0)

  override def deleteByIds(customerids: Array[CustomerId]): ZIO[ZConnection, Throwable, Long] = sql"""delete from "sales"."customer" where "customerid" = ANY(${Segment.paramSegment(customerids)(CustomerId.arraySetter)})""".delete
  ...
  override def insertStreaming(
    ...
    batchSize: Int = 10000
  ): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY "sales"."customer"("customerid", "personid", "storeid", "territoryid", "rowguid", "modifieddate") FROM STDIN""", batchSize, unsaved)(CustomerRow.pgText)

  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(
    unsaved: ZStream[ZConnection, Throwable, CustomerRowUnsaved],
    batchSize: Int = 10000
  ): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY "sales"."customer"("personid", "storeid", "territoryid", "customerid", "rowguid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(CustomerRowUnsaved.pgText)
  ...
  override def selectAll: ZStream[ZConnection, Throwable, CustomerRow] = sql"""select "customerid", "personid", "storeid", "territoryid", "rowguid", "modifieddate"::text from "sales"."customer"""".query(CustomerRow.jdbcDecoder).selectStream()

  override def selectById(customerid: CustomerId): ZIO[ZConnection, Throwable, Option[CustomerRow]] = sql"""select "customerid", "personid", "storeid", "territoryid", "rowguid", "modifieddate"::text from "sales"."customer" where "customerid" = ${Segment.paramSegment(customerid)(CustomerId.setter)}""".query(CustomerRow.jdbcDecoder).selectOne

  override def selectByIds(customerids: Array[CustomerId]): ZStream[ZConnection, Throwable, CustomerRow] = sql"""select "customerid", "personid", "storeid", "territoryid", "rowguid", "modifieddate"::text from "sales"."customer" where "customerid" = ANY(${Segment.paramSegment(customerids)(CustomerId.arraySetter)})""".query(CustomerRow.jdbcDecoder).selectStream()
  ...
  override def upsertStreaming(
    ...
    val copied = streamingInsert(s"""copy customer_TEMP("customerid", "personid", "storeid", "territoryid", "rowguid", "modifieddate") from stdin""", batchSize, unsaved)(CustomerRow.pgText)
