...
case class ProductmodelproductdescriptioncultureRepoMock(
  ...
  override def deleteById(compositeId: ProductmodelproductdescriptioncultureId): ZIO[ZConnection, Throwable, Boolean] = ZIO.succeed(map.remove(compositeId).isDefined)

  override def deleteByIds(compositeIds: Array[ProductmodelproductdescriptioncultureId]): ZIO[ZConnection, Throwable, Long] = ZIO.succeed(compositeIds.map(id => map.remove(id)).count(_.isDefined).toLong)
  ...
  override def insert(unsaved: ProductmodelproductdescriptioncultureRowUnsaved): ZIO[ZConnection, Throwable, ProductmodelproductdescriptioncultureRow] = insert(toRow(unsaved))
  ...
  override def select: SelectBuilder[ProductmodelproductdescriptioncultureFields, ProductmodelproductdescriptioncultureRow] = SelectBuilderMock(ProductmodelproductdescriptioncultureFields.structure, ZIO.succeed(Chunk.fromIterable(map.values)), SelectParams.empty)

  override def selectAll: ZStream[ZConnection, Throwable, ProductmodelproductdescriptioncultureRow] = ZStream.fromIterable(map.values)

  override def selectById(compositeId: ProductmodelproductdescriptioncultureId): ZIO[ZConnection, Throwable, Option[ProductmodelproductdescriptioncultureRow]] = ZIO.succeed(map.get(compositeId))

  override def selectByIds(compositeIds: Array[ProductmodelproductdescriptioncultureId]): ZStream[ZConnection, Throwable, ProductmodelproductdescriptioncultureRow] = ZStream.fromIterable(compositeIds.flatMap(map.get))
