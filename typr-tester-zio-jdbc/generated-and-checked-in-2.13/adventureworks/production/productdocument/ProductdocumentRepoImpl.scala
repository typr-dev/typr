...
class ProductdocumentRepoImpl extends ProductdocumentRepo {
  ...
  override def insertStreaming(
    ...
    batchSize: Int = 10000
  ): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY "production"."productdocument"("productid", "modifieddate", "documentnode") FROM STDIN""", batchSize, unsaved)(ProductdocumentRow.pgText)

  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(
    unsaved: ZStream[ZConnection, Throwable, ProductdocumentRowUnsaved],
    batchSize: Int = 10000
  ): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY "production"."productdocument"("productid", "modifieddate", "documentnode") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(ProductdocumentRowUnsaved.pgText)
  ...
  override def selectAll: ZStream[ZConnection, Throwable, ProductdocumentRow] = sql"""select "productid", "modifieddate"::text, "documentnode" from "production"."productdocument"""".query(ProductdocumentRow.jdbcDecoder).selectStream()

  override def selectById(compositeId: ProductdocumentId): ZIO[ZConnection, Throwable, Option[ProductdocumentRow]] = sql"""select "productid", "modifieddate"::text, "documentnode" from "production"."productdocument" where "productid" = ${Segment.paramSegment(compositeId.productid)(ProductId.setter)} AND "documentnode" = ${Segment.paramSegment(compositeId.documentnode)(DocumentId.setter)}""".query(ProductdocumentRow.jdbcDecoder).selectOne
  ...
  override def upsertStreaming(
    ...
    val copied = streamingInsert(s"""copy productdocument_TEMP("productid", "modifieddate", "documentnode") from stdin""", batchSize, unsaved)(ProductdocumentRow.pgText)
