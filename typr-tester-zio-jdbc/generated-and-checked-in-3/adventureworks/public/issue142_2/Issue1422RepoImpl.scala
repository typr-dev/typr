...
class Issue1422RepoImpl extends Issue1422Repo {
  ...
  override def deleteById(tabellkode: Issue142Id): ZIO[ZConnection, Throwable, Boolean] = sql"""delete from "public"."issue142_2" where "tabellkode" = ${Segment.paramSegment(tabellkode)(using Issue142Id.setter)}""".delete.map(_ > 0)

  override def deleteByIds(tabellkodes: Array[Issue142Id]): ZIO[ZConnection, Throwable, Long] = sql"""delete from "public"."issue142_2" where "tabellkode" = ANY(${Segment.paramSegment(tabellkodes)(using Issue142Id.arraySetter)})""".delete
  ...
  override def insertStreaming(
    ...
    batchSize: Int = 10000
  ): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY "public"."issue142_2"("tabellkode") FROM STDIN""", batchSize, unsaved)(using Issue1422Row.pgText)
  ...
  override def selectAll: ZStream[ZConnection, Throwable, Issue1422Row] = sql"""select "tabellkode" from "public"."issue142_2"""".query(using Issue1422Row.jdbcDecoder).selectStream()

  override def selectById(tabellkode: Issue142Id): ZIO[ZConnection, Throwable, Option[Issue1422Row]] = sql"""select "tabellkode" from "public"."issue142_2" where "tabellkode" = ${Segment.paramSegment(tabellkode)(using Issue142Id.setter)}""".query(using Issue1422Row.jdbcDecoder).selectOne

  override def selectByIds(tabellkodes: Array[Issue142Id]): ZStream[ZConnection, Throwable, Issue1422Row] = sql"""select "tabellkode" from "public"."issue142_2" where "tabellkode" = ANY(${Segment.paramSegment(tabellkodes)(using Issue142Id.arraySetter)})""".query(using Issue1422Row.jdbcDecoder).selectStream()
  ...
  override def upsertStreaming(
    ...
    val copied = streamingInsert(s"""copy issue142_2_TEMP("tabellkode") from stdin""", batchSize, unsaved)(using Issue1422Row.pgText)
