...
class PmiViewRepoImpl extends PmiViewRepo {
  ...
  override def selectAll: ZStream[ZConnection, Throwable, PmiViewRow] = sql"""select "productmodelid", "illustrationid", "modifieddate"::text from "pr"."pmi"""".query(using PmiViewRow.jdbcDecoder).selectStream()
}