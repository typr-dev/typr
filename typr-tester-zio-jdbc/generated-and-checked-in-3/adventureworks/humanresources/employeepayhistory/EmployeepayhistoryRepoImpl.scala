...
class EmployeepayhistoryRepoImpl extends EmployeepayhistoryRepo {
  ...
  override def deleteByIds(compositeIds: Array[EmployeepayhistoryId]): ZIO[ZConnection, Throwable, Long] = {
    ...
    in (select unnest(${Segment.paramSegment(businessentityid)(using BusinessentityId.arraySetter)}), unnest(${Segment.paramSegment(ratechangedate)(using TypoLocalDateTime.arraySetter)}))
    """.delete
  }
  ...
  override def upsertStreaming(
    ...
    val copied = streamingInsert(s"""copy employeepayhistory_TEMP("businessentityid", "ratechangedate", "rate", "payfrequency", "modifieddate") from stdin""", batchSize, unsaved)(using EmployeepayhistoryRow.pgText)
