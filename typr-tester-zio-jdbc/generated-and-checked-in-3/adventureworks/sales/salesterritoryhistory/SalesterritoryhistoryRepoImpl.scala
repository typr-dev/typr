...
class SalesterritoryhistoryRepoImpl extends SalesterritoryhistoryRepo {
  ...
  override def deleteByIds(compositeIds: Array[SalesterritoryhistoryId]): ZIO[ZConnection, Throwable, Long] = {
    ...
    in (select unnest(${Segment.paramSegment(businessentityid)(using BusinessentityId.arraySetter)}), unnest(${Segment.paramSegment(startdate)(using TypoLocalDateTime.arraySetter)}), unnest(${Segment.paramSegment(territoryid)(using SalesterritoryId.arraySetter)}))
    """.delete
  }
  ...
  override def upsertStreaming(
    ...
    val copied = streamingInsert(s"""copy salesterritoryhistory_TEMP("businessentityid", "territoryid", "startdate", "enddate", "rowguid", "modifieddate") from stdin""", batchSize, unsaved)(using SalesterritoryhistoryRow.pgText)
