...
object TypoLine {
  given arrayJdbcDecoder: JdbcDecoder[Array[TypoLine]] = {
    JdbcDecoder[Array[TypoLine]]((rs: ResultSet) => (i: Int) =>
      rs.getArray(i) match {
        case null => null
        case arr => arr.getArray.asInstanceOf[Array[AnyRef]].map(x => new TypoLine(x.asInstanceOf[PGline].a, x.asInstanceOf[PGline].b, x.asInstanceOf[PGline].c))
      },
      "Array[org.postgresql.geometric.PGline]"
    )
  }

  given arrayJdbcEncoder: JdbcEncoder[Array[TypoLine]] = JdbcEncoder.singleParamEncoder(using arraySetter)

  given arraySetter: Setter[Array[TypoLine]] = {
  ...
  given jdbcDecoder: JdbcDecoder[TypoLine] = {
    JdbcDecoder[TypoLine](
      (rs: ResultSet) => (i: Int) => {
        val v = rs.getObject(i)
        if (v eq null) null else new TypoLine(v.asInstanceOf[PGline].a, v.asInstanceOf[PGline].b, v.asInstanceOf[PGline].c)
      },
      "org.postgresql.geometric.PGline"
    )
  }

  given jdbcEncoder: JdbcEncoder[TypoLine] = JdbcEncoder.singleParamEncoder(using setter)

  given jsonDecoder: JsonDecoder[TypoLine] = {
    JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
      val a = jsonObj.get("a").toRight("Missing field 'a'").flatMap(_.as(using JsonDecoder.double))
      val b = jsonObj.get("b").toRight("Missing field 'b'").flatMap(_.as(using JsonDecoder.double))
      val c = jsonObj.get("c").toRight("Missing field 'c'").flatMap(_.as(using JsonDecoder.double))
  ...
  given jsonEncoder: JsonEncoder[TypoLine] = {
