/**
 * File has been automatically generated by `typo`.
 *
 * IF YOU CHANGE THIS FILE YOUR CHANGES WILL BE OVERWRITTEN.
 */
package adventureworks.customtypes

import adventureworks.Text
import java.sql.ResultSet
import java.sql.Types
import org.postgresql.util.PGobject
import typr.dsl.Bijection
import typr.dsl.PGType
import zio.jdbc.JdbcDecoder
import zio.jdbc.JdbcEncoder
import zio.jdbc.SqlFragment.Setter
import zio.json.JsonDecoder
import zio.json.JsonEncoder

/** inet (via PGObject) */
case class TypoInet(value: String)

object TypoInet {
  given arrayJdbcDecoder: JdbcDecoder[Array[TypoInet]] = {
    JdbcDecoder[Array[TypoInet]]((rs: ResultSet) => (i: Int) =>
      rs.getArray(i) match {
        case null => null
        case arr => arr.getArray.asInstanceOf[Array[AnyRef]].map(x => new TypoInet(x.asInstanceOf[PGobject].getValue))
      },
      "Array[org.postgresql.util.PGobject]"
    )
  }

  given arrayJdbcEncoder: JdbcEncoder[Array[TypoInet]] = JdbcEncoder.singleParamEncoder(using arraySetter)

  given arraySetter: Setter[Array[TypoInet]] = {
    Setter.forSqlType((ps, i, v) =>
      ps.setArray(
        i,
        ps.getConnection.createArrayOf(
          "inet",
          v.map { vv =>
            {
              val obj = new PGobject()
              obj.setType("inet")
              obj.setValue(vv.value)
              obj
            }
          }
        )
      ),
      Types.ARRAY
    )
  }

  given bijection: Bijection[TypoInet, String] = Bijection.apply[TypoInet, String](_.value)(TypoInet.apply)

  given jdbcDecoder: JdbcDecoder[TypoInet] = {
    JdbcDecoder[TypoInet](
      (rs: ResultSet) => (i: Int) => {
        val v = rs.getObject(i)
        if (v eq null) null else new TypoInet(v.asInstanceOf[PGobject].getValue)
      },
      "org.postgresql.util.PGobject"
    )
  }

  given jdbcEncoder: JdbcEncoder[TypoInet] = JdbcEncoder.singleParamEncoder(using setter)

  given jsonDecoder: JsonDecoder[TypoInet] = JsonDecoder.string.map(TypoInet.apply)

  given jsonEncoder: JsonEncoder[TypoInet] = JsonEncoder.string.contramap(_.value)

  given pgText: Text[TypoInet] = {
    new Text[TypoInet] {
      override def unsafeEncode(v: TypoInet, sb: StringBuilder): Unit = Text.stringInstance.unsafeEncode(v.value, sb)
      override def unsafeArrayEncode(v: TypoInet, sb: StringBuilder): Unit = Text.stringInstance.unsafeArrayEncode(v.value, sb)
    }
  }

  given pgType: PGType[TypoInet] = PGType.instance[TypoInet]("inet", Types.OTHER)

  given setter: Setter[TypoInet] = {
    Setter.other(
      (ps, i, v) => {
        ps.setObject(
          i,
          {
            val obj = new PGobject()
            obj.setType("inet")
            obj.setValue(v.value)
            obj
          }
        )
      },
      "inet"
    )
  }
}