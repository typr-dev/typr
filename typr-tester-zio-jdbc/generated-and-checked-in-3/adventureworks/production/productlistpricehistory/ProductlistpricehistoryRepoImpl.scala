...
class ProductlistpricehistoryRepoImpl extends ProductlistpricehistoryRepo {
  ...
  override def deleteById(compositeId: ProductlistpricehistoryId): ZIO[ZConnection, Throwable, Boolean] = sql"""delete from "production"."productlistpricehistory" where "productid" = ${Segment.paramSegment(compositeId.productid)(using ProductId.setter)} AND "startdate" = ${Segment.paramSegment(compositeId.startdate)(using TypoLocalDateTime.setter)}""".delete.map(_ > 0)
  ...
  override def insert(unsaved: ProductlistpricehistoryRow): ZIO[ZConnection, Throwable, ProductlistpricehistoryRow] = {
    ...
    values (${Segment.paramSegment(unsaved.productid)(using ProductId.setter)}::int4, ${Segment.paramSegment(unsaved.startdate)(using TypoLocalDateTime.setter)}::timestamp, ${Segment.paramSegment(unsaved.enddate)(using Setter.optionParamSetter(using TypoLocalDateTime.setter))}::timestamp, ${Segment.paramSegment(unsaved.listprice)(using Setter.bigDecimalScalaSetter)}::numeric, ${Segment.paramSegment(unsaved.modifieddate)(using TypoLocalDateTime.setter)}::timestamp)
  ...
  override def insertStreaming(
    ...
    batchSize: Int = 10000
  ): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY "production"."productlistpricehistory"("productid", "startdate", "enddate", "listprice", "modifieddate") FROM STDIN""", batchSize, unsaved)(using ProductlistpricehistoryRow.pgText)

  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(
    unsaved: ZStream[ZConnection, Throwable, ProductlistpricehistoryRowUnsaved],
    batchSize: Int = 10000
  ): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY "production"."productlistpricehistory"("productid", "startdate", "enddate", "listprice", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(using ProductlistpricehistoryRowUnsaved.pgText)
  ...
  override def selectAll: ZStream[ZConnection, Throwable, ProductlistpricehistoryRow] = sql"""select "productid", "startdate"::text, "enddate"::text, "listprice", "modifieddate"::text from "production"."productlistpricehistory"""".query(using ProductlistpricehistoryRow.jdbcDecoder).selectStream()

  override def selectById(compositeId: ProductlistpricehistoryId): ZIO[ZConnection, Throwable, Option[ProductlistpricehistoryRow]] = sql"""select "productid", "startdate"::text, "enddate"::text, "listprice", "modifieddate"::text from "production"."productlistpricehistory" where "productid" = ${Segment.paramSegment(compositeId.productid)(using ProductId.setter)} AND "startdate" = ${Segment.paramSegment(compositeId.startdate)(using TypoLocalDateTime.setter)}""".query(using ProductlistpricehistoryRow.jdbcDecoder).selectOne
  ...
  override def update(row: ProductlistpricehistoryRow): ZIO[ZConnection, Throwable, Option[ProductlistpricehistoryRow]] = {
    ...
    returning "productid", "startdate"::text, "enddate"::text, "listprice", "modifieddate"::text"""
      .query(using ProductlistpricehistoryRow.jdbcDecoder)
      .selectOne
  }
  ...
  override def upsertStreaming(
    ...
    val copied = streamingInsert(s"""copy productlistpricehistory_TEMP("productid", "startdate", "enddate", "listprice", "modifieddate") from stdin""", batchSize, unsaved)(using ProductlistpricehistoryRow.pgText)
