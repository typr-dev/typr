...
public class GroupedBuilderMock<Fields, Row> implements GroupedBuilder<Fields, Row> {
  ...
  record GroupedSelectBuilderMock<
     ...
     * simpler.
     */
    private static Object convertSumToObject(
        BigDecimal value, dev.typr.foundations.DbType<?> targetType) {
      // Check for common numeric types by comparing to known PgTypes
      if (targetType == dev.typr.foundations.PgTypes.int8) {
        return Long.valueOf(value.longValue());
      } else if (targetType == dev.typr.foundations.PgTypes.int4) {
        return Integer.valueOf(value.intValue());
      } else if (targetType == dev.typr.foundations.PgTypes.int2) {
        return Short.valueOf(value.shortValue());
      } else if (targetType == dev.typr.foundations.PgTypes.float8) {
        return Double.valueOf(value.doubleValue());
      } else if (targetType == dev.typr.foundations.PgTypes.float4) {
        return Float.valueOf(value.floatValue());
      } else if (targetType == dev.typr.foundations.PgTypes.numeric) {
        return value;
      } else {
