/**
 * File has been automatically generated by `typo`.
 *
 * IF YOU CHANGE THIS FILE YOUR CHANGES WILL BE OVERWRITTEN.
 */
package testdb.shipping_carriers;

import com.fasterxml.jackson.annotation.JsonProperty;
import java.util.Optional;
import testdb.customtypes.Defaulted;
import typo.runtime.MariaText;
import typo.runtime.MariaTypes;
import typo.runtime.RowParser;
import typo.runtime.RowParsers;

/** Table: shipping_carriers
  * Primary key: carrier_id
  */
public record ShippingCarriersRow(
  /** 
    * AUTO_INCREMENT
    */
  @JsonProperty("carrier_id") ShippingCarriersId carrierId,
  /**  */
  String code,
  /**  */
  String name,
  /** 
    * Default: NULL
    */
  @JsonProperty("tracking_url_template") Optional<String> trackingUrlTemplate,
  /** 
    * Default: NULL
    */
  @JsonProperty("api_config") Optional<String> apiConfig,
  /** 
    * Default: 1
    */
  @JsonProperty("is_active") Boolean isActive
) {
  /** 
    * AUTO_INCREMENT
    */
  public ShippingCarriersRow withCarrierId(ShippingCarriersId carrierId) {
    return new ShippingCarriersRow(carrierId, code, name, trackingUrlTemplate, apiConfig, isActive);
  };

  /**  */
  public ShippingCarriersRow withCode(String code) {
    return new ShippingCarriersRow(carrierId, code, name, trackingUrlTemplate, apiConfig, isActive);
  };

  /**  */
  public ShippingCarriersRow withName(String name) {
    return new ShippingCarriersRow(carrierId, code, name, trackingUrlTemplate, apiConfig, isActive);
  };

  /** 
    * Default: NULL
    */
  public ShippingCarriersRow withTrackingUrlTemplate(Optional<String> trackingUrlTemplate) {
    return new ShippingCarriersRow(carrierId, code, name, trackingUrlTemplate, apiConfig, isActive);
  };

  /** 
    * Default: NULL
    */
  public ShippingCarriersRow withApiConfig(Optional<String> apiConfig) {
    return new ShippingCarriersRow(carrierId, code, name, trackingUrlTemplate, apiConfig, isActive);
  };

  /** 
    * Default: 1
    */
  public ShippingCarriersRow withIsActive(Boolean isActive) {
    return new ShippingCarriersRow(carrierId, code, name, trackingUrlTemplate, apiConfig, isActive);
  };

  static RowParser<ShippingCarriersRow> _rowParser = RowParsers.of(ShippingCarriersId.pgType, MariaTypes.varchar, MariaTypes.varchar, MariaTypes.varchar.opt(), MariaTypes.longtext.opt(), MariaTypes.bool, ShippingCarriersRow::new, row -> new Object[]{row.carrierId(), row.code(), row.name(), row.trackingUrlTemplate(), row.apiConfig(), row.isActive()});;

  static public MariaText<ShippingCarriersRow> mariaText =
    MariaText.from(_rowParser);

  public ShippingCarriersId id() {
    return carrierId;
  };

  public ShippingCarriersRowUnsaved toUnsavedRow(
    Defaulted<Optional<String>> trackingUrlTemplate,
    Defaulted<Optional<String>> apiConfig,
    Defaulted<Boolean> isActive
  ) {
    return new ShippingCarriersRowUnsaved(code, name, trackingUrlTemplate, apiConfig, isActive);
  };
}