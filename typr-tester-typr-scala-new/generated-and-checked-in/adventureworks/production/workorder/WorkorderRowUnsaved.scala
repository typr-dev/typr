...
object WorkorderRowUnsaved {
  given pgText: PgText[WorkorderRowUnsaved] = PgText.instance((row, sb) => { ProductId.pgType.pgText.unsafeEncode(row.productid, sb); sb.append(PgText.DELIMETER); ScalaDbTypes.PgTypes.int4.pgText.unsafeEncode(row.orderqty, sb); sb.append(PgText.DELIMETER); ScalaDbTypes.PgTypes.int2.pgText.unsafeEncode(row.scrappedqty, sb); sb.append(PgText.DELIMETER); PgTypes.timestamp.pgText.unsafeEncode(row.startdate, sb); sb.append(PgText.DELIMETER); PgTypes.timestamp.nullable.pgText.unsafeEncode(row.enddate, sb); sb.append(PgText.DELIMETER); PgTypes.timestamp.pgText.unsafeEncode(row.duedate, sb); sb.append(PgText.DELIMETER); ScrapreasonId.pgType.nullable.pgText.unsafeEncode(row.scrapreasonid, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using WorkorderId.pgType.pgText).unsafeEncode(row.workorderid, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using PgTypes.timestamp.pgText).unsafeEncode(row.modifieddate, sb) })
}