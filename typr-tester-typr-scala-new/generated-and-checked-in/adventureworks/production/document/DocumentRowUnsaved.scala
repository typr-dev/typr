...
object DocumentRowUnsaved {
  given pgText: PgText[DocumentRowUnsaved] = PgText.instance((row, sb) => { PgTypes.text.pgText.unsafeEncode(row.title, sb); sb.append(PgText.DELIMETER); BusinessentityId.pgType.pgText.unsafeEncode(row.owner, sb); sb.append(PgText.DELIMETER); PgTypes.text.pgText.unsafeEncode(row.filename, sb); sb.append(PgText.DELIMETER); PgTypes.text.nullable.pgText.unsafeEncode(row.fileextension, sb); sb.append(PgText.DELIMETER); PgTypes.bpchar.pgText.unsafeEncode(row.revision, sb); sb.append(PgText.DELIMETER); ScalaDbTypes.PgTypes.int2.pgText.unsafeEncode(row.status, sb); sb.append(PgText.DELIMETER); PgTypes.text.nullable.pgText.unsafeEncode(row.documentsummary, sb); sb.append(PgText.DELIMETER); PgTypes.bytea.nullable.pgText.unsafeEncode(row.document, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using Flag.pgType.pgText).unsafeEncode(row.folderflag, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using ScalaDbTypes.PgTypes.int4.pgText).unsafeEncode(row.changenumber, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using PgTypes.uuid.pgText).unsafeEncode(row.rowguid, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using PgTypes.timestamp.pgText).unsafeEncode(row.modifieddate, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using DocumentId.pgType.pgText).unsafeEncode(row.documentnode, sb) })
}