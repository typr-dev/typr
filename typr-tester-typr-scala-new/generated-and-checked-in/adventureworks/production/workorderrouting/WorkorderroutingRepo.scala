...
trait WorkorderroutingRepo {
  ...
  def update(row: WorkorderroutingRow)(using c: Connection): Boolean

  def upsert(unsaved: WorkorderroutingRow)(using c: Connection): WorkorderroutingRow

  def upsertBatch(unsaved: Iterator[WorkorderroutingRow])(using c: Connection): List[WorkorderroutingRow]
