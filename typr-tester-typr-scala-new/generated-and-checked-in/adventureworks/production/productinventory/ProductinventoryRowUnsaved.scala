...
case class ProductinventoryRowUnsaved(
   ...
   */
  quantity: Defaulted[Short] = new UseDefault(),
...
object ProductinventoryRowUnsaved {
  given pgText: PgText[ProductinventoryRowUnsaved] = PgText.instance((row, sb) => { ProductId.pgType.pgText.unsafeEncode(row.productid, sb); sb.append(PgText.DELIMETER); LocationId.pgType.pgText.unsafeEncode(row.locationid, sb); sb.append(PgText.DELIMETER); PgTypes.text.pgText.unsafeEncode(row.shelf, sb); sb.append(PgText.DELIMETER); ScalaDbTypes.PgTypes.int2.pgText.unsafeEncode(row.bin, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using ScalaDbTypes.PgTypes.int2.pgText).unsafeEncode(row.quantity, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using PgTypes.uuid.pgText).unsafeEncode(row.rowguid, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using PgTypes.timestamp.pgText).unsafeEncode(row.modifieddate, sb) })
}