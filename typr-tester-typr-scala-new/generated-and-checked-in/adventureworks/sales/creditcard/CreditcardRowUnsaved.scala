...
object CreditcardRowUnsaved {
  given pgText: PgText[CreditcardRowUnsaved] = PgText.instance((row, sb) => { PgTypes.text.pgText.unsafeEncode(row.cardtype, sb); sb.append(PgText.DELIMETER); PgTypes.text.pgText.unsafeEncode(row.cardnumber, sb); sb.append(PgText.DELIMETER); ScalaDbTypes.PgTypes.int2.pgText.unsafeEncode(row.expmonth, sb); sb.append(PgText.DELIMETER); ScalaDbTypes.PgTypes.int2.pgText.unsafeEncode(row.expyear, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using CustomCreditcardId.pgType.pgText).unsafeEncode(row.creditcardid, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using PgTypes.timestamp.pgText).unsafeEncode(row.modifieddate, sb) })
}