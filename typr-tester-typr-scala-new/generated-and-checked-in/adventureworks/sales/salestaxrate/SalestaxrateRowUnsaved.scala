...
object SalestaxrateRowUnsaved {
  given pgText: PgText[SalestaxrateRowUnsaved] = PgText.instance((row, sb) => { StateprovinceId.pgType.pgText.unsafeEncode(row.stateprovinceid, sb); sb.append(PgText.DELIMETER); ScalaDbTypes.PgTypes.int2.pgText.unsafeEncode(row.taxtype, sb); sb.append(PgText.DELIMETER); Name.pgType.pgText.unsafeEncode(row.name, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using SalestaxrateId.pgType.pgText).unsafeEncode(row.salestaxrateid, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using ScalaDbTypes.PgTypes.numeric.pgText).unsafeEncode(row.taxrate, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using PgTypes.uuid.pgText).unsafeEncode(row.rowguid, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using PgTypes.timestamp.pgText).unsafeEncode(row.modifieddate, sb) })
}