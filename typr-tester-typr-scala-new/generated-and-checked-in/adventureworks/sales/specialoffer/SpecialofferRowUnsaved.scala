...
object SpecialofferRowUnsaved {
  given pgText: PgText[SpecialofferRowUnsaved] = PgText.instance((row, sb) => { PgTypes.text.pgText.unsafeEncode(row.description, sb); sb.append(PgText.DELIMETER); PgTypes.text.pgText.unsafeEncode(row.`type`, sb); sb.append(PgText.DELIMETER); PgTypes.text.pgText.unsafeEncode(row.category, sb); sb.append(PgText.DELIMETER); PgTypes.timestamp.pgText.unsafeEncode(row.startdate, sb); sb.append(PgText.DELIMETER); PgTypes.timestamp.pgText.unsafeEncode(row.enddate, sb); sb.append(PgText.DELIMETER); ScalaDbTypes.PgTypes.int4.nullable.pgText.unsafeEncode(row.maxqty, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using SpecialofferId.pgType.pgText).unsafeEncode(row.specialofferid, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using ScalaDbTypes.PgTypes.numeric.pgText).unsafeEncode(row.discountpct, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using ScalaDbTypes.PgTypes.int4.pgText).unsafeEncode(row.minqty, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using PgTypes.uuid.pgText).unsafeEncode(row.rowguid, sb); sb.append(PgText.DELIMETER); Defaulted.pgText(using PgTypes.timestamp.pgText).unsafeEncode(row.modifieddate, sb) })
}