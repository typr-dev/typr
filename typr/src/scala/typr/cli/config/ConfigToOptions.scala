...
import typr.config.generated.{KeyType as ConfigKeyType}
...
object ConfigToOptions {
  ...
  def convertAvroBoundary(
    ...
    val topicKeys = config.topic_keys.getOrElse(Map.empty).map { case (topic, configKey) =>
      topic -> parseKeyType(configKey)
    }

    val defaultKeyType = config.default_key_type.map(parseKeyType).getOrElse(typr.avro.KeyType.StringKey)
  ...
  def toEffectType(s: Option[String]): EffectType = s match {
  ...
  private def parseKeyType(configKey: ConfigKeyType): typr.avro.KeyType = configKey match {
    case KeyTypeEnum("string") => typr.avro.KeyType.StringKey
    case KeyTypeEnum("uuid")   => typr.avro.KeyType.UUIDKey
    case KeyTypeEnum("long")   => typr.avro.KeyType.LongKey
    case KeyTypeEnum("int")    => typr.avro.KeyType.IntKey
    case KeyTypeEnum("bytes")  => typr.avro.KeyType.BytesKey
    case KeyTypeEnum(_)        => typr.avro.KeyType.StringKey
    case obj: KeyTypeObject    => typr.avro.KeyType.SchemaKey(obj.schema)
  }
  ...
  def convertDatabaseSource(name: String, dbConfig: DatabaseBoundary): Either[String, BoundaryConfig] =
