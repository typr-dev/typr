  ...
  def parseSource(json: Json): Either[String, ParsedSource] = {
    ...
    sourceType match {
      ...
      case Some("jsonschema") =>
        json.as[JsonschemaBoundary].map(ParsedSource.JsonSchema.apply).left.map(e => s"Failed to parse JSON Schema source: ${e.getMessage}")
      case Some("avro") =>
...
object ParsedSource {
  case class Database(config: DatabaseBoundary) extends ParsedSource {
  ...
  case class JsonSchema(config: JsonschemaBoundary) extends ParsedSource {
