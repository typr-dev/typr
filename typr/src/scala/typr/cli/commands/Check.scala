...
object Check {
  ...
  def run(
    ...
    val result = for {
      ...
      buildDir = Option(Paths.get(configPath).getParent).getOrElse(Path.of("."))
      _ <- IO.whenA(debug)(IO.println(s"Loading source entities from build dir: $buildDir"))

      sourceEntities <- IO.fromEither(
        SourceEntityLoader
          .loadSourceEntities(config, sourceDeclarations, buildDir, quiet)
          .left
          .map(new Exception(_))
      )
      _ <- IO.whenA(debug)(IO.println(s"Loaded ${sourceEntities.values.map(_.size).sum} source entities from ${sourceEntities.size} sources"))
      ...
      .collect { case (name, ct: DomainType) =>
        ...
        val alignedDecls = (ct.alignedSources.getOrElse(Map.empty) ++ ct.projections.getOrElse(Map.empty)).map { case (key, aligned) =>
          ...
          key -> SourceDeclaration(
            ...
            direction = aligned.direction.flatMap(FlowDirection.fromString).getOrElse(FlowDirection.InOut),
            mode = CompatibilityMode.fromString(aligned.mode.getOrElse("superset")).getOrElse(CompatibilityMode.Superset),
            mappings = aligned.mappings.getOrElse(Map.empty),
            exclude = aligned.exclude.getOrElse(Nil).toSet,
            includeExtra = aligned.include_extra.getOrElse(Nil),
            readonly = aligned.readonly.getOrElse(false),
            defaultTypePolicy = aligned.type_policy.flatMap(TypePolicy.fromString).getOrElse(TypePolicy.Exact),
            fieldOverrides = parseFieldOverrides(aligned.field_overrides.getOrElse(Map.empty))
          )
        }
  ...
  private def parseFieldOverrides(overrides: Map[String, ConfigFieldOverride]): Map[String, FieldOverride] =
    overrides.map { case (fieldName, configOverride) =>
      fieldName -> (configOverride match {
        case FieldOverrideEnum("forward") =>
          FieldOverride.Forward(sourceFieldName = None, typePolicy = None, directionOverride = None)
        case FieldOverrideEnum("drop") =>
          FieldOverride.Drop(reason = None)
        case FieldOverrideEnum(_) =>
          FieldOverride.Forward(sourceFieldName = None, typePolicy = None, directionOverride = None)
        case obj: FieldOverrideObject =>
          obj.action.getOrElse("forward") match {
            case "drop" =>
              FieldOverride.Drop(reason = obj.reason)
            case "custom" =>
              ...
              FieldOverride.Custom(kind)
            case _ =>
              FieldOverride.Forward(
                sourceFieldName = obj.source_field,
                typePolicy = obj.type_policy.flatMap(TypePolicy.fromString),
                directionOverride = obj.direction.flatMap(FieldDirection.fromString)
              )
          }
      })
    }
