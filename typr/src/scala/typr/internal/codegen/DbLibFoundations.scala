...
class DbLibFoundations(
  ...
  val streamingInsert: jvm.Type.Qualified = FoundationsTypes.streamingInsert
  val Fragment: jvm.Type.Qualified = lang.dsl.Fragment
  val SqlStringInterpolation: jvm.Type.Qualified = Fragment
  val KotlinNullableExtension: jvm.Type.Qualified = FoundationsTypes.kotlin.KotlinNullableExtension
  val KotlinQueryExtension: jvm.Type.Qualified = FoundationsTypes.kotlin.KotlinQueryExtension
  val ScalaDbTypeOps: jvm.Type.Qualified = FoundationsTypes.scala.ScalaDbTypeOps
  ...
  val ScalaIteratorOps: jvm.Type.Qualified = FoundationsTypes.scala.ScalaIteratorOps
  val ScalaCollectionConverters: jvm.Type.Qualified = jvm.Type.Qualified("scala.jdk.CollectionConverters")
  ...
  override def rowInstances(tpe: jvm.Type, cols: NonEmptyList[ComputedColumn], rowType: DbLib.RowType): List[jvm.ClassMember] = {
    def rowParser = {
      jvm.Value(
        ...
        Some {
          val dbTypes = cols.toList.map(x => lookupType(x)).mkCode(code", ")
          val row = jvm.Ident("row")
          ...
          val decodeLambda: jvm.Code = lang match {
            case LangScala(_, TypeSupportJava, _) if cols.length > 22 =>
              ...
              val functionTypeName = FoundationsTypes.RowParsersFunctionN(cols.length)
            ...
            case _: LangKotlin =>
              ...
              jvm.Lambda(params.map(p => jvm.LambdaParam(p.name)), jvm.Body.Expr(tpe.construct(args*)))
            case _ =>
          ...
          lang.dsl match {
            case DslQualifiedNames.Scala =>
              code"${lang.dsl.RowParsers}.of($dbTypes)($decodeLambda)($encodeLambda)"
            case _ =>
              code"${lang.dsl.RowParsers}.of($dbTypes, $decodeLambda, $encodeLambda)"
          }
        },
  ...
  override def structInstances(computed: ComputedOracleObjectType): List[jvm.ClassMember] = {
    if (computed.attributes.isEmpty) {
      ...
      val builderStart = jvm.GenericMethodCall(
        target = FoundationsTypes.oracle.OracleObject,
      ...
      List(
        jvm.Value(
          ...
          tpe = FoundationsTypes.oracle.OracleType.of(computed.tpe),
          body = Some(fullChain),
          isLazy = false,
          isOverride = false
        )
  ...
  override def collectionInstances(computed: ComputedOracleCollectionType): List[jvm.ClassMember] = {
    ...
    val baseFactoryCall = (computed.underlying: @unchecked) match {
      case v: db.OracleType.VArray =>
        ...
        code"${FoundationsTypes.oracle.OracleVArray}.of($typeName, ${v.maxSize}, $elementTypeInstance)"
      case n: db.OracleType.NestedTable =>
        val typeName = jvm.StrLit(n.name.name)
        code"${FoundationsTypes.oracle.OracleNestedTable}.of($typeName, $elementTypeInstance)"
    }
