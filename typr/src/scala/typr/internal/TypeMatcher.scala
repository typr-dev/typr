...
/** Evaluates TypeDefinitions predicates against database columns and OpenAPI fields. */
object TypeMatcher {
  ...
  object MatchSource {
    ...
    case class ApiField(spec: String, path: String, fieldName: String, kind: ApiFieldKind) extends MatchSource
  }

  sealed trait ApiFieldKind
  object ApiFieldKind {
    case object ModelProperty extends ApiFieldKind
    case object Parameter extends ApiFieldKind
  }
  ...
  def matchesDb(
    ...
    matchesGlob(m.database, ctx.databaseName) &&
    matchesGlob(m.schema, ctx.schema) &&
    matchesGlob(m.table, ctx.table) &&
    matchesGlob(m.column, col.name.value) &&
    matchesGlob(m.dbType, typeName(col.tpe)) &&
    ...
    matchesGlob(m.references, ctx.references.mkString(",")) &&
    matchesGlob(m.comment, col.comment.getOrElse("")) &&
    matchesGlob(m.annotation, extractAnnotations(col.comment).mkString(","))
  }
  ...
  /** Check if a value matches any of the glob patterns (OR semantics). Empty pattern list means "match any".
    */
  private def matchesGlob(patterns: List[String], value: String): Boolean =
    patterns.isEmpty || patterns.exists(p => globMatch(p, value))
  ...
  // ═══════════════════════════════════════════════════════════════════════════════
  // OpenAPI Field Matching
  // ═══════════════════════════════════════════════════════════════════════════════

  /** Context needed to match an OpenAPI field */
  case class ApiFieldContext(
      specName: String,
      operationId: String,
      httpMethod: String,
      path: String,
  ...
  object ApiFieldContext {
    ...
    def fromParameter(
      ...
      val location = param.in match {
        case openapi.ParameterIn.Path   => ApiLocation.PathParam
        case openapi.ParameterIn.Query  => ApiLocation.QueryParam
        case openapi.ParameterIn.Header => ApiLocation.HeaderParam
        case openapi.ParameterIn.Cookie => ApiLocation.CookieParam
      }
      ApiFieldContext(
        specName = specName,
    ...
    /** Create context for a request body field */
    def fromRequestBody(
        specName: String,
        method: openapi.ApiMethod,
      ...
      ApiFieldContext(
        ...
        httpMethod = httpMethodString(method.httpMethod),
        path = method.path,
        location = ApiLocation.RequestBody,
        fieldName = jsonPath.split("/").lastOption.getOrElse(""),
        jsonPath = jsonPath,
    ...
    /** Create context for a response body */
    def fromResponseBody(
        specName: String,
        method: openapi.ApiMethod,
        response: openapi.ApiResponse,
        jsonPath: String
    ): Option[ApiFieldContext] =
      response.typeInfo.map { typeInfo =>
        ApiFieldContext(
          specName = specName,
          operationId = method.name,
          httpMethod = httpMethodString(method.httpMethod),
          path = method.path,
          location = ApiLocation.ResponseBody,
          fieldName = jsonPath.split("/").lastOption.getOrElse(""),
          jsonPath = jsonPath,
          typeInfo = typeInfo,
          required = true,
  ...
  /** Find all TypeEntry matches for an API field */
  def findApiMatches(
      definitions: TypeDefinitions,
      ctx: ApiFieldContext
  ): List[MatchResult] = {
    val kind = ctx.location match {
      case ApiLocation.ResponseBody | ApiLocation.RequestBody | ApiLocation.AnyBody => ApiFieldKind.ModelProperty
      case _                                                                        => ApiFieldKind.Parameter
    }
    definitions.entries
      .filter { entry =>
      ...
      .map { entry =>
        MatchResult(
          entry = entry,
          source = MatchSource.ApiField(ctx.specName, ctx.path, ctx.fieldName, kind),
          dbType = typeInfoToDbType(ctx.typeInfo)
        )
      }
  }

  /** Evaluate an ApiMatch predicate against an API field context */
  def matchesApi(
      m: ApiMatch,
      ctx: ApiFieldContext
  ): Boolean = {
    matchesLocation(m.location, ctx.location) &&
    matchesGlob(m.spec, ctx.specName) &&
    matchesGlob(m.operationId, ctx.operationId) &&
    matchesGlob(m.httpMethod, ctx.httpMethod) &&
    matchesGlob(m.path, ctx.path) &&
    matchesGlob(m.name, ctx.fieldName) &&
    matchesGlob(m.jsonPath, ctx.jsonPath) &&
    matchesGlob(m.schemaType, schemaTypeName(ctx.typeInfo)) &&
    matchesGlob(m.format, schemaFormat(ctx.typeInfo)) &&
    matchesGlob(m.schemaRef, schemaRefName(ctx.typeInfo)) &&
  ...
  private def matchesLocation(patterns: List[ApiLocation], value: ApiLocation): Boolean =
    patterns.isEmpty || patterns.exists { pattern =>
      pattern == value || expandLocation(pattern).contains(value)
    }

  /** Expand convenience groupings */
  private def expandLocation(loc: ApiLocation): Set[ApiLocation] = loc match {
    case ApiLocation.AnyParam => Set(ApiLocation.PathParam, ApiLocation.QueryParam, ApiLocation.HeaderParam, ApiLocation.CookieParam)
    case ApiLocation.AnyBody  => Set(ApiLocation.RequestBody, ApiLocation.ResponseBody)
  ...
  /** Extract schema $ref name for matching */
  private def schemaRefName(typeInfo: openapi.TypeInfo): String = typeInfo match {
    case openapi.TypeInfo.Ref(name)       => name
    case openapi.TypeInfo.Optional(inner) => schemaRefName(inner)
    case openapi.TypeInfo.ListOf(inner)   => schemaRefName(inner)
    case _                                => ""
  }
  ...
  def scanOpenApi(
    ...
    if (definitions.isEmpty || definitions.entries.forall(_.api == ApiMatch.Empty)) {
      return OpenApiScanResult(Map.empty, Nil, Nil)
    }

    // Collect matches from model properties
    val modelMatches: List[MatchResult] = spec.models.flatMap {
      case obj: openapi.ModelClass.ObjectType =>
        obj.properties.flatMap { prop =>
          val ctx = ApiFieldContext(
            specName = specName,
            operationId = "",
            httpMethod = "",
            path = obj.name,
    ...
    val paramMatches: List[MatchResult] = spec.apis.flatMap { api =>
      api.methods.flatMap { method =>
        method.parameters.flatMap { param =>
          val ctx = ApiFieldContext.fromParameter(specName, method, param)
          findApiMatches(definitions, ctx)
        }
      }
    }
