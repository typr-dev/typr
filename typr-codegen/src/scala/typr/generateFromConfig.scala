  ...
  private def generateDbInstance(
    ...
    val body: jvm.Code = alignment.transform match {
      case TypoType.AlignmentTransform.Direct =>
        ...
        // e.g., MariaTypes.bool.to(Bijection.of(IsActive::new, IsActive::value))
        ...
        code"$baseTypeLookup.to(${lang.dsl.Bijection}.of(${jvm.ConstructorMethodRef(wrapperType)}, ${jvm.FieldGetterRef(wrapperType, value)}))"

      case TypoType.AlignmentTransform.Unpack =>
        // Unpack: bimap through intermediate domain type using proper AST nodes
        // e.g., Name.pgType.to(Bijection.of(x -> new FirstName(x.value()), w -> new Name(w.value())))
        ...
        domainType match {
          case Some(domain) =>
            ...
            code"${jvm.Import(domain)}$domain.${adapter.typeFieldName}.to(${lang.dsl.Bijection}.of($toWrapper, $fromWrapper))"
          case None =>
