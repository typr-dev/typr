...
class ModelCodegen(
  ...
  private def generateObjectType(obj: ModelClass.ObjectType): jvm.File = {
    ...
    // Filter out discriminator properties since they'll be added as override vals
    val discriminatorPropertyNames = obj.discriminatorValues.keySet
    val params = obj.properties.filterNot(prop => discriminatorPropertyNames.contains(prop.originalName)).map { prop =>
  ...
  private def generateWrapperType(wrapper: ModelClass.WrapperType): jvm.File = {
    ...
    // If this wrapper has sum type parents, it can't be an AnyVal (needs to extend trait)
    val hasSumTypeParent = wrapper.sumTypeParents.nonEmpty
    val parentTraits = wrapper.sumTypeParents.map(p => jvm.Type.Qualified(modelPkg / jvm.Ident(p)))
    ...
    val record = jvm.Adt.Record(
      ...
      isWrapper = !hasSumTypeParent, // Can't be AnyVal if extending a trait
      privateConstructor = false,
      comments = comments,
      name = tpe,
      tparams = Nil,
  ...
  def generateSumType(sumType: SumType): jvm.File = {
    ...
    // Add discriminator property method (only for property-based discrimination, not type inference)
    val discriminatorMethod = if (sumType.usesTypeInference) {
      None
    } else {
      Some(
        jvm.Method(
    ...
    val sumAdt = jvm.Adt.Sum(
      ...
      name = tpe,
      tparams = Nil,
      members = discriminatorMethod.toList ++ commonMethods,
      implements = Nil,
