...
object TypeSupportScala extends TypeSupport {
  ...
  override object IteratorOps extends IteratorSupport {
    ...
    def toJavaIterator(iterator: jvm.Code): jvm.Code = {
      val iteratorHasAsJava = jvm.Type.Qualified("scala.jdk.CollectionConverters.IteratorHasAsJava")
      code"${jvm.Import(iteratorHasAsJava, isStatic = false)}$iterator.asJava"
    }
  }
