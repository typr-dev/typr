...
case class OracleTypeMapperDb(
  ...
  private def parseTypeParams(typeName: String): (String, Option[(Int, Option[Int])]) = {
    // Match params at end (e.g., "NUMBER(10,2)") or in the middle (e.g., "TIMESTAMP(6) WITH TIME ZONE").
    val patternEnd = """^([A-Z0-9_ ]+)\s*\(\s*(\d+)(?:\s*,\s*(\d+))?\s*\)$""".r
    val patternMiddle = """^([A-Z0-9_]+)\s*\(\s*(\d+)(?:\s*,\s*(\d+))?\s*\)\s+(.+)$""".r
    typeName match {
      case patternEnd(base, p1, p2) =>
        val precision = p1.toInt
        val scale = Option(p2).map(_.toInt)
        (base.trim, Some((precision, scale)))
      case patternMiddle(prefix, p1, p2, suffix) =>
        val precision = p1.toInt
        val scale = Option(p2).map(_.toInt)
        ((prefix.trim + " " + suffix.trim).trim, Some((precision, scale)))
      case _ =>
        (typeName, None)
    }
  }
