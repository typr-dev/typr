    ...
    results.map { result =>
      val databases = result.matches.flatMap { m =>
        m.source match {
          ...
          case _: TypeMatcher.MatchSource.ModelField =>
            None
          case _: TypeMatcher.MatchSource.ApiParam =>
            None
        }
      }.toSet

      val hasApiUsage = result.matches.exists {
        case TypeMatcher.MatchResult(_, _: TypeMatcher.MatchSource.ModelField, _) => true
        case TypeMatcher.MatchResult(_, _: TypeMatcher.MatchSource.ApiParam, _)   => true
        case _                                                                    => false
      }
