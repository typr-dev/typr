...
case class TypeMapperJvmOld(lang: Lang, typeOverride: TypeOverride, nullabilityOverride: NullabilityOverride, naming: Naming, customTypes: CustomTypes)
  ...
  /** Legacy DSLs (Anorm/Doobie/ZioJdbc) use Array-shaped codecs (`arrayToStatement`, `ParameterMetaData[Array[X]]`), so we keep Array as the container for array columns instead of List. */
  override protected def arrayContainerOf(elementType: jvm.Type): jvm.Type = jvm.Type.ArrayOf(elementType)
    ...
    tpe match {
      case x: db.PgType =>
        x match {
          ...
          case db.PgType.Bit(_)                 => lang.String // bit types as strings for legacy libraries
          case db.PgType.Varbit(_)              => lang.String // varbit types as strings for legacy libraries
