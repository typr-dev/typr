  ...
  override def col(relation: db.RelationName, col: db.Col, typeFromFK: Option[jvm.Type]): jvm.Type = {
    ...
    val maybePrecise = col.tpe match {
      ...
      case db.OracleType.TimestampWithLocalTimeZone(Some(fsp)) if fsp > 0 =>
        Some(jvm.Type.Qualified(naming.preciseInstantNName(fsp)))
    ...
    tpe match {
      case x: db.PgType =>
        x match {
          ...
          case db.PgType.Bit(_)                 => TypesJava.runtime.Bit
          case db.PgType.Varbit(_)              => TypesJava.runtime.Varbit
      ...
      case x: db.OracleType =>
        x match {
          ...
          case db.OracleType.TimestampWithTimeZone(_)      => TypesJava.ZonedDateTime
          case db.OracleType.TimestampWithLocalTimeZone(_) => TypesJava.Instant
