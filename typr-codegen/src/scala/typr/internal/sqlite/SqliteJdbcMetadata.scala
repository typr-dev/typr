...
/** SQLite-specific JDBC metadata extraction for views and queries.
  *
  * The xerial JDBC driver returns much of the column metadata we need via the standard {@link java.sql.ResultSetMetaData} contract — including `getColumnTypeName`, `isNullable`, `getColumnLabel` — so
  * this mirrors the DuckDB shape.
  */
case class SqliteJdbcMetadata(columns: MaybeReturnsRows[NonEmptyList[MetadataColumn]])

object SqliteJdbcMetadata {

  def from(sql: String)(implicit c: Connection): Either[String, SqliteJdbcMetadata] = {
    val ps = c.prepareStatement(sql)
    try from(ps)
    finally ps.close()
  }

  def from(ps: PreparedStatement): Either[String, SqliteJdbcMetadata] = {
    def nonEmpty(str: String): Option[String] = if (str == null || str.isEmpty) None else Some(str)

    Option(ps.getMetaData) match {
      case None =>
        Right(SqliteJdbcMetadata(MaybeReturnsRows.Update))
      ...
      case Some(metadata: ResultSetMetaData) =>
        ...
        NonEmptyList.fromList(cols.toList) match {
          case Some(cols) =>
            Right(SqliteJdbcMetadata(MaybeReturnsRows.Query(cols)))
          case None =>
