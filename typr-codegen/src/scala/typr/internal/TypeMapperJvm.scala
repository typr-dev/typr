...
abstract class TypeMapperJvm(lang: Lang, typeOverride: TypeOverride, nullabilityOverride: NullabilityOverride) {
  ...
  /** Container used for PG/DuckDB array columns. Modern foundations DSL (TypeMapperJvmNew) uses `List`; legacy DSLs (TypeMapperJvmOld) keep `Array` because their generated
    * `arrayToStatement`/`ParameterMetaData[Array[X]]` codecs are array-shaped.
    */
  protected def arrayContainerOf(elementType: jvm.Type): jvm.Type = lang.ListType.tpe.of(elementType)

  // Internal alias for callers in this file.
  private def listOf(elementType: jvm.Type): jvm.Type = arrayContainerOf(elementType)
  ...
  def sqlFile(maybeOverridden: Option[jvm.Type], dbType: db.Type, nullability: Nullability): jvm.Type = {
    ...
    val base = dbType match {
      case db.PgType.Array(tpe) =>
        listOf(go(tpe, stripArrays = true))
      case db.DuckDbType.ListType(tpe) =>
        listOf(go(tpe, stripArrays = true))
      case db.DuckDbType.ArrayType(tpe, _) =>
        listOf(go(tpe, stripArrays = true))
  ...
  /** Peels off Option/array/list wrappers — used when reusing a column's type via FK or override. RC5 switched arrays to lists, so the List wrapper also peels here. */
  def stripOptionAndArray(tpe: jvm.Type): jvm.Type = {
    val listType = lang.ListType.tpe
    tpe match {
      case jvm.Type.ArrayOf(tpe)                          => stripOptionAndArray(tpe)
      case jvm.Type.TApply(`listType`, List(elementType)) => stripOptionAndArray(elementType)
