...
case class ComputedTable(
  ...
  /** Container type for selectByIds/deleteByIds id arguments. Modern foundations DSL works with `List`; legacy DSLs (Anorm/Doobie/ZioJdbc) keep `Array` because their generated `arrayToStatement`
    * codecs are array-shaped.
    */
  private def idsContainerType(elemType: jvm.Type): jvm.Type =
    options.dbLib match {
      case Some(_: codegen.DbLibLegacy) => jvm.Type.ArrayOf(elemType)
      case _                            => lang.ListType.tpe.of(elemType)
    }
  ...
  val repoMethods: Option[NonEmptyList[RepoMethod]] = {
    val maybeMethods = List(
      List[Iterable[RepoMethod]](
        ...
        maybeId
          .collect {
            case unary: IdComputed.Unary =>
              RepoMethod.SelectByIds(dbTable.name, cols, unary, jvm.Param(unary.paramName.appended("s"), idsContainerType(unary.tpe)), names.RowName)
            case x: IdComputed.Composite if x.cols.forall(col => col.dbCol.nullability == Nullability.NoNulls) =>
              RepoMethod.SelectByIds(dbTable.name, cols, x, jvm.Param(x.paramName.appended("s"), idsContainerType(x.tpe)), names.RowName)
          }
          .toList
        ...
        maybeId.collect {
          case unary: IdComputed.Unary =>
            RepoMethod.DeleteByIds(dbTable.name, unary, jvm.Param(unary.paramName.appended("s"), idsContainerType(unary.tpe)))
          case x: IdComputed.Composite if x.cols.forall(col => col.dbCol.nullability == Nullability.NoNulls) =>
            RepoMethod.DeleteByIds(dbTable.name, x, jvm.Param(x.paramName.appended("s"), idsContainerType(x.tpe)))
        }
      ).flatten,
      dbTable.uniqueKeys
        .flatMap { uk =>
