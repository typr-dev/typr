...
object generate {
  ...
  def apply(
    ...
    // Generate bridge composite types
    val computedBridgeCompositeTypes = ComputedBridgeCompositeType.fromConfig(publicOptions.bridgeCompositeTypes, pkg, language)
    ...
    val projectsWithFiles: ProjectGraph[Files, List[jvm.File]] =
      graph.valueFromProject { project =>
            ...
            val preciseConstraints = allCols.flatMap { case (relName, dbCol) =>
              if (!options.enablePreciseTypes.include(relName)) None
                ...
                dbCol.tpe match {
                  ...
                  case db.OracleType.TimestampWithLocalTimeZone(Some(fsp)) if fsp > 0     => Some(PreciseConstraint.InstantN(fsp, dbCol.tpe))
        ...
        val bridgeCompositeTypeFiles = computedBridgeCompositeTypes.map { computed =>
          FileBridgeCompositeType(computed, options, name => computedRelationsByName.get(name))
        }
        ...
        val keptMostFiles: List[jvm.File] = {
          ...
          minimize(
            mostFiles,
            entryPoints =
              sqlFileFiles ++ keptRelations ++ domainFiles ++ oracleObjectTypeFiles ++ oracleCollectionTypeFiles ++ duckDbStructTypeFiles ++ compositeEntryPoints ++ sharedTypeFiles ++ bridgeCompositeTypeFiles
          )
        }
