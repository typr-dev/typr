...
object DuckDbAdapter extends DbAdapter {
  ...
  override val KotlinTypes: jvm.Type.Qualified = jvm.Type.Qualified("dev.typr.foundationskt.DuckDbTypes")
  override val ScalaTypes: jvm.Type.Qualified = jvm.Type.Qualified("dev.typr.foundationssc.DuckDbTypes")
  ...
  /** DuckDbType.list is a no-arg method on the Scala wrapper but list() on Java/Kotlin. */
  private def listCall(typeSupport: TypeSupport): Code = typeSupport match {
    case TypeSupportScala => code".list"
    case _                => code".list()"
  }

  /** Wrap a Java `.array()` call in the appropriate Kotlin/Scala type constructor. DuckDB's `.array()` method only exists on Java types, so for Kotlin/Scala we need to wrap the result.
    */
  private def wrapArrayCall(javaArrayCode: Code, typeSupport: TypeSupport): Code =
    typeSupport match {
      case TypeSupportScala =>
        val tc = typeClassFor(typeSupport)
        code"$tc($javaArrayCode)"
      case TypeSupportKotlin =>
        val tc = typeClassFor(typeSupport)
        code"$tc($javaArrayCode)"
      case _ => javaArrayCode
    }
  ...
  def lookupType(typoType: TypoType, naming: Naming, typeSupport: TypeSupport): Code = {
    ...
    typoType match {
      ...
      case TypoType.Array(_, element) =>
        element match {
          case TypoType.Generated(_, _, qualifiedType)         => code"$qualifiedType.$typeArrayFieldName"
          case TypoType.UserDefined(_, _, Left(qualifiedType)) => code"$qualifiedType.$typeArrayFieldName"
          case _                                               => code"${lookupType(element, naming, typeSupport)}${listCall(typeSupport)}"
        }
    ...
    dbType match {
      case dbType: db.DuckDbType =>
        dbType match {
          ...
          case db.DuckDbType.Decimal(_, _) => primitiveType("numeric")

          case db.DuckDbType.HugeInt     => primitiveType("hugeint")
          case db.DuckDbType.UTinyInt    => primitiveType("utinyint")
          case db.DuckDbType.USmallInt   => primitiveType("usmallint")
          case db.DuckDbType.UInteger    => primitiveType("uinteger")
          case db.DuckDbType.UBigInt     => primitiveType("ubigint")
          case db.DuckDbType.UHugeInt    => primitiveType("uhugeint")
          case db.DuckDbType.VarChar(_)  => primitiveType("varchar")
          case db.DuckDbType.Char(_)     => primitiveType("char_")
          case db.DuckDbType.Text        => primitiveType("text")
          case db.DuckDbType.Blob        => primitiveType("blob")
          case db.DuckDbType.Bit(_)      => primitiveType("bit")
          case db.DuckDbType.Date        => primitiveType("date")
          case db.DuckDbType.Time        => primitiveType("time")
          case db.DuckDbType.Timestamp   => primitiveType("timestamp")
          case db.DuckDbType.TimestampTz => primitiveType("timestamptz")
          case db.DuckDbType.TimestampS  => primitiveType("timestamp")
          case db.DuckDbType.TimestampMS => primitiveType("timestamp")
          case db.DuckDbType.TimestampNS => primitiveType("timestamp")
          case db.DuckDbType.TimeTz      => primitiveType("timetz")
          case db.DuckDbType.Interval    => primitiveType("interval")
          case db.DuckDbType.UUID        => primitiveType("uuid")
          case db.DuckDbType.Json        => primitiveType("json")
          ...
          case db.DuckDbType.ListType(elementType) =>
            code"${lookupByDbType(elementType, naming, typeSupport)}${listCall(typeSupport)}"
          case db.DuckDbType.ArrayType(elementType, size) =>
            val elem = lookupByDbType(elementType, naming, typeSupport)
            size match {
              case Some(n) => code"$elem.array($n)"
              case None    => code"$elem${listCall(typeSupport)}"
            }
