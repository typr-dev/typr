...
trait DbAdapter {
  ...
  /** Runtime types object for Java (PgTypes or MariaTypes) */
  def Types: jvm.Type.Qualified

  /** Runtime types object for Scala wrappers */
  def ScalaTypes: jvm.Type.Qualified

  /** Runtime types object for Kotlin wrappers */
  def KotlinTypes: jvm.Type.Qualified

  /** Runtime types object dispatched by language */
  def typesFor(typeSupport: TypeSupport): jvm.Type.Qualified = typeSupport match {
    case TypeSupportScala  => ScalaTypes
    case TypeSupportKotlin => KotlinTypes
    case _                 => Types
  }

  /** Runtime type class for Java (PgType or MariaType) */
  def TypeClass: jvm.Type.Qualified

  /** Resolve any Java foundation class to its language-specific package */
  def foundationClassFor(javaClass: jvm.Type.Qualified, typeSupport: TypeSupport): jvm.Type.Qualified = {
    val langTypes = typesFor(typeSupport)
    val pkg = langTypes.value.parentOpt.getOrElse(langTypes.value)
    jvm.Type.Qualified(pkg / javaClass.value.idents.last)
  }

  /** Runtime type class dispatched by language */
  def typeClassFor(typeSupport: TypeSupport): jvm.Type.Qualified =
    foundationClassFor(TypeClass, typeSupport)
  ...
  protected def nullableType(innerCode: Code): Code =
  ...
  /** Language-aware version of `numericTypeCode` — used in precise type instance bodies. */
  def numericTypeCodeFor(typeSupport: TypeSupport): jvm.Code = code"${typesFor(typeSupport)}.${jvm.Ident(numericTypeName)}"
