...
case object LangJava extends Lang {
  ...
  override def renderTree(tree: jvm.Tree, ctx: Ctx): jvm.Code =
    tree match {
      ...
      case jvm.BlockExpr(statements, result)  =>
        // Java switch expression: { stmt1; stmt2; yield result; }
        val stmts = statements.map(s => code"$s;")
        code"{ ${stmts.mkCode(" ")} yield $result; }"
      ...
      case jvm.TypeSwitch(value, cases, nullCase, defaultCase, _) =>
        // In Java switch case arms with `->`: blocks `{ ... }` don't need semicolons,
        // but expressions (including switch expressions) do.
        ...
        def needsSemicolon(body: jvm.Code): String = {
          ...
          if (rendered.endsWith("}") && !rendered.startsWith("switch")) "" else ";"
        }
      ...
      case jvm.StringInterpolate(_, _, _) =>
        ...
        sys.error("StringInterpolate should not reach LangJava. DbLibFoundations.SQL() should have rewritten it to Fragment.concat() calls.")
      ...
      case cls: jvm.Adt.Record =>
        ...
        val body: List[jvm.Code] =
          cls.staticMembers.map(x => renderTree(x, staticMemberCtx)) ++ cls.members.sortBy(_.name).map(m => renderTree(m, memberCtx))
      ...
      case sum: jvm.Adt.Sum =>
        ...
        val body: List[jvm.Code] =
          List(
            ...
            sum.staticMembers.map(x => renderTree(x, staticMemberCtx)),
      ...
      case cls: jvm.Class =>
        ...
        val body: List[jvm.Code] =
          cls.staticMembers.map(x => renderTree(x, staticMemberCtx)) ++ sortedMembers.map(m => renderTree(m, memberCtx))
  ...
  override def bigDecimalZero: jvm.Code =
    code"${TypesJava.BigDecimal}.ZERO"

  override def bigIntegerZero: jvm.Code =
    code"${TypesJava.BigInteger}.ZERO"

  override def bigDecimalFromLong(v: jvm.Code): jvm.Code =
    code"${TypesJava.BigDecimal}.valueOf($v)"

  override def bigIntegerFromLong(v: jvm.Code): jvm.Code =
    code"${TypesJava.BigInteger}.valueOf($v)"
