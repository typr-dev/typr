  ...
  def apply(language: Lang, knownNamesByPkg: Map[jvm.QIdent, Map[jvm.Ident, jvm.Type.Qualified]], file: jvm.File): jvm.File = {
    ...
    def addImport(currentQName: jvm.Type.Qualified, isStatic: Boolean): jvm.Type.Qualified = {
      if (currentQName.value.idents.length <= 1) currentQName
        ...
        // Check if type is nested (more idents than package + 1) — nested types need explicit imports in Java
        // Only applies to same-package types, not builtins or already-imported types
        val isNestedType = currentQName.value.idents.length > file.pkg.idents.length + 1
        val pkgMatch = knownNamesByPkg.get(file.pkg).flatMap(_.get(currentName))
        val resolved = pkgMatch.orElse(language.BuiltIn.get(currentName)).orElse(targetMap.get(currentName)).orElse(otherMap.get(currentName))
        resolved match {
          case Some(alreadyAvailable) if alreadyAvailable == currentQName && isNestedType && pkgMatch.isDefined =>
            targetMap += ((currentName, currentQName))
            shortenedQName
          case Some(alreadyAvailable) if alreadyAvailable == currentQName =>
            shortenedQName
          case Some(_) =>
            currentQName
          case None =>
  ...
  def shortenNames(tree: jvm.Tree, typeImport: jvm.Type.Qualified => jvm.Type.Qualified, staticImport: jvm.Type.Qualified => jvm.Type.Qualified): jvm.Tree =
    tree match {
      ...
      case jvm.BlockExpr(statements, result) =>
        jvm.BlockExpr(
          statements.map(_.mapTrees(t => shortenNames(t, typeImport, staticImport))),
          result.mapTrees(t => shortenNames(t, typeImport, staticImport))
        )
