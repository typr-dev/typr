...
class DbLibFoundations(
  ...
  /** Lang-aware StreamingInsert: foundationssc.StreamingInsert (Scala), foundations.StreamingInsert (Java), foundationskt.StreamingInsert (Kotlin). */
  def streamingInsert: jvm.Type.Qualified = foundationsDsl.StreamingInsert

  /** lang.dsl narrowed to DslQualifiedNamesFoundations (DbLibFoundations only uses foundations-backed DSLs). */
  private val foundationsDsl: DslQualifiedNamesFoundations = lang.dsl match {
    case f: DslQualifiedNamesFoundations => f
    case other                           => sys.error(s"DbLibFoundations requires a foundations-backed DSL, got: $other")
  }
  val Fragment: jvm.Type.Qualified = lang.dsl.Fragment
  val SqlStringInterpolation: jvm.Type.Qualified = Fragment
  val KotlinQueryExtension: jvm.Type.Qualified = FoundationsTypes.kotlin.KotlinQueryExtension
  val DbText: jvm.Type.Qualified = FoundationsTypes.DbText

  /** Wrap a type code to make it nullable - uses ApplyNullary for correct Scala/Java/Kotlin rendering */
  def nullableType(innerCode: jvm.Code): jvm.Code =
    jvm.ApplyNullary(innerCode, jvm.Ident("opt")).code
  ...
  val OperationListOps: jvm.Type.Qualified = FoundationsTypes.scala.OperationListOps
  val OperationOptionalToOptionOps: jvm.Type.Qualified = FoundationsTypes.scala.OperationOptionalToOptionOps

  val c: jvm.Param[jvm.Type] = jvm.Param(jvm.Ident("c"), foundationsDsl.Connection)

  /** Read-only connection parameter for select/query methods. */
  val cRead: jvm.Param[jvm.Type] = jvm.Param(jvm.Ident("c"), foundationsDsl.ConnectionRead)

  /** `.run(using c)` (Scala DSL) or `.run(c)` (Java/Kotlin DSL) against the writable connection. */
  def runC: jvm.Code = runOn(c)

  /** `.run(using c)` (Scala DSL) or `.run(c)` (Java/Kotlin DSL) against the read-only connection. */
  def runReadC: jvm.Code = runOn(cRead)

  /** Only the Scala wrapper has `using` clauses on `run`; Java/Kotlin wrappers expose plain methods, so emit positional. */
  private val useUsingForRun: Boolean = foundationsDsl == DslQualifiedNames.Scala

  private def runOn(connParam: jvm.Param[jvm.Type]): jvm.Code =
    jvm.Call(code".run", List(jvm.Call.ArgGroup(List(jvm.Arg.Pos(connParam.name.code)), isImplicit = useUsingForRun))).code

  /** Generate the appropriate run call for methods returning List */
  def queryAllRunUnchecked: jvm.Code = runReadC

  /** Generate the appropriate run call for methods returning Optional */
  def queryFirstRunUnchecked: jvm.Code = runReadC
  ...
  /** Convert Iterator for use with StreamingInsert.of. The Scala wrapper accepts Scala Iterator and converts internally; Java/Kotlin need a Java Iterator. */
  ...
  def SQL(content: jvm.Code): jvm.Code = {
    if (useNativeStringInterpolation) {
      ...
      // For everything else: directly generate Fragment.concat() calls
      ...
      val processedParts: List[jvm.Code] = linearized.map {
        ...
        case jvm.Code.Str(str) =>
          // String parts: wrap in Fragment.of() with proper string literal
          code"$Fragment.of(${jvm.StrLit(str)})"
        case other =>
          // For other Code types, render them and wrap in Fragment.of()
      ...
      val finalParts = linearized.lastOption match {
        case Some(jvm.Code.Tree(jvm.RuntimeInterpolation(_))) =>
          ...
          processedParts :+ code"$Fragment.of(${jvm.StrLit("")})"
        case _ =>
          processedParts
      }

      // For Kotlin, use qualified name Fragment.concat() directly (static imports don't work with companion objects)
      // For Java, use static import and unqualified name
      lang match {
        case _: LangKotlin =>
          // Create qualified call: Fragment.concat(...)
  ...
  val rowParserName: jvm.Ident = jvm.Ident("rowCodec")
  ...
  override val additionalFiles: List[typr.jvm.File] = Nil
  ...
  /** Lookup type and unwrap to Java DbType for use in DSL constructors (Field, IdField, OptField). Kotlin/Scala DSL constructors expect `dev.typr.foundations.DbType[T]` (Java), but `lookupType`
    * returns wrapper types like `DuckDbType[T]` for Kotlin/Scala. This method adds `.underlying` to unwrap the wrapper types.
    */
  def lookupTypeForDsl(typoType: TypoType): jvm.Code = {
    val base = lookupType(typoType)
    lang.typeSupport match {
      case TypeSupportScala | TypeSupportKotlin => code"$base.underlying"
      case _                                    => base
    }
  }

  /** Call `.renamed(sqlType)` on a type expression. For Kotlin/Scala wrapper types that lack `renamed()`, unwrap to Java type, call renamed, then rewrap.
    */
  private def callRenamed(baseCode: jvm.Code, sqlTypeLit: jvm.StrLit): jvm.Code =
    lang.typeSupport match {
      case TypeSupportScala | TypeSupportKotlin =>
        val typeClass = adapter.typeClassFor(lang.typeSupport)
        code"$typeClass($baseCode.underlying.renamed($sqlTypeLit))"
      case _ =>
        code"$baseCode.renamed($sqlTypeLit)"
    }

  /** Call `.renamedDropPrecision(sqlType)` on a type expression. For Kotlin/Scala wrapper types that lack `renamedDropPrecision()`, unwrap to Java type, call it, then rewrap.
    */
  private def callRenamedDropPrecision(baseCode: jvm.Code, sqlTypeLit: jvm.StrLit): jvm.Code =
    lang.typeSupport match {
      case TypeSupportScala | TypeSupportKotlin =>
        val typeClass = adapter.typeClassFor(lang.typeSupport)
        code"$typeClass($baseCode.underlying.renamedDropPrecision($sqlTypeLit))"
      case _ =>
        code"$baseCode.renamedDropPrecision($sqlTypeLit)"
    }
  ...
  /** DbLibFoundations does not need constAs expressions for generateCompositeIn */
  override def needsConstAsForCompositeIn: Boolean = false

  /** Generate a composite IN expression using TupleExpr.of(...).among(...) pattern. ID types implement Tuple interface, so they can be passed directly to among(). Renders as: TupleExpr.of(field1,
    * field2).among(compositeIds)
    */
  override def generateCompositeIn(
    ...
    val tupleExpr = code"${lang.dsl.TupleExpr}.of(${fieldExprs.mkCode(", ")})"
    code"$tupleExpr.among($idsExpr)"
  }
          ...
          case x: IdComputed.Composite =>
            ...
            val vals = x.cols.map { col =>
              jvm
                .Value(
                  Nil,
                  col.name,
                  lang.ListType.tpe.of(col.tpe),
                  Some(lang.ListType.map(idsParam.name.code, jvm.FieldGetterRef(x.tpe, col.name).code)),
                  isLazy = false,
                  isOverride = false
                )
                .code
            ...
            val sql = SQL {
              ...
              def unnestCol(col: ComputedColumn): jvm.Code = {
                val arrayTypoType = TypoType.Array(lang.ListType.tpe.of(col.tpe), col.typoType)
        ...
        id match {
          ...
          case x: IdComputed.Composite =>
            ...
            val encodedCols = x.cols.toList.map { col =>
              ...
              code"$Fragment.of(${jvm.StrLit(s"$quotedCol = ")}), $Fragment.encode(${lookupType(col)}, ${lang.prop(idIdent.code, col.name)})"
            }
            val andLit = code"$Fragment.of(${jvm.StrLit(" AND ")})"
            val andArgs = encodedCols.flatMap(e => List(andLit, e)).drop(1)
            val openParen = code"$Fragment.of(${jvm.StrLit("(")})"
            val closeParen = code"$Fragment.of(${jvm.StrLit(")")})"
            val orClauseForId = code"$Fragment.concat($openParen, ${andArgs.mkCode(", ")}, $closeParen)"
            ...
            jvm.Body.Stmts(
              List(
                ...
                jvm.Return(
                  code"""$Fragment.concat($Fragment.of(${jvm.StrLit(
                      s"select $colNamesStr\nfrom $qRelNameStr\nwhere "
                    )}), $Fragment.or(${collectionForOr(orClauses.code)}), $Fragment.of(${jvm.StrLit("\n")})).query(${resultSetParserFor(rowType, "all")})$queryAllRunUnchecked"""
                )
              )
            )
        }
      ...
      case DbType.MariaDB =>
        id match {
              ...
              List(
                ...
                jvm.Return(
                  code"""$Fragment.concat($Fragment.of(${jvm.StrLit(
                    ...
                    )}), $Fragment.comma(${collectionForComma(fragments.code)}), $Fragment.of(${jvm.StrLit(s")")})).query(${resultSetParserFor(rowType, "all")})$queryAllRunUnchecked"""
                )
              )
            )
          ...
          case x: IdComputed.Composite =>
            ...
            val commaLit = code"$Fragment.of(${jvm.StrLit(", ")})"
            val openParen = code"$Fragment.of(${jvm.StrLit("(")})"
            val closeParen = code"$Fragment.of(${jvm.StrLit(")")})"
            val tupleArgs = encodedCols.flatMap(e => List(commaLit, e)).drop(1)
            val tupleExpr = code"$Fragment.concat($openParen, ${tupleArgs.mkCode(", ")}, $closeParen)"
              ...
              List(
                ...
                jvm.Return(
                  code"""$Fragment.concat($Fragment.of(${jvm.StrLit(
                    ...
                    )}), $Fragment.comma(${collectionForComma(fragments.code)}), $Fragment.of(${jvm.StrLit(")")})).query(${resultSetParserFor(rowType, "all")})$queryAllRunUnchecked"""
                )
              )
            )
        }
    ...
    adapter.dbType match {
      case DbType.PostgreSQL =>
        id match {
          case x: IdComputed.Unary =>
            // PostgreSQL: WHERE col = ANY(array)
            val arrayTypoType = TypoType.Array(lang.ListType.tpe.of(x.tpe), x.typoType)
            val sql = SQL {
              code"""|delete
                     |from $qRelName
                     |where ${jvm.Code.Str(adapter.quoteIdent(x.col.dbName.value))} = ANY(${runtimeInterpolateValue(idsParam.name, arrayTypoType)})""".stripMargin
          ...
          case x: IdComputed.Composite =>
            ...
            jvm.Body.Stmts(vals.toList :+ jvm.Return(code"$sql.update()$runC").code)
        }
          ...
          case x: IdComputed.Unary =>
            ...
            jvm.Body.Stmts(
              List(
                ...
                lang.arrayForEach(idsParam.name.code, idIdent, jvm.Body.Expr(addStmt)),
                jvm.Return(
                  code"""$Fragment.concat($Fragment.of(${jvm.StrLit(s"delete from $qRelNameStr where $colName in (")}), $Fragment.comma(${collectionForComma(
                      fragments.code
                    )}), $Fragment.of(${jvm
                      .StrLit(s")")})).update()$runC"""
                )
              )
            )
              ...
              List(
                ...
                jvm.Return(
                  code"""$Fragment.concat($Fragment.of(${jvm.StrLit(
                    ...
                    )}), $Fragment.comma(${collectionForComma(fragments.code)}), $Fragment.of(${jvm.StrLit(")")})).update()$runC"""
                )
              )
            )
        }
  ...
  override def repoSig(repoMethod: RepoMethod): Right[Nothing, jvm.Method] = {
    ...
    repoMethod match {
      ...
      case RepoMethod.SelectAll(_, _, rowType) =>
        sig(params = Nil, implicitParams = List(cRead), returnType = lang.ListType.tpe.of(rowType))
      case RepoMethod.SelectById(_, _, id, rowType) =>
        sig(params = List(id.param), implicitParams = List(cRead), returnType = lang.Optional.tpe(rowType))
      case RepoMethod.SelectByIds(_, _, _, idsParam, rowType) =>
        sig(params = List(idsParam), implicitParams = List(cRead), returnType = lang.ListType.tpe.of(rowType))
      case RepoMethod.SelectByIdsTracked(x) =>
        sig(params = List(x.idsParam), implicitParams = List(cRead), returnType = lang.MapOps.tpe.of(x.idComputed.tpe, x.rowType))
      case RepoMethod.SelectByUnique(_, keyColumns, _, rowType) =>
        sig(params = keyColumns.toList.map(_.param), implicitParams = List(cRead), returnType = lang.Optional.tpe(rowType))
      case RepoMethod.SelectByFieldValues(_, _, _, fieldValueOrIdsParam, rowType) =>
        sig(params = List(fieldValueOrIdsParam), implicitParams = List(cRead), returnType = lang.ListType.tpe.of(rowType))
      ...
      case RepoMethod.SqlFile(sqlScript) =>
        ...
        val (retType, connParam) = sqlScript.maybeRowName match {
          case MaybeReturnsRows.Query(rowName) => (lang.ListType.tpe.of(rowName), cRead)
          case MaybeReturnsRows.Update         => (lang.Int, c)
        }
        sig(params = params, implicitParams = List(connParam), returnType = retType)
    }
  }
  ...
  override def repoImpl(repoMethod: RepoMethod): jvm.Body =
    repoMethod match {
      case RepoMethod.SelectBuilder(relName, fieldsType, rowType) =>
        val structure = jvm.Select(fieldsType.code, jvm.Ident("structure")).code
      ...
      case RepoMethod.UpdateBuilder(relName, fieldsType, rowType) =>
        val structure = jvm.Select(fieldsType.code, jvm.Ident("structure")).code
        jvm.Body.Expr(code"${lang.dsl.UpdateBuilder}.of(${jvm.StrLit(quotedRelNameStr(relName))}, $structure, $rowType.$rowParserName, ${adapter.dialectRef(lang)})")
      ...
      case RepoMethod.Update(relName, _, id, param, colsUnsaved) =>
        ...
        jvm.Body.Stmts(
          List(
            ...
            jvm.Return(code"$sql.update()$runC > 0")
          )
        )
      ...
      case RepoMethod.Insert(relName, cols, _, unsavedParam, writeableColumnsWithId, returningStrategy) =>
        ...
        returningStrategy match {
          ...
          case ReturningStrategy.GeneratedKeysAllColumns(_, allCols) =>
            ...
            jvm.Body.Expr(
              code"""|$insertSql
                     |  .updateReturningGeneratedKeys($columnNamesArray, $code)$runC"""
            )
      ...
      case RepoMethod.Upsert(relName, cols, id, unsavedParam, rowType, writeableColumnsWithId, upsertStrategy) =>
        ...
        // because Upsert operates on Row type (not RowUnsaved) which has the ID.
        // MariaDB allows explicit values in AUTO_INCREMENT columns.
        val upsertCols = if (!adapter.supportsArrays) cols else writeableColumnsWithId

        val values = upsertCols.map { c =>
          runtimeInterpolateValue(prop(unsavedParam.name.code, c.name), c.typoType).code ++ adapter.columnWriteCast(c)
        }

        // Generate the ON CONFLICT/ON DUPLICATE KEY UPDATE clause - delegated to adapter
        val nonPkCols = upsertCols.toList.filterNot(c => id.cols.exists(_.name == c.name))
        ...
        upsertStrategy match {
          case UpsertStrategy.NotSupported =>
            val sql = SQL {
              adapter.upsertSql(
                tableName = quotedRelName(relName),
                columns = dbNames(upsertCols, isRead = false),
      ...
      case RepoMethod.UpsertStreaming(relName, id, rowType, writeableColumnsWithId) =>
        ...
        if (!adapter.supportsCopyStreaming) {
          ...
          jvm.Body.Stmts(
            List(
              jvm.IgnoreResult(code"${SQL(code"create temporary table $tempTablename (like ${quotedRelName(relName)}) on commit drop")}.update()$runC"),
              jvm.IgnoreResult(code"$streamingInsert.of($copySql, batchSize, $unsavedArg, $rowType.${adapter.textFieldName})$runC"),
              jvm.Return(code"$mergeSql.update()$runC")
            )
          )
        }
      ...
      case RepoMethod.InsertUnsaved(relName, cols, unsaved, unsavedParam, _, _, returningStrategy) =>
        ...
        val cases0 = unsaved.normalColumns.flatMap { col =>
          ...
          List(
            lang.typeSupport.MutableListOps.add(columns.name.code, code"$Fragment.of(${jvm.StrLit(quotedName)})"),
          ...
          case ReturningStrategy.SqlReturning(_) =>
            ...
            jvm.Body.Stmts(
              List[List[jvm.Code]](
                ...
                List(
                  q,
                  jvm.Return(code"q.updateReturning($parser)$runC")
                )
              ).flatten
            )
          ...
          case ReturningStrategy.GeneratedKeysAllColumns(_, allCols) =>
            ...
            jvm.Body.Stmts(
              List[List[jvm.Code]](
                ...
                List(
                  q,
                  jvm.Return(code"q.updateReturningGeneratedKeys($columnNamesArray, $parser)$runC")
                )
              ).flatten
            )
      ...
      case RepoMethod.InsertStreaming(relName, rowType, writeableColumnsWithId) =>
        ...
        val unsavedArg = iteratorToJavaAlways(code"unsaved")
        jvm.Body.Expr(code"$streamingInsert.of($sql, batchSize, $unsavedArg, $rowType.${adapter.textFieldName})$runC")
      case RepoMethod.InsertUnsavedStreaming(relName, unsaved) =>
        ...
        val unsavedArg = iteratorToJavaAlways(code"unsaved")
        jvm.Body.Expr(code"$streamingInsert.of($sql, batchSize, $unsavedArg, ${unsaved.tpe}.${adapter.textFieldName})$runC")
      ...
      case RepoMethod.Delete(relName, id) =>
        ...
        jvm.Body.Expr(code"$sql.update()$runC > 0")
      ...
      case RepoMethod.SqlFile(sqlScript) =>
        ...
        jvm.Body.Expr(ret.getOrElse(code"${SQL(renderedScript)}.update()$runC"))
    }
  ...
  private def testInsertMethodJava(x: ComputedTestInserts.InsertMethod, returnType: jvm.Type): jvm.Method = {
    val inserterType = FoundationsTypes.Inserter.of(x.cls, returnType)
    ...
    val newRepo = jvm.New(x.table.names.RepoImplName, Nil)
    val insertLambda = jvm.Lambda(
      List(jvm.LambdaParam(row, Some(x.cls)), jvm.LambdaParam(conn, Some(foundationsDsl.Connection))),
      jvm.Body.Expr(jvm.Call.withImplicits(code"($newRepo).insert", List(jvm.Arg.Pos(row.code)), List(jvm.Arg.Pos(conn.code))).code)
    )

    val body = code"${FoundationsTypes.Inserter}.of($newRow, $insertLambda)"
  ...
  override def stringEnumInstances(wrapperType: jvm.Type, underlyingTypoType: TypoType, sqlType: String, openEnum: Boolean): List[jvm.Given] = {
    ...
    val arrayWrapper = lang.ListType.tpe.of(wrapperType)
    ...
    List(
      ...
      if (!adapter.supportsArrays) None
        ...
        Some(
          jvm.Given(
            tparams = Nil,
            name = dbTypeArrayName,
            implicitParams = Nil,
            tpe = adapter.typeClassFor(lang.typeSupport).of(arrayWrapper),
            body = {
              val xs = jvm.Ident("xs")
              val lambda1 = jvm.Lambda(xs, lang.ListType.map(xs.code, create.code))
              val lambda2 = jvm.Lambda(xs, lang.ListType.map(xs.code, extract.code))
              ...
              val base =
                code"""|${lookupType(arrayDbType)}
                       |  .to(${lang.dsl.Bijection}.of($lambda1, $lambda2))""".stripMargin
              if (openEnum) base
              else callRenamedDropPrecision(base, sqlTypeLit)
            }
          )
        ),
      Some(
        jvm.Given(
          ...
          implicitParams = Nil,
          tpe = adapter.typeClassFor(lang.typeSupport).of(wrapperType),
          body = {
            val base = code"${lookupType(underlyingDbType)}.to(${lang.dsl.Bijection}.of($create, $extract))"
            if (openEnum) base
            else callRenamedDropPrecision(base, sqlTypeLit)
  ...
  override def wrapperTypeInstances(wrapperType: jvm.Type.Qualified, underlyingJvmType: jvm.Type, underlyingDbType: db.Type, overrideDbType: Option[String]): List[jvm.Given] = {
    val xs = jvm.Ident("xs")
    // When overrideDbType is provided (e.g., for domain types), we need to rename the dbType
    // to use the correct SQL type name for proper casting
    def maybeRename(baseCode: jvm.Code, sqlType: String): jvm.Code =
      callRenamed(baseCode, jvm.StrLit(sqlType))
    ...
    List[Option[jvm.Given]](
      // pgType/duckDbType must come before dbTypeArray because the array body references it
      // (Java rejects forward references between static fields).
      Some(
        jvm.Given(
          tparams = Nil,
          name = adapter.typeFieldName,
          ...
          tpe = adapter.typeClassFor(lang.typeSupport).of(wrapperType),
          body = {
            val base = code"$getBaseDbType.to(${lang.dsl.Bijection}.of(${jvm.ConstructorMethodRef(wrapperType)}, ${jvm.FieldGetterRef(wrapperType, jvm.Ident("value"))}))"
      ...
      if (!adapter.supportsArrays) None
        ...
        Some(
          jvm.Given(
            ...
            name = dbTypeArrayName,
            implicitParams = Nil,
            tpe = adapter.typeClassFor(lang.typeSupport).of(lang.ListType.tpe.of(wrapperType)),
            body = {
              // The scalar `pgType`/`duckDbType` already wraps via Bijection.of(new, value), and
              // the typename it carries (renamed if overrideDbType applies) gets a "[]" suffix
              // automatically from .array()/.list(). So we just call it on our own scalar type.
              val arrayMethod = adapter.dbType match {
                case DbType.PostgreSQL => "array"
                case DbType.DuckDB     => "list"
                case _                 => sys.error("unreachable: dbTypeArray only emitted when supportsArrays is true")
              }
              jvm.ApplyNullary(adapter.typeFieldName.code, jvm.Ident(arrayMethod)).code
            }
          )
        ),
      ...
      if (adapter.dbType != DbType.Oracle) None
        ...
        Some(
          jvm.Given(
            ...
            name = rowParserName,
            implicitParams = Nil,
            tpe = lang.dsl.RowCodec.of(wrapperType),
            body = {
              val x = jvm.Ident("x")
              val id = jvm.Ident("id")
              val encodeLambda = jvm.Lambda(id, lang.arrayOf(List(id.code)))
              val inlineType = code"$getBaseDbType.to(${lang.dsl.Bijection}.of(${jvm.ConstructorMethodRef(wrapperType)}, ${jvm.FieldGetterRef(wrapperType, jvm.Ident("value"))}))"
              ...
              lang.dsl match {
                case DslQualifiedNames.Scala =>
                  ...
                  code"${lang.dsl.RowCodecs}.of($inlineType)($decodeLambda)($encodeLambda)"
                case _ =>
                  // Java/Kotlin RowCodecs.of: def of[T0, Row](t0: DbType[T0], decode: Function1[T0, Row], encode: Function[Row, Object[]])
                  code"${lang.dsl.RowCodecs}.of($inlineType, $decodeLambda, $encodeLambda)"
              }
            }
          )
  ...
  /** Generate instances for Unpack transform - bimap through an intermediate wrapper type.
    *
    * For unified FirstName(String) where PostgreSQL uses Name domain: `Name.pgType.to(Bijection.of(n -> new FirstName(n.value()), fn -> new Name(fn.value())))`
    */
  private def generateUnpackInstances(
    ...
    List[jvm.ClassMember](
      jvm.Given(
        ...
        tpe = adapter.typeClassFor(lang.typeSupport).of(wrapperType),
        body = {
          // Generate: IntermediateType.pgType.to(Bijection.of(x -> new WrapperType(x.value()), w -> new IntermediateType(w.value())))
          ...
          val base = code"$intermediateQualified.${adapter.typeFieldName}.to(${lang.dsl.Bijection}.of($toWrapper, $fromWrapper))"
          overrideDbType.fold(base)(maybeRename(base, _))
        }
      )
    )
  ...
  override def rowInstances(tpe: jvm.Type, cols: NonEmptyList[ComputedColumn], rowType: DbLib.RowType): List[jvm.ClassMember] = {
    def rowParser = {
      jvm.Value(
        Nil,
        rowParserName,
        lang.dsl.RowCodec.of(tpe),
        Some {
          val dbTypes = cols.toList.map(x => lookupType(x.typoType)).mkCode(code", ")
          val row = jvm.Ident("row")
          val newArray = lang.arrayOf(cols.toList.map(c => lang.propertyGetterAccess(row.code, c.name)))
          val encodeLambda = lang match {
            case _: LangKotlin =>
              jvm.Lambda(List(jvm.LambdaParam.typed(row, tpe)), jvm.Body.Expr(newArray))
            case _ =>
              jvm.Lambda(row, newArray)
          }
          ...
          val decodeLambda: jvm.Code = lang match {
            case LangScala(_, TypeSupportJava, _) if cols.length > 22 =>
              ...
              val applyMethod = jvm.Method(
                ...
                tpe = tpe,
                throws = Nil,
                body = jvm.Body.Expr(tpe.construct(args*)),
                isOverride = true,
                isDefault = false
              )
              val functionTypeName = FoundationsTypes.RowCodecsFunctionN(cols.length)
              ...
              jvm
                ...
                .code
            case _: LangKotlin if cols.length <= 22 =>
              val params = cols.toList.zipWithIndex.map { case (col, i) =>
                jvm.Param(jvm.Ident(s"t$i"), col.tpe)
              }
              ...
              jvm.Lambda(params.map(p => jvm.LambdaParam.typed(p.name, p.tpe)), jvm.Body.Expr(tpe.construct(args*)))
            case _: LangKotlin =>
              // >22 columns: Kotlin Function types only go to 22, so we can't use typed lambdas.
              // This case is handled below by generating RowCodec.create directly.
              null
            case _ =>
          ...
          lang match {
            case _: LangKotlin if cols.length > 22 =>
              // >22 columns: bypass RowCodecs.of (limited to 22) and use Java RowCodec.create directly
              val dbTypesJava = cols.toList.map(x => lookupTypeForDsl(x.typoType)).mkCode(code", ")
              val a = jvm.Ident("a")
              val arrayArgs = cols.toList.zipWithIndex.map { case (col, i) =>
                code"$a[$i] as ${col.tpe}"
              }
              val decodeLambdaArr = jvm.Lambda(a, tpe.construct(arrayArgs*))
              code"${lang.dsl.RowCodec}(${DslQualifiedNames.Java.RowCodec}.create(listOf($dbTypesJava), $decodeLambdaArr, $encodeLambda))"
            case _ if lang.dsl == DslQualifiedNames.Scala =>
              // For Scala DSL, use curried calls
              code"${lang.dsl.RowCodecs}.of($dbTypes)($decodeLambda)($encodeLambda)"
            case _ =>
              code"${lang.dsl.RowCodecs}.of($dbTypes, $decodeLambda, $encodeLambda)"
          }
        },
    ...
    rowType match {
      case DbLib.RowType.Writable =>
        val text = if (enableStreamingInserts && adapter.supportsCopyStreaming) {
          val row = jvm.Ident("row")
          val sb = jvm.Ident("sb")
          val textMethod = adapter.textFieldName
          val textCols: NonEmptyList[jvm.Code] = cols.map { col =>
            val text = col.tpe match {
              case jvm.Type.TApply(default.Defaulted, List(_)) =>
                ...
                val innerText = code"${lookupType(col)}.$textMethod()"
                val target = jvm.Select(default.Defaulted.code, adapter.textFieldName)
                jvm.Call(target.code, List(jvm.Call.ArgGroup(List(jvm.Arg.Pos(innerText)), isImplicit = true))).code
              case _ =>
                code"${lookupType(col)}.$textMethod()"
            }
  ...
  override def structInstances(computed: ComputedOracleObjectType): List[jvm.ClassMember] = {
    if (computed.attributes.isEmpty) {
      ...
      val OracleTypes = adapter.typesFor(lang.typeSupport)
      val OracleTypeForLang = adapter.typeClassFor(lang.typeSupport)
      ...
      // Each attribute: .field(name, oracleType, getter)
      // Scala wrapper has curried `.field(name, tpe)(getter)`; Java/Kotlin builders take all three positionally.
      val addAttributeCalls = computed.attributes
        .map { attr =>
          ...
          if (lang.dsl == DslQualifiedNames.Scala) code".field($attrName, $oracleTypeCall)($methodRef)"
          else code".field($attrName, $oracleTypeCall, $methodRef)"
        }
        .mkCode(code"")

      // .build(decoder)
      val constructorLambda: jvm.Code = lang match {
        case _: LangScala =>
          val params = computed.attributes.zipWithIndex.map { case (_, i) => jvm.Ident(s"t$i") }
          val args = computed.attributes.zip(params).map { case (a, p) => code"${a.name} = $p" }
      ...
      // OracleTypes.compositeOf("TYPE_NAME", RowCodec.namedBuilder()...build())
      val codec = code"$builderStart$addAttributeCalls$buildCall"
      val fullChain = code"$OracleTypes.compositeOf(${jvm.StrLit(oracleTypeName)}, $codec)"

      List(
        jvm.Value(
          annotations = Nil,
          name = jvm.Ident("oracleType"),
          tpe = OracleTypeForLang.of(computed.tpe),
          body = Some(fullChain),
          isLazy = false,
          isOverride = false
        )
  ...
  override def collectionInstances(computed: ComputedOracleCollectionType): List[jvm.ClassMember] = {
    ...
    // OracleVArray/OracleNestedTable are Java-only APIs, need Java types for element
    val elementTypeJava = lang.typeSupport match {
      case TypeSupportScala | TypeSupportKotlin => code"$elementTypeInstance.underlying"
      case _                                    => elementTypeInstance
    }

    val baseFactoryCall = (computed.underlying: @unchecked) match {
      case v: db.OracleType.VArray =>
        val typeName = jvm.StrLit(v.name.name)
        code"${FoundationsTypes.oracle.OracleVArray}.of($typeName, ${v.maxSize}, $elementTypeJava)"
      case n: db.OracleType.NestedTable =>
        val typeName = jvm.StrLit(n.name.name)
        code"${FoundationsTypes.oracle.OracleNestedTable}.of($typeName, $elementTypeJava)"
    }
    ...
    val (toWrapperLambda, fromWrapperLambda) = lang match {
      ...
      case scala: LangScala if scala.typeSupport == TypeSupportJava =>
        ...
        val valueAccess = lang.prop(wrapperIdent, "value")
        // Use spread operator * to pass array as varargs to List.of()
        val listOf = code"$listType.of($valueAccess*)"
        val fromWrapper = jvm.Lambda("wrapper", jvm.Body(List(listOf)))
        (toWrapper, fromWrapper)
    ...
    // baseFactoryCall returns a *Java* OracleType — chain `.to(...)` with the Java Bijection so the result stays Java; we wrap with the language-specific OracleType once at the end.
    val javaFactoryCall = code"$baseFactoryCall.to(${DslQualifiedNames.Java.Bijection}.of($toWrapperLambda, $fromWrapperLambda))"
    // Wrap Java result in language-specific wrapper (OracleVArray/OracleNestedTable have no Kotlin/Scala wrappers)
    val factoryCall = lang.typeSupport match {
      case TypeSupportScala | TypeSupportKotlin => code"$OracleTypeForLang($javaFactoryCall)"
      case _                                    => javaFactoryCall
    }
