...
object Db2Adapter extends DbAdapter {
  ...
  override val KotlinTypes: jvm.Type.Qualified = jvm.Type.Qualified("dev.typr.foundationskt.Db2Types")
  override val ScalaTypes: jvm.Type.Qualified = jvm.Type.Qualified("dev.typr.foundationssc.Db2Types")
  ...
  def lookupPrimitive(primitive: analysis.WellKnownPrimitive, typeSupport: TypeSupport): Code = {
    ...
    primitive match {
      ...
      case analysis.WellKnownPrimitive.UUID          => primitiveType("char_") // UUID as CHAR(36) in DB2
    }
  }
    ...
    dbType match {
      case dbType: db.DB2Type =>
        dbType match {
          ...
          case db.DB2Type.Char(_)    => primitiveType("char_")
          case db.DB2Type.VarChar(_) => primitiveType("varchar")
          case db.DB2Type.Clob       => primitiveType("clob")
          case db.DB2Type.Long       => primitiveType("varchar") // Deprecated, map to VARCHAR

          // Character types (DBCS/Graphic)
          case db.DB2Type.Graphic(_)     => primitiveType("graphic")
          case db.DB2Type.VarGraphic(_)  => primitiveType("vargraphic")
          case db.DB2Type.DbClob         => primitiveType("dbclob")
          case db.DB2Type.LongVarGraphic => primitiveType("vargraphic") // Deprecated

          // Binary types
          case db.DB2Type.Binary(_)    => primitiveType("binary")
          case db.DB2Type.VarBinary(_) => primitiveType("varbinary")
          case db.DB2Type.Blob         => primitiveType("blob")

          // Date/Time types
          case db.DB2Type.Date         => primitiveType("date")
          case db.DB2Type.Time         => primitiveType("time")
          case db.DB2Type.Timestamp(_) => primitiveType("timestamp")

          // XML type
          case db.DB2Type.Xml => primitiveType("xml")

          // Row ID
          case db.DB2Type.RowId => primitiveType("rowid")

          // Distinct types - use the underlying type
          case db.DB2Type.DistinctType(_, _) => primitiveType("varchar") // Fallback
