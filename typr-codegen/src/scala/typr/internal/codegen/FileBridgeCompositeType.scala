...
object FileBridgeCompositeType {
  ...
  def apply(
      ...
      externalRecordLookup: String => Option[FileBridgeProjectionMapper.ExternalRecord] = _ => None
  ): jvm.File = {
    ...
    val mapperMethods = FileBridgeProjectionMapper.generateMapperMethods(computed, options, relationLookup, externalRecordLookup)
