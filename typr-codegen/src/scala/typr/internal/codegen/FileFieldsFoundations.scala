        ...
        body = jvm.Body.Expr(
          cls
            ...
            .construct(
              ...
              dbLib.lookupTypeForDsl(typoTypeForLookup)
            )
        ),
        isOverride = false,
        isDefault = false
  ...
  private def rowParserMethod(): jvm.Method = {
    val (rowParserType, rowParserBody) = dsl match {
      case DslQualifiedNames.Kotlin | DslQualifiedNames.Scala =>
        val javaRowCodec = dsl.RowCodecJava.of(names.RowName)
        (javaRowCodec, code"${names.RowName}.rowCodec.underlying")
      case _ =>
        (dsl.RowCodec.of(names.RowName), code"${names.RowName}.rowCodec")
    }
    jvm.Method(
      annotations = Nil,
      comments = jvm.Comments.Empty,
      tparams = Nil,
      name = jvm.Ident("rowCodec"),
      params = Nil,
      implicitParams = Nil,
      tpe = rowParserType,
      throws = Nil,
