...
import typr.internal.codegen.DbLib.RowType
        ...
        val (underlyingType, underlyingTypoType, sqlType): (jvm.Type.Qualified, TypoType, String) =
          x.openEnum match {
            case OpenEnum.Text(_) =>
              val textDbType = table.dbType.adapter(table.scalaTypeMapper.needsTimestampCasts).textType
              (lang.String, TypoType.Standard(lang.String, textDbType), "text")
