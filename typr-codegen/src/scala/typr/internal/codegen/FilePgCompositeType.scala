...
/** Generates a file for a PostgreSQL composite type.
  ...
  *   - A static `dbStruct` field using PgStruct.builder()
  *   - A static `dbType` field that calls dbStruct.asType()
...
object FilePgCompositeType {
  ...
  private def pgStructInstances(
    ...
    if (computed.fields.isEmpty) {
      ...
      // Create the dbStruct field
      val dbStructField = jvm.Given(
        tparams = Nil,
        name = jvm.Ident("dbStruct"),
        implicitParams = Nil,
        tpe = PgStruct.of(computed.tpe),
        body = buildCall
      )

      // Create the dbType field that calls dbStruct.asType() - named dbType for consistency with domains
      val dbTypeField = jvm.Given(
        tparams = Nil,
        name = jvm.Ident("dbType"),
        implicitParams = Nil,
        tpe = PgType.of(computed.tpe),
        body = code"dbStruct.asType()"
      )

      // Create the dbTypeArray field for array of this composite type
      ...
      val compositeTextGetter = lang match {
        case _: LangScala  => code"dbType.pgCompositeText()"
        case _: LangKotlin => code"dbType.pgCompositeText()"
        case _             => code"dbType.pgCompositeText()"
      }
      val dbTypeArrayField = jvm.Given(
        tparams = Nil,
        name = jvm.Ident("dbTypeArray"),
        implicitParams = Nil,
        tpe = PgType.of(arrayType),
        body = code"dbType.array($PgRead.readCompositeArray($compositeTextGetter, $arrayFactory), $arrayFactory)"
      )

      List(dbStructField, dbTypeField, dbTypeArrayField)
    }
  }
  ...
  private def lookupFieldType(
    ...
    dbType match {
      case c: db.PgType.CompositeType =>
        // Nested composite - look up its type and reference its dbType field
        compositeLookup.get(c.name) match {
          case Some(computed) =>
            code"${computed.tpe}.dbType"
          case None =>
            sys.error(s"Nested composite type not found in lookup: ${c.name.value}")
        }
      case db.PgType.Array(c: db.PgType.CompositeType) =>
        // Array of composites - use the dbTypeArray field
        compositeLookup.get(c.name) match {
          case Some(computed) =>
            code"${computed.tpe}.dbTypeArray"
          case None =>
            sys.error(s"Nested composite type not found in lookup: ${c.name.value}")
        }
      case _ =>
