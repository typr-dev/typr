...
class DbLibAnorm(pkg: jvm.QIdent, inlineImplicits: Boolean, default: ComputedDefault, enableStreamingInserts: Boolean, override val lang: LangScala) extends DbLibLegacy {
  ...
  def lookupColumnFor(tpe: jvm.Type): jvm.Code =
    if (!inlineImplicits) jvm.Summon(Column.of(tpe)).code
      ...
      jvm.Type.base(tpe) match {
        ...
        case jvm.Type.ArrayOf(targ) => code"$Column.columnToArray[$targ](${dialect.usingCall}${lookupColumnFor(targ)}, ${dialect.implicitlyCall})"
  ...
  override def resolveConstAs(typoType: TypoType): jvm.Code = {
    ...
    typoType match {
      case TypoType.Nullable(_, inner) =>
        code"${lang.dsl.ConstAsAsOpt}[${inner.jvmType}]($ToParameterValue(${dialect.usingCall}null, ${lookupToStatementFor(tpe)}), ${lookupParameterMetaDataFor(tpe)})"
      case _ =>
        code"${lang.dsl.ConstAsAs}[$tpe](${dialect.usingCall}$ToParameterValue(${dialect.usingCall}null, ${lookupToStatementFor(tpe)}), ${lookupParameterMetaDataFor(tpe)})"
    }
  }
    ...
    List(
      ...
      Some(
        jvm.Given(
          ...
          body = code"$Column.columnToArray(${dialect.usingCall}$columnName, ${dialect.implicitlyCall})"
        )
      ),
      Some(
