...
object FileMariaSet {
  ...
  private def generateWrapper(options: InternalOptions, setType: ComputedMariaSet, memberEnumType: jvm.Type.Qualified, adapter: DbAdapter): jvm.File = {
    ...
    val dbTypeInstances: List[jvm.ClassMember] = options.dbLib.toList.flatMap { _ =>
      mariaTypeInstance(setType, memberEnumType, lang, adapter)
    }
  ...
  private def mariaTypeInstance(setType: ComputedMariaSet, memberEnumType: jvm.Type, lang: Lang, adapter: DbAdapter): List[jvm.ClassMember] = {
    val MariaSetType = jvm.Type.Qualified("dev.typr.foundations.data.maria.MariaSet")
    val MariaTypes = adapter.typesFor(lang.typeSupport)
    val MariaType = adapter.typeClassFor(lang.typeSupport)
    ...
    val body = code"$MariaTypes.set.to(${lang.dsl.Bijection}.of($fromMariaSet, $toMariaSet))"
