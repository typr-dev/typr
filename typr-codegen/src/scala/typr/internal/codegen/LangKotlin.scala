...
case class LangKotlin(typeSupport: TypeSupport) extends Lang {
  ...
  override def arrayEquals(left: jvm.Code, right: jvm.Code): jvm.Code =
    code"$left.contentEquals($right)"

  override def arrayHashCode(arr: jvm.Code): jvm.Code =
    code"$arr.contentHashCode()"
  ...
  override def renderTree(tree: jvm.Tree, ctx: Ctx): jvm.Code =
    tree match {
      ...
      case jvm.BlockExpr(statements, result)  =>
        // Kotlin: { stmt1; stmt2; result } - last expression is the value
      ...
      case jvm.StringInterpolate(_, _, _) =>
        ...
        sys.error("StringInterpolate should not reach LangKotlin. DbLibFoundations.SQL() should have rewritten it to Fragment.concat() calls.")
      ...
      case cls: jvm.Adt.Record =>
        ...
        val staticMembers = cls.staticMembers.map(x => renderTree(x, memberCtx))
      ...
      case sum: jvm.Adt.Sum =>
        ...
        val companionBody = sum.staticMembers.map(x => renderTree(x, memberCtx.withStatic))
      ...
      case cls: jvm.Class =>
        ...
        val staticMembers: List[jvm.Code] = cls.staticMembers.map(x => renderTree(x, memberCtx))
