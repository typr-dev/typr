...
case class LangScala(dialect: Dialect, typeSupport: TypeSupport, dsl: DslQualifiedNames) extends Lang {
  ...
  override def renderTree(tree: jvm.Tree, ctx: Ctx): jvm.Code =
    tree match {
      ...
      case jvm.BlockExpr(statements, result) =>
        // Scala: { stmt1; stmt2; result } - last expression is the value
        val stmts = statements.map(s => code"$s")
        code"""|{
               |  ${stmts.mkCode("\n")}
               |  $result
               |}""".stripMargin
...
""")
  ...
  override def bijection(wrapperType: jvm.Type, underlying: jvm.Type, getter: jvm.FieldGetterRef, constructor: jvm.ConstructorMethodRef): jvm.Code = {
    ...
    dsl match {
      case DslQualifiedNames.Legacy =>
        code"$bijection[$wrapperType, $underlying]($getter)($constructor)"
      case _ =>
        code"$bijection.of[$wrapperType, $underlying]($getter, $constructor)"
    }
  }
  ...
  // Iterate a list. With the Scala-wrapper DSL the param is a Scala `List` (use `.foreach`); with the Java DSL it's `java.util.List` (use Java's `.forEach(Consumer)`, which Scala SAM-converts).
  override def arrayForEach(array: jvm.Code, elemVar: jvm.Ident, body: jvm.Body): jvm.Code = {
    val method = if (dsl == DslQualifiedNames.Scala) "foreach" else "forEach"
    body match {
      case jvm.Body.Expr(expr)   => code"$array.$method { $elemVar => $expr }"
      case jvm.Body.Stmts(stmts) => code"$array.$method { $elemVar => ${stmts.mkCode("; ")} }"
      case jvm.Body.Abstract     => code"$array.$method { _ => () }"
    }
  }
  ...
  override def arrayEquals(left: jvm.Code, right: jvm.Code): jvm.Code =
    code"${TypesJava.Arrays}.equals($left, $right)"

  override def arrayHashCode(arr: jvm.Code): jvm.Code =
    code"${TypesJava.Arrays}.hashCode($arr)"

  override def strip(expr: jvm.Code): jvm.Code =
    code"$expr.strip()"

  override def stripTrailing(expr: jvm.Code): jvm.Code =
    code"$expr.stripTrailing()"

  override def bigDecimalZero: jvm.Code = typeSupport match {
    case TypeSupportScala => code"${TypesScala.BigDecimal}(0)"
    case _                => code"${TypesJava.BigDecimal}.ZERO"
  }

  override def bigIntegerZero: jvm.Code = typeSupport match {
    case TypeSupportScala => code"${TypesScala.BigInt}(0)"
    case _                => code"${TypesJava.BigInteger}.ZERO"
  }

  override def bigDecimalFromLong(v: jvm.Code): jvm.Code = typeSupport match {
    case TypeSupportScala => code"${TypesScala.BigDecimal}($v)"
    case _                => code"${TypesJava.BigDecimal}.valueOf($v)"
  }

  override def bigIntegerFromLong(v: jvm.Code): jvm.Code = typeSupport match {
    case TypeSupportScala => code"${TypesScala.BigInt}($v)"
    case _                => code"${TypesJava.BigInteger}.valueOf($v)"
  }

  override def getClassName(expr: jvm.Code): jvm.Code =
    code"$expr.getClass().getName()"
