...
object EffectType {
  ...
  private object MutinyUniOps extends EffectTypeOps {
    ...
    def foreachDiscard(iterable: jvm.Code, elementVar: jvm.Ident, body: jvm.Code): jvm.Code = {
      val lambda = jvm.Lambda(List(jvm.LambdaParam(elementVar)), jvm.Body.Expr(body))
      code"$MultiType.createFrom().iterable($iterable).onItem().transformToUniAndConcatenate($lambda).collect().last()"
    }
    ...
    def toCompletableFuture(effect: jvm.Code): jvm.Code =
      code"$effect.subscribeAsCompletionStage()"
    def awaitBlocking(effect: jvm.Code): jvm.Code =
      code"$effect.await().indefinitely()"
  }
  ...
  private object ReactorMonoOps extends EffectTypeOps {
    ...
    def foreachDiscard(iterable: jvm.Code, elementVar: jvm.Ident, body: jvm.Code): jvm.Code = {
      val lambda = jvm.Lambda(List(jvm.LambdaParam(elementVar)), jvm.Body.Expr(body))
      code"$FluxType.fromIterable($iterable).flatMap($lambda).then()"
    }
  ...
  private object CompletableFutureOps extends EffectTypeOps {
    ...
    def foreachDiscard(iterable: jvm.Code, elementVar: jvm.Ident, body: jvm.Code): jvm.Code = {
      val lambda = jvm.Lambda(List(jvm.LambdaParam(elementVar)), jvm.Body.Expr(body))
      code"$tpe.allOf($StreamSupportType.stream($iterable.spliterator(), false).map($lambda).toArray($tpe[]::new))"
    }
  ...
  private object CatsIOOps extends EffectTypeOps {
    ...
    def subscribeWith(effect: jvm.Code, onItem: jvm.Code, onFailure: jvm.Code): jvm.Code = {
      // For Cats IO, run synchronously and let the caller inline the body
      // We pass a placeholder that the codegen should replace with inline code
      // Since this doesn't work well with type inference, just run and call onItem
      val result = jvm.Ident("_result")
      code"{ val $result = $effect.unsafeRunSync()(cats.effect.unsafe.IORuntime.global); ($onItem)($result) }"
    }
    def defer(supplier: jvm.Code): jvm.Code =
      code"$tpe.delay(($supplier).apply())"
    def toCompletableFuture(effect: jvm.Code): jvm.Code =
      code"$effect.unsafeToCompletableFuture()(cats.effect.unsafe.IORuntime.global)"
    def awaitBlocking(effect: jvm.Code): jvm.Code =
      code"$effect.unsafeRunSync()(cats.effect.unsafe.IORuntime.global)"
  }
  ...
  private object ZIOOps extends EffectTypeOps {
    ...
    def toCompletableFuture(effect: jvm.Code): jvm.Code = {
      val UnsafeType = jvm.Type.Qualified(jvm.QIdent("zio.Unsafe"))
      val RuntimeType = jvm.Type.Qualified(jvm.QIdent("zio.Runtime"))
      code"$UnsafeType.unsafe { implicit u => $RuntimeType.default.unsafe.runToFuture($effect) }"
    }
    def awaitBlocking(effect: jvm.Code): jvm.Code = {
      val UnsafeType = jvm.Type.Qualified(jvm.QIdent("zio.Unsafe"))
      val RuntimeType = jvm.Type.Qualified(jvm.QIdent("zio.Runtime"))
      code"$UnsafeType.unsafe { implicit u => $RuntimeType.default.unsafe.run($effect).getOrThrowFiberFailure() }"
    }
  }
