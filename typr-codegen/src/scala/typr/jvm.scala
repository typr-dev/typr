...
object jvm {
  ...
  object Call {
    ...
    def withImplicits(target: Code, args: List[Arg], implicitArgs: List[Arg]): Call = {
      val groups = List(
        if (args.nonEmpty) Some(ArgGroup(args, isImplicit = false)) else None,
  ...
  /** Block expression - executes statements then evaluates to result. Java: { stmt1; stmt2; yield result; } Scala/Kotlin: { stmt1; stmt2; result }
    */
  case class BlockExpr(statements: List[Code], result: Code) extends Tree
