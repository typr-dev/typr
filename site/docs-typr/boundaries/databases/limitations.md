...
the interface of the generated code is not final yet. You can keep using a particular version of Typr
~forever, but if you want to upgrade to the newest version you need to expect at least some API churn.

### Correctness

Typr tries to make a guarantee that all generated code will not fail with a type error.
You can still break [constraints](other-features/constraints.md) naturally, but the types should align.

Typr is close to delivering on this, but JDBC makes it *incredibly* difficult to get all the details exactly right.
