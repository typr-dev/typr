...
| `silentBanner`           | Controls whether to suppress the Typr banner while running Typr (default is `false`).                                                                                                                         |
...
| `openEnums`              | Controls if you want to tag table ids as [open enums](../type-safety/open-enums.md)                                                                                                            |
...
Typr supports multiple Scala database libraries, each with specific optimizations:
...
Typr can generate code for different Scala versions using the `dialect` option:
