...
Typr has a mechanism by which you can choose which relations to generate code for.
...
So in Typr we say that relations have dependencies, see [flow typing](../type-safety/type-flow.md). 
...
Typr can optionally generate code for these dependencies as well. 

If you want that, you can  [customize](overview.md) Typr and set the `keepDependencies` parameter to `true` to generate code for those dependencies as well. 
