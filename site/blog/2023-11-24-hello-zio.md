...
the [COPY API for streaming inserts](/db/other-features/streaming-inserts).
...
PostgreSQL features through `zio-schema`, and we wanted to generate code which is as [fast to compile](/db/other-features/faster-compilation) as possible.

### Also support for `zio-json`

Typo supports generating [JSON codecs](/db/other-features/json) for all the row types.
