...
Thanks to the efforts of Jules Ivanic in [PR #57](https://github.com/oyvindberg/typr/pull/57),
Typo now supports using `zio-jdbc` as a database library.

### Never heard of Typo? 

You can check out the [introduction](https://oyvindberg.github.io/typo/db/).
...
the [COPY API for streaming inserts](/typr/boundaries/databases/other-features/streaming-inserts).
...
PostgreSQL features through `zio-schema`, and we wanted to generate code which is as [fast to compile](/typr/boundaries/databases/other-features/faster-compilation) as possible.

### Also support for `zio-json`

Typo supports generating [JSON codecs](/typr/boundaries/databases/other-features/json) for all the row types.
