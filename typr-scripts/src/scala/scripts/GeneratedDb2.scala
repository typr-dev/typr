...
object GeneratedDb2 {
  ...
  def main(args: Array[String]): Unit =
    Loggers
      ...
      .use { logger =>
        val ds = TypoDataSource.hikariDb2(
          server = "localhost",
          port = 50000,
          databaseName = "typr",
          username = "db2inst1",
        ...
        val metadb = Await.result(MetaDb.fromDb(typoLogger, ds, selector, schemaMode = SchemaMode.SingleSchema("DB2INST1"), externalTools), Duration.Inf)

        val variants: Seq[(Lang, DbLibName, JsonLibName, String, String)] = List(
          (LangJava, DbLibName.Typo, JsonLibName.Jackson, "testers/db2/java", ""),
          (LangScala.scalaDsl(Dialect.Scala3, TypeSupportScala), DbLibName.Typo, JsonLibName.Jackson, "testers/db2/scala", ""),
          (LangKotlin(TypeSupportKotlin), DbLibName.Typo, JsonLibName.Jackson, "testers/db2/kotlin", "")
        )
