...
class CustomerRepoImpl extends CustomerRepo {
  ...
  override def selectById(customerid: CustomerId): ConnectionIO[Option[CustomerRow]] = {
    sql"""select "customerid", "personid", "storeid", "territoryid", "rowguid", "modifieddate"::text from "sales"."customer" where "customerid" = ${fromWrite(customerid)(new Write.Single(CustomerId.put))}""".query(CustomerRow.read).option
  }
  ...
  override def upsertStreaming(unsaved: Stream[ConnectionIO, CustomerRow], batchSize: Int = 10000): ConnectionIO[Int] = {
    for {
      ...
      _ <- new FragmentOps(sql"""copy customer_TEMP("customerid", "personid", "storeid", "territoryid", "rowguid", "modifieddate") from stdin""").copyIn(unsaved, batchSize)(CustomerRow.text)
