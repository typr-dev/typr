...
class CurrencyRepoImpl extends CurrencyRepo {
  ...
  override def selectById(currencycode: CurrencyId): ConnectionIO[Option[CurrencyRow]] = {
    sql"""select "currencycode", "name", "modifieddate"::text from "sales"."currency" where "currencycode" = ${fromWrite(currencycode)(new Write.Single(CurrencyId.put))}""".query(CurrencyRow.read).option
  }
  ...
  override def upsertStreaming(unsaved: Stream[ConnectionIO, CurrencyRow], batchSize: Int = 10000): ConnectionIO[Int] = {
    for {
      ...
      _ <- new FragmentOps(sql"""copy currency_TEMP("currencycode", "name", "modifieddate") from stdin""").copyIn(unsaved, batchSize)(CurrencyRow.text)
