...
class TestOrganisasjonRepoImpl extends TestOrganisasjonRepo {
  ...
  override def insertStreaming(unsaved: Stream[ConnectionIO, TestOrganisasjonRow], batchSize: Int = 10000): ConnectionIO[Long] = {
    new FragmentOps(sql"""COPY "public"."test_organisasjon"("organisasjonskode") FROM STDIN""").copyIn(unsaved, batchSize)(TestOrganisasjonRow.text)
  }
  ...
  override def selectAll: Stream[ConnectionIO, TestOrganisasjonRow] = {
    sql"""select "organisasjonskode" from "public"."test_organisasjon"""".query(TestOrganisasjonRow.read).stream
  }
  ...
  override def upsertStreaming(unsaved: Stream[ConnectionIO, TestOrganisasjonRow], batchSize: Int = 10000): ConnectionIO[Int] = {
    for {
      ...
      _ <- new FragmentOps(sql"""copy test_organisasjon_TEMP("organisasjonskode") from stdin""").copyIn(unsaved, batchSize)(TestOrganisasjonRow.text)
