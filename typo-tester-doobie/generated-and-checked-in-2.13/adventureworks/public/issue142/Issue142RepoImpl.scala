...
class Issue142RepoImpl extends Issue142Repo {
  ...
  override def insertStreaming(unsaved: Stream[ConnectionIO, Issue142Row], batchSize: Int = 10000): ConnectionIO[Long] = {
    new FragmentOps(sql"""COPY "public"."issue142"("tabellkode") FROM STDIN""").copyIn(unsaved, batchSize)(Issue142Row.text)
  }
  ...
  override def selectAll: Stream[ConnectionIO, Issue142Row] = {
    sql"""select "tabellkode" from "public"."issue142"""".query(Issue142Row.read).stream
  }
  override def selectById(tabellkode: Issue142Id): ConnectionIO[Option[Issue142Row]] = {
    sql"""select "tabellkode" from "public"."issue142" where "tabellkode" = ${fromWrite(tabellkode)(new Write.Single(Issue142Id.put))}""".query(Issue142Row.read).option
  }
  override def selectByIds(tabellkodes: Array[Issue142Id]): Stream[ConnectionIO, Issue142Row] = {
    sql"""select "tabellkode" from "public"."issue142" where "tabellkode" = ANY(${tabellkodes})""".query(Issue142Row.read).stream
  }
  ...
  override def upsertStreaming(unsaved: Stream[ConnectionIO, Issue142Row], batchSize: Int = 10000): ConnectionIO[Int] = {
    for {
      ...
      _ <- new FragmentOps(sql"""copy issue142_TEMP("tabellkode") from stdin""").copyIn(unsaved, batchSize)(Issue142Row.text)
