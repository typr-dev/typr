...
class IdentityTestRepoImpl extends IdentityTestRepo {
  ...
  override def insertStreaming(unsaved: Stream[ConnectionIO, IdentityTestRow], batchSize: Int = 10000): ConnectionIO[Long] = {
    new FragmentOps(sql"""COPY "public"."identity-test"("default_generated", "name") FROM STDIN""").copyIn(unsaved, batchSize)(IdentityTestRow.text)
  }
  ...
  override def selectById(name: IdentityTestId): ConnectionIO[Option[IdentityTestRow]] = {
    sql"""select "always_generated", "default_generated", "name" from "public"."identity-test" where "name" = ${fromWrite(name)(new Write.Single(IdentityTestId.put))}""".query(IdentityTestRow.read).option
  }
  ...
  override def upsertStreaming(unsaved: Stream[ConnectionIO, IdentityTestRow], batchSize: Int = 10000): ConnectionIO[Int] = {
    for {
      ...
      _ <- new FragmentOps(sql"""copy identity-test_TEMP("default_generated", "name") from stdin""").copyIn(unsaved, batchSize)(IdentityTestRow.text)
