...
class OnlyPkColumnsRepoImpl extends OnlyPkColumnsRepo {
  ...
  override def insertStreaming(unsaved: Stream[ConnectionIO, OnlyPkColumnsRow], batchSize: Int = 10000): ConnectionIO[Long] = {
    new FragmentOps(sql"""COPY "public"."only_pk_columns"("key_column_1", "key_column_2") FROM STDIN""").copyIn(unsaved, batchSize)(OnlyPkColumnsRow.text)
  }
  ...
  override def selectAll: Stream[ConnectionIO, OnlyPkColumnsRow] = {
    sql"""select "key_column_1", "key_column_2" from "public"."only_pk_columns"""".query(OnlyPkColumnsRow.read).stream
  }
  override def selectById(compositeId: OnlyPkColumnsId): ConnectionIO[Option[OnlyPkColumnsRow]] = {
    sql"""select "key_column_1", "key_column_2" from "public"."only_pk_columns" where "key_column_1" = ${fromWrite(compositeId.keyColumn1)(new Write.Single(Meta.StringMeta.put))} AND "key_column_2" = ${fromWrite(compositeId.keyColumn2)(new Write.Single(Meta.IntMeta.put))}""".query(OnlyPkColumnsRow.read).option
  }
  ...
  override def upsertBatch(unsaved: List[OnlyPkColumnsRow]): Stream[ConnectionIO, OnlyPkColumnsRow] = {
    Update[OnlyPkColumnsRow](
      s"""insert into "public"."only_pk_columns"("key_column_1", "key_column_2")
          ...
          do nothing
          returning "key_column_1", "key_column_2""""
    )(OnlyPkColumnsRow.write)
    .updateManyWithGeneratedKeys[OnlyPkColumnsRow]("key_column_1", "key_column_2")(unsaved)(catsStdInstancesForList, OnlyPkColumnsRow.read)
  }
  ...
  override def upsertStreaming(unsaved: Stream[ConnectionIO, OnlyPkColumnsRow], batchSize: Int = 10000): ConnectionIO[Int] = {
    for {
      ...
      _ <- new FragmentOps(sql"""copy only_pk_columns_TEMP("key_column_1", "key_column_2") from stdin""").copyIn(unsaved, batchSize)(OnlyPkColumnsRow.text)
