...
class TestUtdanningstilbudRepoImpl extends TestUtdanningstilbudRepo {
  ...
  override def insertStreaming(unsaved: Stream[ConnectionIO, TestUtdanningstilbudRow], batchSize: Int = 10000): ConnectionIO[Long] = {
    new FragmentOps(sql"""COPY "public"."test_utdanningstilbud"("organisasjonskode", "utdanningsmulighet_kode") FROM STDIN""").copyIn(unsaved, batchSize)(TestUtdanningstilbudRow.text)
  }
  ...
  override def selectAll: Stream[ConnectionIO, TestUtdanningstilbudRow] = {
    sql"""select "organisasjonskode", "utdanningsmulighet_kode" from "public"."test_utdanningstilbud"""".query(TestUtdanningstilbudRow.read).stream
  }
  override def selectById(compositeId: TestUtdanningstilbudId): ConnectionIO[Option[TestUtdanningstilbudRow]] = {
    sql"""select "organisasjonskode", "utdanningsmulighet_kode" from "public"."test_utdanningstilbud" where "organisasjonskode" = ${fromWrite(compositeId.organisasjonskode)(new Write.Single(TestOrganisasjonId.put))} AND "utdanningsmulighet_kode" = ${fromWrite(compositeId.utdanningsmulighetKode)(new Write.Single(Meta.StringMeta.put))}""".query(TestUtdanningstilbudRow.read).option
  }
  ...
  override def upsertStreaming(unsaved: Stream[ConnectionIO, TestUtdanningstilbudRow], batchSize: Int = 10000): ConnectionIO[Int] = {
    for {
      ...
      _ <- new FragmentOps(sql"""copy test_utdanningstilbud_TEMP("organisasjonskode", "utdanningsmulighet_kode") from stdin""").copyIn(unsaved, batchSize)(TestUtdanningstilbudRow.text)
