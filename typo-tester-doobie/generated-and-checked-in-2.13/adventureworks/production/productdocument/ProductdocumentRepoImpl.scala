...
class ProductdocumentRepoImpl extends ProductdocumentRepo {
  ...
  override def selectById(compositeId: ProductdocumentId): ConnectionIO[Option[ProductdocumentRow]] = {
    sql"""select "productid", "modifieddate"::text, "documentnode" from "production"."productdocument" where "productid" = ${fromWrite(compositeId.productid)(new Write.Single(ProductId.put))} AND "documentnode" = ${fromWrite(compositeId.documentnode)(new Write.Single(DocumentId.put))}""".query(ProductdocumentRow.read).option
  }
    ...
    Update[ProductdocumentRow](
      s"""insert into "production"."productdocument"("productid", "modifieddate", "documentnode")
          ...
          do update set
            "modifieddate" = EXCLUDED."modifieddate"
          returning "productid", "modifieddate"::text, "documentnode""""
    )(ProductdocumentRow.write)
    .updateManyWithGeneratedKeys[ProductdocumentRow]("productid", "modifieddate", "documentnode")(unsaved)(catsStdInstancesForList, ProductdocumentRow.read)
  }
  ...
  override def upsertStreaming(unsaved: Stream[ConnectionIO, ProductdocumentRow], batchSize: Int = 10000): ConnectionIO[Int] = {
    for {
      ...
      _ <- new FragmentOps(sql"""copy productdocument_TEMP("productid", "modifieddate", "documentnode") from stdin""").copyIn(unsaved, batchSize)(ProductdocumentRow.text)
