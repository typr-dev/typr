...
class BusinessentityRepoImpl extends BusinessentityRepo {
  ...
  override def selectById(businessentityid: BusinessentityId): ConnectionIO[Option[BusinessentityRow]] = {
    sql"""select "businessentityid", "rowguid", "modifieddate"::text from "person"."businessentity" where "businessentityid" = ${fromWrite(businessentityid)(new Write.Single(BusinessentityId.put))}""".query(BusinessentityRow.read).option
  }
    ...
    Update[BusinessentityRow](
      s"""insert into "person"."businessentity"("businessentityid", "rowguid", "modifieddate")
          ...
          do update set
            ...
            "modifieddate" = EXCLUDED."modifieddate"
          returning "businessentityid", "rowguid", "modifieddate"::text"""
    )(BusinessentityRow.write)
    .updateManyWithGeneratedKeys[BusinessentityRow]("businessentityid", "rowguid", "modifieddate")(unsaved)(catsStdInstancesForList, BusinessentityRow.read)
  }
  ...
  override def upsertStreaming(unsaved: Stream[ConnectionIO, BusinessentityRow], batchSize: Int = 10000): ConnectionIO[Int] = {
    for {
      ...
      _ <- new FragmentOps(sql"""copy businessentity_TEMP("businessentityid", "rowguid", "modifieddate") from stdin""").copyIn(unsaved, batchSize)(BusinessentityRow.text)
