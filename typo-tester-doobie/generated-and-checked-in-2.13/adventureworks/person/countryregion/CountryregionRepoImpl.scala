...
class CountryregionRepoImpl extends CountryregionRepo {
  ...
  override def selectById(countryregioncode: CountryregionId): ConnectionIO[Option[CountryregionRow]] = {
    sql"""select "countryregioncode", "name", "modifieddate"::text from "person"."countryregion" where "countryregioncode" = ${fromWrite(countryregioncode)(new Write.Single(CountryregionId.put))}""".query(CountryregionRow.read).option
  }
    ...
    Update[CountryregionRow](
      s"""insert into "person"."countryregion"("countryregioncode", "name", "modifieddate")
          ...
          do update set
            ...
            "modifieddate" = EXCLUDED."modifieddate"
          returning "countryregioncode", "name", "modifieddate"::text"""
    )(CountryregionRow.write)
    .updateManyWithGeneratedKeys[CountryregionRow]("countryregioncode", "name", "modifieddate")(unsaved)(catsStdInstancesForList, CountryregionRow.read)
  }
  ...
  override def upsertStreaming(unsaved: Stream[ConnectionIO, CountryregionRow], batchSize: Int = 10000): ConnectionIO[Int] = {
    for {
      ...
      _ <- new FragmentOps(sql"""copy countryregion_TEMP("countryregioncode", "name", "modifieddate") from stdin""").copyIn(unsaved, batchSize)(CountryregionRow.text)
