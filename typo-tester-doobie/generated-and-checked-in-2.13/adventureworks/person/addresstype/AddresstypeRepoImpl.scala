...
class AddresstypeRepoImpl extends AddresstypeRepo {
  ...
  override def selectById(addresstypeid: AddresstypeId): ConnectionIO[Option[AddresstypeRow]] = {
    sql"""select "addresstypeid", "name", "rowguid", "modifieddate"::text from "person"."addresstype" where "addresstypeid" = ${fromWrite(addresstypeid)(new Write.Single(AddresstypeId.put))}""".query(AddresstypeRow.read).option
  }
    ...
    Update[AddresstypeRow](
      s"""insert into "person"."addresstype"("addresstypeid", "name", "rowguid", "modifieddate")
          ...
          do update set
            ...
            "modifieddate" = EXCLUDED."modifieddate"
          returning "addresstypeid", "name", "rowguid", "modifieddate"::text"""
    )(AddresstypeRow.write)
    .updateManyWithGeneratedKeys[AddresstypeRow]("addresstypeid", "name", "rowguid", "modifieddate")(unsaved)(catsStdInstancesForList, AddresstypeRow.read)
  }
  ...
  override def upsertStreaming(unsaved: Stream[ConnectionIO, AddresstypeRow], batchSize: Int = 10000): ConnectionIO[Int] = {
    for {
      ...
      _ <- new FragmentOps(sql"""copy addresstype_TEMP("addresstypeid", "name", "rowguid", "modifieddate") from stdin""").copyIn(unsaved, batchSize)(AddresstypeRow.text)
