...
class StateprovinceRepoImpl extends StateprovinceRepo {
  ...
  override def deleteById(stateprovinceid: StateprovinceId): ConnectionIO[Boolean] = {
    sql"""delete from "person"."stateprovince" where "stateprovinceid" = ${fromWrite(stateprovinceid)(using new Write.Single(StateprovinceId.put))}""".update.run.map(_ > 0)
  }
  ...
  override def selectById(stateprovinceid: StateprovinceId): ConnectionIO[Option[StateprovinceRow]] = {
    sql"""select "stateprovinceid", "stateprovincecode", "countryregioncode", "isonlystateprovinceflag", "name", "territoryid", "rowguid", "modifieddate"::text from "person"."stateprovince" where "stateprovinceid" = ${fromWrite(stateprovinceid)(using new Write.Single(StateprovinceId.put))}""".query(using StateprovinceRow.read).option
  }
