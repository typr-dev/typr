...
class DepartmentRepoImpl extends DepartmentRepo {
  ...
  override def deleteById(departmentid: DepartmentId): ConnectionIO[Boolean] = {
    sql"""delete from "humanresources"."department" where "departmentid" = ${fromWrite(departmentid)(using new Write.Single(DepartmentId.put))}""".update.run.map(_ > 0)
  }
  ...
  override def selectById(departmentid: DepartmentId): ConnectionIO[Option[DepartmentRow]] = {
    sql"""select "departmentid", "name", "groupname", "modifieddate"::text from "humanresources"."department" where "departmentid" = ${fromWrite(departmentid)(using new Write.Single(DepartmentId.put))}""".query(using DepartmentRow.read).option
  }
