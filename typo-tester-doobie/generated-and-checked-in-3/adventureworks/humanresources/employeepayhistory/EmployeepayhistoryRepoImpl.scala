...
class EmployeepayhistoryRepoImpl extends EmployeepayhistoryRepo {
  ...
  override def deleteById(compositeId: EmployeepayhistoryId): ConnectionIO[Boolean] = {
    sql"""delete from "humanresources"."employeepayhistory" where "businessentityid" = ${fromWrite(compositeId.businessentityid)(using new Write.Single(BusinessentityId.put))} AND "ratechangedate" = ${fromWrite(compositeId.ratechangedate)(using new Write.Single(TypoLocalDateTime.put))}""".update.run.map(_ > 0)
  }
