...
object CustomerRow {
  given decoder: Decoder[CustomerRow] = Decoder.forProduct6[CustomerRow, CustomerId, Option[BusinessentityId], Option[BusinessentityId], Option[SalesterritoryId], TypoUUID, TypoLocalDateTime]("customerid", "personid", "storeid", "territoryid", "rowguid", "modifieddate")(CustomerRow.apply)(using CustomerId.decoder, Decoder.decodeOption(using BusinessentityId.decoder), Decoder.decodeOption(using BusinessentityId.decoder), Decoder.decodeOption(using SalesterritoryId.decoder), TypoUUID.decoder, TypoLocalDateTime.decoder)
  given encoder: Encoder[CustomerRow] = Encoder.forProduct6[CustomerRow, CustomerId, Option[BusinessentityId], Option[BusinessentityId], Option[SalesterritoryId], TypoUUID, TypoLocalDateTime]("customerid", "personid", "storeid", "territoryid", "rowguid", "modifieddate")(x => (x.customerid, x.personid, x.storeid, x.territoryid, x.rowguid, x.modifieddate))(using CustomerId.encoder, Encoder.encodeOption(using BusinessentityId.encoder), Encoder.encodeOption(using BusinessentityId.encoder), Encoder.encodeOption(using SalesterritoryId.encoder), TypoUUID.encoder, TypoLocalDateTime.encoder)
  given read: Read[CustomerRow] = new Read.CompositeOfInstances(Array(
  ...
  given write: Write[CustomerRow] = new Write.Composite[CustomerRow](
