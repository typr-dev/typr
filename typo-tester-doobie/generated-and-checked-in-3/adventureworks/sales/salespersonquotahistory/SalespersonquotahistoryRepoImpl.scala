...
class SalespersonquotahistoryRepoImpl extends SalespersonquotahistoryRepo {
  ...
  override def deleteById(compositeId: SalespersonquotahistoryId): ConnectionIO[Boolean] = {
    sql"""delete from "sales"."salespersonquotahistory" where "businessentityid" = ${fromWrite(compositeId.businessentityid)(using new Write.Single(BusinessentityId.put))} AND "quotadate" = ${fromWrite(compositeId.quotadate)(using new Write.Single(TypoLocalDateTime.put))}""".update.run.map(_ > 0)
  }
