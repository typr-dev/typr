...
object StoreRow {
  given decoder: Decoder[StoreRow] = Decoder.forProduct6[StoreRow, BusinessentityId, Name, Option[BusinessentityId], Option[TypoXml], TypoUUID, TypoLocalDateTime]("businessentityid", "name", "salespersonid", "demographics", "rowguid", "modifieddate")(StoreRow.apply)(using BusinessentityId.decoder, Name.decoder, Decoder.decodeOption(using BusinessentityId.decoder), Decoder.decodeOption(using TypoXml.decoder), TypoUUID.decoder, TypoLocalDateTime.decoder)
  given encoder: Encoder[StoreRow] = Encoder.forProduct6[StoreRow, BusinessentityId, Name, Option[BusinessentityId], Option[TypoXml], TypoUUID, TypoLocalDateTime]("businessentityid", "name", "salespersonid", "demographics", "rowguid", "modifieddate")(x => (x.businessentityid, x.name, x.salespersonid, x.demographics, x.rowguid, x.modifieddate))(using BusinessentityId.encoder, Name.encoder, Encoder.encodeOption(using BusinessentityId.encoder), Encoder.encodeOption(using TypoXml.encoder), TypoUUID.encoder, TypoLocalDateTime.encoder)
  given read: Read[StoreRow] = new Read.CompositeOfInstances(Array(
  ...
  given text: Text[StoreRow] = Text.instance[StoreRow]{ (row, sb) =>
  ...
  given write: Write[StoreRow] = new Write.Composite[StoreRow](
