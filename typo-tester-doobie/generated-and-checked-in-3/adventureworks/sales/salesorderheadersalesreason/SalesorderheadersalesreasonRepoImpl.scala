...
class SalesorderheadersalesreasonRepoImpl extends SalesorderheadersalesreasonRepo {
  ...
  override def deleteById(compositeId: SalesorderheadersalesreasonId): ConnectionIO[Boolean] = {
    sql"""delete from "sales"."salesorderheadersalesreason" where "salesorderid" = ${fromWrite(compositeId.salesorderid)(using new Write.Single(SalesorderheaderId.put))} AND "salesreasonid" = ${fromWrite(compositeId.salesreasonid)(using new Write.Single(SalesreasonId.put))}""".update.run.map(_ > 0)
  }
