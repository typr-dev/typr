...
class WorkorderroutingRepoImpl extends WorkorderroutingRepo {
  ...
  override def deleteById(compositeId: WorkorderroutingId): ConnectionIO[Boolean] = {
    sql"""delete from "production"."workorderrouting" where "workorderid" = ${fromWrite(compositeId.workorderid)(using new Write.Single(WorkorderId.put))} AND "productid" = ${fromWrite(compositeId.productid)(using new Write.Single(Meta.IntMeta.put))} AND "operationsequence" = ${fromWrite(compositeId.operationsequence)(using new Write.Single(TypoShort.put))}""".update.run.map(_ > 0)
  }
