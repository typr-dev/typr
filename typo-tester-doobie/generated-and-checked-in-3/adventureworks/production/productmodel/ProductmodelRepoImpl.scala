...
class ProductmodelRepoImpl extends ProductmodelRepo {
  ...
  override def deleteById(productmodelid: ProductmodelId): ConnectionIO[Boolean] = {
    sql"""delete from "production"."productmodel" where "productmodelid" = ${fromWrite(productmodelid)(using new Write.Single(ProductmodelId.put))}""".update.run.map(_ > 0)
  }
  ...
  override def selectById(productmodelid: ProductmodelId): ConnectionIO[Option[ProductmodelRow]] = {
    sql"""select "productmodelid", "name", "catalogdescription", "instructions", "rowguid", "modifieddate"::text from "production"."productmodel" where "productmodelid" = ${fromWrite(productmodelid)(using new Write.Single(ProductmodelId.put))}""".query(using ProductmodelRow.read).option
  }
