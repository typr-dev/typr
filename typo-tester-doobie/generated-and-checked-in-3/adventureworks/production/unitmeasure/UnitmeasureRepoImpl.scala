...
class UnitmeasureRepoImpl extends UnitmeasureRepo {
  ...
  override def deleteById(unitmeasurecode: UnitmeasureId): ConnectionIO[Boolean] = {
    sql"""delete from "production"."unitmeasure" where "unitmeasurecode" = ${fromWrite(unitmeasurecode)(using new Write.Single(UnitmeasureId.put))}""".update.run.map(_ > 0)
  }
  ...
  override def selectById(unitmeasurecode: UnitmeasureId): ConnectionIO[Option[UnitmeasureRow]] = {
    sql"""select "unitmeasurecode", "name", "modifieddate"::text from "production"."unitmeasure" where "unitmeasurecode" = ${fromWrite(unitmeasurecode)(using new Write.Single(UnitmeasureId.put))}""".query(using UnitmeasureRow.read).option
  }
