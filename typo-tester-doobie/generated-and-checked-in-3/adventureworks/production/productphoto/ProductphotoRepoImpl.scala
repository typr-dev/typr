...
class ProductphotoRepoImpl extends ProductphotoRepo {
  ...
  override def deleteById(productphotoid: ProductphotoId): ConnectionIO[Boolean] = {
    sql"""delete from "production"."productphoto" where "productphotoid" = ${fromWrite(productphotoid)(using new Write.Single(ProductphotoId.put))}""".update.run.map(_ > 0)
  }
  ...
  override def selectById(productphotoid: ProductphotoId): ConnectionIO[Option[ProductphotoRow]] = {
    sql"""select "productphotoid", "thumbnailphoto", "thumbnailphotofilename", "largephoto", "largephotofilename", "modifieddate"::text from "production"."productphoto" where "productphotoid" = ${fromWrite(productphotoid)(using new Write.Single(ProductphotoId.put))}""".query(using ProductphotoRow.read).option
  }
