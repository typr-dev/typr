...
class DocumentRepoImpl extends DocumentRepo {
  ...
  override def deleteById(documentnode: DocumentId): ConnectionIO[Boolean] = {
    sql"""delete from "production"."document" where "documentnode" = ${fromWrite(documentnode)(using new Write.Single(DocumentId.put))}""".update.run.map(_ > 0)
  }
  ...
  override def selectById(documentnode: DocumentId): ConnectionIO[Option[DocumentRow]] = {
    sql"""select "title", "owner", "folderflag", "filename", "fileextension", "revision", "changenumber", "status", "documentsummary", "document", "rowguid", "modifieddate"::text, "documentnode" from "production"."document" where "documentnode" = ${fromWrite(documentnode)(using new Write.Single(DocumentId.put))}""".query(using DocumentRow.read).option
  }
