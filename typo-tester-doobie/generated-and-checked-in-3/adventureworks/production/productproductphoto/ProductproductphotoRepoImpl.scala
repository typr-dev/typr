...
class ProductproductphotoRepoImpl extends ProductproductphotoRepo {
  ...
  override def deleteById(compositeId: ProductproductphotoId): ConnectionIO[Boolean] = {
    sql"""delete from "production"."productproductphoto" where "productid" = ${fromWrite(compositeId.productid)(using new Write.Single(ProductId.put))} AND "productphotoid" = ${fromWrite(compositeId.productphotoid)(using new Write.Single(ProductphotoId.put))}""".update.run.map(_ > 0)
  }
