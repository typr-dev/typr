...
class ProductmodelproductdescriptioncultureRepoImpl extends ProductmodelproductdescriptioncultureRepo {
  ...
  override def deleteById(compositeId: ProductmodelproductdescriptioncultureId): ConnectionIO[Boolean] = {
    sql"""delete from "production"."productmodelproductdescriptionculture" where "productmodelid" = ${fromWrite(compositeId.productmodelid)(using new Write.Single(ProductmodelId.put))} AND "productdescriptionid" = ${fromWrite(compositeId.productdescriptionid)(using new Write.Single(ProductdescriptionId.put))} AND "cultureid" = ${fromWrite(compositeId.cultureid)(using new Write.Single(CultureId.put))}""".update.run.map(_ > 0)
  }
