...
class ScrapreasonRepoImpl extends ScrapreasonRepo {
  ...
  override def deleteById(scrapreasonid: ScrapreasonId): ConnectionIO[Boolean] = {
    sql"""delete from "production"."scrapreason" where "scrapreasonid" = ${fromWrite(scrapreasonid)(using new Write.Single(ScrapreasonId.put))}""".update.run.map(_ > 0)
  }
  ...
  override def insert(unsaved: ScrapreasonRowUnsaved): ConnectionIO[ScrapreasonRow] = {
    val fs = List(
      ...
      unsaved.scrapreasonid match {
        ...
        case Defaulted.Provided(value) => Some((Fragment.const0(s""""scrapreasonid""""), fr"${fromWrite(value: ScrapreasonId)(using new Write.Single(ScrapreasonId.put))}::int4"))
      },
  ...
  override def selectById(scrapreasonid: ScrapreasonId): ConnectionIO[Option[ScrapreasonRow]] = {
    sql"""select "scrapreasonid", "name", "modifieddate"::text from "production"."scrapreason" where "scrapreasonid" = ${fromWrite(scrapreasonid)(using new Write.Single(ScrapreasonId.put))}""".query(using ScrapreasonRow.read).option
  }
  ...
  override def update(row: ScrapreasonRow): ConnectionIO[Option[ScrapreasonRow]] = {
    ...
    sql"""update "production"."scrapreason"
          ...
          where "scrapreasonid" = ${fromWrite(scrapreasonid)(using new Write.Single(ScrapreasonId.put))}
