...
class ProductinventoryRepoImpl extends ProductinventoryRepo {
  ...
  override def deleteById(compositeId: ProductinventoryId): ConnectionIO[Boolean] = {
    sql"""delete from "production"."productinventory" where "productid" = ${fromWrite(compositeId.productid)(using new Write.Single(ProductId.put))} AND "locationid" = ${fromWrite(compositeId.locationid)(using new Write.Single(LocationId.put))}""".update.run.map(_ > 0)
  }
