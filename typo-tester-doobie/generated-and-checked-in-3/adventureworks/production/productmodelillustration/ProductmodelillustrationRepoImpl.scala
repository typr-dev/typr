...
class ProductmodelillustrationRepoImpl extends ProductmodelillustrationRepo {
  ...
  override def deleteById(compositeId: ProductmodelillustrationId): ConnectionIO[Boolean] = {
    sql"""delete from "production"."productmodelillustration" where "productmodelid" = ${fromWrite(compositeId.productmodelid)(using new Write.Single(ProductmodelId.put))} AND "illustrationid" = ${fromWrite(compositeId.illustrationid)(using new Write.Single(IllustrationId.put))}""".update.run.map(_ > 0)
  }
