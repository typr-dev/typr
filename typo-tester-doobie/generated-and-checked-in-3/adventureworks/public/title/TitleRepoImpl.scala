...
class TitleRepoImpl extends TitleRepo {
  ...
  override def deleteById(code: TitleId): ConnectionIO[Boolean] = {
    sql"""delete from "public"."title" where "code" = ${fromWrite(code)(using new Write.Single(TitleId.put))}""".update.run.map(_ > 0)
  }
  ...
  override def selectById(code: TitleId): ConnectionIO[Option[TitleRow]] = {
    sql"""select "code" from "public"."title" where "code" = ${fromWrite(code)(using new Write.Single(TitleId.put))}""".query(using TitleRow.read).option
  }
