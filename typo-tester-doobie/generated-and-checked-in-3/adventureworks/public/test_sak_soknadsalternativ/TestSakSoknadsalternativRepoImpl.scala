...
class TestSakSoknadsalternativRepoImpl extends TestSakSoknadsalternativRepo {
  ...
  override def deleteById(compositeId: TestSakSoknadsalternativId): ConnectionIO[Boolean] = {
    sql"""delete from "public"."test_sak_soknadsalternativ" where "organisasjonskode_saksbehandler" = ${fromWrite(compositeId.organisasjonskodeSaksbehandler)(using new Write.Single(Meta.StringMeta.put))} AND "utdanningsmulighet_kode" = ${fromWrite(compositeId.utdanningsmulighetKode)(using new Write.Single(Meta.StringMeta.put))}""".update.run.map(_ > 0)
  }
