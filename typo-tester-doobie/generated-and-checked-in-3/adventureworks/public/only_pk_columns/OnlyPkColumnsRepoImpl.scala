...
class OnlyPkColumnsRepoImpl extends OnlyPkColumnsRepo {
  ...
  override def deleteById(compositeId: OnlyPkColumnsId): ConnectionIO[Boolean] = {
    sql"""delete from "public"."only_pk_columns" where "key_column_1" = ${fromWrite(compositeId.keyColumn1)(using new Write.Single(Meta.StringMeta.put))} AND "key_column_2" = ${fromWrite(compositeId.keyColumn2)(using new Write.Single(Meta.IntMeta.put))}""".update.run.map(_ > 0)
  }
  ...
  override def selectById(compositeId: OnlyPkColumnsId): ConnectionIO[Option[OnlyPkColumnsRow]] = {
    sql"""select "key_column_1", "key_column_2" from "public"."only_pk_columns" where "key_column_1" = ${fromWrite(compositeId.keyColumn1)(using new Write.Single(Meta.StringMeta.put))} AND "key_column_2" = ${fromWrite(compositeId.keyColumn2)(using new Write.Single(Meta.IntMeta.put))}""".query(using OnlyPkColumnsRow.read).option
  }
