...
class Issue1422RepoImpl extends Issue1422Repo {
  ...
  override def deleteById(tabellkode: Issue142Id): ConnectionIO[Boolean] = {
    sql"""delete from "public"."issue142_2" where "tabellkode" = ${fromWrite(tabellkode)(using new Write.Single(Issue142Id.put))}""".update.run.map(_ > 0)
  }
  ...
  override def selectById(tabellkode: Issue142Id): ConnectionIO[Option[Issue1422Row]] = {
    sql"""select "tabellkode" from "public"."issue142_2" where "tabellkode" = ${fromWrite(tabellkode)(using new Write.Single(Issue142Id.put))}""".query(using Issue1422Row.read).option
  }
