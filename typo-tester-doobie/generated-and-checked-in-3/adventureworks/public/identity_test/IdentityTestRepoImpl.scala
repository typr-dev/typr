...
class IdentityTestRepoImpl extends IdentityTestRepo {
  ...
  override def deleteById(name: IdentityTestId): ConnectionIO[Boolean] = {
    sql"""delete from "public"."identity-test" where "name" = ${fromWrite(name)(using new Write.Single(IdentityTestId.put))}""".update.run.map(_ > 0)
  }
  ...
  override def insert(unsaved: IdentityTestRowUnsaved): ConnectionIO[IdentityTestRow] = {
    val fs = List(
      ...
      unsaved.defaultGenerated match {
        ...
        case Defaulted.Provided(value) => Some((Fragment.const0(s""""default_generated""""), fr"${fromWrite(value: Int)(using new Write.Single(Meta.IntMeta.put))}::int4"))
      }
    ).flatten
  ...
  override def selectById(name: IdentityTestId): ConnectionIO[Option[IdentityTestRow]] = {
    sql"""select "always_generated", "default_generated", "name" from "public"."identity-test" where "name" = ${fromWrite(name)(using new Write.Single(IdentityTestId.put))}""".query(using IdentityTestRow.read).option
  }
  ...
  override def update(row: IdentityTestRow): ConnectionIO[Option[IdentityTestRow]] = {
    ...
    sql"""update "public"."identity-test"
          ...
          where "name" = ${fromWrite(name)(using new Write.Single(IdentityTestId.put))}
  ...
  override def upsert(unsaved: IdentityTestRow): ConnectionIO[IdentityTestRow] = {
    sql"""insert into "public"."identity-test"("default_generated", "name")
          values (
            ...
            ${fromWrite(unsaved.name)(using new Write.Single(IdentityTestId.put))}
          )
          on conflict ("name")
          do update set
