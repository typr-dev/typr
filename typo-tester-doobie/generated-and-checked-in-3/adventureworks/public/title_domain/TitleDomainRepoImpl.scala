...
class TitleDomainRepoImpl extends TitleDomainRepo {
  ...
  override def deleteById(code: TitleDomainId): ConnectionIO[Boolean] = {
    sql"""delete from "public"."title_domain" where "code" = ${fromWrite(code)(using new Write.Single(TitleDomainId.put))}""".update.run.map(_ > 0)
  }
  ...
  override def selectById(code: TitleDomainId): ConnectionIO[Option[TitleDomainRow]] = {
    sql"""select "code" from "public"."title_domain" where "code" = ${fromWrite(code)(using new Write.Single(TitleDomainId.put))}""".query(using TitleDomainRow.read).option
  }
