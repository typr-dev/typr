...
class PersonDetailSqlRepoImpl extends PersonDetailSqlRepo {
  override def apply(
    ...
    val sql =
      ...
      where s.businessentityid = ${fromWrite(businessentityid)(using new Write.Single(/* user-picked */ BusinessentityId.put))}::int4
        and p.modifieddate > ${fromWrite(modifiedAfter)(using new Write.Single(TypoLocalDateTime.put))}::timestamp"""
