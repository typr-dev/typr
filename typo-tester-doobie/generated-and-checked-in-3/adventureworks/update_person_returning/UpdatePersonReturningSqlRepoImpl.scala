  ...
  override def apply(suffix: /* nullability unknown */ Option[String], cutoff: /* nullability unknown */ Option[TypoLocalDateTime]): Stream[ConnectionIO, UpdatePersonReturningSqlRow] = {
    val sql =
      sql"""with row as (
              ...
              set firstname = firstname || '-' || ${fromWrite(suffix)(using new Write.SingleOpt(Meta.StringMeta.put))}
              where modifieddate < ${fromWrite(cutoff)(using new Write.SingleOpt(TypoLocalDateTime.put))}::timestamp
