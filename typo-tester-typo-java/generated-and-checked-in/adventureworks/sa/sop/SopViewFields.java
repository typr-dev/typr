...
public interface SopViewFields extends FieldsExpr<SopViewRow> {
  record Impl(List<Path> _path) implements SopViewFields, RelationStructure<SopViewFields, SopViewRow> {
    @Override
    ...
    public Field<UUID, SopViewRow> rowguid() {
      return new Field<UUID, SopViewRow>(_path, "rowguid", SopViewRow::rowguid, Optional.empty(), Optional.empty(), (row, value) -> row.withRowguid(value), PgTypes.uuid);
    };

    @Override
    public Field<LocalDateTime, SopViewRow> modifieddate() {
      return new Field<LocalDateTime, SopViewRow>(_path, "modifieddate", SopViewRow::modifieddate, Optional.empty(), Optional.empty(), (row, value) -> row.withModifieddate(value), PgTypes.timestamp);
    };

    @Override
    public List<FieldLike<?, SopViewRow>> columns() {
      return java.util.List.of(this.id(), this.specialofferid(), this.productid(), this.rowguid(), this.modifieddate());
    };
