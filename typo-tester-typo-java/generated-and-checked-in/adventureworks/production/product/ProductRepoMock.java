...
public record ProductRepoMock(
  ...
  public ProductRepoMock withToRow(
      java.util.function.Function<ProductRowUnsaved, ProductRow> toRow) {
  ...
  public SelectBuilder<ProductFields, ProductRow> select() {
    return new SelectBuilderMock<>(
        ProductFields.structure(), () -> new ArrayList<>(map.values()), SelectParams.empty());
  }
  ;
  ...
  public Map<ProductId, ProductRow> selectByIdsTracked(ProductId[] productids, Connection c) {
    return selectByIds(productids, c).stream()
        .collect(Collectors.toMap((ProductRow row) -> row.productid(), Function.identity()));
  }
  ;
