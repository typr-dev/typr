...
public interface SalestaxrateFields extends FieldsExpr<SalestaxrateRow> {
  record Impl(List<Path> _path) implements SalestaxrateFields, RelationStructure<SalestaxrateFields, SalestaxrateRow> {
    @Override
    ...
    public Field<Short, SalestaxrateRow> taxtype() {
      return new Field<Short, SalestaxrateRow>(_path, "taxtype", SalestaxrateRow::taxtype, Optional.empty(), Optional.of("int2"), (row, value) -> row.withTaxtype(value), PgTypes.int2);
    };
    ...
    public Field<UUID, SalestaxrateRow> rowguid() {
      return new Field<UUID, SalestaxrateRow>(_path, "rowguid", SalestaxrateRow::rowguid, Optional.empty(), Optional.of("uuid"), (row, value) -> row.withRowguid(value), PgTypes.uuid);
    };

    @Override
    public Field<LocalDateTime, SalestaxrateRow> modifieddate() {
      return new Field<LocalDateTime, SalestaxrateRow>(_path, "modifieddate", SalestaxrateRow::modifieddate, Optional.empty(), Optional.of("timestamp"), (row, value) -> row.withModifieddate(value), PgTypes.timestamp);
    };

    @Override
    public List<FieldLike<?, SalestaxrateRow>> columns() {
      return java.util.List.of(this.salestaxrateid(), this.stateprovinceid(), this.taxtype(), this.taxrate(), this.name(), this.rowguid(), this.modifieddate());
    };
  ...
  default ForeignKey<StateprovinceFields, StateprovinceRow> fkPersonStateprovince() {
    return ForeignKey.<StateprovinceFields, StateprovinceRow>of("sales.FK_SalesTaxRate_StateProvince_StateProvinceID").<StateprovinceId>withColumnPair(stateprovinceid(), StateprovinceFields::stateprovinceid);
  };
