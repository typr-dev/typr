...
case class TestInsert(random: Random) {
  ...
  )(using c: Connection): PersonRow = (new PersonRepoImpl).insert(new PersonRowUnsaved(name = name, one = one, two = two))

  def myschemaFootballClub(
    id: FootballClubId = FootballClubId(random.nextLong()),
    name: String = random.alphanumeric.take(20).mkString
  )(using c: Connection): FootballClubRow = (new FootballClubRepoImpl).insert(new FootballClubRow(id = id, name = name))

  def myschemaMaritalStatus(id: MaritalStatusId = MaritalStatusId(random.nextLong()))(using c: Connection): MaritalStatusRow = (new MaritalStatusRepoImpl).insert(new MaritalStatusRow(id = id))
