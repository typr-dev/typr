...
case class PersonRepoMock(
  ...
  override def deleteById(compositeId: PersonId): ConnectionIO[Boolean] = delay(map.remove(compositeId).isDefined)
  ...
  override def selectById(compositeId: PersonId): ConnectionIO[Option[PersonRow]] = delay(map.get(compositeId))
