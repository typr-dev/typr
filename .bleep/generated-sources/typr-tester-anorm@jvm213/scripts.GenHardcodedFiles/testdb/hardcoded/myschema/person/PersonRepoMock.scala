...
case class PersonRepoMock(
  ...
  override def deleteById(id: PersonId)(implicit c: Connection): Boolean = map.remove(id).isDefined

  override def deleteByIds(ids: Array[PersonId])(implicit c: Connection): Int = ids.map(id => map.remove(id)).count(_.isDefined)
  ...
  override def selectById(id: PersonId)(implicit c: Connection): Option[PersonRow] = map.get(id)

  override def selectByIds(ids: Array[PersonId])(implicit c: Connection): List[PersonRow] = ids.flatMap(map.get).toList
