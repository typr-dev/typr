...
case class MaritalStatusRepoMock(map: scala.collection.mutable.Map[MaritalStatusId, MaritalStatusRow] = scala.collection.mutable.Map.empty[MaritalStatusId, MaritalStatusRow]) extends MaritalStatusRepo {
  ...
  override def deleteById(id: MaritalStatusId)(implicit c: Connection): Boolean = map.remove(id).isDefined

  override def deleteByIds(ids: Array[MaritalStatusId])(implicit c: Connection): Int = ids.map(id => map.remove(id)).count(_.isDefined)
  ...
  override def selectAll(implicit c: Connection): List[MaritalStatusRow] = map.values.toList

  override def selectByFieldValues(fieldValues: List[MaritalStatusFieldValue[?]])(implicit c: Connection): List[MaritalStatusRow] = {
    fieldValues.foldLeft(map.values) {
      case (acc, MaritalStatusFieldValue.id(value)) => acc.filter(_.id == value)
    }.toList
  }

  override def selectById(id: MaritalStatusId)(implicit c: Connection): Option[MaritalStatusRow] = map.get(id)

  override def selectByIds(ids: Array[MaritalStatusId])(implicit c: Connection): List[MaritalStatusRow] = ids.flatMap(map.get).toList
