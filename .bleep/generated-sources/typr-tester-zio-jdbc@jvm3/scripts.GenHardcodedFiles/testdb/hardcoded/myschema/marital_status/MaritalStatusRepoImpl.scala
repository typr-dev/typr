...
class MaritalStatusRepoImpl extends MaritalStatusRepo {
  ...
  override def deleteById(id: MaritalStatusId): ZIO[ZConnection, Throwable, Boolean] = sql"""delete from "myschema"."marital_status" where "id" = ${Segment.paramSegment(id)(using MaritalStatusId.setter)}""".delete.map(_ > 0)

  override def deleteByIds(ids: Array[MaritalStatusId]): ZIO[ZConnection, Throwable, Long] = sql"""delete from "myschema"."marital_status" where "id" = ANY(${Segment.paramSegment(ids)(using MaritalStatusId.arraySetter)})""".delete
  ...
  override def insert(unsaved: MaritalStatusRow): ZIO[ZConnection, Throwable, MaritalStatusRow] = {
    ...
    values (${Segment.paramSegment(unsaved.id)(using MaritalStatusId.setter)}::int8)
    returning "id"
  ...
  override def insertStreaming(
    ...
    batchSize: Int = 10000
  ): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY "myschema"."marital_status"("id") FROM STDIN""", batchSize, unsaved)(using MaritalStatusRow.pgText)
  ...
  override def selectAll: ZStream[ZConnection, Throwable, MaritalStatusRow] = sql"""select "id" from "myschema"."marital_status"""".query(using MaritalStatusRow.jdbcDecoder).selectStream()
    ...
    fieldValues match {
      ...
      case nonEmpty =>
        val wheres = SqlFragment.empty.and(
          nonEmpty.map {
            case MaritalStatusFieldValue.id(value) => sql""""id" = ${Segment.paramSegment(value)(using MaritalStatusId.setter)}"""
          }
        )
  ...
  override def selectById(id: MaritalStatusId): ZIO[ZConnection, Throwable, Option[MaritalStatusRow]] = sql"""select "id" from "myschema"."marital_status" where "id" = ${Segment.paramSegment(id)(using MaritalStatusId.setter)}""".query(using MaritalStatusRow.jdbcDecoder).selectOne

  override def selectByIds(ids: Array[MaritalStatusId]): ZStream[ZConnection, Throwable, MaritalStatusRow] = sql"""select "id" from "myschema"."marital_status" where "id" = ANY(${Segment.paramSegment(ids)(using MaritalStatusId.arraySetter)})""".query(using MaritalStatusRow.jdbcDecoder).selectStream()
  ...
  override def updateFieldValues(
    ...
    NonEmptyChunk.fromIterableOption(fieldValues) match {
      ...
      case Some(nonEmpty) =>
        val updates = nonEmpty.map { case MaritalStatusFieldValue.id(value) => sql""""id" = ${Segment.paramSegment(value)(using MaritalStatusId.setter)}::int8""" }.mkFragment(SqlFragment(", "))
        sql"""update "myschema"."marital_status"
        set $updates
        where "id" = ${Segment.paramSegment(id)(using MaritalStatusId.setter)}
        """.update.map(_ > 0)
    }
  }
  ...
  override def upsert(unsaved: MaritalStatusRow): ZIO[ZConnection, Throwable, UpdateResult[MaritalStatusRow]] = {
    sql"""insert into "myschema"."marital_status"("id")
    values (
      ${Segment.paramSegment(unsaved.id)(using MaritalStatusId.setter)}::int8
    )
    on conflict ("id")
  ...
  override def upsertStreaming(
    ...
    val copied = streamingInsert(s"""copy marital_status_TEMP("id") from stdin""", batchSize, unsaved)(using MaritalStatusRow.pgText)
