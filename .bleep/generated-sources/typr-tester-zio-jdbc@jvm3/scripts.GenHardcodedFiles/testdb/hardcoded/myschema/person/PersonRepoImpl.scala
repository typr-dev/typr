...
class PersonRepoImpl extends PersonRepo {
  ...
  override def updateFieldValues(
    ...
    NonEmptyChunk.fromIterableOption(fieldValues) match {
      ...
      case Some(nonEmpty) =>
        val updates = nonEmpty.map { case PersonFieldValue.id(value) => sql""""id" = ${Segment.paramSegment(value)(using PersonId.setter)}::int8"""
                                     case PersonFieldValue.favouriteFootballClubId(value) => sql""""favourite_football_club_id" = ${Segment.paramSegment(value)(using FootballClubId.setter)}"""
                                     case PersonFieldValue.name(value) => sql""""name" = ${Segment.paramSegment(value)(using Setter.stringSetter)}"""
                                     case PersonFieldValue.nickName(value) => sql""""nick_name" = ${Segment.paramSegment(value)(using Setter.optionParamSetter(using Setter.stringSetter))}"""
                                     case PersonFieldValue.blogUrl(value) => sql""""blog_url" = ${Segment.paramSegment(value)(using Setter.optionParamSetter(using Setter.stringSetter))}"""
  ...
  override def upsertStreaming(
    ...
    val copied = streamingInsert(s"""copy person_TEMP("id", "favourite_football_club_id", "name", "nick_name", "blog_url", "email", "phone", "likes_pizza", "marital_status_id", "work_email", "favorite_number") from stdin""", batchSize, unsaved)(using PersonRow.pgText)
