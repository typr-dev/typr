...
case class PersonRepoMock(
  ...
  override def deleteById(id: PersonId): ZIO[ZConnection, Throwable, Boolean] = ZIO.succeed(map.remove(id).isDefined)

  override def deleteByIds(ids: Array[PersonId]): ZIO[ZConnection, Throwable, Long] = ZIO.succeed(ids.map(id => map.remove(id)).count(_.isDefined).toLong)
  ...
  override def selectById(id: PersonId): ZIO[ZConnection, Throwable, Option[PersonRow]] = ZIO.succeed(map.get(id))

  override def selectByIds(ids: Array[PersonId]): ZStream[ZConnection, Throwable, PersonRow] = ZStream.fromIterable(ids.flatMap(map.get))
