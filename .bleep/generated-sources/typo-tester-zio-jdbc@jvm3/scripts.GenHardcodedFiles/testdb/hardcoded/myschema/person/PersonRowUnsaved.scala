...
object PersonRowUnsaved {
  given jsonDecoder: JsonDecoder[PersonRowUnsaved] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
    ...
    val id = jsonObj.get("id").toRight("Missing field 'id'").flatMap(_.as(using Defaulted.jsonDecoder(using PersonId.jsonDecoder)))
    val maritalStatusId = jsonObj.get("marital_status_id").toRight("Missing field 'marital_status_id'").flatMap(_.as(using Defaulted.jsonDecoder(using MaritalStatusId.jsonDecoder)))
    val favoriteNumber = jsonObj.get("favorite_number").toRight("Missing field 'favorite_number'").flatMap(_.as(using Defaulted.jsonDecoder(using Number.jsonDecoder)))
  ...
  given jsonEncoder: JsonEncoder[PersonRowUnsaved] = new JsonEncoder[PersonRowUnsaved] {
    override def unsafeEncode(a: PersonRowUnsaved, indent: Option[Int], out: Write): Unit = {
      ...
      out.write(",")
      out.write(""""id":""")
      Defaulted.jsonEncoder(using PersonId.jsonEncoder).unsafeEncode(a.id, indent, out)
      out.write(",")
      out.write(""""marital_status_id":""")
      Defaulted.jsonEncoder(using MaritalStatusId.jsonEncoder).unsafeEncode(a.maritalStatusId, indent, out)
      out.write(",")
      out.write(""""favorite_number":""")
      Defaulted.jsonEncoder(using Number.jsonEncoder).unsafeEncode(a.favoriteNumber, indent, out)
      out.write("}")
    }
  }
