...
class MaritalStatusRepoImpl extends MaritalStatusRepo {
  ...
  override def deleteById(id: MaritalStatusId): ZIO[ZConnection, Throwable, Boolean] = {
    sql"""delete from "myschema"."marital_status" where "id" = ${Segment.paramSegment(id)(using MaritalStatusId.setter)}""".delete.map(_ > 0)
  }
  ...
  override def insertStreaming(unsaved: ZStream[ZConnection, Throwable, MaritalStatusRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    streamingInsert(s"""COPY "myschema"."marital_status"("id") FROM STDIN""", batchSize, unsaved)(using MaritalStatusRow.text)
  }
  ...
  override def selectById(id: MaritalStatusId): ZIO[ZConnection, Throwable, Option[MaritalStatusRow]] = {
    sql"""select "id" from "myschema"."marital_status" where "id" = ${Segment.paramSegment(id)(using MaritalStatusId.setter)}""".query(using MaritalStatusRow.jdbcDecoder).selectOne
  }
  override def selectByIds(ids: Array[MaritalStatusId]): ZStream[ZConnection, Throwable, MaritalStatusRow] = {
    sql"""select "id" from "myschema"."marital_status" where "id" = ANY(${Segment.paramSegment(ids)(using MaritalStatusId.arraySetter)})""".query(using MaritalStatusRow.jdbcDecoder).selectStream()
  }
  ...
  override def upsertStreaming(unsaved: ZStream[ZConnection, Throwable, MaritalStatusRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = {
    ...
    val copied = streamingInsert(s"""copy marital_status_TEMP("id") from stdin""", batchSize, unsaved)(using MaritalStatusRow.text)
