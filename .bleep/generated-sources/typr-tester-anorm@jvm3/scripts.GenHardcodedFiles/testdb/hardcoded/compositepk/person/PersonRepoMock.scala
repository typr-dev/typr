...
case class PersonRepoMock(
  ...
  override def deleteById(compositeId: PersonId)(using c: Connection): Boolean = map.remove(compositeId).isDefined
  ...
  override def selectById(compositeId: PersonId)(using c: Connection): Option[PersonRow] = map.get(compositeId)
