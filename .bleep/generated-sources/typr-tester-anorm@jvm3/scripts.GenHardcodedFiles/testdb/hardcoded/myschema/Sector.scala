...
object Sector {
  given arrayColumn: Column[Array[Sector]] = Column.columnToArray[String](using Column.columnToString, summon).map(_.map(Sector.force))
  ...
  given reads: Reads[Sector] = Reads[Sector]{(value: JsValue) => value.validate(using Reads.StringReads).flatMap(str => Sector(str).fold(JsError.apply, JsSuccess(_)))}
