...
case class FootballClubRepoMock(map: scala.collection.mutable.Map[FootballClubId, FootballClubRow] = scala.collection.mutable.Map.empty[FootballClubId, FootballClubRow]) extends FootballClubRepo {
  ...
  override def deleteById(id: FootballClubId)(using c: Connection): Boolean = map.remove(id).isDefined

  override def deleteByIds(ids: Array[FootballClubId])(using c: Connection): Int = ids.map(id => map.remove(id)).count(_.isDefined)
  ...
  override def selectAll(using c: Connection): List[FootballClubRow] = map.values.toList
  ...
  override def selectById(id: FootballClubId)(using c: Connection): Option[FootballClubRow] = map.get(id)

  override def selectByIds(ids: Array[FootballClubId])(using c: Connection): List[FootballClubRow] = ids.flatMap(map.get).toList
