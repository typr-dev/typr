...
class MaritalStatusRepoImpl extends MaritalStatusRepo {
    ...
    NonEmptyList.fromList(fieldValues) match {
      ...
      case Some(nonEmpty) =>
        val updates = fragments.set(
          nonEmpty.map {
            case MaritalStatusFieldValue.id(value) => fr""""id" = ${fromWrite(value)(new Write.Single(MaritalStatusId.put))}::int8"""
          }
        )
        sql"""update "myschema"."marital_status"
        $updates
        where "id" = ${fromWrite(id)(new Write.Single(MaritalStatusId.put))}""".update.run.map(_ > 0)
    }
  }
  ...
  override def upsertBatch(unsaved: List[MaritalStatusRow]): Stream[ConnectionIO, MaritalStatusRow] = {
    Update[MaritalStatusRow](
      ...
      do nothing
      returning "id""""
    )(MaritalStatusRow.write)
    .updateManyWithGeneratedKeys[MaritalStatusRow]("id")(unsaved)(catsStdInstancesForList, MaritalStatusRow.read)
  }
  ...
  override def upsertStreaming(
    ...
    for {
      ...
      _ <- new FragmentOps(sql"""copy marital_status_TEMP("id") from stdin""").copyIn(unsaved, batchSize)(MaritalStatusRow.pgText)
