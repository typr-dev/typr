...
public record PaymentsRepoMock(
  ...
  public PaymentsRepoMock withToRow(
      java.util.function.Function<PaymentsRowUnsaved, PaymentsRow> toRow) {
  ...
  public SelectBuilder<PaymentsFields, PaymentsRow> select() {
    return new SelectBuilderMock<>(
        PaymentsFields.structure(), () -> new ArrayList<>(map.values()), SelectParams.empty());
  }
  ;
  ...
  public Map<PaymentsId, PaymentsRow> selectByIdsTracked(PaymentsId[] paymentIds, Connection c) {
    return selectByIds(paymentIds, c).stream()
        .collect(Collectors.toMap((PaymentsRow row) -> row.paymentId(), Function.identity()));
  }
  ;
  ...
  public Boolean update(PaymentsRow row, Connection c) {
    var shouldUpdate =
        Optional.ofNullable(map.get(row.paymentId()))
