...
public sealed interface Fragment {
  ...
  /**
   * Creates an IN clause fragment: `column IN (?, ?, ...)` Dynamically expands based on the number
   * of values. Used primarily for MariaDB which doesn't support array parameters.
   *
  ...
  /**
   * Creates an IN clause fragment for use in WHERE conditions. Returns FALSE if the values array is
   * empty (standard SQL behavior for empty IN).
   */
  ...
  record CompositeInClause(String[] columns, List<Object[]> tuples, DbType<?>[] types)
      implements Fragment {
    @Override
    public void render(StringBuilder sb) {
      if (tuples.isEmpty()) {
