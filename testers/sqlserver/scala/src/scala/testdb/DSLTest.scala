...
/** Tests for the DSL query builder functionality. Tests type-safe query building with where, orderBy, limit, count, and projection.
  */
class DSLTest {
  private val testInsert = TestInsert(Random(1645377545))
  ...
  @Test
  def testProductDSLQuery(): Unit = withConnection {
  ...
  @Test
  def testOrdersDSLQuery(): Unit = withConnection {
  ...
  @Test
  def testComplexWhereClause(): Unit = withConnection {
