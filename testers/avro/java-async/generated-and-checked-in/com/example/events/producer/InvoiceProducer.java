...
public record InvoiceProducer(Producer<String, Invoice> producer, String topic)
  ...
  public CompletableFuture<RecordMetadata> send(String key, Invoice value) {
    return CompletableFuture.supplyAsync(
            () -> {
              CompletableFuture<RecordMetadata> future = new CompletableFuture<>();
              producer.send(
                  new ProducerRecord<String, Invoice>(topic, key, value),
                  (result, exception) -> {
                    if (exception != null) {
  ...
  public CompletableFuture<RecordMetadata> send(
      String key, Invoice value, StandardHeaders headers) {
    return CompletableFuture.supplyAsync(
            () -> {
              CompletableFuture<RecordMetadata> future = new CompletableFuture<>();
              producer.send(
