...
public record LinkedListNodeProducer(Producer<String, LinkedListNode> producer, String topic)
  ...
  public Future<RecordMetadata> send(String key, LinkedListNode value) {
    return producer.send(new ProducerRecord<String, LinkedListNode>(topic, key, value));
  }

  /** Send a message with headers to the topic */
  public Future<RecordMetadata> send(String key, LinkedListNode value, StandardHeaders headers) {
    return producer.send(
        new ProducerRecord<String, LinkedListNode>(topic, null, key, value, headers.toHeaders()));
  }
}
