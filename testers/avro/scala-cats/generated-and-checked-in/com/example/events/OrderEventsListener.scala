...
trait OrderEventsListener {
  ...
  def receive(record: ConsumerRecord[String, Any]): IO[Unit] = {
    return record.value match {
      ...
      case e: PaymentCallback => onPaymentCallback(e, record.headers)
      case e: PaymentCharged => onPaymentCharged(e, record.headers)
    }
  }
  ...
  /** Handle PaymentCallback event */
  def onPaymentCallback(
    event: PaymentCallback,
    metadata: Headers
  ): IO[Unit]

  /** Handle PaymentCharged event */
  def onPaymentCharged(
