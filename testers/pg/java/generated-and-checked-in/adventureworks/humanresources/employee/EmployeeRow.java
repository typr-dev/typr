...
public record EmployeeRow(
...
) implements Tuple15<BusinessentityId, String, String, String, LocalDate, String, String, LocalDate, Flag, Short, Short, Flag, UUID, LocalDateTime, Optional<String>> {
  /** Primary key for Employee records.  Foreign key to BusinessEntity.BusinessEntityID.
    * Points to {@link adventureworks.person.person.PersonRow#businessentityid()}
    */
  ...
  public static RowParser<EmployeeRow> _rowParser = RowParsers.of(BusinessentityId.dbType, PgTypes.text, PgTypes.text, PgTypes.text, PgTypes.date, PgTypes.bpchar, PgTypes.bpchar, PgTypes.date, Flag.dbType, PgTypes.int2, PgTypes.int2, Flag.dbType, PgTypes.uuid, PgTypes.timestamp, PgTypes.text.opt(), EmployeeRow::new, row -> new Object[]{row.businessentityid(), row.nationalidnumber(), row.loginid(), row.jobtitle(), row.birthdate(), row.maritalstatus(), row.gender(), row.hiredate(), row.salariedflag(), row.vacationhours(), row.sickleavehours(), row.currentflag(), row.rowguid(), row.modifieddate(), row.organizationnode()});;
