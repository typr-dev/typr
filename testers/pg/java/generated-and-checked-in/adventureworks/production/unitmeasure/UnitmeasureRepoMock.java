...
public record UnitmeasureRepoMock(
  ...
  public UnitmeasureRepoMock(
      java.util.function.Function<UnitmeasureRowUnsaved, UnitmeasureRow> toRow) {
    this(toRow, new HashMap<UnitmeasureId, UnitmeasureRow>());
  }
  ;

  public UnitmeasureRepoMock withToRow(
      java.util.function.Function<UnitmeasureRowUnsaved, UnitmeasureRow> toRow) {
  ...
  public UnitmeasureRow insert(UnitmeasureRow unsaved, Connection c) {
    if (map.containsKey(unsaved.unitmeasurecode())) {
      throw new RuntimeException("id " + unsaved.unitmeasurecode() + " already exists");
    }
    ;
  ...
  public Map<UnitmeasureId, UnitmeasureRow> selectByIdsTracked(
    ...
    return selectByIds(unitmeasurecodes, c).stream()
        .collect(
            Collectors.toMap((UnitmeasureRow row) -> row.unitmeasurecode(), Function.identity()));
  }
  ;
  ...
  public Boolean update(UnitmeasureRow row, Connection c) {
    var shouldUpdate =
        Optional.ofNullable(map.get(row.unitmeasurecode()))
  ...
  public Integer upsertStreaming(
      Iterator<UnitmeasureRow> unsaved, Integer batchSize, Connection c) {
    var count = 0;
    while (unsaved.hasNext()) {
      var row = unsaved.next();
      map.put(row.unitmeasurecode(), row);
