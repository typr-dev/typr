...
object EmployeeRow {
  given jdbcDecoder: JdbcDecoder[EmployeeRow] = {
      ...
      override def unsafeDecode(columIndex: Int, rs: ResultSet): (Int, EmployeeRow) =
        columIndex + 14 ->
          EmployeeRow(
            ...
            salariedflag = SalariedFlag.jdbcDecoder.unsafeDecode(columIndex + 8, rs)._2,
            ...
            currentflag = CurrentFlag.jdbcDecoder.unsafeDecode(columIndex + 11, rs)._2,
            ...
            organizationnode = JdbcDecoder.optionDecoder(using JdbcDecoder.stringDecoder).unsafeDecode(columIndex + 14, rs)._2
          )
    }
  }

  given jsonDecoder: JsonDecoder[EmployeeRow] = {
    JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
      ...
      val salariedflag = jsonObj.get("salariedflag").toRight("Missing field 'salariedflag'").flatMap(_.as(using SalariedFlag.jsonDecoder))
      val vacationhours = jsonObj.get("vacationhours").toRight("Missing field 'vacationhours'").flatMap(_.as(using TypoShort.jsonDecoder))
      val sickleavehours = jsonObj.get("sickleavehours").toRight("Missing field 'sickleavehours'").flatMap(_.as(using TypoShort.jsonDecoder))
      val currentflag = jsonObj.get("currentflag").toRight("Missing field 'currentflag'").flatMap(_.as(using CurrentFlag.jsonDecoder))
      ...
      val organizationnode = jsonObj.get("organizationnode").fold[Either[String, Option[String]]](Right(None))(_.as(using JsonDecoder.option(using JsonDecoder.string)))
  ...
  given jsonEncoder: JsonEncoder[EmployeeRow] = {
    new JsonEncoder[EmployeeRow] {
      override def unsafeEncode(a: EmployeeRow, indent: Option[Int], out: Write): Unit = {
        ...
        SalariedFlag.jsonEncoder.unsafeEncode(a.salariedflag, indent, out)
        out.write(",")
        ...
        CurrentFlag.jsonEncoder.unsafeEncode(a.currentflag, indent, out)
        out.write(",")
        out.write(""""rowguid":""")
  ...
  given pgText: Text[EmployeeRow] = {
    Text.instance[EmployeeRow]{ (row, sb) =>
      ...
      sb.append(Text.DELIMETER)
      Text.option(using Text.stringInstance).unsafeEncode(row.organizationnode, sb)
    }
  }
}