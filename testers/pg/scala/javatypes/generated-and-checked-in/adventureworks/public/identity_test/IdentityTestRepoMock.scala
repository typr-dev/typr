...
case class IdentityTestRepoMock(
  ...
  override def selectAll(using c: ConnectionRead): java.util.List[IdentityTestRow] = new ArrayList(map.values())

  override def selectById(name: IdentityTestId)(using c: ConnectionRead): Optional[IdentityTestRow] = Optional.ofNullable(map.get(name))
  ...
  override def selectByIdsTracked(names: java.util.List[IdentityTestId])(using c: ConnectionRead): java.util.Map[IdentityTestId, IdentityTestRow] = selectByIds(names)(using c).stream().collect(Collectors.toMap((row: IdentityTestRow) => row.name, Function.identity()))
