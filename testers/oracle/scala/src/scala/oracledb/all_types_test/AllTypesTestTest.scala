...
class AllTypesTestTest extends AnyFunSuite {
  ...
  def createTestStruct(prefix: String): AllTypesStructNoLobs = {
    val coords = new CoordinatesT(new java.math.BigDecimal("40.7128"), new java.math.BigDecimal("-74.0060"))
    ...
    new AllTypesStructNoLobs(
      ...
      new java.math.BigDecimal("1234567890"),
      new java.math.BigDecimal("1234567890123456789"),
      java.lang.Float.valueOf(1.5f),
  ...
  test("insert and select all types struct") {
    withConnection { c =>
      ...
      val unsaved = new AllTypesTestRowUnsaved(
        ...
        java.util.Optional.empty[AllTypesStructNoLobsArray](),
      ...
      val _ = assert(inserted.data.isPresent)
      ...
      val _ = assert(retrieved.numberField == new java.math.BigDecimal("12345.6789"))
  ...
  test("roundtrip all fields") {
    withConnection { c =>
      ...
      val insertedId = repo.insert(unsaved)
      val retrieved = repo.selectById(insertedId).orElseThrow()

      val data = retrieved.data.orElseThrow()
  ...
  test("insert with array") {
    withConnection { c =>
      ...
      val unsaved = new AllTypesTestRowUnsaved(
        "Array Test",
        java.util.Optional.of(struct1),
        java.util.Optional.of(dataArray),
        Defaulted.UseDefault[AllTypesTestId]()
      )
      ...
      val _ = assert(retrieved.dataArray.isPresent)
  ...
  test("update data") {
    withConnection { c =>
      ...
      val updatedRow = inserted.copy(data = java.util.Optional.of(updated))

      val wasUpdated = repo.update(updatedRow)
      val _ = assert(wasUpdated)

      val fetched = repo.selectById(insertedId).orElseThrow()
      val _ = assert(fetched.data.orElseThrow().varcharField == "Updated_varchar")
    }
  }
  ...
  test("delete") {
    withConnection { c =>
      ...
      val insertedId = repo.insert(unsaved)
      val _ = assert(repo.selectById(insertedId).isPresent)
