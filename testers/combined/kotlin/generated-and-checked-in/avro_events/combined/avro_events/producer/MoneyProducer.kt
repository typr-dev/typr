...
data class MoneyProducer(
  ...
  fun send(
    ...
    return Uni.createFrom().emitter({ em -> producer.send(ProducerRecord<kotlin.String, Money>(topic, key, value), { result, exception -> if (exception != null) {
      em.fail(exception)
    } else {
      em.complete(result)
    } }) })
  ...
  fun send(
    ...
    return Uni.createFrom().emitter({ em -> producer.send(ProducerRecord<kotlin.String, Money>(topic, null, key, value, headers.toHeaders()), { result, exception -> if (exception != null) {
      em.fail(exception)
    } else {
      em.complete(result)
    } }) })
