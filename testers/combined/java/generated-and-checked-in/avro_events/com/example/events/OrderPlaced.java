...
public record OrderPlaced(
  ...
  static public OrderPlaced fromGenericRecord(GenericRecord record) {
    return new OrderPlaced(UUID.fromString(record.get("orderId").toString()), ((Long) record.get("customerId")), Decimal10_2.unsafeForce(new BigDecimal(new BigInteger(((ByteBuffer) record.get("totalAmount")).array()), 2)), Instant.ofEpochMilli(((Long) record.get("placedAt"))), ((List<?>) record.get("items")).stream().map(e -> e.toString()).toList(), Optional.ofNullable((record.get("shippingAddress") == null
      ? null
