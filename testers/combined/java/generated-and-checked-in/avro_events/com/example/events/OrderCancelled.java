...
import combined.avro_events.precisetypes.Decimal10_2;
...
public record OrderCancelled(
  ...
  static public OrderCancelled fromGenericRecord(GenericRecord record) {
    return new OrderCancelled(UUID.fromString(record.get("orderId").toString()), ((Long) record.get("customerId")), Optional.ofNullable((record.get("reason") == null
      ? null
      : record.get("reason").toString())), Instant.ofEpochMilli(((Long) record.get("cancelledAt"))), Optional.ofNullable((record.get("refundAmount") == null
      ? null
      : Decimal10_2.unsafeForce(new BigDecimal(new BigInteger(((ByteBuffer) record.get("refundAmount")).array()), 2)))));
  }
  ...
  public GenericRecord toGenericRecord() {
    ...
    record.put("reason", (this.reason().isEmpty()
      ? null
      : this.reason().get()));
    record.put("cancelledAt", this.cancelledAt().toEpochMilli());
    record.put("refundAmount", (this.refundAmount().isEmpty()
      ? null
      : ByteBuffer.wrap(this.refundAmount().get().decimalValue().setScale(2, RoundingMode.HALF_UP).unscaledValue().toByteArray())));
    return record;
  }
}