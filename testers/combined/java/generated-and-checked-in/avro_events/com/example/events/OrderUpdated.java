...
public record OrderUpdated(
  ...
  static public OrderUpdated fromGenericRecord(GenericRecord record) {
    return new OrderUpdated(UUID.fromString(record.get("orderId").toString()), OrderStatus.valueOf(record.get("previousStatus").toString()), OrderStatus.valueOf(record.get("newStatus").toString()), Instant.ofEpochMilli(((Long) record.get("updatedAt"))), Optional.ofNullable((record.get("shippingAddress") == null
      ? null
  ...
  public GenericRecord toGenericRecord() {
    ...
    record.put("previousStatus", new EnumSymbol(OrderUpdated.SCHEMA.getField("previousStatus").schema(), this.previousStatus().name()));
    record.put("newStatus", new EnumSymbol(OrderUpdated.SCHEMA.getField("newStatus").schema(), this.newStatus().name()));
    record.put("updatedAt", this.updatedAt().toEpochMilli());
    record.put("shippingAddress", (this.shippingAddress().isEmpty()
      ? null
      : this.shippingAddress().get().toGenericRecord()));
    return record;
  }
}