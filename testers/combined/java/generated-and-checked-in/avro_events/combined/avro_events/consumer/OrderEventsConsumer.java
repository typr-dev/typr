...
public record OrderEventsConsumer(
  ...
  public CompletableFuture<Void> poll(Duration timeout) {
    ...
    return CompletableFuture.allOf(StreamSupport.stream(records.spliterator(), false).map(record -> switch (record.value()) {
      case OrderCancelled e -> handler.handleOrderCancelled(record.key(), e, StandardHeaders.fromHeaders(record.headers()));
      case OrderPlaced e -> handler.handleOrderPlaced(record.key(), e, StandardHeaders.fromHeaders(record.headers()));
      case OrderUpdated e -> handler.handleOrderUpdated(record.key(), e, StandardHeaders.fromHeaders(record.headers()));
      case PaymentCallback e -> handler.handlePaymentCallback(record.key(), e, StandardHeaders.fromHeaders(record.headers()));
      case PaymentCharged e -> handler.handlePaymentCharged(record.key(), e, StandardHeaders.fromHeaders(record.headers()));
      default -> handler.handleUnknown(record.key(), record.value(), StandardHeaders.fromHeaders(record.headers()));
