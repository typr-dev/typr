...
public record DynamicValueProducer(
  Producer<String, DynamicValue> producer,
  String topic
  ...
  public CompletableFuture<RecordMetadata> send(
    ...
    return CompletableFuture.supplyAsync(() -> {
      ...
      producer.send(new ProducerRecord<String, DynamicValue>(topic, key, value), (result, exception) -> {
      if (exception != null) {
  ...
  public CompletableFuture<RecordMetadata> send(
    ...
    return CompletableFuture.supplyAsync(() -> {
      CompletableFuture<RecordMetadata> future = new CompletableFuture<>();
      producer.send(new ProducerRecord<String, DynamicValue>(topic, null, key, value, headers.toHeaders()), (result, exception) -> {
      if (exception != null) {
      future.completeExceptionally(exception);
    } else {
      future.complete(result);
