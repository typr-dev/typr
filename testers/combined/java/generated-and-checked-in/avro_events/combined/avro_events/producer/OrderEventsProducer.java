...
public record OrderEventsProducer(
  ...
  public CompletableFuture<RecordMetadata> send(
    ...
    return CompletableFuture.supplyAsync(() -> {
      ...
      producer.send(new ProducerRecord<String, OrderEvents>(topic, key, value), (result, exception) -> {
      if (exception != null) {
  ...
  public CompletableFuture<RecordMetadata> send(
    ...
    return CompletableFuture.supplyAsync(() -> {
      CompletableFuture<RecordMetadata> future = new CompletableFuture<>();
      producer.send(new ProducerRecord<String, OrderEvents>(topic, null, key, value, headers.toHeaders()), (result, exception) -> {
      if (exception != null) {
      future.completeExceptionally(exception);
    } else {
      future.complete(result);
