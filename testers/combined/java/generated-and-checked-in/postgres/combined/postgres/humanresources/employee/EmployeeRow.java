...
public record EmployeeRow(
...
) implements Tuple15<BusinessentityId, String, String, String, LocalDate, String, String, LocalDate, /* user-picked */ SalariedFlag, Short, Short, /* user-picked */ CurrentFlag, UUID, LocalDateTime, Optional<String>> {
  /** Primary key for Employee records.  Foreign key to BusinessEntity.BusinessEntityID.
    * Points to {@link combined.postgres.person.person.PersonRow#businessentityid()}
    */
  ...
  public static RowCodec<EmployeeRow> rowCodec = RowCodecs.of(BusinessentityId.pgType, PgTypes.text, PgTypes.text, PgTypes.text, PgTypes.date, PgTypes.bpchar, PgTypes.bpchar, PgTypes.date, SalariedFlag.pgType, PgTypes.int2, PgTypes.int2, CurrentFlag.pgType, PgTypes.uuid, PgTypes.timestamp, PgTypes.text.opt(), EmployeeRow::new, row -> new Object[]{row.businessentityid(), row.nationalidnumber(), row.loginid(), row.jobtitle(), row.birthdate(), row.maritalstatus(), row.gender(), row.hiredate(), row.salariedflag(), row.vacationhours(), row.sickleavehours(), row.currentflag(), row.rowguid(), row.modifieddate(), row.organizationnode()});
