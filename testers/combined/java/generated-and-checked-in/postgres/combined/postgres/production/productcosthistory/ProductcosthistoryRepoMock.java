...
public record ProductcosthistoryRepoMock(
  ...
  public ProductcosthistoryRepoMock(java.util.function.Function<ProductcosthistoryRowUnsaved, ProductcosthistoryRow> toRow) {
    this(toRow, new HashMap<ProductcosthistoryId, ProductcosthistoryRow>());
  }

  public ProductcosthistoryRepoMock withToRow(java.util.function.Function<ProductcosthistoryRowUnsaved, ProductcosthistoryRow> toRow) {
    return new ProductcosthistoryRepoMock(toRow, map);
  }

  public ProductcosthistoryRepoMock withMap(HashMap<ProductcosthistoryId, ProductcosthistoryRow> map) {
    return new ProductcosthistoryRepoMock(toRow, map);
  }

  @Override
  public DeleteBuilder<ProductcosthistoryFields, ProductcosthistoryRow> delete() {
    return new DeleteBuilderMock<>(ProductcosthistoryFields.structure, () -> new ArrayList<>(map.values()), DeleteParams.empty(), row -> row.compositeId(), id -> map.remove(id));
  }
  ...
  public SelectBuilder<ProductcosthistoryFields, ProductcosthistoryRow> select() {
    return new SelectBuilderMock<>(ProductcosthistoryFields.structure, () -> new ArrayList<>(map.values()), SelectParams.empty());
  }
  ...
  public Map<ProductcosthistoryId, ProductcosthistoryRow> selectByIdsTracked(
    ...
    return selectByIds(compositeIds, c).stream().collect(Collectors.toMap((ProductcosthistoryRow row) -> row.compositeId(), Function.identity()));
  }

  @Override
  public UpdateBuilder<ProductcosthistoryFields, ProductcosthistoryRow> update() {
    return new UpdateBuilderMock<>(ProductcosthistoryFields.structure, () -> new ArrayList<>(map.values()), UpdateParams.empty(), row -> row);
  }
