...
public record CustomersRow(
  ...
  /** 
    * Default: 'pending'
    * Points to {@link combined.mariadb.customer_status.CustomerStatusRow#statusCode()}
    */
...
) implements Tuple14<CustomersId, /* user-picked */ Email, byte[], /* user-picked */ FirstName, /* user-picked */ LastName, Optional<String>, CustomerStatusId, String, Optional<Json>, Optional<EmailMailPushSmsSet>, Optional<String>, LocalDateTime, LocalDateTime, Optional<LocalDateTime>> {
  /** 
    * AUTO_INCREMENT
    */
  public CustomersRow withCustomerId(CustomersId customerId) {
    return new CustomersRow(customerId, email, passwordHash, firstName, lastName, phone, status, tier, preferences, marketingFlags, notes, createdAt, updatedAt, lastLoginAt);
  }
  ...
  public static RowCodec<CustomersRow> rowCodec = RowCodecs.of(CustomersId.mariaType, Email.mariaType, MariaTypes.binary, FirstName.mariaType, LastName.mariaType, MariaTypes.varchar.opt(), CustomerStatusId.mariaType, MariaTypes.text, MariaTypes.json.opt(), EmailMailPushSmsSet.mariaType.opt(), MariaTypes.text.opt(), MariaTypes.datetime, MariaTypes.datetime, MariaTypes.datetime.opt(), CustomersRow::new, row -> new Object[]{row.customerId(), row.email(), row.passwordHash(), row.firstName(), row.lastName(), row.phone(), row.status(), row.tier(), row.preferences(), row.marketingFlags(), row.notes(), row.createdAt(), row.updatedAt(), row.lastLoginAt()});
