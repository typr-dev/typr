/**
 * File has been automatically generated by `typo`.
 *
 * IF YOU CHANGE THIS FILE YOUR CHANGES WILL BE OVERWRITTEN.
 */
package combined.mariadb.product_images;

import com.fasterxml.jackson.annotation.JsonProperty;
import combined.mariadb.customtypes.Defaulted;
import combined.mariadb.products.ProductsId;
import combined.mariadb.userdefined.IsPrimary;
import dev.typr.dsl.RowCodecs;
import dev.typr.foundations.MariaTypes;
import dev.typr.foundations.RowCodec;
import dev.typr.foundations.Tuple.Tuple8;
import dev.typr.foundations.data.Uint1;
import java.util.Optional;

/** Table: product_images
  * Primary key: image_id
  */
public record ProductImagesRow(
  /** 
    * AUTO_INCREMENT
    */
  @JsonProperty("image_id") ProductImagesId imageId,
  /** 
    * Points to {@link combined.mariadb.products.ProductsRow#productId()}
    */
  @JsonProperty("product_id") ProductsId productId,
  /**  */
  @JsonProperty("image_url") String imageUrl,
  /** 
    * Default: NULL
    */
  @JsonProperty("thumbnail_url") Optional<String> thumbnailUrl,
  /** 
    * Default: NULL
    */
  @JsonProperty("alt_text") Optional<String> altText,
  /** 
    * Default: 0
    */
  @JsonProperty("sort_order") Uint1 sortOrder,
  /** 
    * Default: 0
    */
  @JsonProperty("is_primary") /* user-picked */ IsPrimary isPrimary,
  /** Optional embedded image data
    * Default: NULL
    */
  @JsonProperty("image_data") Optional<byte[]> imageData
) implements Tuple8<ProductImagesId, ProductsId, String, Optional<String>, Optional<String>, Uint1, /* user-picked */ IsPrimary, Optional<byte[]>> {
  /** 
    * AUTO_INCREMENT
    */
  public ProductImagesRow withImageId(ProductImagesId imageId) {
    return new ProductImagesRow(imageId, productId, imageUrl, thumbnailUrl, altText, sortOrder, isPrimary, imageData);
  }

  /** 
    * Points to {@link combined.mariadb.products.ProductsRow#productId()}
    */
  public ProductImagesRow withProductId(ProductsId productId) {
    return new ProductImagesRow(imageId, productId, imageUrl, thumbnailUrl, altText, sortOrder, isPrimary, imageData);
  }

  /**  */
  public ProductImagesRow withImageUrl(String imageUrl) {
    return new ProductImagesRow(imageId, productId, imageUrl, thumbnailUrl, altText, sortOrder, isPrimary, imageData);
  }

  /** 
    * Default: NULL
    */
  public ProductImagesRow withThumbnailUrl(Optional<String> thumbnailUrl) {
    return new ProductImagesRow(imageId, productId, imageUrl, thumbnailUrl, altText, sortOrder, isPrimary, imageData);
  }

  /** 
    * Default: NULL
    */
  public ProductImagesRow withAltText(Optional<String> altText) {
    return new ProductImagesRow(imageId, productId, imageUrl, thumbnailUrl, altText, sortOrder, isPrimary, imageData);
  }

  /** 
    * Default: 0
    */
  public ProductImagesRow withSortOrder(Uint1 sortOrder) {
    return new ProductImagesRow(imageId, productId, imageUrl, thumbnailUrl, altText, sortOrder, isPrimary, imageData);
  }

  /** 
    * Default: 0
    */
  public ProductImagesRow withIsPrimary(/* user-picked */ IsPrimary isPrimary) {
    return new ProductImagesRow(imageId, productId, imageUrl, thumbnailUrl, altText, sortOrder, isPrimary, imageData);
  }

  /** Optional embedded image data
    * Default: NULL
    */
  public ProductImagesRow withImageData(Optional<byte[]> imageData) {
    return new ProductImagesRow(imageId, productId, imageUrl, thumbnailUrl, altText, sortOrder, isPrimary, imageData);
  }

  public static RowCodec<ProductImagesRow> rowCodec = RowCodecs.of(ProductImagesId.mariaType, ProductsId.mariaType, MariaTypes.varchar, MariaTypes.varchar.opt(), MariaTypes.varchar.opt(), MariaTypes.tinyintUnsigned, IsPrimary.mariaType, MariaTypes.longblob.opt(), ProductImagesRow::new, row -> new Object[]{row.imageId(), row.productId(), row.imageUrl(), row.thumbnailUrl(), row.altText(), row.sortOrder(), row.isPrimary(), row.imageData()});

  @Override
  public ProductImagesId _1() {
    return imageId;
  }

  @Override
  public ProductsId _2() {
    return productId;
  }

  @Override
  public String _3() {
    return imageUrl;
  }

  @Override
  public Optional<String> _4() {
    return thumbnailUrl;
  }

  @Override
  public Optional<String> _5() {
    return altText;
  }

  @Override
  public Uint1 _6() {
    return sortOrder;
  }

  @Override
  public /* user-picked */ IsPrimary _7() {
    return isPrimary;
  }

  @Override
  public Optional<byte[]> _8() {
    return imageData;
  }

  public ProductImagesId id() {
    return imageId;
  }

  public ProductImagesRowUnsaved toUnsavedRow(
    Defaulted<Optional<String>> thumbnailUrl,
    Defaulted<Optional<String>> altText,
    Defaulted<Uint1> sortOrder,
    Defaulted</* user-picked */ IsPrimary> isPrimary,
    Defaulted<Optional<byte[]>> imageData
  ) {
    return new ProductImagesRowUnsaved(productId, imageUrl, thumbnailUrl, altText, sortOrder, isPrimary, imageData);
  }
}