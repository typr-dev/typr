...
public record OrderHistoryRow(
...
) implements Tuple8<OrderHistoryId, OrdersId, Optional<String>, String, Optional<String>, Optional<String>, Optional<Json>, LocalDateTime> {
  /** 
    * AUTO_INCREMENT
    */
  public OrderHistoryRow withHistoryId(OrderHistoryId historyId) {
    return new OrderHistoryRow(historyId, orderId, previousStatus, newStatus, changedBy, changeReason, metadata, createdAt);
  }
  ...
  public static RowCodec<OrderHistoryRow> rowCodec = RowCodecs.of(OrderHistoryId.mariaType, OrdersId.mariaType, MariaTypes.text.opt(), MariaTypes.text, MariaTypes.varchar.opt(), MariaTypes.varchar.opt(), MariaTypes.json.opt(), MariaTypes.datetime, OrderHistoryRow::new, row -> new Object[]{row.historyId(), row.orderId(), row.previousStatus(), row.newStatus(), row.changedBy(), row.changeReason(), row.metadata(), row.createdAt()});
