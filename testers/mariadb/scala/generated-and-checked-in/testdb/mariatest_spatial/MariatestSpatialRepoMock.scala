...
case class MariatestSpatialRepoMock(
  ...
  override def selectAll(using c: ConnectionRead): List[MariatestSpatialRow] = map.values.toList

  override def selectById(id: MariatestSpatialId)(using c: ConnectionRead): Option[MariatestSpatialRow] = map.get(id)

  override def selectByIds(ids: List[MariatestSpatialId])(using c: ConnectionRead): List[MariatestSpatialRow] = ids.flatMap(map.get(_)).toList

  override def selectByIdsTracked(ids: List[MariatestSpatialId])(using c: ConnectionRead): Map[MariatestSpatialId, MariatestSpatialRow] = selectByIds(ids)(using c).map(x => (((row: MariatestSpatialRow) => row.id).apply(x), x)).toMap
