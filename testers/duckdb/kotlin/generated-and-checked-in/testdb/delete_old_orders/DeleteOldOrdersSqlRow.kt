/**
 * File has been automatically generated by `typo`.
 *
 * IF YOU CHANGE THIS FILE YOUR CHANGES WILL BE OVERWRITTEN.
 */
package testdb.delete_old_orders

import com.fasterxml.jackson.annotation.JsonProperty
import dev.typr.dslkt.RowCodecs
import dev.typr.foundations.Tuple.Tuple5
import dev.typr.foundationskt.DuckDbTypes
import dev.typr.foundationskt.RowCodec
import java.math.BigDecimal
import java.time.LocalDate
import testdb.orders.OrdersId

/** SQL file: delete_old_orders.sql */
data class DeleteOldOrdersSqlRow(
  /** Points to [testdb.orders.OrdersRow.orderId] */
  @field:JsonProperty("order_id") val orderId: OrdersId,
  /** Points to [testdb.orders.OrdersRow.customerId] */
  @field:JsonProperty("customer_id") val customerId: Int,
  /** Points to [testdb.orders.OrdersRow.orderDate] */
  @field:JsonProperty("order_date") val orderDate: LocalDate,
  /** Points to [testdb.orders.OrdersRow.totalAmount] */
  @field:JsonProperty("total_amount") val totalAmount: BigDecimal?,
  /** Points to [testdb.orders.OrdersRow.status] */
  val status: kotlin.String?
) : Tuple5<OrdersId, Int, LocalDate, BigDecimal?, kotlin.String?> {
  override fun _1(): OrdersId = orderId

  override fun _2(): Int = customerId

  override fun _3(): LocalDate = orderDate

  override fun _4(): BigDecimal? = totalAmount

  override fun _5(): kotlin.String? = status

  companion object {
    val rowCodec: RowCodec<DeleteOldOrdersSqlRow> = RowCodecs.of(OrdersId.duckDbType, DuckDbTypes.integer, DuckDbTypes.date, DuckDbTypes.numeric.opt(), DuckDbTypes.varchar.opt(), { t0: OrdersId, t1: Int, t2: LocalDate, t3: BigDecimal?, t4: kotlin.String? -> DeleteOldOrdersSqlRow(t0, t1, t2, t3, t4) }, { row: DeleteOldOrdersSqlRow -> arrayOf<Any?>(row.orderId, row.customerId, row.orderDate, row.totalAmount, row.status) })
  }
}