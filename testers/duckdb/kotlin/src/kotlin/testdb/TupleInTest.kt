...
/**
 * Comprehensive tests for tuple IN functionality on DuckDB. Tests cover:
...
class TupleInTest {
        ...
        val result = repos.departmentsRepo
            ...
            .where { d ->
                SqlExpr.all(
                    ...
                    d.budget().greaterThan(BigDecimal("200000"))
                )
            }
            .toList(c)
    ...
    fun tupleInSubqueryBasic(repos: Repos, c: Connection) {
        val row1 = DepartmentsRow(
            "SMALL1", "REG1", "Small Dept 1",
            BigDecimal("10000"))
        val row2 = DepartmentsRow(
            "SMALL2", "REG2", "Small Dept 2",
            BigDecimal("20000"))
        val row3 = DepartmentsRow(
            "LARGE", "REG3", "Large Dept",
            BigDecimal("1000000"))
                ...
                d.deptCode()
                    ...
                    .among(
                        repos.departmentsRepo
                            .select()
                            .where { inner -> inner.budget().lessThan(BigDecimal("100000")) }
    ...
    fun tupleInSubqueryWithNoMatches(repos: Repos, c: Connection) {
        val row = DepartmentsRow("TEST1", "REGION1", "Test Dept 1", BigDecimal("50000"))
        repos.departmentsRepo.insert(row, c)
    ...
    fun tupleInSubqueryCombinedWithOtherConditions(repos: Repos, c: Connection) {
        val row1 = DepartmentsRow("A", "X", "Dept A", BigDecimal("1000"))
        val row2 = DepartmentsRow("B", "X", "Dept B", BigDecimal("2000"))
        val row3 = DepartmentsRow("C", "X", "Dept C", BigDecimal("3000"))
    ...
    fun tupleInWithNullableColumn(repos: Repos, c: Connection) {
        ...
        val row3 = DepartmentsRow("BUDGET", "REG3", "Dept With Budget", BigDecimal("500000"))
        ...
        val result = repos.departmentsRepo
            ...
            .where { d ->
                d.deptCode()
                    .tupleWith(d.budget())
                    .among(listOf(
                        dev.typr.foundations.Tuple.of("NULL1", null as BigDecimal?),
                        dev.typr.foundations.Tuple.of("NULL2", null as BigDecimal?)
                    ))
            }
            .toList(c)
    ...
    fun readNestedTupleFromDatabase(repos: Repos, c: Connection) {
        // Insert test data
        val row1 = DepartmentsRow("READ1", "REG1", "Read Dept 1", BigDecimal("100000"))
        val row2 = DepartmentsRow("READ2", "REG2", "Read Dept 2", BigDecimal("200000"))
        val row3 = DepartmentsRow("READ3", "REG3", "Read Dept 3", BigDecimal("300000"))
