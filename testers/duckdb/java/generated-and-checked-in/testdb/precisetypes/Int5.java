...
public record Int5(@JsonValue BigInteger value) implements DecimalN {
  ...
  static public DuckDbType<Int5> duckDbType =
    DuckDbTypes.numeric.to(Bijection.of(bd -> new Int5(bd.toBigIntegerExact()), v -> new BigDecimal(v.value())));

  static public DuckDbType<List<Int5>> dbTypeArray =
    DuckDbTypes.numeric.list().to(Bijection.of(xs -> xs.stream().map(bd -> new Int5(bd.toBigIntegerExact())).toList(), xs -> xs.stream().map(v -> new BigDecimal(v.value())).toList()));
