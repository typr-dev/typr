...
public record OrdersRow(
  ...
  public static RowCodec<OrdersRow> rowCodec = RowCodecs.of(OrdersId.duckDbType, DuckDbTypes.integer, DuckDbTypes.date, DuckDbTypes.numeric.opt(), DuckDbTypes.varchar.opt(), OrdersRow::new, row -> new Object[]{row.orderId(), row.customerId(), row.orderDate(), row.totalAmount(), row.status()});
