...
class OrderServiceServer(val delegate: OrderService) extends BindableService {
  override def bindService: ServerServiceDefinition = {
    return ServerServiceDefinition.builder("testgrpc.OrderService").addMethod(OrderServiceServer.GET_CUSTOMER, ServerCalls.asyncUnaryCall((request, responseObserver) => { { val `_result` = delegate.getCustomer(request).unsafeRunSync()(cats.effect.unsafe.IORuntime.global); ((response: GetCustomerResponse) => { responseObserver.onNext(response); responseObserver.onCompleted() })(`_result`) } })).addMethod(OrderServiceServer.CREATE_ORDER, ServerCalls.asyncUnaryCall((request, responseObserver) => { { val `_result` = delegate.createOrder(request).unsafeRunSync()(cats.effect.unsafe.IORuntime.global); ((response: CreateOrderResponse) => { responseObserver.onNext(response); responseObserver.onCompleted() })(`_result`) } })).addMethod(OrderServiceServer.LIST_ORDERS, ServerCalls.asyncServerStreamingCall((request, responseObserver) => { val results = delegate.listOrders(request); while (results.hasNext()) {
      responseObserver.onNext(results.next())
    }; responseObserver.onCompleted() })).addMethod(OrderServiceServer.SUBMIT_ORDERS, ServerCalls.asyncClientStreamingCall(responseObserver => { throw new UnsupportedOperationException("Client streaming not yet implemented in server adapter") })).addMethod(OrderServiceServer.CHAT, ServerCalls.asyncBidiStreamingCall(responseObserver => { throw new UnsupportedOperationException("Bidi streaming not yet implemented in server adapter") })).build()
  }
}
