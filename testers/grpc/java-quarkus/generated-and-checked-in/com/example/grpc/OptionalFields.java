...
public record OptionalFields(
  ...
  static public OptionalFields parseFrom(CodedInputStream input) throws IOException {
    Optional<String> name = Optional.empty();
    Optional<Integer> age = Optional.empty();
    Optional<Customer> customer = Optional.empty();
    while (!input.isAtEnd()) {
      var tag = input.readTag();
      if (WireFormat.getTagFieldNumber(tag) == 1) { name = Optional.of(input.readString()); }
      else if (WireFormat.getTagFieldNumber(tag) == 2) { age = Optional.of(input.readInt32()); }
  ...
  static public Marshaller<OptionalFields> MARSHALLER =
