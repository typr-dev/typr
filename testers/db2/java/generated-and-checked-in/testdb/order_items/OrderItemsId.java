/**
 * File has been automatically generated by `typo`.
 *
 * IF YOU CHANGE THIS FILE YOUR CHANGES WILL BE OVERWRITTEN.
 */
package testdb.order_items;

import com.fasterxml.jackson.annotation.JsonProperty;
import dev.typr.dsl.RowCodecs;
import dev.typr.foundations.Db2Types;
import dev.typr.foundations.RowCodec;
import dev.typr.foundations.Tuple.Tuple2;
import testdb.orders.OrdersId;

/** Type for the composite primary key of table `ORDER_ITEMS` */
public record OrderItemsId(
  @JsonProperty("ORDER_ID") OrdersId orderId,
  @JsonProperty("ITEM_NUMBER") Integer itemNumber
) implements Tuple2<OrdersId, Integer> {
  public OrderItemsId withOrderId(OrdersId orderId) {
    return new OrderItemsId(orderId, itemNumber);
  }

  public OrderItemsId withItemNumber(Integer itemNumber) {
    return new OrderItemsId(orderId, itemNumber);
  }

  public static RowCodec<OrderItemsId> rowCodec = RowCodecs.of(OrdersId.db2Type, Db2Types.integer, OrderItemsId::new, row -> new Object[]{row.orderId(), row.itemNumber()});

  @Override
  public OrdersId _1() {
    return orderId;
  }

  @Override
  public Integer _2() {
    return itemNumber;
  }
}