...
class OrdersRepoImpl extends OrdersRepo {
  override def delete: DeleteBuilder[OrdersFields, OrdersRow] = DeleteBuilder.of(""""orders"""", OrdersFields.structure, Dialect.SQLITE)

  override def deleteById(orderId: OrdersId)(using c: Connection): Boolean = sql"""delete from "orders" where "order_id" = ${Fragment.encode(OrdersId.sqliteType, orderId)}""".update().run(using c) > 0

  override def deleteByIds(orderIds: List[OrdersId])(using c: Connection): Int = {
    val fragments: ListBuffer[Fragment] = ListBuffer()
    orderIds.foreach { id => fragments.addOne(Fragment.encode(OrdersId.sqliteType, id)): @scala.annotation.nowarn }
  ...
  sql"""insert into "orders"("order_id", "customer_id", "order_date", "total_amount", "status")
    values (${Fragment.encode(OrdersId.sqliteType, unsaved.orderId)}, ${Fragment.encode(CustomersId.sqliteType, unsaved.customerId)}, ${Fragment.encode(SqliteTypes.date, unsaved.orderDate)}, ${Fragment.encode(SqliteTypes.numeric.opt, unsaved.totalAmount)}, ${Fragment.encode(SqliteTypes.text, unsaved.status)})
  ...
  override def insert(unsaved: OrdersRowUnsaved)(using c: Connection): OrdersRow = {
    ...
    columns.addOne(Fragment.of(""""order_date"""")): @scala.annotation.nowarn
    values.addOne(sql"${Fragment.encode(SqliteTypes.date, unsaved.orderDate)}"): @scala.annotation.nowarn
    columns.addOne(Fragment.of(""""total_amount"""")): @scala.annotation.nowarn
    values.addOne(sql"${Fragment.encode(SqliteTypes.numeric.opt, unsaved.totalAmount)}"): @scala.annotation.nowarn
    unsaved.status.visit(
      {  },
      value => { columns.addOne(Fragment.of(""""status"""")): @scala.annotation.nowarn; values.addOne(sql"${Fragment.encode(SqliteTypes.text, value)}"): @scala.annotation.nowarn }
    );
    val q: Fragment = {
  ...
  override def select: SelectBuilder[OrdersFields, OrdersRow] = SelectBuilder.of(""""orders"""", OrdersFields.structure, OrdersRow.rowCodec, Dialect.SQLITE)
  ...
  override def selectById(orderId: OrdersId)(using c: ConnectionRead): Option[OrdersRow] = {
    sql"""select "order_id", "customer_id", "order_date", "total_amount", "status"
    from "orders"
    where "order_id" = ${Fragment.encode(OrdersId.sqliteType, orderId)}""".query(OrdersRow.rowCodec.first()).run(using c)
  }
  ...
  override def update: UpdateBuilder[OrdersFields, OrdersRow] = UpdateBuilder.of(""""orders"""", OrdersFields.structure, OrdersRow.rowCodec, Dialect.SQLITE)

  override def update(row: OrdersRow)(using c: Connection): Boolean = {
    val orderId: OrdersId = row.orderId
    return sql"""update "orders"
    set "customer_id" = ${Fragment.encode(CustomersId.sqliteType, row.customerId)},
    "order_date" = ${Fragment.encode(SqliteTypes.date, row.orderDate)},
    "total_amount" = ${Fragment.encode(SqliteTypes.numeric.opt, row.totalAmount)},
    "status" = ${Fragment.encode(SqliteTypes.text, row.status)}
    where "order_id" = ${Fragment.encode(OrdersId.sqliteType, orderId)}""".update().run(using c) > 0
  }
