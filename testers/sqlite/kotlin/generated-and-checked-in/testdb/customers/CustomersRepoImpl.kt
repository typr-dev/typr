...
class CustomersRepoImpl() : CustomersRepo {
  override fun delete(): DeleteBuilder<CustomersFields, CustomersRow> = DeleteBuilder.of("\"customers\"", CustomersFields.structure, Dialect.SQLITE)

  override fun deleteById(
    customerId: CustomersId,
    c: Connection
  ): kotlin.Boolean = Fragment.concat(Fragment.of("delete from \"customers\" where \"customer_id\" = "), Fragment.encode(CustomersId.sqliteType, customerId), Fragment.of("")).update().run(c) > 0
  ...
  override fun deleteByIds(
    ...
    for (id in customerIds) { fragments.add(Fragment.encode(CustomersId.sqliteType, id)) }
  ...
  override fun insert(
    ...
    values.add(Fragment.concat(Fragment.encode(SqliteTypes.text, unsaved.name), Fragment.of("")))
    columns.add(Fragment.of("\"email\""))
    values.add(Fragment.concat(Fragment.encode(Email.sqliteType.opt(), unsaved.email), Fragment.of("")))
    unsaved.createdAt.visit(
      {  },
      { value -> columns.add(Fragment.of("\"created_at\""))
      values.add(Fragment.concat(Fragment.encode(SqliteTypes.timestamp, value), Fragment.of(""))) }
    );
  ...
  override fun select(): SelectBuilder<CustomersFields, CustomersRow> = SelectBuilder.of("\"customers\"", CustomersFields.structure, CustomersRow.rowCodec, Dialect.SQLITE)
  ...
  override fun update(): UpdateBuilder<CustomersFields, CustomersRow> = UpdateBuilder.of("\"customers\"", CustomersFields.structure, CustomersRow.rowCodec, Dialect.SQLITE)
  ...
  override fun upsertBatch(
    ...
    c: Connection
  ): List<CustomersRow> = Fragment.concat(Fragment.of("INSERT INTO \"customers\"(\"customer_id\", \"name\", \"email\", \"created_at\")\nVALUES (?, ?, ?, ?)\nON CONFLICT (\"customer_id\")\nDO UPDATE SET\n  \"name\" = EXCLUDED.\"name\",\n\"email\" = EXCLUDED.\"email\",\n\"created_at\" = EXCLUDED.\"created_at\"\nRETURNING \"customer_id\", \"name\", \"email\", \"created_at\""))
