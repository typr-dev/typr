...
trait PetsApiServer extends PetsApi {
  ...
  def createPetEndpoint(body: PetCreate): IO[Response[IO]] = {
    createPet(body).flatMap((response: testapi.api.Response201400[?, ?]) =>
  ...
  def deletePetEndpoint(
    ...
    deletePet(petId).flatMap((response: testapi.api.Response404Default[?]) =>
      (response: @unchecked) match {
  ...
  def getPetEndpoint(
    ...
    getPet(petId).flatMap((response: testapi.api.Response200404[?, ?]) =>
      (response: @unchecked) match {
