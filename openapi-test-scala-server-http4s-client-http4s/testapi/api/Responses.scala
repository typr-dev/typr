...
case class Ok[+T](value: T) extends Response200404[T, Nothing] with Response2004XX5XX[T] {
  ...
  /** Convert this response to an HTTP4s Response */
  def toResponse(using encoder: EntityEncoder[IO, T @uncheckedVariance]): IO[Response[IO]] = cats.effect.IO.pure(org.http4s.Response.apply(org.http4s.Status.Ok).withEntity(value))
}
