...
public record SortOrder<T>(SqlExpr<T> expr, boolean ascending, boolean nullsFirst) {
  ...
  public Fragment render(RenderCtx ctx, AtomicInteger counter) {
    ...
    result = result.append(Fragment.of(" ")).append(Fragment.of(ascending ? "ASC" : "DESC"));
    // Only add NULLS FIRST/LAST if the dialect supports it
    if (ctx.dialect().supportsNullsFirstLast()) {
      if (nullsFirst) {
        result = result.append(Fragment.of(" NULLS FIRST"));
      } else if (!ascending) {
        ...
        result = result.append(Fragment.of(" NULLS LAST"));
      }
    }
    return result;
  }
