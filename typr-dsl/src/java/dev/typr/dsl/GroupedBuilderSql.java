...
public class GroupedBuilderSql<Fields, Row> implements GroupedBuilder<Fields, Row> {
  ...
  static class GroupedSelectBuilderSql<
    ...
    public Query<NewFields, NewRow> collectQuery(RenderCtx ctx, AtomicInteger counter) {
      ...
      RowCodec<NewRow> rowCodec = buildGroupedRowCodec(projectedExprs);
      ...
      return new Query<>(
          groupedTable, List.of(), SelectParams.empty(), projectedFields, startCol -> rowCodec);
    }

    @SuppressWarnings("unchecked")
    private RowCodec<NewRow> buildGroupedRowCodec(List<SqlExpr<?>> projectedExprs) {
      // Delegate to the buildRowCodec that returns a proper RowCodec
      return (RowCodec<NewRow>) buildRowCodec(projectedExprs);
    }
    ...
    protected Tuple.Tuple2<Fragment, RowCodec<NewRow>> getSqlAndRowCodec() {
      ...
      Fragment selectClause = Fragment.of("select ").append(Fragment.comma(selectFragments));
      ...
      if (!havingPredicates.isEmpty()) {
        ...
        for (int i = 1; i < havingFragments.size(); i++) {
          havingCombined =
              havingCombined.append(Fragment.of(" AND ")).append(havingFragments.get(i));
        }
        havingClause = Fragment.of("\nhaving ").append(havingCombined);
      }
      ...
      // Build row parser
      RowCodec<NewRow> rowCodec = buildRowCodec(projectedExprs);
    ...
    private Fragment renderGroupedTableRefFull(
      ...
      // FROM clause
      Fragment from =
          Fragment.of(" FROM ").append(renderTableRef(sourceQuery.firstTable(), dialect, counter));
    ...
    @SuppressWarnings("unchecked")
    private RowCodec<NewRow> buildRowCodec(List<SqlExpr<?>> projectedExprs) {
  ...
  public record SyntheticField<T>(
    ...
    @Override
    public dev.typr.foundations.Either<String, Object> set(Object row, Optional<T> value) {
      // Not used for grouped results
      return dev.typr.foundations.Either.left("SyntheticField.set is not supported");
    }
    ...
    public Fragment render(RenderCtx ctx, AtomicInteger counter) {
      return Fragment.of(tableAlias + "." + column);
    }
