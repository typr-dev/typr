...
public sealed interface AnimalsApiClient extends AnimalsApi {
  ...
  default ListAnimalsResponse listAnimals() {
    try {
        ...
        if (response.getStatus() == 200) { return new Status200(response.readEntity(List<Animal>.class)); }
        ...
        if (e.getResponse().getStatus() == 200) { return new Status200(e.getResponse().readEntity(List<Animal>.class)); }
