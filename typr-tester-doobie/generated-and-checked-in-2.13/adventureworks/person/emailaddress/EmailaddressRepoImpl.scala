...
class EmailaddressRepoImpl extends EmailaddressRepo {
  ...
  override def deleteByIds(compositeIds: Array[EmailaddressId]): ConnectionIO[Int] = {
    ...
    in (select unnest(${fromWrite(businessentityid)(new Write.Single(BusinessentityId.arrayPut))}), unnest(${fromWrite(emailaddressid)(new Write.Single(adventureworks.IntegerArrayMeta.put))}))
    """.update.run
  }
  ...
  override def insertStreaming(
    ...
    batchSize: Int = 10000
  ): ConnectionIO[Long] = new FragmentOps(sql"""COPY "person"."emailaddress"("businessentityid", "emailaddressid", "emailaddress", "rowguid", "modifieddate") FROM STDIN""").copyIn(unsaved, batchSize)(EmailaddressRow.pgText)

  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(
    unsaved: Stream[ConnectionIO, EmailaddressRowUnsaved],
    batchSize: Int = 10000
  ): ConnectionIO[Long] = new FragmentOps(sql"""COPY "person"."emailaddress"("businessentityid", "emailaddress", "emailaddressid", "rowguid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""").copyIn(unsaved, batchSize)(EmailaddressRowUnsaved.pgText)
  ...
  override def selectAll: Stream[ConnectionIO, EmailaddressRow] = sql"""select "businessentityid", "emailaddressid", "emailaddress", "rowguid", "modifieddate"::text from "person"."emailaddress"""".query(EmailaddressRow.read).stream
  ...
  override def selectByIds(compositeIds: Array[EmailaddressId]): Stream[ConnectionIO, EmailaddressRow] = {
    ...
    where ("businessentityid", "emailaddressid")
    in (select unnest(${fromWrite(businessentityid)(new Write.Single(BusinessentityId.arrayPut))}), unnest(${fromWrite(emailaddressid)(new Write.Single(adventureworks.IntegerArrayMeta.put))}))
    """.query(EmailaddressRow.read).stream
  }
  ...
  override def update(row: EmailaddressRow): ConnectionIO[Option[EmailaddressRow]] = {
    ...
    returning "businessentityid", "emailaddressid", "emailaddress", "rowguid", "modifieddate"::text""".query(EmailaddressRow.read).option
  }
  ...
  override def upsertBatch(unsaved: List[EmailaddressRow]): Stream[ConnectionIO, EmailaddressRow] = {
    Update[EmailaddressRow](
      ...
      "modifieddate" = EXCLUDED."modifieddate"
      returning "businessentityid", "emailaddressid", "emailaddress", "rowguid", "modifieddate"::text"""
    )(EmailaddressRow.write)
    .updateManyWithGeneratedKeys[EmailaddressRow]("businessentityid", "emailaddressid", "emailaddress", "rowguid", "modifieddate")(unsaved)(catsStdInstancesForList, EmailaddressRow.read)
  }
  ...
  override def upsertStreaming(
    ...
    for {
      ...
      _ <- new FragmentOps(sql"""copy emailaddress_TEMP("businessentityid", "emailaddressid", "emailaddress", "rowguid", "modifieddate") from stdin""").copyIn(unsaved, batchSize)(EmailaddressRow.pgText)
