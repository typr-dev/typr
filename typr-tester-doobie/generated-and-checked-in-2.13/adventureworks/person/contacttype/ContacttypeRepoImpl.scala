...
class ContacttypeRepoImpl extends ContacttypeRepo {
  ...
  override def deleteById(contacttypeid: ContacttypeId): ConnectionIO[Boolean] = sql"""delete from "person"."contacttype" where "contacttypeid" = ${fromWrite(contacttypeid)(new Write.Single(ContacttypeId.put))}""".update.run.map(_ > 0)

  override def deleteByIds(contacttypeids: Array[ContacttypeId]): ConnectionIO[Int] = sql"""delete from "person"."contacttype" where "contacttypeid" = ANY(${fromWrite(contacttypeids)(new Write.Single(ContacttypeId.arrayPut))})""".update.run
  ...
  override def insertStreaming(
    ...
    batchSize: Int = 10000
  ): ConnectionIO[Long] = new FragmentOps(sql"""COPY "person"."contacttype"("contacttypeid", "name", "modifieddate") FROM STDIN""").copyIn(unsaved, batchSize)(ContacttypeRow.pgText)

  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(
    unsaved: Stream[ConnectionIO, ContacttypeRowUnsaved],
    batchSize: Int = 10000
  ): ConnectionIO[Long] = new FragmentOps(sql"""COPY "person"."contacttype"("name", "contacttypeid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""").copyIn(unsaved, batchSize)(ContacttypeRowUnsaved.pgText)
  ...
  override def selectAll: Stream[ConnectionIO, ContacttypeRow] = sql"""select "contacttypeid", "name", "modifieddate"::text from "person"."contacttype"""".query(ContacttypeRow.read).stream

  override def selectById(contacttypeid: ContacttypeId): ConnectionIO[Option[ContacttypeRow]] = sql"""select "contacttypeid", "name", "modifieddate"::text from "person"."contacttype" where "contacttypeid" = ${fromWrite(contacttypeid)(new Write.Single(ContacttypeId.put))}""".query(ContacttypeRow.read).option

  override def selectByIds(contacttypeids: Array[ContacttypeId]): Stream[ConnectionIO, ContacttypeRow] = sql"""select "contacttypeid", "name", "modifieddate"::text from "person"."contacttype" where "contacttypeid" = ANY(${fromWrite(contacttypeids)(new Write.Single(ContacttypeId.arrayPut))})""".query(ContacttypeRow.read).stream
  ...
  override def update(row: ContacttypeRow): ConnectionIO[Option[ContacttypeRow]] = {
    ...
    returning "contacttypeid", "name", "modifieddate"::text""".query(ContacttypeRow.read).option
  }
  ...
  override def upsertBatch(unsaved: List[ContacttypeRow]): Stream[ConnectionIO, ContacttypeRow] = {
    Update[ContacttypeRow](
      ...
      "modifieddate" = EXCLUDED."modifieddate"
      returning "contacttypeid", "name", "modifieddate"::text"""
    )(ContacttypeRow.write)
    .updateManyWithGeneratedKeys[ContacttypeRow]("contacttypeid", "name", "modifieddate")(unsaved)(catsStdInstancesForList, ContacttypeRow.read)
  }
  ...
  override def upsertStreaming(
    ...
    for {
      ...
      _ <- new FragmentOps(sql"""copy contacttype_TEMP("contacttypeid", "name", "modifieddate") from stdin""").copyIn(unsaved, batchSize)(ContacttypeRow.pgText)
