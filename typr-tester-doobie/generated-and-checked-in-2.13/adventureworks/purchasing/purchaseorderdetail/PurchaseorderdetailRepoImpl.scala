...
class PurchaseorderdetailRepoImpl extends PurchaseorderdetailRepo {
  ...
  override def selectByIds(compositeIds: Array[PurchaseorderdetailId]): Stream[ConnectionIO, PurchaseorderdetailRow] = {
    ...
    in (select unnest(${fromWrite(purchaseorderid)(new Write.Single(PurchaseorderheaderId.arrayPut))}), unnest(${fromWrite(purchaseorderdetailid)(new Write.Single(adventureworks.IntegerArrayMeta.put))}))
