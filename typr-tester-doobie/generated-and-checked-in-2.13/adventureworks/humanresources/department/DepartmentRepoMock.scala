...
case class DepartmentRepoMock(
  ...
  override def deleteById(departmentid: DepartmentId): ConnectionIO[Boolean] = delay(map.remove(departmentid).isDefined)

  override def deleteByIds(departmentids: Array[DepartmentId]): ConnectionIO[Int] = delay(departmentids.map(id => map.remove(id)).count(_.isDefined))
  ...
  override def insert(unsaved: DepartmentRowUnsaved): ConnectionIO[DepartmentRow] = insert(toRow(unsaved))
  ...
  override def select: SelectBuilder[DepartmentFields, DepartmentRow] = SelectBuilderMock(DepartmentFields.structure, delay(map.values.toList), SelectParams.empty)

  override def selectAll: Stream[ConnectionIO, DepartmentRow] = Stream.emits(map.values.toList)

  override def selectById(departmentid: DepartmentId): ConnectionIO[Option[DepartmentRow]] = delay(map.get(departmentid))

  override def selectByIds(departmentids: Array[DepartmentId]): Stream[ConnectionIO, DepartmentRow] = Stream.emits(departmentids.flatMap(map.get).toList)
