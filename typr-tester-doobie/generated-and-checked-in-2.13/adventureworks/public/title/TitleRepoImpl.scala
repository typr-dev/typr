...
class TitleRepoImpl extends TitleRepo {
  ...
  override def deleteById(code: TitleId): ConnectionIO[Boolean] = sql"""delete from "public"."title" where "code" = ${fromWrite(code)(new Write.Single(TitleId.put))}""".update.run.map(_ > 0)

  override def deleteByIds(codes: Array[TitleId]): ConnectionIO[Int] = sql"""delete from "public"."title" where "code" = ANY(${fromWrite(codes)(new Write.Single(TitleId.arrayPut))})""".update.run
  ...
  override def insert(unsaved: TitleRow): ConnectionIO[TitleRow] = {
    ...
    returning "code"
    """.query(TitleRow.read).unique
  }

  override def insertStreaming(
    unsaved: Stream[ConnectionIO, TitleRow],
    batchSize: Int = 10000
  ): ConnectionIO[Long] = new FragmentOps(sql"""COPY "public"."title"("code") FROM STDIN""").copyIn(unsaved, batchSize)(TitleRow.pgText)

  override def select: SelectBuilder[TitleFields, TitleRow] = SelectBuilder.of(""""public"."title"""", TitleFields.structure, TitleRow.read)

  override def selectAll: Stream[ConnectionIO, TitleRow] = sql"""select "code" from "public"."title"""".query(TitleRow.read).stream

  override def selectById(code: TitleId): ConnectionIO[Option[TitleRow]] = sql"""select "code" from "public"."title" where "code" = ${fromWrite(code)(new Write.Single(TitleId.put))}""".query(TitleRow.read).option

  override def selectByIds(codes: Array[TitleId]): Stream[ConnectionIO, TitleRow] = sql"""select "code" from "public"."title" where "code" = ANY(${fromWrite(codes)(new Write.Single(TitleId.arrayPut))})""".query(TitleRow.read).stream
  ...
  override def upsertBatch(unsaved: List[TitleRow]): Stream[ConnectionIO, TitleRow] = {
    ...
    .updateManyWithGeneratedKeys[TitleRow]("code")(unsaved)(catsStdInstancesForList, TitleRow.read)
  }
  ...
  override def upsertStreaming(
    ...
    for {
      ...
      _ <- new FragmentOps(sql"""copy title_TEMP("code") from stdin""").copyIn(unsaved, batchSize)(TitleRow.pgText)
