...
class TitleDomainRepoImpl extends TitleDomainRepo {
  ...
  override def deleteById(code: TitleDomainId): ConnectionIO[Boolean] = sql"""delete from "public"."title_domain" where "code" = ${fromWrite(code)(new Write.Single(TitleDomainId.put))}""".update.run.map(_ > 0)

  override def deleteByIds(codes: Array[TitleDomainId]): ConnectionIO[Int] = sql"""delete from "public"."title_domain" where "code" = ANY(${fromWrite(codes)(new Write.Single(TitleDomainId.arrayPut))})""".update.run
  ...
  override def insert(unsaved: TitleDomainRow): ConnectionIO[TitleDomainRow] = {
    ...
    returning "code"
    """.query(TitleDomainRow.read).unique
  }

  override def insertStreaming(
    unsaved: Stream[ConnectionIO, TitleDomainRow],
    batchSize: Int = 10000
  ): ConnectionIO[Long] = new FragmentOps(sql"""COPY "public"."title_domain"("code") FROM STDIN""").copyIn(unsaved, batchSize)(TitleDomainRow.pgText)
  ...
  override def selectAll: Stream[ConnectionIO, TitleDomainRow] = sql"""select "code" from "public"."title_domain"""".query(TitleDomainRow.read).stream

  override def selectById(code: TitleDomainId): ConnectionIO[Option[TitleDomainRow]] = sql"""select "code" from "public"."title_domain" where "code" = ${fromWrite(code)(new Write.Single(TitleDomainId.put))}""".query(TitleDomainRow.read).option

  override def selectByIds(codes: Array[TitleDomainId]): Stream[ConnectionIO, TitleDomainRow] = sql"""select "code" from "public"."title_domain" where "code" = ANY(${fromWrite(codes)(new Write.Single(TitleDomainId.arrayPut))})""".query(TitleDomainRow.read).stream
  ...
  override def upsertBatch(unsaved: List[TitleDomainRow]): Stream[ConnectionIO, TitleDomainRow] = {
    Update[TitleDomainRow](
      ...
      do nothing
      returning "code""""
    )(TitleDomainRow.write)
    .updateManyWithGeneratedKeys[TitleDomainRow]("code")(unsaved)(catsStdInstancesForList, TitleDomainRow.read)
  }
  ...
  override def upsertStreaming(
    ...
    for {
      ...
      _ <- new FragmentOps(sql"""copy title_domain_TEMP("code") from stdin""").copyIn(unsaved, batchSize)(TitleDomainRow.pgText)
