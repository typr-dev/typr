...
class ScrapreasonRepoImpl extends ScrapreasonRepo {
  ...
  override def deleteById(scrapreasonid: ScrapreasonId): ConnectionIO[Boolean] = sql"""delete from "production"."scrapreason" where "scrapreasonid" = ${fromWrite(scrapreasonid)(new Write.Single(ScrapreasonId.put))}""".update.run.map(_ > 0)

  override def deleteByIds(scrapreasonids: Array[ScrapreasonId]): ConnectionIO[Int] = sql"""delete from "production"."scrapreason" where "scrapreasonid" = ANY(${fromWrite(scrapreasonids)(new Write.Single(ScrapreasonId.arrayPut))})""".update.run
  ...
  override def insertStreaming(
    ...
    batchSize: Int = 10000
  ): ConnectionIO[Long] = new FragmentOps(sql"""COPY "production"."scrapreason"("scrapreasonid", "name", "modifieddate") FROM STDIN""").copyIn(unsaved, batchSize)(ScrapreasonRow.pgText)

  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(
    unsaved: Stream[ConnectionIO, ScrapreasonRowUnsaved],
    batchSize: Int = 10000
  ): ConnectionIO[Long] = new FragmentOps(sql"""COPY "production"."scrapreason"("name", "scrapreasonid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""").copyIn(unsaved, batchSize)(ScrapreasonRowUnsaved.pgText)
  ...
  override def selectAll: Stream[ConnectionIO, ScrapreasonRow] = sql"""select "scrapreasonid", "name", "modifieddate"::text from "production"."scrapreason"""".query(ScrapreasonRow.read).stream

  override def selectById(scrapreasonid: ScrapreasonId): ConnectionIO[Option[ScrapreasonRow]] = sql"""select "scrapreasonid", "name", "modifieddate"::text from "production"."scrapreason" where "scrapreasonid" = ${fromWrite(scrapreasonid)(new Write.Single(ScrapreasonId.put))}""".query(ScrapreasonRow.read).option

  override def selectByIds(scrapreasonids: Array[ScrapreasonId]): Stream[ConnectionIO, ScrapreasonRow] = sql"""select "scrapreasonid", "name", "modifieddate"::text from "production"."scrapreason" where "scrapreasonid" = ANY(${fromWrite(scrapreasonids)(new Write.Single(ScrapreasonId.arrayPut))})""".query(ScrapreasonRow.read).stream
  ...
  override def update(row: ScrapreasonRow): ConnectionIO[Option[ScrapreasonRow]] = {
    ...
    returning "scrapreasonid", "name", "modifieddate"::text""".query(ScrapreasonRow.read).option
  }
  ...
  override def upsertBatch(unsaved: List[ScrapreasonRow]): Stream[ConnectionIO, ScrapreasonRow] = {
    Update[ScrapreasonRow](
      ...
      "modifieddate" = EXCLUDED."modifieddate"
      returning "scrapreasonid", "name", "modifieddate"::text"""
    )(ScrapreasonRow.write)
    .updateManyWithGeneratedKeys[ScrapreasonRow]("scrapreasonid", "name", "modifieddate")(unsaved)(catsStdInstancesForList, ScrapreasonRow.read)
  }
  ...
  override def upsertStreaming(
    ...
    for {
      ...
      _ <- new FragmentOps(sql"""copy scrapreason_TEMP("scrapreasonid", "name", "modifieddate") from stdin""").copyIn(unsaved, batchSize)(ScrapreasonRow.pgText)
