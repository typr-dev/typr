...
class ProductcosthistoryRepoImpl extends ProductcosthistoryRepo {
  ...
  override def deleteById(compositeId: ProductcosthistoryId): ConnectionIO[Boolean] = sql"""delete from "production"."productcosthistory" where "productid" = ${fromWrite(compositeId.productid)(new Write.Single(ProductId.put))} AND "startdate" = ${fromWrite(compositeId.startdate)(new Write.Single(TypoLocalDateTime.put))}""".update.run.map(_ > 0)
  ...
  override def insertStreaming(
    ...
    batchSize: Int = 10000
  ): ConnectionIO[Long] = new FragmentOps(sql"""COPY "production"."productcosthistory"("productid", "startdate", "enddate", "standardcost", "modifieddate") FROM STDIN""").copyIn(unsaved, batchSize)(ProductcosthistoryRow.pgText)

  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(
    unsaved: Stream[ConnectionIO, ProductcosthistoryRowUnsaved],
    batchSize: Int = 10000
  ): ConnectionIO[Long] = new FragmentOps(sql"""COPY "production"."productcosthistory"("productid", "startdate", "enddate", "standardcost", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""").copyIn(unsaved, batchSize)(ProductcosthistoryRowUnsaved.pgText)
  ...
  override def selectAll: Stream[ConnectionIO, ProductcosthistoryRow] = sql"""select "productid", "startdate"::text, "enddate"::text, "standardcost", "modifieddate"::text from "production"."productcosthistory"""".query(ProductcosthistoryRow.read).stream

  override def selectById(compositeId: ProductcosthistoryId): ConnectionIO[Option[ProductcosthistoryRow]] = sql"""select "productid", "startdate"::text, "enddate"::text, "standardcost", "modifieddate"::text from "production"."productcosthistory" where "productid" = ${fromWrite(compositeId.productid)(new Write.Single(ProductId.put))} AND "startdate" = ${fromWrite(compositeId.startdate)(new Write.Single(TypoLocalDateTime.put))}""".query(ProductcosthistoryRow.read).option
  ...
  override def selectByIds(compositeIds: Array[ProductcosthistoryId]): Stream[ConnectionIO, ProductcosthistoryRow] = {
    ...
    where ("productid", "startdate")
    in (select unnest(${fromWrite(productid)(new Write.Single(ProductId.arrayPut))}), unnest(${fromWrite(startdate)(new Write.Single(TypoLocalDateTime.arrayPut))}))
    """.query(ProductcosthistoryRow.read).stream
  }
  ...
  override def update(row: ProductcosthistoryRow): ConnectionIO[Option[ProductcosthistoryRow]] = {
    ...
    returning "productid", "startdate"::text, "enddate"::text, "standardcost", "modifieddate"::text""".query(ProductcosthistoryRow.read).option
  }
  ...
  override def upsertBatch(unsaved: List[ProductcosthistoryRow]): Stream[ConnectionIO, ProductcosthistoryRow] = {
    Update[ProductcosthistoryRow](
      ...
      "modifieddate" = EXCLUDED."modifieddate"
      returning "productid", "startdate"::text, "enddate"::text, "standardcost", "modifieddate"::text"""
    )(ProductcosthistoryRow.write)
    .updateManyWithGeneratedKeys[ProductcosthistoryRow]("productid", "startdate", "enddate", "standardcost", "modifieddate")(unsaved)(catsStdInstancesForList, ProductcosthistoryRow.read)
  }
  ...
  override def upsertStreaming(
    ...
    for {
      ...
      _ <- new FragmentOps(sql"""copy productcosthistory_TEMP("productid", "startdate", "enddate", "standardcost", "modifieddate") from stdin""").copyIn(unsaved, batchSize)(ProductcosthistoryRow.pgText)
