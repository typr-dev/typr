...
class ProductdocumentRepoImpl extends ProductdocumentRepo {
  ...
  override def deleteByIds(compositeIds: Array[ProductdocumentId]): ConnectionIO[Int] = {
    ...
    in (select unnest(${fromWrite(productid)(new Write.Single(ProductId.arrayPut))}), unnest(${fromWrite(documentnode)(new Write.Single(DocumentId.arrayPut))}))
    """.update.run
  }
  ...
  override def upsertStreaming(
    ...
    for {
      ...
      _ <- new FragmentOps(sql"""copy productdocument_TEMP("productid", "modifieddate", "documentnode") from stdin""").copyIn(unsaved, batchSize)(ProductdocumentRow.pgText)
