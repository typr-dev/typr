...
class ProductinventoryRepoImpl extends ProductinventoryRepo {
  ...
  override def deleteById(compositeId: ProductinventoryId): ConnectionIO[Boolean] = sql"""delete from "production"."productinventory" where "productid" = ${fromWrite(compositeId.productid)(new Write.Single(ProductId.put))} AND "locationid" = ${fromWrite(compositeId.locationid)(new Write.Single(LocationId.put))}""".update.run.map(_ > 0)
  ...
  override def insertStreaming(
    ...
    batchSize: Int = 10000
  ): ConnectionIO[Long] = new FragmentOps(sql"""COPY "production"."productinventory"("productid", "locationid", "shelf", "bin", "quantity", "rowguid", "modifieddate") FROM STDIN""").copyIn(unsaved, batchSize)(ProductinventoryRow.pgText)

  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(
    unsaved: Stream[ConnectionIO, ProductinventoryRowUnsaved],
    batchSize: Int = 10000
  ): ConnectionIO[Long] = new FragmentOps(sql"""COPY "production"."productinventory"("productid", "locationid", "shelf", "bin", "quantity", "rowguid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""").copyIn(unsaved, batchSize)(ProductinventoryRowUnsaved.pgText)
  ...
  override def selectAll: Stream[ConnectionIO, ProductinventoryRow] = sql"""select "productid", "locationid", "shelf", "bin", "quantity", "rowguid", "modifieddate"::text from "production"."productinventory"""".query(ProductinventoryRow.read).stream

  override def selectById(compositeId: ProductinventoryId): ConnectionIO[Option[ProductinventoryRow]] = sql"""select "productid", "locationid", "shelf", "bin", "quantity", "rowguid", "modifieddate"::text from "production"."productinventory" where "productid" = ${fromWrite(compositeId.productid)(new Write.Single(ProductId.put))} AND "locationid" = ${fromWrite(compositeId.locationid)(new Write.Single(LocationId.put))}""".query(ProductinventoryRow.read).option
  ...
  override def update(row: ProductinventoryRow): ConnectionIO[Option[ProductinventoryRow]] = {
    ...
    returning "productid", "locationid", "shelf", "bin", "quantity", "rowguid", "modifieddate"::text""".query(ProductinventoryRow.read).option
  }
  ...
  override def upsertBatch(unsaved: List[ProductinventoryRow]): Stream[ConnectionIO, ProductinventoryRow] = {
    Update[ProductinventoryRow](
      ...
      "modifieddate" = EXCLUDED."modifieddate"
      returning "productid", "locationid", "shelf", "bin", "quantity", "rowguid", "modifieddate"::text"""
    )(ProductinventoryRow.write)
    .updateManyWithGeneratedKeys[ProductinventoryRow]("productid", "locationid", "shelf", "bin", "quantity", "rowguid", "modifieddate")(unsaved)(catsStdInstancesForList, ProductinventoryRow.read)
  }
  ...
  override def upsertStreaming(
    ...
    for {
      ...
      _ <- new FragmentOps(sql"""copy productinventory_TEMP("productid", "locationid", "shelf", "bin", "quantity", "rowguid", "modifieddate") from stdin""").copyIn(unsaved, batchSize)(ProductinventoryRow.pgText)
