...
case class CultureRepoMock(
  ...
  override def deleteById(cultureid: CultureId): ConnectionIO[Boolean] = delay(map.remove(cultureid).isDefined)

  override def deleteByIds(cultureids: Array[CultureId]): ConnectionIO[Int] = delay(cultureids.map(id => map.remove(id)).count(_.isDefined))
  ...
  override def insert(unsaved: CultureRowUnsaved): ConnectionIO[CultureRow] = insert(toRow(unsaved))
  ...
  override def select: SelectBuilder[CultureFields, CultureRow] = SelectBuilderMock(CultureFields.structure, delay(map.values.toList), SelectParams.empty)

  override def selectAll: Stream[ConnectionIO, CultureRow] = Stream.emits(map.values.toList)

  override def selectById(cultureid: CultureId): ConnectionIO[Option[CultureRow]] = delay(map.get(cultureid))

  override def selectByIds(cultureids: Array[CultureId]): Stream[ConnectionIO, CultureRow] = Stream.emits(cultureids.flatMap(map.get).toList)
