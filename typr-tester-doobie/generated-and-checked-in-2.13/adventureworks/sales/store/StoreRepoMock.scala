...
case class StoreRepoMock(
  ...
  override def insert(unsaved: StoreRowUnsaved): ConnectionIO[StoreRow] = insert(toRow(unsaved))
  ...
  override def select: SelectBuilder[StoreFields, StoreRow] = SelectBuilderMock(StoreFields.structure, delay(map.values.toList), SelectParams.empty)

  override def selectAll: Stream[ConnectionIO, StoreRow] = Stream.emits(map.values.toList)

  override def selectById(businessentityid: BusinessentityId): ConnectionIO[Option[StoreRow]] = delay(map.get(businessentityid))

  override def selectByIds(businessentityids: Array[BusinessentityId]): Stream[ConnectionIO, StoreRow] = Stream.emits(businessentityids.flatMap(map.get).toList)
