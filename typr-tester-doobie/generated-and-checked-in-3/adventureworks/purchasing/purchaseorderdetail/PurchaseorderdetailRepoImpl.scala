...
class PurchaseorderdetailRepoImpl extends PurchaseorderdetailRepo {
  ...
  override def selectAll: Stream[ConnectionIO, PurchaseorderdetailRow] = sql"""select "purchaseorderid", "purchaseorderdetailid", "duedate"::text, "orderqty", "productid", "unitprice", "receivedqty", "rejectedqty", "modifieddate"::text from "purchasing"."purchaseorderdetail"""".query(using PurchaseorderdetailRow.read).stream

  override def selectById(compositeId: PurchaseorderdetailId): ConnectionIO[Option[PurchaseorderdetailRow]] = sql"""select "purchaseorderid", "purchaseorderdetailid", "duedate"::text, "orderqty", "productid", "unitprice", "receivedqty", "rejectedqty", "modifieddate"::text from "purchasing"."purchaseorderdetail" where "purchaseorderid" = ${fromWrite(compositeId.purchaseorderid)(using new Write.Single(PurchaseorderheaderId.put))} AND "purchaseorderdetailid" = ${fromWrite(compositeId.purchaseorderdetailid)(using new Write.Single(Meta.IntMeta.put))}""".query(using PurchaseorderdetailRow.read).option
  ...
  override def selectByIds(compositeIds: Array[PurchaseorderdetailId]): Stream[ConnectionIO, PurchaseorderdetailRow] = {
    ...
    in (select unnest(${fromWrite(purchaseorderid)(using new Write.Single(PurchaseorderheaderId.arrayPut))}), unnest(${fromWrite(purchaseorderdetailid)(using new Write.Single(adventureworks.IntegerArrayMeta.put))}))
