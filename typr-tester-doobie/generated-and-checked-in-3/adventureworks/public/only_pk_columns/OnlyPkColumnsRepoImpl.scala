...
class OnlyPkColumnsRepoImpl extends OnlyPkColumnsRepo {
  ...
  override def deleteById(compositeId: OnlyPkColumnsId): ConnectionIO[Boolean] = sql"""delete from "public"."only_pk_columns" where "key_column_1" = ${fromWrite(compositeId.keyColumn1)(using new Write.Single(Meta.StringMeta.put))} AND "key_column_2" = ${fromWrite(compositeId.keyColumn2)(using new Write.Single(Meta.IntMeta.put))}""".update.run.map(_ > 0)
  ...
  override def deleteByIds(compositeIds: Array[OnlyPkColumnsId]): ConnectionIO[Int] = {
    ...
    in (select unnest(${fromWrite(keyColumn1)(using new Write.Single(adventureworks.StringArrayMeta.put))}), unnest(${fromWrite(keyColumn2)(using new Write.Single(adventureworks.IntegerArrayMeta.put))}))
    """.update.run
  }
  ...
  override def insert(unsaved: OnlyPkColumnsRow): ConnectionIO[OnlyPkColumnsRow] = {
    ...
    values (${fromWrite(unsaved.keyColumn1)(using new Write.Single(Meta.StringMeta.put))}, ${fromWrite(unsaved.keyColumn2)(using new Write.Single(Meta.IntMeta.put))}::int4)
  ...
  override def insertStreaming(
    ...
    batchSize: Int = 10000
  ): ConnectionIO[Long] = new FragmentOps(sql"""COPY "public"."only_pk_columns"("key_column_1", "key_column_2") FROM STDIN""").copyIn(unsaved, batchSize)(using OnlyPkColumnsRow.pgText)
  ...
  override def selectAll: Stream[ConnectionIO, OnlyPkColumnsRow] = sql"""select "key_column_1", "key_column_2" from "public"."only_pk_columns"""".query(using OnlyPkColumnsRow.read).stream

  override def selectById(compositeId: OnlyPkColumnsId): ConnectionIO[Option[OnlyPkColumnsRow]] = sql"""select "key_column_1", "key_column_2" from "public"."only_pk_columns" where "key_column_1" = ${fromWrite(compositeId.keyColumn1)(using new Write.Single(Meta.StringMeta.put))} AND "key_column_2" = ${fromWrite(compositeId.keyColumn2)(using new Write.Single(Meta.IntMeta.put))}""".query(using OnlyPkColumnsRow.read).option
  ...
  override def upsert(unsaved: OnlyPkColumnsRow): ConnectionIO[OnlyPkColumnsRow] = {
    sql"""insert into "public"."only_pk_columns"("key_column_1", "key_column_2")
    values (
      ${fromWrite(unsaved.keyColumn1)(using new Write.Single(Meta.StringMeta.put))},
    ${fromWrite(unsaved.keyColumn2)(using new Write.Single(Meta.IntMeta.put))}::int4
    )
  ...
  override def upsertStreaming(
    ...
    for {
      ...
      _ <- new FragmentOps(sql"""copy only_pk_columns_TEMP("key_column_1", "key_column_2") from stdin""").copyIn(unsaved, batchSize)(using OnlyPkColumnsRow.pgText)
