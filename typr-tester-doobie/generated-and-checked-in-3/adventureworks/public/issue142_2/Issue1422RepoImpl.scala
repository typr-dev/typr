...
class Issue1422RepoImpl extends Issue1422Repo {
  ...
  override def deleteById(tabellkode: Issue142Id): ConnectionIO[Boolean] = sql"""delete from "public"."issue142_2" where "tabellkode" = ${fromWrite(tabellkode)(using new Write.Single(Issue142Id.put))}""".update.run.map(_ > 0)

  override def deleteByIds(tabellkodes: Array[Issue142Id]): ConnectionIO[Int] = sql"""delete from "public"."issue142_2" where "tabellkode" = ANY(${fromWrite(tabellkodes)(using new Write.Single(Issue142Id.arrayPut))})""".update.run
  ...
  override def insertStreaming(
    ...
    batchSize: Int = 10000
  ): ConnectionIO[Long] = new FragmentOps(sql"""COPY "public"."issue142_2"("tabellkode") FROM STDIN""").copyIn(unsaved, batchSize)(using Issue1422Row.pgText)
  ...
  override def selectAll: Stream[ConnectionIO, Issue1422Row] = sql"""select "tabellkode" from "public"."issue142_2"""".query(using Issue1422Row.read).stream

  override def selectById(tabellkode: Issue142Id): ConnectionIO[Option[Issue1422Row]] = sql"""select "tabellkode" from "public"."issue142_2" where "tabellkode" = ${fromWrite(tabellkode)(using new Write.Single(Issue142Id.put))}""".query(using Issue1422Row.read).option

  override def selectByIds(tabellkodes: Array[Issue142Id]): Stream[ConnectionIO, Issue1422Row] = sql"""select "tabellkode" from "public"."issue142_2" where "tabellkode" = ANY(${fromWrite(tabellkodes)(using new Write.Single(Issue142Id.arrayPut))})""".query(using Issue1422Row.read).stream
  ...
  override def upsertStreaming(
    ...
    for {
      ...
      _ <- new FragmentOps(sql"""copy issue142_2_TEMP("tabellkode") from stdin""").copyIn(unsaved, batchSize)(using Issue1422Row.pgText)
