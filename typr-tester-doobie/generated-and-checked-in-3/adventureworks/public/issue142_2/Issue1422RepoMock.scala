...
case class Issue1422RepoMock(map: scala.collection.mutable.Map[Issue142Id, Issue1422Row] = scala.collection.mutable.Map.empty[Issue142Id, Issue1422Row]) extends Issue1422Repo {
  ...
  override def select: SelectBuilder[Issue1422Fields, Issue1422Row] = SelectBuilderMock(Issue1422Fields.structure, delay(map.values.toList), SelectParams.empty)

  override def selectAll: Stream[ConnectionIO, Issue1422Row] = Stream.emits(map.values.toList)

  override def selectById(tabellkode: Issue142Id): ConnectionIO[Option[Issue1422Row]] = delay(map.get(tabellkode))

  override def selectByIds(tabellkodes: Array[Issue142Id]): Stream[ConnectionIO, Issue1422Row] = Stream.emits(tabellkodes.flatMap(map.get).toList)
