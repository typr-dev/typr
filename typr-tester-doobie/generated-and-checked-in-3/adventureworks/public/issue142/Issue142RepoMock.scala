...
case class Issue142RepoMock(map: scala.collection.mutable.Map[Issue142Id, Issue142Row] = scala.collection.mutable.Map.empty[Issue142Id, Issue142Row]) extends Issue142Repo {
  ...
  override def deleteById(tabellkode: Issue142Id): ConnectionIO[Boolean] = delay(map.remove(tabellkode).isDefined)

  override def deleteByIds(tabellkodes: Array[Issue142Id]): ConnectionIO[Int] = delay(tabellkodes.map(id => map.remove(id)).count(_.isDefined))
  ...
  override def select: SelectBuilder[Issue142Fields, Issue142Row] = SelectBuilderMock(Issue142Fields.structure, delay(map.values.toList), SelectParams.empty)

  override def selectAll: Stream[ConnectionIO, Issue142Row] = Stream.emits(map.values.toList)

  override def selectById(tabellkode: Issue142Id): ConnectionIO[Option[Issue142Row]] = delay(map.get(tabellkode))

  override def selectByIds(tabellkodes: Array[Issue142Id]): Stream[ConnectionIO, Issue142Row] = Stream.emits(tabellkodes.flatMap(map.get).toList)
