...
class EmailaddressRepoImpl extends EmailaddressRepo {
  ...
  override def deleteByIds(compositeIds: Array[EmailaddressId]): ConnectionIO[Int] = {
    ...
    in (select unnest(${fromWrite(businessentityid)(using new Write.Single(BusinessentityId.arrayPut))}), unnest(${fromWrite(emailaddressid)(using new Write.Single(adventureworks.IntegerArrayMeta.put))}))
    """.update.run
  }
  ...
  override def insert(unsaved: EmailaddressRowUnsaved): ConnectionIO[EmailaddressRow] = {
    val fs = List(
      ...
      unsaved.emailaddressid match {
        ...
        case Defaulted.Provided(value) => Some((Fragment.const0(s""""emailaddressid""""), fr"${fromWrite(value: Int)(using new Write.Single(Meta.IntMeta.put))}::int4"))
      },
  ...
  override def update(row: EmailaddressRow): ConnectionIO[Option[EmailaddressRow]] = {
    ...
    where "businessentityid" = ${fromWrite(compositeId.businessentityid)(using new Write.Single(BusinessentityId.put))} AND "emailaddressid" = ${fromWrite(compositeId.emailaddressid)(using new Write.Single(Meta.IntMeta.put))}
  ...
  override def upsert(unsaved: EmailaddressRow): ConnectionIO[EmailaddressRow] = {
    ...
    ${fromWrite(unsaved.emailaddressid)(using new Write.Single(Meta.IntMeta.put))}::int4,
    ${fromWrite(unsaved.emailaddress)(using new Write.SingleOpt(Meta.StringMeta.put))},
  ...
  override def upsertStreaming(
    ...
    for {
      ...
      _ <- new FragmentOps(sql"""copy emailaddress_TEMP("businessentityid", "emailaddressid", "emailaddress", "rowguid", "modifieddate") from stdin""").copyIn(unsaved, batchSize)(using EmailaddressRow.pgText)
