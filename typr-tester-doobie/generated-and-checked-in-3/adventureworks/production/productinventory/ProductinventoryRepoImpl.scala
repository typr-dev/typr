...
class ProductinventoryRepoImpl extends ProductinventoryRepo {
  ...
  override def deleteByIds(compositeIds: Array[ProductinventoryId]): ConnectionIO[Int] = {
    ...
    in (select unnest(${fromWrite(productid)(using new Write.Single(ProductId.arrayPut))}), unnest(${fromWrite(locationid)(using new Write.Single(LocationId.arrayPut))}))
    """.update.run
  }
  ...
  override def insert(unsaved: ProductinventoryRowUnsaved): ConnectionIO[ProductinventoryRow] = {
    val fs = List(
      ...
      Some((Fragment.const0(s""""shelf""""), fr"${fromWrite(unsaved.shelf)(using new Write.Single(Meta.StringMeta.put))}")),
      Some((Fragment.const0(s""""bin""""), fr"${fromWrite(unsaved.bin)(using new Write.Single(TypoShort.put))}::int2")),
      unsaved.quantity match {
        case Defaulted.UseDefault() => None
        case Defaulted.Provided(value) => Some((Fragment.const0(s""""quantity""""), fr"${fromWrite(value: TypoShort)(using new Write.Single(TypoShort.put))}::int2"))
      },
  ...
  override def update(row: ProductinventoryRow): ConnectionIO[Option[ProductinventoryRow]] = {
    ...
    "bin" = ${fromWrite(row.bin)(using new Write.Single(TypoShort.put))}::int2,
    "quantity" = ${fromWrite(row.quantity)(using new Write.Single(TypoShort.put))}::int2,
    ...
    where "productid" = ${fromWrite(compositeId.productid)(using new Write.Single(ProductId.put))} AND "locationid" = ${fromWrite(compositeId.locationid)(using new Write.Single(LocationId.put))}
  ...
  override def upsert(unsaved: ProductinventoryRow): ConnectionIO[ProductinventoryRow] = {
    ...
    ${fromWrite(unsaved.shelf)(using new Write.Single(Meta.StringMeta.put))},
    ${fromWrite(unsaved.bin)(using new Write.Single(TypoShort.put))}::int2,
    ${fromWrite(unsaved.quantity)(using new Write.Single(TypoShort.put))}::int2,
  ...
  override def upsertStreaming(
    ...
    for {
      ...
      _ <- new FragmentOps(sql"""copy productinventory_TEMP("productid", "locationid", "shelf", "bin", "quantity", "rowguid", "modifieddate") from stdin""").copyIn(unsaved, batchSize)(using ProductinventoryRow.pgText)
