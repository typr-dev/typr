...
case class TransactionhistoryarchiveRepoMock(
  ...
  override def deleteById(transactionid: TransactionhistoryarchiveId): ConnectionIO[Boolean] = delay(map.remove(transactionid).isDefined)

  override def deleteByIds(transactionids: Array[TransactionhistoryarchiveId]): ConnectionIO[Int] = delay(transactionids.map(id => map.remove(id)).count(_.isDefined))
  ...
  override def insert(unsaved: TransactionhistoryarchiveRowUnsaved): ConnectionIO[TransactionhistoryarchiveRow] = insert(toRow(unsaved))
  ...
  override def select: SelectBuilder[TransactionhistoryarchiveFields, TransactionhistoryarchiveRow] = SelectBuilderMock(TransactionhistoryarchiveFields.structure, delay(map.values.toList), SelectParams.empty)

  override def selectAll: Stream[ConnectionIO, TransactionhistoryarchiveRow] = Stream.emits(map.values.toList)

  override def selectById(transactionid: TransactionhistoryarchiveId): ConnectionIO[Option[TransactionhistoryarchiveRow]] = delay(map.get(transactionid))

  override def selectByIds(transactionids: Array[TransactionhistoryarchiveId]): Stream[ConnectionIO, TransactionhistoryarchiveRow] = Stream.emits(transactionids.flatMap(map.get).toList)
