...
class BillofmaterialsRepoImpl extends BillofmaterialsRepo {
  ...
  override def insert(unsaved: BillofmaterialsRowUnsaved): ConnectionIO[BillofmaterialsRow] = {
    val fs = List(
      ...
      Some((Fragment.const0(s""""bomlevel""""), fr"${fromWrite(unsaved.bomlevel)(using new Write.Single(TypoShort.put))}::int2")),
      unsaved.billofmaterialsid match {
        case Defaulted.UseDefault() => None
        case Defaulted.Provided(value) => Some((Fragment.const0(s""""billofmaterialsid""""), fr"${fromWrite(value: Int)(using new Write.Single(Meta.IntMeta.put))}::int4"))
      },
      unsaved.startdate match {
        case Defaulted.UseDefault() => None
        case Defaulted.Provided(value) => Some((Fragment.const0(s""""startdate""""), fr"${fromWrite(value: TypoLocalDateTime)(using new Write.Single(TypoLocalDateTime.put))}::timestamp"))
      },
      unsaved.perassemblyqty match {
        case Defaulted.UseDefault() => None
        case Defaulted.Provided(value) => Some((Fragment.const0(s""""perassemblyqty""""), fr"${fromWrite(value: BigDecimal)(using new Write.Single(Meta.ScalaBigDecimalMeta.put))}::numeric"))
      },
  ...
  override def update(row: BillofmaterialsRow): ConnectionIO[Option[BillofmaterialsRow]] = {
    ...
    "componentid" = ${fromWrite(row.componentid)(using new Write.Single(ProductId.put))}::int4,
    ...
    "bomlevel" = ${fromWrite(row.bomlevel)(using new Write.Single(TypoShort.put))}::int2,
    "perassemblyqty" = ${fromWrite(row.perassemblyqty)(using new Write.Single(Meta.ScalaBigDecimalMeta.put))}::numeric,
    "modifieddate" = ${fromWrite(row.modifieddate)(using new Write.Single(TypoLocalDateTime.put))}::timestamp
    where "billofmaterialsid" = ${fromWrite(billofmaterialsid)(using new Write.Single(Meta.IntMeta.put))}
  ...
  override def upsert(unsaved: BillofmaterialsRow): ConnectionIO[BillofmaterialsRow] = {
    ...
    ${fromWrite(unsaved.productassemblyid)(using new Write.SingleOpt(ProductId.put))}::int4,
    ${fromWrite(unsaved.componentid)(using new Write.Single(ProductId.put))}::int4,
    ...
    ${fromWrite(unsaved.bomlevel)(using new Write.Single(TypoShort.put))}::int2,
    ${fromWrite(unsaved.perassemblyqty)(using new Write.Single(Meta.ScalaBigDecimalMeta.put))}::numeric,
  ...
  override def upsertStreaming(
    ...
    for {
      ...
      _ <- new FragmentOps(sql"""copy billofmaterials_TEMP("billofmaterialsid", "productassemblyid", "componentid", "startdate", "enddate", "unitmeasurecode", "bomlevel", "perassemblyqty", "modifieddate") from stdin""").copyIn(unsaved, batchSize)(using BillofmaterialsRow.pgText)
