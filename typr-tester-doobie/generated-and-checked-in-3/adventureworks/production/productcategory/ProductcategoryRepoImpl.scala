...
class ProductcategoryRepoImpl extends ProductcategoryRepo {
  override def delete: DeleteBuilder[ProductcategoryFields, ProductcategoryRow] = DeleteBuilder.of(""""production"."productcategory"""", ProductcategoryFields.structure, ProductcategoryRow.read)

  override def deleteById(productcategoryid: ProductcategoryId): ConnectionIO[Boolean] = sql"""delete from "production"."productcategory" where "productcategoryid" = ${fromWrite(productcategoryid)(using new Write.Single(ProductcategoryId.put))}""".update.run.map(_ > 0)

  override def deleteByIds(productcategoryids: Array[ProductcategoryId]): ConnectionIO[Int] = sql"""delete from "production"."productcategory" where "productcategoryid" = ANY(${fromWrite(productcategoryids)(using new Write.Single(ProductcategoryId.arrayPut))})""".update.run
  ...
  override def insert(unsaved: ProductcategoryRowUnsaved): ConnectionIO[ProductcategoryRow] = {
    val fs = List(
      ...
      unsaved.productcategoryid match {
        ...
        case Defaulted.Provided(value) => Some((Fragment.const0(s""""productcategoryid""""), fr"${fromWrite(value: ProductcategoryId)(using new Write.Single(ProductcategoryId.put))}::int4"))
      },
  ...
  override def insertStreaming(
    unsaved: Stream[ConnectionIO, ProductcategoryRow],
    batchSize: Int = 10000
  ): ConnectionIO[Long] = new FragmentOps(sql"""COPY "production"."productcategory"("productcategoryid", "name", "rowguid", "modifieddate") FROM STDIN""").copyIn(unsaved, batchSize)(using ProductcategoryRow.pgText)

  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  override def insertUnsavedStreaming(
    unsaved: Stream[ConnectionIO, ProductcategoryRowUnsaved],
    batchSize: Int = 10000
  ): ConnectionIO[Long] = new FragmentOps(sql"""COPY "production"."productcategory"("name", "productcategoryid", "rowguid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""").copyIn(unsaved, batchSize)(using ProductcategoryRowUnsaved.pgText)

  override def select: SelectBuilder[ProductcategoryFields, ProductcategoryRow] = SelectBuilder.of(""""production"."productcategory"""", ProductcategoryFields.structure, ProductcategoryRow.read)

  override def selectAll: Stream[ConnectionIO, ProductcategoryRow] = sql"""select "productcategoryid", "name", "rowguid", "modifieddate"::text from "production"."productcategory"""".query(using ProductcategoryRow.read).stream

  override def selectById(productcategoryid: ProductcategoryId): ConnectionIO[Option[ProductcategoryRow]] = sql"""select "productcategoryid", "name", "rowguid", "modifieddate"::text from "production"."productcategory" where "productcategoryid" = ${fromWrite(productcategoryid)(using new Write.Single(ProductcategoryId.put))}""".query(using ProductcategoryRow.read).option

  override def selectByIds(productcategoryids: Array[ProductcategoryId]): Stream[ConnectionIO, ProductcategoryRow] = sql"""select "productcategoryid", "name", "rowguid", "modifieddate"::text from "production"."productcategory" where "productcategoryid" = ANY(${fromWrite(productcategoryids)(using new Write.Single(ProductcategoryId.arrayPut))})""".query(using ProductcategoryRow.read).stream
  ...
  override def update: UpdateBuilder[ProductcategoryFields, ProductcategoryRow] = UpdateBuilder.of(""""production"."productcategory"""", ProductcategoryFields.structure, ProductcategoryRow.read)
  ...
  override def update(row: ProductcategoryRow): ConnectionIO[Option[ProductcategoryRow]] = {
    ...
    where "productcategoryid" = ${fromWrite(productcategoryid)(using new Write.Single(ProductcategoryId.put))}
  ...
  override def upsertStreaming(
    ...
    for {
      ...
      _ <- new FragmentOps(sql"""copy productcategory_TEMP("productcategoryid", "name", "rowguid", "modifieddate") from stdin""").copyIn(unsaved, batchSize)(using ProductcategoryRow.pgText)
