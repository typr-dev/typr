...
class ContactsTest extends AnyFunSuite {
  ...
  test("update tags") {
    withConnection { c =>
      ...
      val _ = assert(fetched.tags.get.value.sameElements(Array("new", "updated")))
    }
  }
  ...
  test("clear emails") {
    withConnection { c =>
      ...
      val cleared = inserted.copy(emails = None)
  ...
  test("select all") {
    withConnection { c =>
      ...
      val unsaved2 = new ContactsRowUnsaved(
        "Contact 2",
        Some(emails2),
        Some(new TagVarrayT(Array("test"))),
        Defaulted.UseDefault[ContactsId]()
      )
