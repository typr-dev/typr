...
data class CustomCreditcardId(val value: Int) {
    companion object {
        val pgText: PgText<CustomCreditcardId> = PgText.instance { v, sb -> PgText.textInteger.unsafeEncode(v.value, sb) }
        val pgType: PgType<CustomCreditcardId> = PgTypes.int4.bimap(::CustomCreditcardId) { it.value }
        val pgTypeArray: PgType<Array<CustomCreditcardId>> = PgTypes.int4Array.bimap(
            { arr -> arrayMap.map(arr, ::CustomCreditcardId, CustomCreditcardId::class.java) },
