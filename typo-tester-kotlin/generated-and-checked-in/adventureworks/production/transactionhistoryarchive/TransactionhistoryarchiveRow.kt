...
data class TransactionhistoryarchiveRow(
  ...
  companion object {
    val _rowParser: RowParser<TransactionhistoryarchiveRow> = RowParsers.of(TransactionhistoryarchiveId.pgType, PgTypes.int4, PgTypes.int4, PgTypes.int4, TypoLocalDateTime.pgType, PgTypes.text, PgTypes.int4, PgTypes.numeric, TypoLocalDateTime.pgType, ::TransactionhistoryarchiveRow, { row -> Tuple9(row.transactionid, row.productid, row.referenceorderid, row.referenceorderlineid, row.transactiondate, row.transactiontype, row.quantity, row.actualcost, row.modifieddate) })
