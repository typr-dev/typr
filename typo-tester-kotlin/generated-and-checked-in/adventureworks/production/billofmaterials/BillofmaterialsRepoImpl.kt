...
data class BillofmaterialsRepoImpl() : BillofmaterialsRepo {
  fun deleteById(
  ...
  fun selectAll(c: Connection): kotlin.collections.List<BillofmaterialsRow> = interpolate(typo.runtime.Fragment.lit("""
    ...
    from "production"."billofmaterials"
  """.trimMargin())).as(BillofmaterialsRow._rowParser.all()).runUnchecked(c)
  ...
  fun selectById(
    ...
    PgTypes.int4.encode(billofmaterialsid),
    typo.runtime.Fragment.lit("")
  ).as(BillofmaterialsRow._rowParser.first()).runUnchecked(c)

  fun selectByIds(
    billofmaterialsids: Array<Int>,
    c: Connection
  ): kotlin.collections.List<BillofmaterialsRow> = interpolate(
    typo.runtime.Fragment.lit("""
      select "billofmaterialsid", "productassemblyid", "componentid", "startdate"::text, "enddate"::text, "unitmeasurecode", "bomlevel", "perassemblyqty", "modifieddate"::text
      from "production"."billofmaterials"
      where "billofmaterialsid" = ANY(""".trimMargin()),
  ...
  fun selectByIdsTracked(
    ...
    val ret: java.util.Map<Int, BillofmaterialsRow> = HashMap()
      for (var row : selectByIds(billofmaterialsids, c)) {
          ret.put(row.id(), row);
      }
    return ret
  }
  ...
  fun upsertBatch(
    unsaved: Iterator<BillofmaterialsRow>,
    c: Connection
  ): kotlin.collections.List<BillofmaterialsRow> = interpolate(typo.runtime.Fragment.lit("""
                                                     insert into "production"."billofmaterials"("billofmaterialsid", "productassemblyid", "componentid", "startdate", "enddate", "unitmeasurecode", "bomlevel", "perassemblyqty", "modifieddate")
