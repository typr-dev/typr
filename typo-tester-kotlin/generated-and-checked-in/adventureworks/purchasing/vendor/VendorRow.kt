...
data class VendorRow(
  ...
  companion object {
    val _rowParser: RowParser<VendorRow> = RowParsers.of(BusinessentityId.pgType, AccountNumber.pgType, Name.pgType, TypoShort.pgType, Flag.pgType, Flag.pgType, PgTypes.text.opt(), TypoLocalDateTime.pgType, ::VendorRow, { row -> Tuple8(row.businessentityid, row.accountnumber, row.name, row.creditrating, row.preferredvendorstatus, row.activeflag, row.purchasingwebserviceurl, row.modifieddate) })
