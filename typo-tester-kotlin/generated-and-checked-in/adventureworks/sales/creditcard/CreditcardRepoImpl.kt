...
data class CreditcardRepoImpl() : CreditcardRepo {
  fun deleteById(
  ...
  fun selectAll(c: Connection): kotlin.collections.List<CreditcardRow> = interpolate(typo.runtime.Fragment.lit("""
    select "creditcardid", "cardtype", "cardnumber", "expmonth", "expyear", "modifieddate"::text
    from "sales"."creditcard"
  """.trimMargin())).as(CreditcardRow._rowParser.all()).runUnchecked(c)
  ...
  fun selectByIds(
    creditcardids: Array</* user-picked */ CustomCreditcardId>,
    c: Connection
  ): kotlin.collections.List<CreditcardRow> = interpolate(
    typo.runtime.Fragment.lit("""
      select "creditcardid", "cardtype", "cardnumber", "expmonth", "expyear", "modifieddate"::text
      from "sales"."creditcard"
      where "creditcardid" = ANY(""".trimMargin()),
  ...
  fun selectByIdsTracked(
    ...
    val ret: java.util.Map</* user-picked */ CustomCreditcardId, CreditcardRow> = HashMap()
      for (var row : selectByIds(creditcardids, c)) {
          ret.put(row.id(), row);
      }
    return ret
  }
  ...
  fun upsertBatch(
    unsaved: Iterator<CreditcardRow>,
    c: Connection
  ): kotlin.collections.List<CreditcardRow> = interpolate(typo.runtime.Fragment.lit("""
                                                insert into "sales"."creditcard"("creditcardid", "cardtype", "cardnumber", "expmonth", "expyear", "modifieddate")
