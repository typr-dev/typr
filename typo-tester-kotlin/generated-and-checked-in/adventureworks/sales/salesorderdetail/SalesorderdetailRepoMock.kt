/**
 * File has been automatically generated by `typo`.
 *
 * IF YOU CHANGE THIS FILE YOUR CHANGES WILL BE OVERWRITTEN.
 */
package adventureworks.sales.salesorderdetail

import java.sql.Connection
import java.util.ArrayList
import java.util.Iterator
import kotlin.collections.List
import kotlin.collections.Map
import kotlin.collections.MutableMap
import typo.runtime.internal.stringInterpolator.str

data class SalesorderdetailRepoMock(
  val toRow: (SalesorderdetailRowUnsaved) -> SalesorderdetailRow,
  val map: MutableMap<SalesorderdetailId, SalesorderdetailRow>
) : SalesorderdetailRepo {
  constructor(toRow: (SalesorderdetailRowUnsaved) -> SalesorderdetailRow) : this(toRow, mutableMapOf<SalesorderdetailId, SalesorderdetailRow>())

  fun deleteById(
    compositeId: SalesorderdetailId,
    c: Connection
  ): Boolean = (map.remove(compositeId) != null)

  fun deleteByIds(
    compositeIds: Array<SalesorderdetailId>,
    c: Connection
  ): Int {
    var count = 0
      for (var id : compositeIds) {
        if ((map.remove(id) != null)) {
          count = count + 1
        }
      }
    return count
  }

  fun insert(
    unsaved: SalesorderdetailRow,
    c: Connection
  ): SalesorderdetailRow {
    if (map.containsKey(unsaved.compositeId)) {
        throw new RuntimeException(str("id $unsaved.compositeId already exists"))
      }
      map[unsaved.compositeId] = unsaved
    return unsaved
  }

  fun insert(
    unsaved: SalesorderdetailRowUnsaved,
    c: Connection
  ): SalesorderdetailRow = insert(toRow(unsaved), c)

  fun insertStreaming(
    unsaved: Iterator<SalesorderdetailRow>,
    batchSize: Int,
    c: Connection
  ): Long {
    var count = 0L
      while (unsaved.hasNext()) {
        var row = unsaved.next()
        map[row.compositeId] = row
        count = count + 1L
      }
    return count
  }

  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  fun insertUnsavedStreaming(
    unsaved: Iterator<SalesorderdetailRowUnsaved>,
    batchSize: Int,
    c: Connection
  ): Long {
    var count = 0L
      while (unsaved.hasNext()) {
        var unsavedRow = unsaved.next()
        var row = toRow(unsavedRow)
        map[row.compositeId] = row
        count = count + 1L
      }
    return count
  }

  fun selectAll(c: Connection): List<SalesorderdetailRow> = map.values.toList()

  fun selectById(
    compositeId: SalesorderdetailId,
    c: Connection
  ): SalesorderdetailRow? = map[compositeId]

  fun selectByIds(
    compositeIds: Array<SalesorderdetailId>,
    c: Connection
  ): List<SalesorderdetailRow> = compositeIds.mapNotNull { { id -> map[id] }(it) }

  fun selectByIdsTracked(
    compositeIds: Array<SalesorderdetailId>,
    c: Connection
  ): Map<SalesorderdetailId, SalesorderdetailRow> = selectByIds(compositeIds, c).associateBy { { row -> row.compositeId }(it) }

  fun update(
    row: SalesorderdetailRow,
    c: Connection
  ): Boolean {
    val shouldUpdate = (map[row.compositeId]?.takeIf { { oldRow -> !oldRow.equals(row) }(it) } != null)
      if (shouldUpdate) {
        map[row.compositeId] = row
      }
    return shouldUpdate
  }

  fun upsert(
    unsaved: SalesorderdetailRow,
    c: Connection
  ): SalesorderdetailRow {
    map[unsaved.compositeId] = unsaved
    return unsaved
  }

  fun upsertBatch(
    unsaved: Iterator<SalesorderdetailRow>,
    c: Connection
  ): List<SalesorderdetailRow> {
    var result = ArrayList<SalesorderdetailRow>()
      while (unsaved.hasNext()) {
        var row = unsaved.next()
        map[row.compositeId] = row
        result.add(row)
      }
    return result
  }

  /** NOTE: this functionality is not safe if you use auto-commit mode! it runs 3 SQL statements */
  fun upsertStreaming(
    unsaved: Iterator<SalesorderdetailRow>,
    batchSize: Int,
    c: Connection
  ): Int {
    var count = 0
      while (unsaved.hasNext()) {
        var row = unsaved.next()
        map[row.compositeId] = row
        count = count + 1
      }
    return count
  }
}