...
import typo.dsl.SelectBuilderSql.ColumnTuple;
import typo.dsl.SelectBuilderSql.GroupedTableState;
import typo.dsl.SelectBuilderSql.JoinInfo;
import typo.dsl.SelectBuilderSql.Query;
import typo.dsl.SelectBuilderSql.Tuple2;
...
public class GroupedBuilderSql<Fields, Row> implements GroupedBuilder<Fields, Row> {
     ...
     */
    static class GroupedSelectBuilderSql<Fields, Row, NewFields extends Tuples.TupleExpr<NewRow>, NewRow extends Tuples.Tuple>
        ...
        public SelectBuilder<NewFields, NewRow> withParams(SelectParams<NewFields, NewRow> newParams) {
            return new GroupedSelectBuilderSql<>(source, groupByExprs, havingPredicates, projection, newParams);
        }
        ...
        public Query<NewFields, NewRow> collectQuery(RenderCtx ctx, AtomicInteger counter) {
            ...
            GroupedTableState groupedTable = new GroupedTableState(
                    alias,
                    columns,
                    sourceQuery,
                    groupByExprList,
        ...
        @Override
        public <Fields2, Row2> SelectBuilder<Structure.Tuple2<NewFields, Fields2>, Structure.Tuple2<NewRow, Row2>>
        joinOn(SelectBuilder<Fields2, Row2> other, Function<Structure.Tuple2<NewFields, Fields2>, SqlExpr<Boolean>> pred) {
        ...
        @Override
        public <Fields2, Row2> SelectBuilder<Structure.Tuple2<NewFields, Fields2>, Structure.Tuple2<NewRow, Optional<Row2>>>
        leftJoinOn(SelectBuilder<Fields2, Row2> other, Function<Structure.Tuple2<NewFields, Fields2>, SqlExpr<Boolean>> pred) {
            if (!(other instanceof SelectBuilderSql<Fields2, Row2> otherSql)) {
                throw new IllegalArgumentException("Can only join with SQL-based SelectBuilder");
            }
        ...
        @Override
        public GroupedBuilder<NewFields, NewRow> groupByExpr(Function<NewFields, List<SqlExpr<?>>> groupKeys) {
        ...
        protected Tuple2<Fragment, RowParser<NewRow>> getSqlAndRowParser() {
            ...
            RenderCtx joinCtx = ctx.withJoinContext(true).withAliasToCteMap(SelectBuilderSql.buildFullAliasMap(allTables));
            ...
            if (!havingPredicates.isEmpty()) {
                ...
                for (int i = 1; i < havingFragments.size(); i++) {
                    havingCombined = havingCombined.append(Fragment.lit(" AND ")).append(havingFragments.get(i));
                }
        ...
        private Fragment renderGroupedTableRefNested(GroupedTableState grouped, Dialect dialect, AtomicInteger counter) {
         ...
         */
        private Fragment renderGroupedTableRefFull(GroupedTableState grouped, Dialect dialect, AtomicInteger counter) {
            ...
            // FROM clause
            Fragment from = Fragment.lit(" FROM ").append(renderTableRef(sourceQuery.firstTable(), dialect, counter));
        ...
        private Fragment renderCompositeTableRef(CompositeTableState composite, Dialect dialect, AtomicInteger counter) {
            Query<?, ?> innerQuery = composite.innerQuery();
            List<TableState> allTables = innerQuery.allTables();

            RenderCtx innerCtx = composite.renderCtx().withJoinContext(true).withAliasToCteMap(SelectBuilderSql.buildSimpleAliasMap(allTables));
            ...
            for (TableState table : allTables) {
                for (ColumnTuple col : table.columns()) {
                    ...
                    Fragment colRef = Fragment.lit(table.alias() + "." + dialect.quoteIdent(col.column().name())
                            + " AS " + uniqueAlias);
    ...
    /**
     * A synthetic field representing a computed expression in a grouped query.
     * Used when grouped results need to be joined with other tables.
     * The row type R is Object since we don't know the actual row type at this point.
     */
    @SuppressWarnings({"unchecked", "rawtypes"})
    public record SyntheticField<T>(
            typo.runtime.DbType<T> pgType,
            String column,
            String tableAlias
        ...
        @Override
        public typo.runtime.Either<String, Object> set(Object row, Optional<T> value) {
            // Not used for grouped results
            return typo.runtime.Either.left("SyntheticField.set is not supported");
        }
        ...
        @Override
        public typo.runtime.DbType<T> dbType() {
            return pgType;
        }
