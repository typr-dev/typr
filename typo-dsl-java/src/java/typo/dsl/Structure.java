...
public interface Structure<Fields, Row> {
    Fields fields();
    /** Returns all field definitions in this structure. Used for field lookup and SQL column generation. */
     ...
     * <p>
     * <b>Mock-only:</b> This method is only used by {@link SelectBuilderMock} for in-memory
     * query evaluation. It is not used by {@link SelectBuilderSql} which generates SQL strings.
     * <p>
     ...
     * <p>
     * <b>Mock-only:</b> This method is only used by {@link SelectBuilderMock} for in-memory
     * query evaluation. It provides a visitor-based interpreter for all SqlExpr types,
     * allowing mock repositories to filter, sort, and project rows without a database.
     * <p>
     * {@link SelectBuilderSql} does not use this method - it renders expressions to SQL strings instead.
     * <p>
     * It's up to you to ensure that the Row in field is the same type as row.
     */
    default <T> Optional<T> untypedEval(SqlExpr<T> expr, Row row) {
        ...
        SqlExprVisitor<T, Row, Optional<T>> visitor = new SqlExprVisitor<T, Row, Optional<T>>() {
            ...
            @Override
            public <F extends Tuples.TupleExpr<R>, R extends Tuples.Tuple> Optional<T> visitInSubqueryExpr(SqlExpr.InSubquery<?, F, R> inSubquery) {
            ...
            public Optional<T> visitCountStar(SqlExpr.CountStar countStar) {
                throw new UnsupportedOperationException("Aggregate function COUNT(*) cannot be evaluated against a single row");
            }
            ...
            public <U> Optional<T> visitCountDistinct(SqlExpr.CountDistinct<U> countDistinct) {
                throw new UnsupportedOperationException("Aggregate function COUNT(DISTINCT) cannot be evaluated against a single row");
            }

            @Override
            public <U, R> Optional<T> visitSum(SqlExpr.Sum<U, R> sum) {
                throw new UnsupportedOperationException("Aggregate function SUM cannot be evaluated against a single row");
            }

            @Override
            public <U> Optional<T> visitAvg(SqlExpr.Avg<U> avg) {
                throw new UnsupportedOperationException("Aggregate function AVG cannot be evaluated against a single row");
            }

            @Override
            public <U> Optional<T> visitMin(SqlExpr.Min<U> min) {
                throw new UnsupportedOperationException("Aggregate function MIN cannot be evaluated against a single row");
            }

            @Override
            public <U> Optional<T> visitMax(SqlExpr.Max<U> max) {
                throw new UnsupportedOperationException("Aggregate function MAX cannot be evaluated against a single row");
            }

            @Override
            public Optional<T> visitStringAgg(SqlExpr.StringAgg stringAgg) {
                throw new UnsupportedOperationException("Aggregate function STRING_AGG cannot be evaluated against a single row");
            }

            @Override
            public <U> Optional<T> visitArrayAgg(SqlExpr.ArrayAgg<U> arrayAgg) {
                throw new UnsupportedOperationException("Aggregate function ARRAY_AGG cannot be evaluated against a single row");
            }

            @Override
            public <U> Optional<T> visitJsonAgg(SqlExpr.JsonAgg<U> jsonAgg) {
                throw new UnsupportedOperationException("Aggregate function JSON_AGG cannot be evaluated against a single row");
            }

            @Override
            public Optional<T> visitBoolAnd(SqlExpr.BoolAnd boolAnd) {
                throw new UnsupportedOperationException("Aggregate function BOOL_AND cannot be evaluated against a single row");
            }

            @Override
            public Optional<T> visitBoolOr(SqlExpr.BoolOr boolOr) {
                throw new UnsupportedOperationException("Aggregate function BOOL_OR cannot be evaluated against a single row");
            }
    ...
    // Interface for relations - can be implemented by records
    // Uses _path to avoid conflicts with tables that have a 'path' column
    interface Relation<Fields, Row> extends Structure<Fields, Row> {
        List<Path> _path();
        Relation<Fields, Row> copy(List<Path> _path);
        ...
        default List<SqlExpr.FieldLike<?, ?>> allFields() {
            // Generated Fields implement both Relation and FieldsExpr.
    ...
    record Tupled<Fields1, Fields2, Row1, Row2>(
            List<Path> path,
            Structure<Fields1, Row1> left,
            Structure<Fields2, Row2> right
        ...
        @Override
        public <T> Optional<T> untypedGetFieldValue(SqlExpr.FieldLike<T, ?> field, Tuple2<Row1, Row2> row) {
    ...
    record LeftTupled<Fields1, Fields2, Row1, Row2>(
        ...
        @Override
        public <T> Optional<T> untypedGetFieldValue(SqlExpr.FieldLike<T, ?> field, Tuple2<Row1, Optional<Row2>> row) {
            if (containsField(left.allFields(), field)) {
                return left.untypedGetFieldValue(field, row._1());
            } else {
    ...
    /**
     * Mock-only: Check if a field exists in a columns list by path and column name.
     * This is needed because Field records contain Function/BiFunction which don't have structural equality.
     * Used by Tupled and LeftTupled to route field lookups to the correct side of a join.
     */
    ...
    /**
     * Mock-only: Evaluate an IN subquery expression against a row.
     * Returns true if the expression value is in the subquery results.
     */
    private static <T, F extends Tuples.TupleExpr<R>, R extends Tuples.Tuple, Row> Boolean evaluateInSubqueryExpression(
    ...
    /**
     * Mock-only: Evaluate an EXISTS expression.
    ...
    /**
     * Mock-only: Evaluate a TupleExpr by evaluating all its sub-expressions and constructing the corresponding Tuple.
     */
    @SuppressWarnings("unchecked")
    private static <Row> Optional<Tuples.Tuple> evaluateTupleExpr(Structure<?, Row> structure, Tuples.TupleExpr<?> tupleExpr, Row row) {
