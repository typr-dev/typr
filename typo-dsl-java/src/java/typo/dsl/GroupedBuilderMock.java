 ...
 *
 * <p>Evaluates GROUP BY and aggregate functions against in-memory data,
...
public class GroupedBuilderMock<Fields, Row> implements GroupedBuilder<Fields, Row> {
    ...
    SelectBuilder<NewFields, NewRow> select(Function<Fields, NewFields> projection) {
        return new GroupedSelectBuilderMock<>(source, groupByExprs, havingPredicates, projection);
    }
     ...
     */
    static class GroupedSelectBuilderMock<Fields, Row, NewFields extends Tuples.TupleExpr<NewRow>, NewRow extends Tuples.Tuple>
        ...
        public SelectBuilder<NewFields, NewRow> withParams(SelectParams<NewFields, NewRow> newParams) {
            return new GroupedSelectBuilderMock<>(source, groupByExprs, havingPredicates, projection, newParams);
        }
        ...
        /**
         * Convert this grouped builder to a regular SelectBuilderMock by materializing results.
         * This enables joins and other operations on grouped results.
         */
        ...
        SelectBuilder<NewFields2, NewRow2> mapExpr(Function<NewFields, NewFields2> projection) {
            ...
            SelectBuilderMock.ProjectedMockStructure<NewFields, NewRow> projectedStructure =
                    new SelectBuilderMock.ProjectedMockStructure<>(structure().fields(), structure().fields().exprs());
         ...
         */
        private List<Object> evaluateGroupKey(Structure<Fields, Row> structure, List<SqlExpr<?>> keyExprs, Row row) {
         ...
         */
        @SuppressWarnings("unchecked")
        private <T> T evaluateAggregate(Structure<Fields, Row> structure, SqlExpr<T> expr, List<Row> groupRows) {
            return switch (expr) {
                ...
                case SqlExpr.Avg<?> a -> {
                    ...
                    yield (T) (count == 0 ? null : sum.divide(BigDecimal.valueOf(count), 10, RoundingMode.HALF_UP).doubleValue());
                }
        ...
        /**
         * Convert a BigDecimal to the appropriate numeric type based on the result type.
         * Uses the PgTypes constants to determine the target type.
         * Returns Object to make type inference simpler.
         */
        private static Object convertSumToObject(BigDecimal value, typo.runtime.DbType<?> targetType) {
            // Check for common numeric types by comparing to known PgTypes
            if (targetType == typo.runtime.PgTypes.int8) {
                return Long.valueOf(value.longValue());
            } else if (targetType == typo.runtime.PgTypes.int4) {
                return Integer.valueOf(value.intValue());
            } else if (targetType == typo.runtime.PgTypes.int2) {
                return Short.valueOf(value.shortValue());
            } else if (targetType == typo.runtime.PgTypes.float8) {
                return Double.valueOf(value.doubleValue());
            } else if (targetType == typo.runtime.PgTypes.float4) {
                return Float.valueOf(value.floatValue());
            } else if (targetType == typo.runtime.PgTypes.numeric) {
                return value;
            } else {
    ...
    /**
     * Structure implementation for grouped query results.
     * Supports joins by properly implementing withPath and field access.
     */
    record GroupedMockStructure<NewFields extends Tuples.TupleExpr<NewRow>, NewRow extends Tuples.Tuple>(
            NewFields fields,
        ...
        public <T> Optional<T> untypedEval(SqlExpr<T> expr, NewRow row) {
            // For grouped results, first check if the expression matches a projected expression by position
            Object[] values = row.asArray();
            for (int i = 0; i < projectedExprs.size(); i++) {
