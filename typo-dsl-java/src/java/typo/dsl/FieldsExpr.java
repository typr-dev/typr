...
/**
 * Marker interface for generated Fields types that can be used as SqlExpr.
 * This allows passing entire field structures through map() operations.
 * <p>
 * Generated code (e.g., PersonFields) will implement this interface,
 * providing access to all columns as a composite expression.
 * <p>
 * This interface is non-sealed to allow generated code in user packages
 * to implement it while still being part of the SqlExpr hierarchy.
 *
...
public non-sealed interface FieldsExpr<Row> extends SqlExpr<Row> {
     ...
     * Used for rendering and projection operations.
     */
    List<SqlExpr.FieldLike<?, Row>> columns();

    /**
     * Returns the row parser for this fields structure.
     * Contains column types, decode, and encode functions.
     */
    RowParser<Row> rowParser();

    /**
     * Returns the database type for this row.
     * Delegates to the first column's dbType - the actual parsing is handled by RowParser.
     */
    @Override
    ...
    /**
     * Returns the total number of columns this fields expression produces.
