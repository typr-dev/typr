...
/**
 * Example showing how the Java DSL might be used with generated code.
 * This demonstrates the type-safe query building capabilities.
 */
public class PersonExample {
    ...
    public static void exampleQueries(PersonRepo repo, Connection conn) {
        ...
        List<PersonRow> adults = repo.select()
                .where(p -> p.age().greaterThanOrEqual(18))
                .toList(conn);

        // Select with multiple conditions
        List<PersonRow> smiths = repo.select()
        ...
        List<PersonRow> complexQuery = repo.select()
                .where(p -> p.age().lessThan(18)
                        .or(p.age().greaterThan(65), Bijection.asBool()))
        ...
        int deleted = repo.delete()
                .where(p -> p.age().lessThan(0))
                .execute(conn);

        // Using IN operator with array
        List<PersonRow> specificPeople = repo.select()
                .where(p -> p.personId().in(1L, 2L, 3L, 4L))
                .toList(conn);

        // Working with optional fields
        List<PersonRow> withEmail = repo.select()
                .where(p -> p.email().isNull().not(Bijection.asBool()))
    ...
    public static void joinExample(
        ...
        var query = personRepo.select()
                ...
                .<PersonFields, PersonRow>on(tuple -> {
        ...
        List<Structure.Tuple2<PersonRow, AddressRow>> results = query.toList(conn);

        // Left join example
        var leftJoinQuery = personRepo.select()
                .join(addressRepo.select())
                .<PersonFields, PersonRow>leftOn(tuple -> {
                    PersonFields person = tuple._1();
                    AddressFields address = tuple._2();
                    return person.personId().isEqual(address.personId());
                });

        List<Structure.Tuple2<PersonRow, Optional<AddressRow>>> leftResults =
                leftJoinQuery.toList(conn);
    }
